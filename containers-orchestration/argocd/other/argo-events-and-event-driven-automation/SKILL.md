...
Argo Events is a [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-native event framework that decouples *event
...
Workflow, creating/patching a [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) resource, calling another
...
- Triggering an Argo Workflows pipeline (see
  ...
  from an external event: a [GitHub](../../../../ci-cd/github-actions/other/github/SKILL.md) webhook, a file landing in S3/GCS, a
...
- Replacing a bespoke webhook-receiver Deployment/polling script with a
  declarative, [GitOps](../../../common/gitops/gitops/SKILL.md)-managed `EventSource`/`Sensor` pair.
...
- Network reachability for the event's origin to hit the cluster: for
  webhook-based `EventSource`s ([GitHub](../../../../ci-cd/github-actions/other/github/SKILL.md), Gitlab, generic webhook), an
...
- If triggers target Argo Workflows, the Argo Workflows controller
  ...
  [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) API, so ordinary RBAC applies.
...
2. **Define an `EventSource`** for the origin of events. Webhook example
   (generic HTTP webhook — [GitHub](../../../../ci-cd/github-actions/other/github/SKILL.md)/GitLab have dedicated event source
...
6. **Verify and debug:**
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get eventsource,sensor,eventbus -n automation
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) logs -n automation deploy/upload-webhook-eventsource -f
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) logs -n automation deploy/upload-processor-sensor -f
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) describe sensor upload-processor -n automation   # trigger conditions/status
   ```
...
- Use dedicated `EventSource` types ([GitHub](../../../../ci-cd/github-actions/other/github/SKILL.md), GitLab, Stripe, etc.) with
...
- Version and [GitOps](../../../common/gitops/gitops/SKILL.md)-manage `EventSource`/`Sensor`/`EventBus` manifests
  the same as any other cluster resource — treat them as part of the
  [GitOps](../../../common/gitops/gitops/SKILL.md)-tracked config repo, not a one-off `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply` nobody
  remembers making.
...
- **Symptom:** A `Sensor` trigger fires but the target `Workflow` never
  ...
  **Fix:** Check the `Sensor` Pod's own logs and `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) describe
...
- **Symptom:** A generic webhook `EventSource` is receiving forged/
  ...
  **Fix:** The generic `webhook` EventSource type has no built-in
  ...
  type (e.g., `[github](../../../../ci-cd/github-actions/other/github/SKILL.md)` with a configured webhook secret) which validates
...
triggers the pipeline, and `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) describe sensor upload-processor -n
