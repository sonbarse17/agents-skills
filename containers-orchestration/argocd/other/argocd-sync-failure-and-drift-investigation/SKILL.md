...
- Someone ran `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) edit`/`[kubectl](../../../kubernetes/other/kubectl/SKILL.md) patch`/`[kubectl](../../../kubernetes/other/kubectl/SKILL.md) scale` directly
...
- `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)` access to the destination cluster for direct inspection —
  the `[argocd](../argocd/SKILL.md)` CLI's view is derived from the same live state but is
  sometimes stale relative to a fresh `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get`.
...
- For hook failures: `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) logs`/`[kubectl](../../../kubernetes/other/kubectl/SKILL.md) describe pod` access in the
...
1. **Start from the `Application`'s own status, not just the UI's
   ...
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get application payments-api-prod -n [argocd](../argocd/SKILL.md) -o jsonpath='{.status.conditions}'
   ```
...
2. **Get the resource-level diff, not the app-level summary.** `[argocd](../argocd/SKILL.md)
   ...
   spanning many resources (usually a bad merge, a botched [Kustomize](../../../kustomize/other/kustomize/SKILL.md)
...
4. **Distinguish manual drift from genuine Git/cluster divergence** —
   ...
   - If the field *is* in Git and the live value differs, check
     `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get <resource> -o yaml` for
     `metadata.managedFields` (`[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get deploy payments-api -n
     ...
     or a race), while `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)-edit`/`[kubectl](../../../kubernetes/other/kubectl/SKILL.md)-patch`/a human's kubeconfig
...
5. **Investigate a stuck/failing sync hook by going straight to the
   ...
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get jobs -n payments-prod -l [argocd](../argocd/SKILL.md).argoproj.io/hook-type
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) logs job/payments-api-migrate -n payments-prod
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) describe pod -l job-name=payments-api-migrate -n payments-prod
   ```
   ...
   requests/limits or a missing image pull secret — check `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)
...
6. **If a hook is stuck because a previous failed run's Job object still
   ...
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get jobs -n payments-prod -l [argocd](../argocd/SKILL.md).argoproj.io/hook-type=PreSync
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) delete job payments-api-migrate -n payments-prod   # only if hook-delete-policy is missing/insufficient
...
7. **When a `Degraded`/stuck `Progressing` resource is a custom CRD**,
   ...
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get application payments-api-prod -n [argocd](../argocd/SKILL.md) -o jsonpath='{.status.resources[?(@.kind=="DatabaseClaim")]}'
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get databaseclaim payments-api-db -n payments-prod -o yaml
   ```
   If `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)` shows the resource genuinely healthy but Argo CD reports
...
- **Symptom:** A `PreSync` hook Job is stuck `Running` far past when it
  ...
  **Fix:** Check `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) describe pod` for the hook — a common cause is
...
3. Check whether `replicas` is even meaningfully declared in Git as `3`,
   ...
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get hpa payments-api -n payments-prod -o yaml
   ```
   ...
   real drift; it's the well-known HPA-vs-[GitOps](../../../common/gitops/gitops/SKILL.md) interaction.
