...
# Complete [GitOps](../../../common/gitops/gitops/SKILL.md)/Argo CD Deployment on EKS, From Scratch
...
policy) is identical to any other [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) target and is covered in depth
by the linked skills, not repeated here.

## When to use

- Starting from a bare, already-provisioned EKS cluster with nothing
  [GitOps](../../../common/gitops/gitops/SKILL.md)-related installed, and needing a working Argo CD + first deployed
...
- Standing up a second/third environment (staging, prod) for a service
  already [GitOps](../../../common/gitops/gitops/SKILL.md)-managed in one EKS cluster.
- Migrating a team off manual `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply`/`helm upgrade` deploys to EKS
...
- An EKS cluster already provisioned and reachable via `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)` — cluster
...
- `helm` ≥ 3.12, `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)`, `eksctl` or Terraform's `aws` provider, and
...
- A Git repository already reachable from the cluster's network path to
  hold the [GitOps](../../../common/gitops/gitops/SKILL.md) config (Application manifests, [Kustomize](../../../kustomize/other/kustomize/SKILL.md)/Helm overlays).
...
[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get pods -n [argocd](../argocd/SKILL.md)   # wait for all Deployments/StatefulSet Ready
```
...
This is the AWS-specific step with no equivalent on a plain [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)
...
2. **`[argocd](../argocd/SKILL.md)-repo-server` (and any workload pulling private images) needs
   ECR pull access without a static [Docker](../../../docker/other/docker/SKILL.md) config secret.** IRSA-bind the
   ...
   eksctl create iamserviceaccount \
     ...
     --override-existing-serviceaccounts --approve
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) patch deployment [argocd](../argocd/SKILL.md)-repo-server -n [argocd](../argocd/SKILL.md) --type=json \
...
[kubectl](../../../kubernetes/other/kubectl/SKILL.md) exec -n [argocd](../argocd/SKILL.md) deploy/[argocd](../argocd/SKILL.md)-repo-server -- aws sts get-caller-identity
```
...
[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -f [argocd](../argocd/SKILL.md)-server-ingress.yaml
