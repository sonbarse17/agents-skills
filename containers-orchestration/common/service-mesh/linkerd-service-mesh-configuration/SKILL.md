...
- A [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) cluster meeting Linkerd's minimum supported [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)
  version for your chosen Linkerd release (check the release's install
  docs; Linkerd tracks a rolling window of recent [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) minor
...
1. **Install CRDs, then the control plane, as separate steps**, and
   ...
   ```bash
   linkerd install --crds | [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -f -
   linkerd install | [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -f -
   linkerd check
   ```
...
2. **Install the `viz` extension** if you need metrics, the dashboard, or
   ...
   ```bash
   linkerd viz install | [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -f -
   linkerd viz check
...
3. **Provide your own trust anchor and issuer certs in production**
   ...
   linkerd install \
     ...
     | [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -f -
   ```
...
4. **Enable proxy injection per namespace**, not cluster-wide, so rollout
   ...
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) annotate namespace payments linkerd.io/inject=enabled
   ```
...
- **Symptom:** A namespace was annotated `linkerd.io/inject=enabled` but
  a `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get pods` shows only one container per pod, no
  `linkerd-proxy`.
  **Fix:** The annotation only affects pods created *after* it's applied,
  ...
  (`[kubectl](../../../kubernetes/other/kubectl/SKILL.md) rollout restart deployment -n <ns>`) to actually get injected.
