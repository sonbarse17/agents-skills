...
- Istio ≥ 1.21 (ambient mesh mode — sidecar-less data plane via ztunnel
  ...
  [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) version range in Istio's release notes before installing —
  Istio typically supports the last 3–4 [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) minor versions.
- `istioctl` matching (or within one minor version of) the control
  plane version being installed.
- A [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) cluster with a CNI that permits Istio's `istio-init`/CNI
  ...
  clusters, like [OpenShift](../../../openshift/other/openshift/SKILL.md), that restrict privileged init containers).
...
- If also running an Ingress controller, decide up front whether Istio's
  ...
  [ingress-nginx-configuration](../[ingress-nginx-configuration](../ingress-nginx-configuration/SKILL.md)/SKILL.md)).
...
2. **Verify the install** before enabling injection anywhere:
   ...
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get pods -n istio-system
   ```

3. **Enable sidecar injection per namespace, not cluster-wide**, so
   rollout is deliberate and reversible per team/service:
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) label namespace payments istio-injection=enabled
   ```
   ...
   running pods:
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) rollout restart deployment -n payments
   ```
...
- If most traffic is north-south through one Ingress and mTLS/circuit-
  ...
  justified versus [ingress-nginx-configuration](../[ingress-nginx-configuration](../ingress-nginx-configuration/SKILL.md)/SKILL.md)
...
- **Symptom:** Sidecar injection was enabled on a namespace but existing
  ...
  **Fix:** The `istio-injection=enabled` label only affects *new* pod
  ...
  (`[kubectl](../../../kubernetes/other/kubectl/SKILL.md) rollout restart deployment -n <ns>`) to pick up the sidecar.
...
```bash
istioctl analyze -n payments
[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -f canary.yaml
watch -n5 '[kubectl](../../../kubernetes/other/kubectl/SKILL.md) exec deploy/payments-api-v2 -n payments -c istio-proxy -- pilot-agent request GET stats | grep 5xx'
```
