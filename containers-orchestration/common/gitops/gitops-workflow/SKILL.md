...
[GitOps](../gitops/SKILL.md) replaces "push-based" deployment (a pipeline runs `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply` or
...
- Introducing [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) deployment automation for a new cluster or
  migrating off imperative `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply` / `helm upgrade` pipelines.
- Setting up Argo CD or Flux and deciding repo structure
  (app-of-apps, mono-repo vs. multi-repo, per-environment overlays).
- Diagnosing configuration drift (someone `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) edit`'d a resource
...
- A running [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) cluster (or clusters) and a [GitOps](../gitops/SKILL.md) operator
...
- [Kustomize](../../../kustomize/other/kustomize/SKILL.md) ≥ 5.0 or Helm ≥ 3.x for templating environment-specific values,
...
- A secrets strategy decided before rollout: Sealed Secrets, SOPS, or an
  external secrets operator pulling from a [vault](../../../../Security/cryptography-secrets/vault/SKILL.md) (AWS Secrets Manager,
  HashiCorp [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md), Azure Key [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md)) — plaintext [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) `Secret` objects
...
1. **Choose repo topology.** For most teams: one app repo per service
   ...
   environments/config repo containing [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) manifests organized by
...
2. **Define the desired state declaratively with an overlay per
   environment** ([Kustomize](../../../kustomize/other/kustomize/SKILL.md) example):
   ```yaml
...
6. **Handle secrets out-of-band of plaintext Git.** With Sealed Secrets,
   ...
   (`ExternalSecret` CR pointing at a [vault](../../../../Security/cryptography-secrets/vault/SKILL.md) path), never the value.
...
7. **Roll back by reverting Git, not by manual cluster surgery.**
   ...
   automatically. Verify with `[argocd](../../../argocd/other/argocd/SKILL.md) app get payments-api-prod` or
...
8. **Monitor for drift and unhealthy sync state**, not just "did the PR
   merge." `[argocd](../../../argocd/other/argocd/SKILL.md) app diff <app>` / `flux diff kustomization` shows
...
- **Symptom:** Someone runs `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) edit deployment` directly against a
  ...
  **Fix:** This is [GitOps](../gitops/SKILL.md) working as designed — communicate the policy
  clearly (no direct `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) edit`/`apply` in [GitOps](../gitops/SKILL.md)-managed namespaces)
...
4. Because prod's `Application` has `automated` sync disabled, the change
   ...
   `[argocd](../../../argocd/other/argocd/SKILL.md) app sync payments-api-prod` (or CI triggers it via
   `[argocd](../../../argocd/other/argocd/SKILL.md) app sync` in a manual-approval job) — giving a deliberate,
