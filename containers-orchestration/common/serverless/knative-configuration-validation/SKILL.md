...
`[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply` on a Knative `Service` almost always succeeds even when
...
- Adding a CI or admission-time gate specific to Knative resources,
  distinct from generic [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) manifest linting.

## Prerequisites & environment

- `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)` with read access to `services.serving.knative.dev` and
...
5. **Validate revision-level IAM/RBAC and network policy scope**, if
   ...
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get ksvc checkout-api -n prod -o jsonpath='{.spec.template.spec.serviceAccountName}'
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get serviceaccount <name> -n prod -o yaml
   ```
...
6. **Wire these checks into CI as a required gate**, using the cluster's
   ...
   REQ=$([kubectl](../../../kubernetes/other/kubectl/SKILL.md) get ksvc "$SVC" -n "$NS" -o jsonpath='{.spec.template.spec.containers[0].resources.requests}')
   if [ -z "$REQ" ]; then
...
- Run these checks as a required CI/admission-time gate on every
  ...
  outside [GitOps](../../gitops/gitops/SKILL.md), and drift is exactly what re-validation catches.
...
- Cross-check `resources.requests`/`limits` against actual observed
  usage (via `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) top pod` or cluster metrics) periodically, not
...
- **Symptom:** A revision appears healthy in `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get pods` but
  ...
  **Fix:** `resources.requests`/`limits` were unset or under-sized;
  ...
  sized against measured usage, not left as [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) defaults (no
  limit at all).
