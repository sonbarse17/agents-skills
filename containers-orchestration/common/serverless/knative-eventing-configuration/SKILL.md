...
- Knative Eventing installed on the cluster (separate installation from
  ...
  `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get brokers.eventing.knative.dev -A -o wide` and confirm the
  backing implementation before assuming delivery guarantees).
- `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)` access to `brokers.eventing.knative.dev`,
...
- Write specific, narrow Trigger filters per subscriber rather than one
  ...
  topology visible in the [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) resources themselves, not buried
  in application code.
...
- **Symptom:** A Trigger's subscriber never receives any events, even
  ...
  **Fix:** The Trigger's `filter.attributes` likely doesn't match the
  ...
  `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) logs` on the Broker's ingress or a debug subscriber with no
