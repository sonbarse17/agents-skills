...
- Reviewing a [GitOps](../../gitops/gitops/SKILL.md) PR that touches `dapr.io/v1alpha1` `Component` or
...
- `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)` read access to `components.dapr.io` and
...
- Awareness of which secret store/reference mechanism the organization
  standardizes on ([Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) Secrets, [Vault](../../../../Software_Engineering_and_Other/Miscellaneous/vault/SKILL.md), a cloud KMS-backed
...
1. **Validate every component has non-empty `scopes`.** An unscoped
   ...
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get components.dapr.io -n prod -o json | \
...
2. **Validate no component embeds a secret-shaped value directly under
   ...
   [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) `Secret` or the organization's standard secret store
...
3. **Validate a `Resiliency` policy exists for components/apps that
   ...
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get resiliencies.dapr.io -n prod -o json | \
...
6. **Wire these checks into CI as a required gate**:
   ...
   set -euo pipefail
   NS=prod
   UNSCOPED=$([kubectl](../../../kubernetes/other/kubectl/SKILL.md) get components.dapr.io -n "$NS" -o json | \
     jq -r '.items[] | select((.scopes // []) | length == 0) | .metadata.name')
   if [ -n "$UNSCOPED" ]; then
     echo "FAIL: unscoped Dapr component(s) found: $UNSCOPED"
     exit 1
   fi
   INLINE_SECRETS=$([kubectl](../../../kubernetes/other/kubectl/SKILL.md) get components.dapr.io -n "$NS" -o json | \
...
- Run these checks as a required CI/admission gate on every change
  ...
  outside [GitOps](../../gitops/gitops/SKILL.md), and periodic re-validation catches that drift.
...
`orders-redis-secret` [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) Secret and rotate the credential that
