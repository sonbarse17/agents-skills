...
worst time. Chaos engineering moves that first real test from an [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) to a planned
...
An experiment that can take down more than you intended is not an experiment, it's an [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md)
...
day — with the on-call team, the IC, and the actual [alerting](../../../../observability-monitoring-logging/common/alerting/alerting/SKILL.md) and paging path all live — tests
the system *and* the humans and process meant to respond to it. That's usually where the more
valuable findings are: alerts that don't fire, [runbooks](../../../../observability-monitoring-logging/common/incident-detection/runbooks/SKILL.md) that are stale, an on-call engineer
who didn't know this dependency existed.

- **Page for real** — trigger the actual [alerting](../../../../observability-monitoring-logging/common/alerting/alerting/SKILL.md) path, not a simulated one, to test whether
  it works.
- **Have the responder use the real [runbook](../../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md)** — see `[runbooks](../../../../observability-monitoring-logging/common/incident-detection/runbooks/SKILL.md)` — and note where it was wrong
  or missing.
- **Debrief immediately after**, while the experience is fresh, and file findings the same way
  as an [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) postmortem.
