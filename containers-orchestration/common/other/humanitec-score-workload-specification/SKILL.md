...
Postgres RDS instance, a per-namespace CloudSQL database, or a local [Docker](../../../docker/other/docker/SKILL.md)
...
[Docker](../../../docker/other/docker/SKILL.md) Compose, `score-k8s` for plain [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md), or Humanitec's Platform
...
- Migrating a service's deployment config off environment-specific
  [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) manifests or Helm values files and onto a portable Score spec.
...
- For local resolution: `score-compose` ([Docker](../../../docker/other/docker/SKILL.md) Compose target) or
  `score-k8s` (plain [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) target), both open-source CLIs maintained
...
3. **Validate and run locally with `score-compose`** before anything
   ...
   [docker](../../../docker/other/docker/SKILL.md) compose -f compose.yaml up
   ```
...
4. **Run the same file against plain [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) with `score-k8s`** for a
   team not using Humanitec at all:
   ```bash
   score-k8s generate score.yaml -o manifests.yaml
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -f manifests.yaml -n checkout
   ```
...
app team's point of view) in a developer's local [Docker](../../../docker/other/docker/SKILL.md) environment, a
...
Local dev: `score-compose generate score.yaml -o compose.yaml && [docker](../../../docker/other/docker/SKILL.md)
