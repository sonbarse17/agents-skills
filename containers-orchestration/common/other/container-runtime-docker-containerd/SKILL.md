...
# Container Runtime: [Docker](../../../docker/other/docker/SKILL.md) Engine, containerd, CRI-O
...
process entirely and below [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)'s own scheduling logic. Since
[Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) 1.24 removed dockershim, every node runs a CRI-compliant
runtime directly (containerd or CRI-O), and [Docker](../../../docker/other/docker/SKILL.md) Engine itself is no
...
- Choosing the CRI runtime for new [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) nodes (containerd vs.
  CRI-O) post-dockershim.
- Migrating an existing cluster off [Docker](../../../docker/other/docker/SKILL.md) Engine/dockershim to a
  CRI-native runtime.
...
- containerd ≥ 1.7 (bundled with most current [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) node images) or
  CRI-O matched exactly to the [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) minor version being run —
  CRI-O version-locks to [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) (CRI-O 1.30.x targets [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)
  1.30) and does not support mixing minor versions the way containerd's
  versioning is decoupled from [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) releases.
- `crictl` (the CRI-level equivalent of `[docker](../../../docker/other/docker/SKILL.md)`/`nerdctl` CLI) for
...
- Confirm the kubelet's configured `cgroupDriver` (`systemd` is the
  ...
  bootstrap failures.
- [Docker](../../../docker/other/docker/SKILL.md) Engine itself, if used at all in this context, is for local
  development/CI image builds (`[docker](../../../docker/other/docker/SKILL.md) build`), not as the [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)
  node runtime — see
  [container-build-and-release](../../../devops/skills/[container-build-and-release](../container-build-and-release/SKILL.md)/SKILL.md)
  for that usage.
...
1. **Choose containerd vs. CRI-O deliberately**:
   - **containerd**: the most widely used CRI runtime, backs [Docker](../../../docker/other/docker/SKILL.md)
     Engine itself under the hood, has the broadest ecosystem/tooling
     support (`nerdctl` for a [Docker](../../../docker/other/docker/SKILL.md)-CLI-like local experience), and is
     the default on most managed [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) node images (EKS, GKE, AKS
     all ship containerd-based nodes).
   - **CRI-O**: purpose-built for [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) only (no independent
     daemon use case outside a CRI context), version-locked to the
     [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) release, and the default on [OpenShift](../../../openshift/other/openshift/SKILL.md)/ROSA nodes — see
     [openshift-and-rosa-platform](../[openshift-and-rosa-platform](../[openshift](../openshift/SKILL.md)-and-rosa-platform/SKILL.md)/SKILL.md).
   For a self-managed cluster with no [OpenShift](../../../openshift/other/openshift/SKILL.md)/Red Hat requirement,
   containerd is the more broadly supported default; choose CRI-O
   primarily when the platform ([OpenShift](../../../openshift/other/openshift/SKILL.md)) mandates it or the team
   specifically wants its tighter [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-only scope.
...
3. **Configure a registry mirror/pull-through cache** to reduce
   ...
   ```toml
   # /etc/containerd/config.toml
   [plugins."io.containerd.grpc.v1.cri".registry.mirrors."[docker](../../../docker/other/docker/SKILL.md).io"]
     endpoint = ["https://mirror.internal.example.com", "https://registry-1.[docker](../../../docker/other/docker/SKILL.md).io"]
   ```
   ```bash
...
4. **Configure private registry authentication** without embedding
   ...
   ```
   Prefer [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-native `imagePullSecrets` at the workload level for
...
5. **Debug directly at the CRI level** when a pod fails to start but
   `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) describe pod` doesn't give enough detail:
   ```bash
   sudo crictl ps -a
...
6. **Run rootless containers** where host attack-surface reduction is
   the priority (most relevant for standalone [Docker](../../../docker/other/docker/SKILL.md)/containerd use —
   e.g. CI runners — rather than typical [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) node setups, which
   usually rely on Pod-level `securityContext.runAsNonRoot` and user
   namespaces instead):
   ```bash
   # rootless [Docker](../../../docker/other/docker/SKILL.md) (dockerd-rootless-setuptool.sh, bundled with [Docker](../../../docker/other/docker/SKILL.md) ≥ 20.10)
   dockerd-rootless-setuptool.sh install
   systemctl --user start [docker](../../../docker/other/docker/SKILL.md)
   ```
   ```bash
   ...
   ```
   [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)' own user-namespace support (stable since 1.30 via
...
8. **Migrate a cluster off [Docker](../../../docker/other/docker/SKILL.md)/dockershim**, if any node still runs
   it: since [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) 1.24, dockershim no longer exists in-tree at
   ...
   [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) offerings handle this automatically on node group/pool
   replacement), and rejoin:
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) drain <node> --ignore-daemonsets --delete-emptydir-data
   # reprovision node with containerd-based image, then:
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) uncordon <node>
   ```
...
- Use `crictl`/`nerdctl` for node-level runtime debugging instead of
  reaching for a [Docker](../../../docker/other/docker/SKILL.md) Engine install "because it's familiar" — adding
  [Docker](../../../docker/other/docker/SKILL.md) Engine alongside the CRI runtime on a [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) node is
...
- Reserve sandboxed runtimes (gVisor, Kata) for workloads that
  ...
  `runc` everywhere.
- Prefer [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-native `imagePullSecrets`/workload-level
  ...
  node-level rootless mode matters most for standalone [Docker](../../../docker/other/docker/SKILL.md)/CI-runner
  use, not typical multi-tenant [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) nodes.
- Track CRI-O's [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-version lock-step explicitly in upgrade
...
- **Symptom:** A pod requesting a sandboxed `RuntimeClass` (gVisor/Kata)
  ...
  message.
  **Fix:** The `RuntimeClass` [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) object was created, but the
  ...
  Both the [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-level `RuntimeClass` and the node-level runtime
  installation are required; neither alone is sufficient.

- **Symptom:** After a [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) upgrade past 1.23/1.24, a legacy node
  image referencing `[docker](../../../docker/other/docker/SKILL.md)://` as the container runtime fails to
  bootstrap entirely.
  **Fix:** dockershim was removed in-tree in [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) 1.24 with no
...
- **Symptom:** Private registry pulls fail on some nodes but not others
  despite identical [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-level `imagePullSecrets`.
...
- **Symptom:** `crictl` commands fail with a connection error even
  though `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)` shows the node `Ready` and pods running fine.
...
cgroups, mirror [Docker](../../../docker/other/docker/SKILL.md) Hub through an internal cache, and add a gVisor
...
```bash
[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -f gvisor-runtimeclass.yaml -f untrusted-pod.yaml
[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get pod untrusted-job
...
sandbox boundary in addition to standard [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) RBAC/NetworkPolicy
isolation.

## Cross-references

- [lightweight-[kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-k3s](../[lightweight-[kubernetes](../kubernetes/SKILL.md)-k3s](../lightweight-[kubernetes](../kubernetes/SKILL.md)-k3s/SKILL.md)/SKILL.md) — K3s bundles and manages containerd internally; the same cgroup-driver and registry-mirror concepts apply.
- [managed-[kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-eks-aks-gke](../[managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke](../managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke/SKILL.md)/SKILL.md) — node image/runtime choices and constraints on managed [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) node groups/pools.
- [openshift-and-rosa-platform](../[openshift-and-rosa-platform](../[openshift](../openshift/SKILL.md)-and-rosa-platform/SKILL.md)/SKILL.md) — CRI-O as [OpenShift](../../../openshift/other/openshift/SKILL.md)'s default node runtime and its [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-version lock-step.
