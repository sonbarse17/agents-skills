...
- Scoping which applications can access which Dapr components in a
  shared [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) namespace or cluster.
- Deciding whether a given integration belongs in application code,
  behind a Dapr building block, or as a native [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)/cloud-provider
  integration instead.

## Prerequisites & environment

- Dapr control plane installed on the target [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) cluster (or
  Dapr's self-hosted mode for local/non-[Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) development), with a
...
- `dapr` CLI for local development/testing (`dapr run`) and
  `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)` for inspecting sidecar injection and component status on
...
2. **Define a state store component scoped to specific applications**,
   ...
   ```
   `spec.metadata[].secretKeyRef` pulls the password from a [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)
...
4. **Use service invocation for direct service-to-service calls**,
   ...
   via the configured name resolution component ([Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) DNS
   resolution is the default on [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)) and forwards the request
...
- Reference secrets via `secretKeyRef` (backed by [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) Secrets or
  a dedicated Dapr secret store component such as [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md)) in every
...
- Size and monitor the sidecar container's own resource usage and
  ...
  application makes.
- Use a dedicated Dapr secret store component ([Vault](../../../../Security/cryptography-secrets/vault/SKILL.md), cloud KMS-backed
...
- **Symptom:** A component manifest committed to source control
  ...
  **Fix:** Replace the inline `value` field with `secretKeyRef` pointing
  at a [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) `Secret` (or a Dapr secret store component), remove
