...
this repo," "what's the on-call [runbook](../../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md) for this API," and "what's the
...
ownership (an [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) responder can find the owning team and [runbook](../../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md)
...
- Adding or evaluating a Backstage plugin (CI/CD status, cost insight,
  security scorecards, on-call/[incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) data) to surface more signal in
  the portal.
...
1. **Author a `catalog-info.yaml` for an existing service** and [commit](../../../../ci-cd/common/git-workflow/commit/SKILL.md)
   ...
   metadata:
     ...
     tags:
       ...
       - java
     links:
       - url: https://[runbooks](../../../../observability-monitoring-logging/common/incident-detection/runbooks/SKILL.md).internal/payments-api
         title: On-call [runbook](../../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md)
   spec:
     type: service
...
7. **Add a plugin to surface external signal** in the portal, e.g. the
   [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) plugin (shows live pod/deployment status for a
   `Component`) or a cost-insights plugin fed by the same allocation
   data used for [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) showback:
   ```yaml
   # catalog-info.yaml annotation wiring a Component to its cluster resources
   metadata:
     annotations:
       backstage.io/[kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-id: payments-api
   ```
...
- **Symptom:** A service shows up in the catalog with `owner:
  ...
  responsible for it during an [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md).
...
4. Set up TechDocs (step 5) for those same 15 services, generating in
   CI and publishing to an S3 bucket, so each service's on-call [runbook](../../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md)
...
7. After rollout, add the [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) plugin (step 7) so each
...
- [prometheus-and-grafana-[monitoring](../../../../observability-monitoring-logging/common/monitoring-strategy/monitoring/SKILL.md)-stack](../[prometheus-and-grafana-[monitoring](../monitoring/SKILL.md)-stack](../../Containers_and_Orchestration/prometheus-and-grafana-[monitoring](../monitoring/SKILL.md)-stack/SKILL.md)/SKILL.md)
