...
- An ephemeral deploy target dedicated to template testing (a
  `template-ci` [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) namespace, or a disposable Humanitec/Score
...
1. **Scaffold a real instance from the template in CI**, driven the same
   ...
   ```yaml
   # .[github](../../../../ci-cd/github-actions/other/github/SKILL.md)/workflows/validate-golden-path.yml
...
3. **Check out the freshly generated repository and run its own build**,
   ...
   ```bash
   git clone "https://[github](../../../../ci-cd/github-actions/other/github/SKILL.md).com/acme-corp/template-ci-standard-${GITHUB_RUN_ID}.git"
   cd "template-ci-standard-${GITHUB_RUN_ID}"
   [docker](../../../docker/other/docker/SKILL.md) build -t "template-ci:${GITHUB_RUN_ID}" .
   ```
...
4. **Deploy the built artifact to an ephemeral environment**, using
   ...
   [docker](../../../docker/other/docker/SKILL.md) compose -f compose.yaml up -d
...
6. **Tear down every resource this pipeline created**, on both success and
   ...
   ```bash
   [docker](../../../docker/other/docker/SKILL.md) compose -f compose.yaml down -v
   gh repo delete "acme-corp/template-ci-standard-${GITHUB_RUN_ID}" --yes
   ```
   Run teardown in a workflow `if: always()` step ([GitHub](../../../../ci-cd/github-actions/other/github/SKILL.md) Actions) or
...
3. `[docker](../../../docker/other/docker/SKILL.md) build` on the generated repository succeeds — the Dockerfile's
   base image update is compatible with the generated `go.mod`.
4. `score-compose generate` and `[docker](../../../docker/other/docker/SKILL.md) compose up` deploy it locally in
   the CI runner; the smoke test polls `/healthz` and gets `200` within
   4 seconds.
5. Teardown removes the throwaway [GitHub](../../../../ci-cd/github-actions/other/github/SKILL.md) repo and stops the compose stack,
   `if: always()`.
