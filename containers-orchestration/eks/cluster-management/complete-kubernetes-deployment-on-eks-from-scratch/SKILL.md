...
# Complete [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) Deployment on EKS From Scratch
...
`[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get nodes` shows `Ready`) and the result is a cluster that looks
...
- Deploying a brand-new EKS cluster into an AWS account for the first
  time, from a landing zone that already exists but has no [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)
  workload on it yet.
...
- `eksctl` ≥ 0.180, `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)`, `helm` ≥ 3.14, and `awscli` v2 authenticated
...
3. **Phase 3 — CNI: VPC CNI by default.** EKS ships the AWS VPC CNI
   ...
   not an arbitrary [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) default):
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get daemonset aws-node -n kube-system
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) describe configmap amazon-vpc-cni -n kube-system
   ```
...
6. **Phase 6 — Conformance and smoke validation.** Run Sonobuoy quick
   ...
   EKS add-on, not a [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) defect.
...
[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get svc -n ingress-nginx ingress-nginx-controller   # note EXTERNAL-IP
...
[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -f route53-staging-issuer.yaml   # validate, then swap to prod issuer
...
[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get nodes
[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get pdb -A
```
...
- [managed-[kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-eks-aks-gke](../[managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke](../managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke/SKILL.md)/SKILL.md) — full detail for Phase 2's cluster/node group/IRSA provisioning.
