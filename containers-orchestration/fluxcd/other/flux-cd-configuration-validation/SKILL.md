...
`Kustomization` path, a malformed [Kustomize](../../../kustomize/other/kustomize/SKILL.md) overlay, or a `HelmRelease`
...
running the [Kustomize](../../../kustomize/other/kustomize/SKILL.md) build, diffing against the live cluster, rendering
...
- Adding a CI gate that validates [Kustomize](../../../kustomize/other/kustomize/SKILL.md)/Helm manifest changes in a
  `[gitops](../../../common/gitops/gitops/SKILL.md)-config` repo before merge, so problems surface in a PR check
  rather than after Flux reconciles them.
- Reviewing a PR that changes a `Kustomization`'s path, a [Kustomize](../../../kustomize/other/kustomize/SKILL.md)
...
- Debugging a `Kustomization` stuck `Ready: False` to determine whether
  the failure is a build-time error (bad YAML/[Kustomize](../../../kustomize/other/kustomize/SKILL.md) syntax) or a
...
- Flux CLI ≥ v2.3 with `flux diff kustomization` support, plus
  `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)` access to the target cluster context for diffing (the diff
  command needs to read live cluster state to compare against).
- `[kustomize](../../../kustomize/other/kustomize/SKILL.md)` ≥ 5.0 and `helm` ≥ 3.x installed for standalone
...
1. **Validate [Kustomize](../../../kustomize/other/kustomize/SKILL.md) build output structurally, independent of any
   ...
   ```bash
   [kustomize](../../../kustomize/other/kustomize/SKILL.md) build apps/payments-api/overlays/prod > /tmp/rendered.yaml
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply --dry-run=client -f /tmp/rendered.yaml
   ```
   `--dry-run=client` catches malformed [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) object syntax
   without needing real cluster credentials at all — run this in CI on
   every PR touching a [Kustomize](../../../kustomize/other/kustomize/SKILL.md) path, regardless of whether a live
...
2. **Validate against the API server's actual schema/admission chain**
   ...
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply --dry-run=server -f /tmp/rendered.yaml
   ```
...
4. **Render and validate `HelmRelease` values changes with `helm
   ...
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply --dry-run=server -f /tmp/redis-rendered.yaml
   ```
...
5. **Wire structural and server-dry-run checks into CI as a required PR
   ...
   - name: [Kustomize](../../../kustomize/other/kustomize/SKILL.md) build validation
     run: |
       for overlay in $(find apps -maxdepth 3 -type d -name prod); do
         [kustomize](../../../kustomize/other/kustomize/SKILL.md) build "$overlay" | [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply --dry-run=server -f -
       done
   ```
...
6. **Interpret `flux get` status output correctly** once something has
   ...
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get kustomization payments-api-prod -n flux-system -o jsonpath='{.status.conditions}'
   ```
...
7. **For a `Kustomization` stuck `Ready: False`, distinguish a
   ...
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) describe kustomization payments-api-prod -n flux-system
   ```
   A message referencing a [Kustomize](../../../kustomize/other/kustomize/SKILL.md) build error (bad patch, missing
   ...
   be syntactically fine — check `[kustomize](../../../kustomize/other/kustomize/SKILL.md)-controller`'s own RBAC and
...
8. **Validate a `prune: true` refactor specifically for unintended
   ...
   weren't intentionally removed — a renamed base file or a [Kustomize](../../../kustomize/other/kustomize/SKILL.md)
...
- Treat `[kustomize](../../../kustomize/other/kustomize/SKILL.md) build` + `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply --dry-run=server` as a
...
- Scope any CI credentials used for `flux diff`/server-side dry-run to
  ...
  a CI runner defeats the credential-scoping benefit [GitOps](../../../common/gitops/gitops/SKILL.md) is supposed
...
- **Symptom:** A PR passes `[kustomize](../../../kustomize/other/kustomize/SKILL.md) build` validation cleanly in CI,
  but the `Kustomization` still fails to reconcile once merged.
  **Fix:** `[kustomize](../../../kustomize/other/kustomize/SKILL.md) build` alone doesn't catch cluster-side apply
  ...
  (`[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply --dry-run=server`) against a representative cluster
...
- **Symptom:** `flux diff kustomization` run locally shows a clean,
  ...
  **Fix:** Confirm the diff was run against the same `GitRepository`
  ref/[commit](../../../../ci-cd/common/git-workflow/commit/SKILL.md) and the same cluster context Flux is actually reconciling
...
- **Symptom:** A refactor PR that renames a [Kustomize](../../../kustomize/other/kustomize/SKILL.md) base directory
  merges cleanly, and the next reconciliation deletes a resource that
  was still needed.
  **Fix:** This was a `prune: true` refactor validated only by
  `[kustomize](../../../kustomize/other/kustomize/SKILL.md) build` (which shows the new state) without a `flux diff`
...
1. CI's structural check runs first and passes cleanly:
   ```bash
   [kustomize](../../../kustomize/other/kustomize/SKILL.md) build apps/payments-api/overlays/prod | [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply --dry-run=server -f -
   ```
   No errors — the rename didn't break the [Kustomize](../../../kustomize/other/kustomize/SKILL.md) build itself.
...
2. A reviewer, aware that `prune: true` makes renames risky, runs the
   ...
   **deletion** and a *new* `Deployment/payments-prod/payments-api`
   ...
   identity [Kustomize](../../../kustomize/other/kustomize/SKILL.md) generates, `prune: true` would delete the
...
3. The reviewer requests a change: keep the file name stable, or add a
   `moved`-equivalent handling (in Flux/[Kustomize](../../../kustomize/other/kustomize/SKILL.md)'s case, ensuring the
...
4. Separately, the `HelmRelease` chart version bump is validated by
   ...
   ```bash
   helm template redis bitnami/redis --version 19.4.2 -f <([kubectl](../../../kubernetes/other/kubectl/SKILL.md) get helmrelease redis -n payments-prod -o jsonpath='{.spec.values}' | yq -P) \
     | [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply --dry-run=server -f -
   ```
   This passes cleanly, so only the [Kustomize](../../../kustomize/other/kustomize/SKILL.md) rename needs rework
...
- [gitops-workflow](../../../devops/skills/[gitops-workflow](../[gitops](../gitops/SKILL.md)-workflow/SKILL.md)/SKILL.md) — vendor-neutral [GitOps](../../../common/gitops/gitops/SKILL.md) principles (credential scoping, PR-gated changes) this validation workflow implements concretely for Flux.
- [infrastructure-as-code-terraform](../../../devops/skills/[infrastructure-as-code-terraform](../../Infrastructure_as_Code/[infrastructure-as-code](../../Infrastructure_as_Code/infrastructure-as-code/SKILL.md)-terraform/SKILL.md)/SKILL.md) — the `terraform plan`-review discipline this skill's `flux diff`/dry-run workflow mirrors for [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) manifests instead of cloud infrastructure.
