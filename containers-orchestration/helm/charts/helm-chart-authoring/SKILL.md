...
Helm is the de facto packaging format for distributing [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)
...
- Helm ≥ 3.14 (OCI registry support has been stable since 3.8; `helm
  ...
  [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) API for capability lookups (`Capabilities.APIVersions`).
- `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)` context pointed at a real or kind/minikube cluster for
...
4. **Centralize name/label logic in `_helpers.tpl`** so every template
   ...
   app.[kubernetes](../../../kubernetes/other/kubernetes/SKILL.md).io/name: {{ include "payments-api.name" . }}
   app.[kubernetes](../../../kubernetes/other/kubernetes/SKILL.md).io/instance: {{ .Release.Name }}
   app.[kubernetes](../../../kubernetes/other/kubernetes/SKILL.md).io/version: {{ .Chart.AppVersion | quote }}
   app.[kubernetes](../../../kubernetes/other/kubernetes/SKILL.md).io/managed-by: {{ .Release.Service }}
...
6. **Validate before every [commit](../../../../ci-cd/common/git-workflow/commit/SKILL.md)**:
   ...
   helm template payments-api ./payments-api -f values-prod.yaml | [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply --dry-run=server -f -
   ```
...
- **Symptom:** Chart renders fine with `helm template` but
  ...
  **Fix:** `helm template` only renders client-side and does not know
  ...
  Always validate with `helm template ... | [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply --dry-run=server -f -`
...
- [kustomize-overlay-management](../[kustomize-overlay-management](../../../Software_Engineering_and_Other/Frontend/[kustomize](../kustomize/SKILL.md)-overlay-management/SKILL.md)/SKILL.md) — when to patch a chart's rendered output with [Kustomize](../../../kustomize/other/kustomize/SKILL.md) instead of adding more values, or combine both.
