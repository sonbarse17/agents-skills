...
# Complete [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) Deployment on AKS From Scratch
...
- Deploying a brand-new AKS cluster into an Azure subscription for the
  ...
  Management Group hierarchy but has no [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) workload yet.
...
- An Azure DNS public zone already delegated for the domain cert-manager
  ...
  DNS-01 can succeed).
- `az` CLI ≥ 2.60 with the `aks-preview` extension current, `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)`, and
  `helm` ≥ 3.14.
...
2. **Phase 2 — Provision the AKS cluster and node pools.** Use
   ...
   az aks create --name payments-prod --resource-group payments-rg \
     --[kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-version 1.30 --enable-managed-identity \
...
[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get pods -n kube-system -l k8s-app=azure-cni
...
[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -f azure-dns-staging-issuer.yaml   # validate, then swap to prod
