...
- A Linux kernel version new enough for the eBPF features you intend to
  ...
  basic connectivity.
- The `cilium` CLI (separate from `[kubectl](../../other/kubectl/SKILL.md)`) for install, status, and
...
4. **Write `CiliumNetworkPolicy` starting from default-deny per
   ...
   [Kubernetes](../../other/kubernetes/SKILL.md) `NetworkPolicy` semantics but with L7-aware matching:
   ```yaml
...
- Default-deny per namespace with `CiliumNetworkPolicy`, then add
  ...
  discipline used for standard [Kubernetes](../../other/kubernetes/SKILL.md) `NetworkPolicy` — Cilium's L7
...
```bash
[kubectl](../../other/kubectl/SKILL.md) apply -f payments-policies.yaml
