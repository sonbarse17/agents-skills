...
gap — "[Kubernetes](../../other/kubernetes/SKILL.md) says it's assigned" vs. "the network actually routes
...
- Network access to test actual external reachability from outside the
  ...
  vantage point.
- `metallb` CLI or direct `[kubectl](../../other/kubectl/SKILL.md)` access to MetalLB CRDs
...
1. **Check pool allocation state before it becomes a blocker** — don't
   ...
   exhausted:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) -n metallb-system get ipaddresspools -o yaml
   [kubectl](../../other/kubectl/SKILL.md) get svc -A -o jsonpath='{range .items[?(@.spec.type=="LoadBalancer")]}{.metadata.namespace}/{.metadata.name}: {.status.loadBalancer.ip}{"\n"}{end}'
   ```
...
2. **Confirm every `LoadBalancer` Service actually has a real, non-empty
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get svc -A -o wide | grep LoadBalancer
   ```
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) -n metallb-system logs -l app=metallb,component=controller --tail=100
   ```

3. **For Layer2 mode, confirm which node is currently announcing each
   service IP**, and that it's a genuinely healthy node:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) -n metallb-system logs -l app=metallb,component=speaker --tail=200 | grep <service-ip>
   ```
...
4. **For BGP mode, check MetalLB's own view of peer state first, but
   don't stop there**:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) -n metallb-system logs -l app=metallb,component=speaker --tail=200 | grep -i bgp
   ```
   ...
   the network side despite [Kubernetes](../../other/kubernetes/SKILL.md) reporting the Service as having
   an `EXTERNAL-IP`.
...
7. **Simulate a node failure in a non-production validation exercise**
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) cordon <node-currently-announcing-ip>
   # for a real test: power off or [kubectl](../../other/kubectl/SKILL.md) drain --delete-emptydir-data --force the node in a staging cluster
   ```
...
- **Symptom:** `[kubectl](../../other/kubectl/SKILL.md) get svc` shows a real `EXTERNAL-IP`, but the
  ...
  **Fix:** An allocated IP is not the same as a reachable one — check
  ...
  earlier negotiation phase. [Kubernetes](../../other/kubernetes/SKILL.md)-level Service status reflects
...
- **Symptom:** A `BGPPeer` resource exists and MetalLB's logs show no
  ...
  **Fix:** Check for an ASN or peer-address mismatch between the
  ...
  peering failure, and it's invisible from the [Kubernetes](../../other/kubernetes/SKILL.md) side alone
...
- **Symptom:** Someone runs a node-failure test by hard-powering-off a
  ...
  **Fix:** This is a destructive validation action performed against
  ...
  stakeholder notice, following the same discipline as [chaos-engineering](../../../common/other/chaos-engineering/SKILL.md)
  practice in
...
1. Confirm the Service's own status looks fine (allocation succeeded):
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get svc payments-api -n payments
   # EXTERNAL-IP: 10.0.0.210   PORT(S): 443:31820/TCP
   ```

2. Check which node is currently announcing that IP in Layer2 mode:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) -n metallb-system logs -l app=metallb,component=speaker --tail=500 | grep 10.0.0.210
   ```
   Shows `worker-2` announcing the address. Check `worker-2`'s health:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) describe node worker-2 | grep -A5 Conditions
   ```
