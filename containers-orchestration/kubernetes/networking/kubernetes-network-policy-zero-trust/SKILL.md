...
# [Kubernetes](../../other/kubernetes/SKILL.md) NetworkPolicy Zero Trust

## Purpose

By default, every pod in a [Kubernetes](../../other/kubernetes/SKILL.md) cluster can reach every other pod
...
- Implementing cluster-wide, tiered policy (e.g. "no namespace may ever
  ...
  plain [Kubernetes](../../other/kubernetes/SKILL.md) `NetworkPolicy` cannot express.
...
- A CNI plugin that **enforces** `NetworkPolicy` — Calico, Cilium, and
  most managed-[Kubernetes](../../other/kubernetes/SKILL.md) default CNIs on EKS (with the AWS VPC CNI's
  ...
  `NetworkPolicy` does anything (`[kubectl](../../other/kubectl/SKILL.md)` will happily accept and
...
- Understanding of which pods are DNS servers (usually CoreDNS in
  ...
  [Kubernetes](../../other/kubernetes/SKILL.md) API access, or workloads will fail in ways that look
   ...
   spec:
     ...
     egress:
       - to:
           - namespaceSelector:
               matchLabels:
                 [kubernetes](../../other/kubernetes/SKILL.md).io/metadata.name: kube-system
             podSelector:
               matchLabels:
                 k8s-app: kube-dns
         ports:
   ...
   spec:
     ...
     ingress:
       - from:
           - namespaceSelector:
               matchLabels:
                 [kubernetes](../../other/kubernetes/SKILL.md).io/metadata.name: ingress-nginx
             podSelector:
               matchLabels:
                 app.[kubernetes](../../other/kubernetes/SKILL.md).io/name: ingress-nginx
         ports:
           - protocol: TCP
             port: 8443
     egress:
...
8. **Verify enforcement, not just that the object was accepted.**
   `[kubectl](../../other/kubectl/SKILL.md) apply` on a `NetworkPolicy` succeeds even on a CNI that
   doesn't enforce it. Confirm with a connectivity test
   (e.g. `[kubectl](../../other/kubectl/SKILL.md) exec` into a pod and attempt a curl/nc to a
   supposedly-blocked target) rather than assuming the manifest is
   doing anything.

9. **Debug unexpected blocks** by listing every `NetworkPolicy`
   selecting the affected pod (`[kubectl](../../other/kubectl/SKILL.md) get networkpolicy -n
...
- **Label namespaces and pods consistently** (e.g.
  `[kubernetes](../../other/kubernetes/SKILL.md).io/metadata.name` for namespace selection, a standard
  `app`/`app.[kubernetes](../../other/kubernetes/SKILL.md).io/name` label for pod selection) — inconsistent
...
- **Test policy changes against a real connectivity check**, not just
  `[kubectl](../../other/kubectl/SKILL.md) apply` succeeding — the object being accepted proves nothing
...
- **Symptom:** A `NetworkPolicy` is applied, `[kubectl](../../other/kubectl/SKILL.md) get networkpolicy`
...
- **Symptom:** An allow-rule was written for a specific service, but
  ...
  **Fix:** The `podSelector`/`namespaceSelector` labels in the policy
  ...
  `[kubernetes](../../other/kubernetes/SKILL.md).io/metadata.name`, which [Kubernetes](../../other/kubernetes/SKILL.md) doesn't support as a
  literal namespace-name field). Verify actual labels with
  `[kubectl](../../other/kubectl/SKILL.md) get pods --show-labels`/`[kubectl](../../other/kubectl/SKILL.md) get ns --show-labels`
...
- **Symptom:** A namespace has a strict default-deny policy, but a
  ...
  weeks.
  **Fix:** Plain [Kubernetes](../../other/kubernetes/SKILL.md) `NetworkPolicy` has no precedence model —
...
1. In a staging cluster first, apply the default-deny-all policy (step
   ...
   names via `[kubectl](../../other/kubectl/SKILL.md) exec ... -- nslookup payments-db.payments.svc`.
