...
- `[kubectl](../../other/kubectl/SKILL.md)` access to the namespaces being validated, including read
...
2. **Confirm a specific workload actually has the proxy injected**,
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get pod <pod-name> -n payments -o jsonpath='{.spec.containers[*].name}'
...
4. **Dry-run policy and routing changes against a non-production
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) apply -f authz-policy.yaml -n payments-staging
...
5. **Validate `ServiceProfile` and `TrafficSplit` resources reference
   ...
   `service:`/`host:` field is accepted by the [Kubernetes](../../other/kubernetes/SKILL.md) API (it's just
   a string) but silently does nothing at the data plane:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get svc payments-api-v1 payments-api-v2 -n payments
   [kubectl](../../other/kubectl/SKILL.md) apply --dry-run=server -f trafficsplit.yaml
   [kubectl](../../other/kubectl/SKILL.md) get trafficsplit payments-api-canary -n payments -o yaml
   ```
...
7. **Integrate into CI as a gate**, failing the pipeline on any
   ...
   `[kubectl](../../other/kubectl/SKILL.md) apply --dry-run=server`:
   ```bash
...
- **Symptom:** A `TrafficSplit` was applied and `[kubectl](../../other/kubectl/SKILL.md) get
...
- **Symptom:** A new `AuthorizationPolicy` was validated successfully by
  `[kubectl](../../other/kubectl/SKILL.md) apply --dry-run=server` (no schema errors) but breaks
...
[kubectl](../../other/kubectl/SKILL.md) get pods -n payments -l app=payments-api \
...
```bash
[kubectl](../../other/kubectl/SKILL.md) apply -f authz-policy.yaml -n payments
linkerd viz edges deployment -n payments
