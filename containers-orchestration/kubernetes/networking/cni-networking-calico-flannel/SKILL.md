...
- Flannel ≥ 0.24 or Calico ≥ 3.27 (Calico's Helm chart / Tigera
  ...
  target [Kubernetes](../../other/kubernetes/SKILL.md) version's compatibility matrix before install).
- Know your target [Kubernetes](../../other/kubernetes/SKILL.md) CNI plugin conformance requirement — most
  managed [Kubernetes](../../other/kubernetes/SKILL.md) distributions (EKS, AKS, GKE) ship their own
  default CNI (see
  [managed-[kubernetes](../../other/kubernetes/SKILL.md)-eks-aks-gke](../[managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke](../managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke/SKILL.md)/SKILL.md))
...
1. **Decide based on actual requirements, not familiarity**:
   ...
   - Choose **Calico** when `NetworkPolicy` enforcement is required
     ...
     is needed beyond what native [Kubernetes](../../other/kubernetes/SKILL.md) `NetworkPolicy` supports.
...
2. **Install Flannel** (VXLAN backend, the common default):
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) apply -f https://[github](../../../../ci-cd/github-actions/other/github/SKILL.md).com/flannel-io/flannel/releases/latest/download/kube-flannel.yml
   ```
...
3. **Install Calico** via the Tigera Operator (recommended over raw
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) create -f https://raw.githubusercontent.com/projectcalico/calico/v3.28.0/manifests/tigera-operator.yaml
   ```
   ```yaml
   ...
   ```
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) create -f custom-resources.yaml
   watch [kubectl](../../other/kubectl/SKILL.md) get tigerastatus
   ```
...
4. **Choose Calico's data path deliberately**:
   ...
   - Native BGP (no encapsulation): lower per-packet overhead and pod
     ...
     an infrastructure decision, not just a [Kubernetes](../../other/kubernetes/SKILL.md) config change.
...
7. **Validate cross-node pod connectivity directly** when debugging,
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) run net-test --image=busybox:1.36 --rm -it --restart=Never -- \
     wget -qO- --timeout=2 <target-pod-ip>:<port>
   ```
   Combine with `[kubectl](../../other/kubectl/SKILL.md) get pods -o wide` to confirm which nodes the
...
- **Symptom:** `NetworkPolicy` resources apply without error, but
  ...
  **Fix:** Confirm the CNI actually enforces `NetworkPolicy` — plain
  Flannel accepts the resource via the [Kubernetes](../../other/kubernetes/SKILL.md) API (any CNI can, since
  ...
  `[kubectl](../../other/kubectl/SKILL.md) get pods -n kube-system` for a policy-enforcing component
...
- **Symptom:** New nodes join the cluster and stay `NotReady`
  indefinitely with a CNI-related error in `[kubectl](../../other/kubectl/SKILL.md) describe node`.
...
- **Symptom:** Pods on the same node can reach each other, but
  ...
  **Fix:** The overlay's encapsulation traffic (VXLAN UDP/4789 for
  ...
  an infrastructure-firewall problem, not a [Kubernetes](../../other/kubernetes/SKILL.md)-level
  misconfiguration, and is invisible to `[kubectl](../../other/kubectl/SKILL.md)` since the API server
...
- **Symptom:** BGP-mode Calico shows nodes `Ready` in [Kubernetes](../../other/kubernetes/SKILL.md) but
  ...
  **Fix:** BGP session isn't forming — check that nothing between nodes
  ...
  [Kubernetes](../../other/kubernetes/SKILL.md)-level node readiness does not reflect BGP peering health at
  all.
...
[kubectl](../../other/kubectl/SKILL.md) create -f custom-resources.yaml
watch [kubectl](../../other/kubectl/SKILL.md) get tigerastatus
[kubectl](../../other/kubectl/SKILL.md) get nodes    # confirm all nodes reach Ready
```
...
```bash
[kubectl](../../other/kubectl/SKILL.md) apply -f netpol.yaml
# negative test: should time out
[kubectl](../../other/kubectl/SKILL.md) run probe --image=busybox:1.36 --rm -it --restart=Never -n payments -- \
  wget -qO- --timeout=2 payments-api:8080
# positive test from an allowed pod: should succeed
[kubectl](../../other/kubectl/SKILL.md) exec -n payments deploy/frontend -- wget -qO- --timeout=2 payments-api:8080
```
...
- [managed-[kubernetes](../../other/kubernetes/SKILL.md)-eks-aks-gke](../[managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke](../managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke/SKILL.md)/SKILL.md) — CNI choices and constraints on managed clusters, where the default CNI is often fixed or cloud-specific.
