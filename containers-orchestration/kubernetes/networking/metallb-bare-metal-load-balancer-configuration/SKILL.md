...
- A [Kubernetes](../../other/kubernetes/SKILL.md) cluster ≥ 1.26 with no existing cloud-provider
...
1. **Install MetalLB**:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) apply -f https://raw.githubusercontent.com/metallb/metallb/v0.14.8/config/manifests/metallb-native.yaml
   [kubectl](../../other/kubectl/SKILL.md) -n metallb-system get pods   # controller + speaker DaemonSet, wait for Running
   ```
...
2. **If kube-proxy runs in IPVS mode, enable `strictARP`** before
   ...
   unreachable:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get configmap kube-proxy -n kube-system -o yaml | \
     sed -e 's/strictARP: false/strictARP: true/' | \
     [kubectl](../../other/kubectl/SKILL.md) apply -f - -n kube-system
   ```
...
3. **Choose Layer2 or BGP mode based on the actual network**, not
   ...
   - **BGP** when upstream routers support BGP peering and either
     ...
     [Kubernetes](../../other/kubernetes/SKILL.md)-only configuration change.
...
- Reserve the IP range given to MetalLB explicitly with whoever manages
  ...
  conflicts that look like a [Kubernetes](../../other/kubernetes/SKILL.md) bug.
...
- **Symptom:** A `LoadBalancer` Service stays `EXTERNAL-IP: <pending>`
  ...
  **Fix:** Check that an `IPAddressPool` actually exists with
  ...
  (`[kubectl](../../other/kubectl/SKILL.md) -n metallb-system get ipaddresspools -o yaml` and compare
...
- **Symptom:** Someone deletes an `IPAddressPool` that still has
  ...
  **Fix:** Deleting a pool still in use by live Services leaves those
  ...
  traffic — check `[kubectl](../../other/kubectl/SKILL.md) get svc -A -o jsonpath='{range .items[?(@.spec.type=="LoadBalancer")]}{.metadata.namespace}/{.metadata.name}: {.status.loadBalancer.ip}{"\n"}{end}'`
...
```bash
[kubectl](../../other/kubectl/SKILL.md) apply -f https://raw.githubusercontent.com/metallb/metallb/v0.14.8/config/manifests/metallb-native.yaml
[kubectl](../../other/kubectl/SKILL.md) -n metallb-system get pods
```

Confirm kube-proxy mode and enable `strictARP` if IPVS:
```bash
[kubectl](../../other/kubectl/SKILL.md) get configmap kube-proxy -n kube-system -o jsonpath='{.data.config\.conf}' | grep mode
# mode: "ipvs"
[kubectl](../../other/kubectl/SKILL.md) get configmap kube-proxy -n kube-system -o yaml | \
  sed -e 's/strictARP: false/strictARP: true/' | [kubectl](../../other/kubectl/SKILL.md) apply -f - -n kube-system
```
...
`[kubectl](../../other/kubectl/SKILL.md) get svc payments-api -n payments` confirms
