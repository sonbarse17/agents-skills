...
A `CiliumNetworkPolicy` that applies cleanly to the [Kubernetes](../../other/kubernetes/SKILL.md) API server
...
- `[kubectl](../../other/kubectl/SKILL.md)` read access to `CiliumNetworkPolicy`,
  `CiliumClusterwideNetworkPolicy`, and `CiliumIdentity` custom
  resources in the namespaces being validated.
- Exec access into a Cilium agent pod (`[kubectl](../../other/kubectl/SKILL.md) exec -n kube-system
  ...
  [Kubernetes](../../other/kubernetes/SKILL.md) API.
...
2. **Validate policy file syntax and schema offline** before it reaches
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) apply --dry-run=server -f payments-api-ingress.yaml
   ```
...
3. **Confirm the policy's selectors actually match real, running
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get cep -n payments -l app=payments-api
   [kubectl](../../other/kubectl/SKILL.md) exec -n kube-system ds/cilium -- cilium endpoint list | grep payments
   ```
...
4. **Confirm an L7 rule is actually being enforced by the proxy**, not
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) exec -n kube-system ds/cilium -- cilium policy get
...
6. **Prove default-deny is actually in effect** before considering a
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) apply -f default-deny.yaml -f allow-checkout.yaml -n payments-staging
...
7. **Wire this into CI as a gate**, failing the pipeline on a non-zero
   ...
   allow/deny split:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) apply --dry-run=server -f ./cilium-policies/ || exit 1
   ```
...
- Treat `[kubectl](../../other/kubectl/SKILL.md) apply --dry-run=server` as necessary but not
...
- Validate every changed policy's selectors against `[kubectl](../../other/kubectl/SKILL.md) get cep`
...
- **Symptom:** A `CiliumNetworkPolicy` applies with no errors, but
  ...
  **Fix:** Almost always an `endpointSelector`/`fromEndpoints` label
  mismatch. Run `[kubectl](../../other/kubectl/SKILL.md) get cep -n <ns> -l <the exact labels used in
...
- **Symptom:** `[kubectl](../../other/kubectl/SKILL.md) apply --dry-run=server` passes cleanly in CI,
...
[kubectl](../../other/kubectl/SKILL.md) apply --dry-run=server -f payments-policies.yaml

# 3. Apply to staging and confirm selectors actually match
[kubectl](../../other/kubectl/SKILL.md) apply -f payments-policies.yaml -n payments-staging
[kubectl](../../other/kubectl/SKILL.md) get cep -n payments-staging -l app=payments-api
