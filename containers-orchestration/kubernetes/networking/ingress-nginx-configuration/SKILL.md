...
[Kubernetes](../../other/kubernetes/SKILL.md) `Ingress` resource into a running NGINX configuration. Most
...
- `ingress-nginx` ≥ 1.10 (tracks [Kubernetes](../../other/kubernetes/SKILL.md) ≥ 1.29; check the project's
  support matrix — the controller drops support for older [Kubernetes](../../other/kubernetes/SKILL.md)
  minor versions on a rolling basis, and running a controller version
  outside its supported [Kubernetes](../../other/kubernetes/SKILL.md) range risks undocumented webhook/API
  incompatibilities).
...
1. **Install via Helm**, choosing the exposure method for your
   environment:
   ```bash
   helm repo add ingress-nginx https://[kubernetes](../../other/kubernetes/SKILL.md).[github](../../../../ci-cd/github-actions/other/github/SKILL.md).io/ingress-nginx
   helm repo update
...
2. **Confirm the controller is healthy and has an external address**:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get pods -n ingress-nginx
   [kubectl](../../other/kubectl/SKILL.md) get svc -n ingress-nginx ingress-nginx-controller
   ```
...
3. **Write a basic host+path routed Ingress**:
   ...
   metadata:
     ...
     namespace: payments
     annotations:
       nginx.ingress.[kubernetes](../../other/kubernetes/SKILL.md).io/rewrite-target: /
   spec:
...
4. **Terminate TLS** by referencing a Secret of type
   `[kubernetes](../../other/kubernetes/SKILL.md).io/tls`, ideally issued by cert-manager rather than
   manually maintained:
   ```yaml
   spec:
     tls:
...
5. **Add rate limiting** per Ingress (enforced per NGINX worker process,
   ...
   metadata:
     annotations:
       nginx.ingress.[kubernetes](../../other/kubernetes/SKILL.md).io/limit-rps: "20"
       nginx.ingress.[kubernetes](../../other/kubernetes/SKILL.md).io/limit-burst-multiplier: "3"
   ```
...
6. **Adjust body size / timeouts** for endpoints outside the defaults
   ...
   metadata:
     annotations:
       nginx.ingress.[kubernetes](../../other/kubernetes/SKILL.md).io/proxy-body-size: "25m"
       nginx.ingress.[kubernetes](../../other/kubernetes/SKILL.md).io/proxy-read-timeout: "120"
       nginx.ingress.[kubernetes](../../other/kubernetes/SKILL.md).io/proxy-send-timeout: "120"
   ```
...
7. **Set up canary routing** for a simple weighted or header-based
   ...
   metadata:
     ...
     annotations:
       nginx.ingress.[kubernetes](../../other/kubernetes/SKILL.md).io/canary: "true"
       nginx.ingress.[kubernetes](../../other/kubernetes/SKILL.md).io/canary-weight: "10"
   spec:
     ingressClassName: nginx
     rules:
       - host: payments.example.com
...
8. **Validate and reload safely**: the controller watches Ingress
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) exec -n ingress-nginx deploy/ingress-nginx-controller -- \
...
9. **Check controller logs and events for merge conflicts** when
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get events -n payments --field-selector reason=Sync
   ```
...
- Treat annotation typos as a silent-failure risk: `ingress-nginx`
  ...
  resource, so a misspelled `nginx.ingress.[kubernetes](../../other/kubernetes/SKILL.md).io/` annotation
...
- **Symptom:** Ingress applies successfully but traffic still 404s or
  ...
  **Fix:** Check for another Ingress claiming the same host with a
  ...
  the host (`[kubectl](../../other/kubectl/SKILL.md) get ingress -A | grep <host>`) before debugging
  further.

- **Symptom:** Uploads or large POST bodies fail with `413 Request
  Entity Too Large`.
  **Fix:** Default `proxy-body-size` is 1m. Set
  `nginx.ingress.[kubernetes](../../other/kubernetes/SKILL.md).io/proxy-body-size` explicitly on the
...
- **Symptom:** After changing an annotation (e.g. `limit-rps`),
  ...
  **Fix:** Confirm the annotation key/namespace prefix is exactly
  correct (`nginx.ingress.[kubernetes](../../other/kubernetes/SKILL.md).io/...`) — a typo or wrong prefix
  is silently ignored rather than rejected, since [Kubernetes](../../other/kubernetes/SKILL.md) annotations
...
metadata:
  ...
  annotations:
    ...
    nginx.ingress.[kubernetes](../../other/kubernetes/SKILL.md).io/limit-rps: "20"
    nginx.ingress.[kubernetes](../../other/kubernetes/SKILL.md).io/limit-burst-multiplier: "3"
    nginx.ingress.[kubernetes](../../other/kubernetes/SKILL.md).io/proxy-body-size: "10m"
    nginx.ingress.[kubernetes](../../other/kubernetes/SKILL.md).io/rewrite-target: /$2
spec:
  ingressClassName: nginx
  tls:
    - hosts: ["payments.example.com"]
...
```bash
[kubectl](../../other/kubectl/SKILL.md) apply -f payments-api-ingress.yaml
[kubectl](../../other/kubectl/SKILL.md) get certificate -n payments payments-example-com-tls -w   # wait for Ready: True
