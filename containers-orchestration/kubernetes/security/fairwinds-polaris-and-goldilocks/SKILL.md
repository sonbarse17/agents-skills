...
Not every [Kubernetes](../../other/kubernetes/SKILL.md) configuration problem needs a custom-authored policy
...
- The user is auditing an existing cluster and wants a scored report of
  which workloads violate common [Kubernetes](../../other/kubernetes/SKILL.md) best practices, to prioritize
  remediation.
...
- A [Kubernetes](../../other/kubernetes/SKILL.md) cluster (Polaris and Goldilocks both work against any
...
- **Goldilocks** requires the **Vertical Pod Autoscaler (VPA)** CRDs and
  ...
  `[kubernetes](../../other/kubernetes/SKILL.md)/autoscaler` project) — Goldilocks is a UI/controller layer
...
- Both tools are read/report-focused by default (dashboard, CLI exit
  ...
  Polaris's admission webhook, which follows the same [audit](../../../../AI_and_Agents/Operations/common/audit/SKILL.md)-before-enforce
...
1. **Run a one-off Polaris scan** against a live cluster or local
   ...
   persistent:
   ```bash
   polaris [audit](../../../../AI_and_Agents/Operations/common/audit/SKILL.md) --format=pretty
   # or against local YAML without a cluster:
   polaris [audit](../../../../AI_and_Agents/Operations/common/audit/SKILL.md) --[audit](../../../../AI_and_Agents/Operations/common/audit/SKILL.md)-path ./k8s-manifests/ --format=json > polaris-report.json
   ```
...
3. **[Customize](../../../../cloud/azure/ai/customize/SKILL.md)/SKILL.md)/SKILL.md) the check set with a config file** — Polaris ships broad
   ...
   ```
   ```bash
   polaris [audit](../../../../AI_and_Agents/Operations/common/audit/SKILL.md) --config polaris-config.yaml --format=pretty
   ```
...
6. **Install VPA, then Goldilocks, for right-sizing** — Goldilocks needs
   ...
   [kubectl](../../other/kubectl/SKILL.md) apply -f https://[github](../../../../ci-cd/github-actions/other/github/SKILL.md).com/[kubernetes](../../other/kubernetes/SKILL.md)/autoscaler/releases/latest/download/vpa-recommender.yaml
...
7. **Opt namespaces in explicitly** — Goldilocks only generates
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) label namespace payments goldilocks.fairwinds.com/enabled=true
   ```
...
8. **Read recommendations from the dashboard or CLI**, and treat them as
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get vpa -n payments
   [kubectl](../../other/kubectl/SKILL.md) describe vpa payments-api -n payments
   ```
...
10. **Re-run both tools periodically** (weekly dashboard review is
    ...
    workloads change — this isn't a one-time [audit](../../../../AI_and_Agents/Operations/common/audit/SKILL.md).

## Best practices

- Treat Polaris as the fast first pass for well-known [Kubernetes](../../other/kubernetes/SKILL.md)
...
- Enable the Polaris admission webhook only after a warn/dashboard-only
  period, exactly like the [audit](../../../../AI_and_Agents/Operations/common/audit/SKILL.md)-before-enforce discipline for
...
- **Symptom:** The Polaris admission webhook is enabled with default
  ...
  **Fix:** Run Polaris in CLI/dashboard-only (report) mode first, add
  ...
  real cluster workloads — the same [audit](../../../../AI_and_Agents/Operations/common/audit/SKILL.md)-first discipline used for
...
- **Symptom:** A team installs Goldilocks, sees VPA objects appear, and
  ...
  **Fix:** Confirm the VPA `updateMode` is `"Off"` (Goldilocks' default) —
  ...
  `[kubectl](../../other/kubectl/SKILL.md) describe vpa` and a follow-up Polaris scan.
...
A platform team wants a fast baseline [audit](../../../../AI_and_Agents/Operations/common/audit/SKILL.md) before investing in custom
...
```bash
polaris [audit](../../../../AI_and_Agents/Operations/common/audit/SKILL.md) --[audit](../../../../AI_and_Agents/Operations/common/audit/SKILL.md)-path ./k8s/ --config polaris-config.yaml --format=json \
  > polaris-report.json
python3 -c "
import json, sys
...
```bash
[kubectl](../../other/kubectl/SKILL.md) label namespace payments goldilocks.fairwinds.com/enabled=true
[kubectl](../../other/kubectl/SKILL.md) get vpa payments-api -n payments -o jsonpath='{.status.recommendation}'
```
...
re-runs `polaris [audit](../../../../AI_and_Agents/Operations/common/audit/SKILL.md)`, and confirms `memoryLimitsMissing` and
...
- [policy-as-code-guardrails](../../../../Security/common/devsecops/SKILL.md)/skills/[policy-as-code-guardrails](../../../../Security/policy-as-code/policy-as-code/SKILL.md)-guardrails/SKILL.md)/SKILL.md) —
  the broader [audit](../../../../AI_and_Agents/Operations/common/audit/SKILL.md)-before-enforce discipline this skill's webhook
  guidance follows.
