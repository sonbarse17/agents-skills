...
- Both tools are read/report-focused by default (dashboard, CLI exit
  ...
  Polaris's admission webhook, which follows the same [audit](../../Operations/common/audit/SKILL.md)-before-enforce
...
1. **Run a one-off Polaris scan** against a live cluster or local
   ...
   persistent:
   ```bash
   polaris [audit](../../Operations/common/audit/SKILL.md) --format=pretty
   # or against local YAML without a cluster:
   polaris [audit](../../Operations/common/audit/SKILL.md) --[audit](../../Operations/common/audit/SKILL.md)-path ./k8s-manifests/ --format=json > polaris-report.json
   ```
...
3. **[Customize](../../../cloud/azure/ai/customize/SKILL.md)/SKILL.md)/SKILL.md) the check set with a config file** — Polaris ships broad
   ...
   ```
   ```bash
   polaris [audit](../../Operations/common/audit/SKILL.md) --config polaris-config.yaml --format=pretty
   ```
...
10. **Re-run both tools periodically** (weekly dashboard review is
    ...
    workloads change — this isn't a one-time [audit](../../Operations/common/audit/SKILL.md).
...
- Enable the Polaris admission webhook only after a warn/dashboard-only
  period, exactly like the [audit](../../Operations/common/audit/SKILL.md)-before-enforce discipline for
...
- **Symptom:** The Polaris admission webhook is enabled with default
  ...
  **Fix:** Run Polaris in CLI/dashboard-only (report) mode first, add
  ...
  real cluster workloads — the same [audit](../../Operations/common/audit/SKILL.md)-first discipline used for
...
A platform team wants a fast baseline [audit](../../Operations/common/audit/SKILL.md) before investing in custom
...
```bash
polaris [audit](../../Operations/common/audit/SKILL.md) --[audit](../../Operations/common/audit/SKILL.md)-path ./k8s/ --config polaris-config.yaml --format=json \
  > polaris-report.json
python3 -c "
import json, sys
...
re-runs `polaris [audit](../../Operations/common/audit/SKILL.md)`, and confirms `memoryLimitsMissing` and
...
- [policy-as-code-guardrails](../../../Security/common/devsecops/SKILL.md)/skills/[policy-as-code-guardrails](../../../Security/policy-as-code/policy-as-code/SKILL.md)-guardrails/SKILL.md)/SKILL.md) —
  the broader [audit](../../Operations/common/audit/SKILL.md)-before-enforce discipline this skill's webhook
  guidance follows.
