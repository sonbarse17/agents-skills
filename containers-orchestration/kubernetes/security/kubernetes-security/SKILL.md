...
# [Kubernetes](../../other/kubernetes/SKILL.md) Security

[Kubernetes](../../other/kubernetes/SKILL.md) ships permissive by default: any pod gets a service-account token mountable for API
...
**Done when:** `[kubectl](../../other/kubectl/SKILL.md) auth can-i --list` for the workload's service account shows nothing beyond
...
[Kubernetes](../../other/kubernetes/SKILL.md) API. That token is exactly what an attacker inside a compromised container reaches for
...
Only workloads that genuinely call the API (controllers, operators — see `[operators-and-crds](../../workloads/operators-and-crds/SKILL.md)`)
...
A [Kubernetes](../../other/kubernetes/SKILL.md) `Secret` is base64, not encrypted, by default — anyone with `get` on it or etcd access
