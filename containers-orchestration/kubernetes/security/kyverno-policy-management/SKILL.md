...
Kyverno is a [Kubernetes](../../other/kubernetes/SKILL.md)-native policy engine that expresses admission
policies as plain [Kubernetes](../../other/kubernetes/SKILL.md) YAML (`ClusterPolicy`/`Policy` custom
resources) instead of a separate policy language — a team that already
reads and writes [Kubernetes](../../other/kubernetes/SKILL.md) manifests can read and modify a Kyverno rule
...
- The user wants to write a `ClusterPolicy` or namespaced `Policy` to
  validate, mutate, or auto-generate [Kubernetes](../../other/kubernetes/SKILL.md) resources, without
  writing Rego.
...
- A [Kubernetes](../../other/kubernetes/SKILL.md) cluster (1.21+ recommended for the admission webhook
...
8. **Wire `PolicyReport` output into CI or a dashboard** so compliance
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get policyreport -A -o json | \
...
- Choose Kyverno when the target is [Kubernetes](../../other/kubernetes/SKILL.md)-only, the team is more
...
- **Symptom:** A `generate` rule with `synchronize: true` is deleted as
  ...
  **Fix:** Before deleting or disabling a `generate` policy with
  ...
  (`[kubectl](../../other/kubectl/SKILL.md) get networkpolicy -A -l kyverno.io/generated-by-policy) and
...
- **Symptom:** Team picked Kyverno for everything, including a CI-time
  ...
  simply unsupported.
  **Fix:** Kyverno's design center is [Kubernetes](../../other/kubernetes/SKILL.md) admission (and, via
...
A platform team standardizes on Kyverno (team is [Kubernetes](../../other/kubernetes/SKILL.md)-YAML-fluent,
