...
Kubewarden is a [Kubernetes](../../other/kubernetes/SKILL.md) admission policy engine, like OPA/Gatekeeper
...
- A [Kubernetes](../../other/kubernetes/SKILL.md) cluster (1.23+ recommended) with cluster-admin access to
...
1. **Install Kubewarden's controller and a policy-server** via Helm:
   ...
   [kubectl](../../other/kubectl/SKILL.md) get pods -n kubewarden   # controller + policy-server Running
   ```
...
4. **Inspect the actual `AdmissionRequest` payload** a policy receives
   ...
   Kubewarden-specific:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) create --dry-run=server -o json -f pod.yaml > sample-request.json
...
5. **Deploy in `monitor` mode and review before switching to
   `protect`**:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) apply -f allowed-registries-policy.yaml   # mode: monitor
   [kubectl](../../other/kubectl/SKILL.md) get clusteradmissionpolicy allowed-registries -o yaml \
     | grep -A5 "status:"
   ```
...
6. **Switch to `protect` (Kubewarden's enforce mode) once monitor is
   ...
   spec:
     ...
     namespaceSelector:
       matchExpressions:
         - key: [kubernetes](../../other/kubernetes/SKILL.md).io/metadata.name
           operator: NotIn
   ...
   > (`[kubectl](../../other/kubectl/SKILL.md) patch clusteradmissionpolicy <name> --type merge -p
...
request:
```bash
[kubectl](../../other/kubectl/SKILL.md) create --dry-run=server -o json -f pod-using-dockerhub.yaml > sample-request.json
kwctl run policy.wasm -r sample-request.json --settings-json \
  '{"registries": ["registry.example.internal/"]}'
```
```
{"allowed":false,"status":{"message":"image [docker](../../../docker/other/docker/SKILL.md).io/library/nginx:latest is not from an approved registry"}}
```

Deploy in `monitor` mode, review for one week:
```bash
[kubectl](../../other/kubectl/SKILL.md) apply -f require-approved-registry.yaml
[kubectl](../../other/kubectl/SKILL.md) logs -n kubewarden -l app=kubewarden-policy-server --since=168h \
...
```bash
[kubectl](../../other/kubectl/SKILL.md) patch clusteradmissionpolicy require-approved-registry \
  --type merge -p '{"spec":{"mode":"protect"}}'
```
From then on, `[kubectl](../../other/kubectl/SKILL.md) apply -f pod-using-dockerhub.yaml` is rejected at
