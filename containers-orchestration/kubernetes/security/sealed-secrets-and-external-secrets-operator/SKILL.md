...
[GitOps](../../../common/gitops/gitops/SKILL.md) workflows (Argo CD, Flux) want every cluster resource, including
secret-bearing ones, declared in git — but a native [Kubernetes](../../other/kubernetes/SKILL.md) `Secret`
is only base64-encoded, not encrypted, so committing one directly is
equivalent to committing plaintext. Two [Kubernetes](../../other/kubernetes/SKILL.md)-native controller
...
declares *which* secret to fetch from an external system ([Vault](../../../../Security/cryptography-secrets/vault/SKILL.md), AWS
Secrets Manager, Azure Key [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md), GCP Secret Manager, and others), and
...
- The user wants to [commit](../../../../ci-cd/common/git-workflow/commit/SKILL.md) a [Kubernetes](../../other/kubernetes/SKILL.md) `Secret` to a [GitOps](../../../common/gitops/gitops/SKILL.md) repo
...
- The user wants [Kubernetes](../../other/kubernetes/SKILL.md) workloads to consume secrets that live in
  [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md), AWS Secrets Manager, Azure Key [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md), or GCP Secret Manager,
...
- The user is deciding between Sealed Secrets and External Secrets
  Operator (or using both together) for a specific [GitOps](../../../common/gitops/gitops/SKILL.md) setup.
...
- The user needs to set up authentication from ESO to a backing secrets
  manager ([Vault](../../../../Security/cryptography-secrets/vault/SKILL.md) [Kubernetes](../../other/kubernetes/SKILL.md) auth, AWS IRSA, Azure Workload Identity) so
...
- A [Kubernetes](../../other/kubernetes/SKILL.md) cluster with permission to install a cluster-scoped
...
- **External Secrets Operator**: `>= 0.9` for stable
  ...
  [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md)/AWS/Azure/GCP, it does not replace them), and a workload
...
- A [GitOps](../../../common/gitops/gitops/SKILL.md) controller (Argo CD/Flux) if the goal is committing
  `SealedSecret`/`ExternalSecret` manifests to a reconciled repo, though
  neither tool requires [GitOps](../../../common/gitops/gitops/SKILL.md) specifically — both work with plain
  `[kubectl](../../other/kubectl/SKILL.md) apply` too.
...
3. **Create the plaintext Secret locally (never [commit](../../../../ci-cd/common/git-workflow/commit/SKILL.md) it)**, then
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) create secret generic db-credentials \
     --namespace payments \
...
4. **[Commit](../../../../ci-cd/common/git-workflow/commit/SKILL.md) and apply** — the controller watches for `SealedSecret`
   ...
   [kubectl](../../other/kubectl/SKILL.md) apply -f sealed-db-credentials.yaml
   [kubectl](../../other/kubectl/SKILL.md) get secret db-credentials -n payments   # controller-created, decrypted
   ```
...
6. **Back up the controller's private key** immediately after install
   and store it in a proper secrets manager/offline [vault](../../../../Security/cryptography-secrets/vault/SKILL.md) — losing it
...
8. **Define a `ClusterSecretStore`** pointing at the backing secrets
   ...
   ```
   [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md) example ([Kubernetes](../../other/kubernetes/SKILL.md) auth method):
   ```yaml
   apiVersion: external-secrets.io/v1beta1
   kind: ClusterSecretStore
   metadata:
     name: [vault](../../../../Security/cryptography-secrets/vault/SKILL.md)-backend
   spec:
     provider:
       [vault](../../../../Security/cryptography-secrets/vault/SKILL.md):
         server: "https://[vault](../../../../Security/cryptography-secrets/vault/SKILL.md).example.internal:8200"
         path: "myapp"
         version: "v2"
         auth:
           [kubernetes](../../other/kubernetes/SKILL.md):
             mountPath: "[kubernetes](../../other/kubernetes/SKILL.md)"
             role: "payments-prod"
             serviceAccountRef:
               name: external-secrets-sa
               namespace: external-secrets
   ```
   Azure Key [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md) example (Workload Identity):
   ```yaml
   apiVersion: external-secrets.io/v1beta1
   kind: ClusterSecretStore
   metadata:
   ...
   spec:
     provider:
       azurekv:
         ...
         vaultUrl: "https://example-kv.[vault](../../../../Security/cryptography-secrets/vault/SKILL.md).azure.net"
         serviceAccountRef:
           name: external-secrets-sa
           namespace: external-secrets
   ```
...
11. **Verify sync status** rather than assuming success — ESO surfaces
    ...
    ```bash
    [kubectl](../../other/kubectl/SKILL.md) get externalsecret db-credentials -n payments
...
- Authenticate ESO to its backend with workload identity (IRSA, Azure
  Workload Identity, GCP Workload Identity, or [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md)'s [Kubernetes](../../other/kubernetes/SKILL.md) auth
...
- **Symptom:** An `ExternalSecret` stays stuck showing `SecretSynced:
  ...
  **Fix:** Check the `ExternalSecret`'s status conditions and the ESO
  controller logs first (`[kubectl](../../other/kubectl/SKILL.md) describe externalsecret ... ` and
  ...
  [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md) role/policy) or the `remoteRef.key`/`property` not matching
...
A team [GitOps](../../../common/gitops/gitops/SKILL.md)-manages a `payments` namespace with Argo CD. They use
...
```bash
[kubectl](../../other/kubectl/SKILL.md) create secret tls bootstrap-tls \
  --namespace payments \
...
history, satisfying both the [GitOps](../../../common/gitops/gitops/SKILL.md) "everything in git" requirement and
...
- [secrets-management](../../../../Security/common/devsecops/SKILL.md)/skills/[secrets-management](../../../../cloud/common/security/secrets-management/SKILL.md)/SKILL.md) —
  ...
  selection ([Vault](../../../../Security/cryptography-secrets/vault/SKILL.md)/cloud/SOPS), and rotation/response workflow this
