...
[Kubernetes](../../other/kubernetes/SKILL.md)-native, declarative, continuously-reconciled process: a
...
1. **Install cert-manager** via its official Helm chart, including
   ...
   [kubectl](../../other/kubectl/SKILL.md) get pods -n cert-manager   # controller, webhook, cainjector all Running
   ```
...
6. **Wire automated certs into Istio's ingress gateway** rather than
   [Kubernetes](../../other/kubernetes/SKILL.md) Ingress, when the mesh's own gateway is the TLS
   termination point:
   ```yaml
   apiVersion: cert-manager.io/v1
   kind: Certificate
...
7. **Verify issuance and renewal**:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) describe certificate payments-example-com -n payments
   [kubectl](../../other/kubectl/SKILL.md) get certificaterequests -n payments
   [kubectl](../../other/kubectl/SKILL.md) get order,challenge -n payments   # ACME-specific intermediate resources
   ```
   ...
   time (`[kubectl](../../other/kubectl/SKILL.md) get secret <name> -o jsonpath='{.metadata.annotations}'`
...
- **Symptom:** A `Certificate` stays `Ready: False` indefinitely, and
  `[kubectl](../../other/kubectl/SKILL.md) get challenge` shows a challenge stuck in a pending/error
  state.
...
```bash
[kubectl](../../other/kubectl/SKILL.md) apply -f staging-issuer.yaml
[kubectl](../../other/kubectl/SKILL.md) patch ingress payments-api -n payments --type merge \
  -p '{"metadata":{"annotations":{"cert-manager.io/cluster-issuer":"letsencrypt-staging"}}}'
[kubectl](../../other/kubectl/SKILL.md) describe certificate payments-example-com-tls -n payments
```
...
```bash
[kubectl](../../other/kubectl/SKILL.md) apply -f prod-issuer.yaml
[kubectl](../../other/kubectl/SKILL.md) patch ingress payments-api -n payments --type merge \
  -p '{"metadata":{"annotations":{"cert-manager.io/cluster-issuer":"letsencrypt-prod"}}}'
[kubectl](../../other/kubectl/SKILL.md) delete certificate payments-example-com-tls -n payments   # forces re-issuance against prod issuer
[kubectl](../../other/kubectl/SKILL.md) describe certificate payments-example-com-tls -n payments
curl -vI https://payments.example.com 2>&1 | grep -i "issuer"
```

The final `curl` shows a certificate issued by `Let's Encrypt` (not the
staging CA), and `[kubectl](../../other/kubectl/SKILL.md) get certificate payments-example-com-tls -n
