...
- **No built-in cloud workload-identity mechanism.** Unlike EKS/AKS/GKE/
  ...
  plan that credential path (a secrets manager, HashiCorp [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md), or a
...
7. **Phase 7 — Deploy the first workload, via Helm or K3s's built-in
   ...
   for [GitOps](../../../common/gitops/gitops/SKILL.md)-style bootstrap of a chart at cluster creation time without
   ...
   the same [GitOps](../../../common/gitops/gitops/SKILL.md)-applied manifest set as the rest of the cluster's
...
- **Symptom:** A single-server K3s cluster's node goes offline for
  unrelated maintenance, and every `[kubectl](../kubectl/SKILL.md)` command (and any [GitOps](../../../common/gitops/gitops/SKILL.md)
