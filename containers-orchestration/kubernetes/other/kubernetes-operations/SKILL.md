...
[Kubernetes](../kubernetes/SKILL.md) does not execute your intent once — it continuously reconciles the cluster's actual
...
`../../../Global_References/[kubectl](../kubectl/SKILL.md)-debugging.md`.
...
- **CPU limits throttle, they don't fix**: a CPU limit that's too tight shows up as latency, not
  crashes, so check `[kubectl](../kubectl/SKILL.md) top` and cgroup throttling stats before assuming code is slow.
- **Memory limits kill**: exceeding a memory limit is an immediate SIGKILL with reason `OOMKilled`,
  visible in `[kubectl](../kubectl/SKILL.md) describe pod`.
...
`[kubectl](../kubectl/SKILL.md) describe pod` surfaces the scheduler's and kubelet's own diagnosis in the Events section —
it is almost always faster than log-diving, because it tells you what [Kubernetes](../kubernetes/SKILL.md) itself couldn't
do, not what your application printed.

```bash
[kubectl](../kubectl/SKILL.md) get pod <name> -o wide          # node, IP, restart count, phase
[kubectl](../kubectl/SKILL.md) describe pod <name>             # Events: scheduling, pulls, probe failures, OOM
[kubectl](../kubectl/SKILL.md) logs <name> --previous          # the crashed container's own output
```

- **`Pending`** with no node assigned: check Events for scheduling failures — insufficient
  resources, unsatisfied node affinity, or an unbound PVC (see `[kubernetes-storage](../[kubernetes](../kubernetes/SKILL.md)-storage/SKILL.md)`).
...
- **`ImagePullBackOff`**: registry auth or a typo'd tag — see `[container-registry](../../../docker/registry/container-registry/SKILL.md)`.
...
A rollout is not done when `[kubectl](../kubectl/SKILL.md) apply` succeeds — it's done when the new ReplicaSet is fully
available and you've confirmed rollback works before you need it under pressure. `[kubectl](../kubectl/SKILL.md) rollout
...
- **For anything riskier than a straight rolling update** — canary, blue/green — that's a delivery
  concern, not a cluster-ops one; see `[deployment-strategies](../../../../ci-cd/common/deployment/deployment-strategies/SKILL.md)` and `[progressive-delivery](../../../../ci-cd/common/deployment/progressive-delivery/SKILL.md)`.
...
- **securityContext**: run as non-root, drop `ALL` capabilities and add back only what's needed,
  ...
  belong in `[kubernetes-security](../[kubernetes](../kubernetes/SKILL.md)-security/SKILL.md)`.
