...
# [Kubernetes](../../other/kubernetes/SKILL.md) Operator Development
...
- Packaging an Operator for distribution via OLM (Operator Lifecycle
  Manager) on [OpenShift](../../../openshift/other/openshift/SKILL.md) or other OLM-enabled clusters.
...
- `[kubectl](../../other/kubectl/SKILL.md)` and a disposable cluster (`kind` or `minikube`) for manual
...
- If targeting [OpenShift](../../../openshift/other/openshift/SKILL.md)/ROSA distribution, familiarity with OLM
  bundle format — see
...
5. **Use finalizers for any resource with external side effects** that
   must be cleaned up before [Kubernetes](../../other/kubernetes/SKILL.md) garbage-collects the object
   ...
   > anything [Kubernetes](../../other/kubernetes/SKILL.md)-garbage-collection-blocked behind it — becomes
...
8. **Install the CRD and run the controller against a `kind` cluster**
   before shipping:
   ```bash
   make install   # [kubectl](../../other/kubectl/SKILL.md) apply -f config/crd
...
9. **Package for distribution.** For a plain chart-based install, wrap
   ...
   distribution ([OpenShift](../../../openshift/other/openshift/SKILL.md)), use `operator-sdk generate bundle` to
...
- Use `SetControllerReference`/owner references on every child resource
  the controller creates, so [Kubernetes](../../other/kubernetes/SKILL.md) garbage-collects children
...
- **Symptom:** Deleting the custom resource hangs indefinitely; `[kubectl](../../other/kubectl/SKILL.md)
...
- **Symptom:** Deleting the CRD itself (`[kubectl](../../other/kubectl/SKILL.md) delete crd
  ...
  **Fix:** This is expected [Kubernetes](../../other/kubernetes/SKILL.md) behavior — deleting a CRD deletes
  ...
  instances exist (`[kubectl](../../other/kubectl/SKILL.md) get <kind> -A`) or explicitly accept the
...
[kubectl](../../other/kubectl/SKILL.md) apply -f config/samples/cache_v1alpha1_rediscluster.yaml
[kubectl](../../other/kubectl/SKILL.md) get rediscluster sessions-cache -o jsonpath='{.status.conditions}'
```
...
`[kubectl](../../other/kubectl/SKILL.md) get statefulset sessions-cache` shows 3/3 ready replicas, and
`[kubectl](../../other/kubectl/SKILL.md) get statefulset sessions-cache -o jsonpath='{.metadata.ownerReferences}'`
