...
A CRD's spec is a contract every consumer (humans, [GitOps](../../../common/gitops/gitops/SKILL.md) tooling, other controllers) writes
against. Changing field names or semantics after adoption is a breaking change exactly like an
external API's, and [Kubernetes](../../other/kubernetes/SKILL.md) gives you real tools for this — use them, don't wing the schema.
...
- **Validate with OpenAPI schema (`x-[kubernetes](../../other/kubernetes/SKILL.md)-validation` / CEL rules) at the API server**, not
  just in the controller — rejecting a bad spec at `[kubectl](../../other/kubectl/SKILL.md) apply` time is far better UX than a
...
[Kubernetes](../../other/kubernetes/SKILL.md) controllers are built around the level-triggered pattern: reconcile is "given the current
...
maintained by people who will keep it working against new [Kubernetes](../../other/kubernetes/SKILL.md) versions. Writing your own is
...
**Done when:** `[kubectl](../../other/kubectl/SKILL.md) auth can-i --list --as=<operator-sa>` shows only the resource types and
verbs the reconcile loop actually touches.

## 5. Handle finalizers so deletion is as safe as creation

If the operator provisions anything outside [Kubernetes](../../other/kubernetes/SKILL.md)' own garbage collection (cloud resources,
