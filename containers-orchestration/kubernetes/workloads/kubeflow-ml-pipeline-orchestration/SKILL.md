...
[Kubernetes](../../other/kubernetes/SKILL.md)-native orchestrator where every pipeline component is a
...
Profiles on top of raw [Kubernetes](../../other/kubernetes/SKILL.md). Kubeflow's specific value is that a
pipeline step *is* a [Kubernetes](../../other/kubernetes/SKILL.md) pod with normal resource requests, node
affinity, and GPU scheduling — which makes it the natural orchestrator
choice for teams already running [Kubernetes](../../other/kubernetes/SKILL.md)-native ML infrastructure (see
...
- The user is deciding whether Kubeflow Pipelines specifically (vs. Argo
  ...
  Pipelines) is the right fit for a [Kubernetes](../../other/kubernetes/SKILL.md)-native ML platform.
...
- The user is setting up [multi-tenancy](../../../common/other/multi-tenancy/SKILL.md) (Kubeflow Profiles/namespaces) so
...
- A Kubeflow installation on [Kubernetes](../../other/kubernetes/SKILL.md) ≥ 1.24 (full Kubeflow distribution,
...
- A container registry the Kubeflow cluster can pull component images from,
  and [Docker](../../../docker/other/docker/SKILL.md)/Buildkit to build those images.
- `[kubectl](../../other/kubectl/SKILL.md)` access to the Kubeflow namespace(s) for debugging runs directly
...
- **Symptom:** A pipeline run shows "Failed" in the KFP UI with a generic
  ...
  **Fix:** Since KFP compiles to Argo Workflows underneath, inspect the
  underlying `Workflow` object directly with `[kubectl](../../other/kubectl/SKILL.md) get workflow -n
  <profile-namespace>` and `[kubectl](../../other/kubectl/SKILL.md) logs` on the specific failed pod — the
