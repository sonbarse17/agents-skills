...
A KEDA `ScaledObject` that applies cleanly with `[kubectl](../../other/kubectl/SKILL.md) apply` gives no
...
- As a gate in a CI/CD pipeline that provisions `ScaledObject`/`ScaledJob`
  resources via Helm, [Kustomize](../../../kustomize/other/kustomize/SKILL.md), or a [GitOps](../../../common/gitops/gitops/SKILL.md) operator.

## Prerequisites & environment

- `[kubectl](../../other/kubectl/SKILL.md)` access to the namespace containing the `ScaledObject`/
...
1. **Confirm the `ScaledObject`/`ScaledJob` actually produced a working
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get scaledobject order-consumer-scaledobject -n orders
   [kubectl](../../other/kubectl/SKILL.md) describe scaledobject order-consumer-scaledobject -n orders
   ```
   ...
   generated HPA:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get hpa -n orders
   ```
...
2. **Validate `TriggerAuthentication` actually resolves**, not just that
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get triggerauthentication order-consumer-kafka-auth -n orders -o yaml
   [kubectl](../../other/kubectl/SKILL.md) get secret kafka-scaler-credentials -n orders -o jsonpath='{.data}' | jq 'keys'
   ```
...
3. **Pull the KEDA operator's own logs for the specific scaler**, since a
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) -n keda logs -l app=keda-operator --tail=200 | grep -i "order-consumer\|error"
   ```
...
7. **For `ScaledJob`, validate `backoffLimit`, job history limits, and
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get scaledjob video-transcode-scaledjob -n media -o yaml | \
   ...
   `[kubectl](../../other/kubectl/SKILL.md)`/API server performance over time in a namespace with high
   job churn.

8. **If configuration is managed via Helm/[Kustomize](../../../kustomize/other/kustomize/SKILL.md)/[GitOps](../../../common/gitops/gitops/SKILL.md), run this
   ...
   helm template order-consumer ./charts/order-consumer | \
     [kubectl](../../other/kubectl/SKILL.md) apply --dry-run=server -f -
   ```
...
- Track KEDA operator version alongside the cluster's [Kubernetes](../../other/kubernetes/SKILL.md)
...
- **Symptom:** A `ScaledObject` shows `READY: True` in `[kubectl](../../other/kubectl/SKILL.md)
