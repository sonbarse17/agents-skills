...
```yaml
[kubectl](../../other/kubectl/SKILL.md) apply -f https://[github](../../../../ci-cd/github-actions/other/github/SKILL.md).com/[kubernetes](../../other/kubernetes/SKILL.md)-sigs/metrics-server/releases/latest/download/components.yaml
```
...
```bash
[kubectl](../../other/kubectl/SKILL.md) describe vpa app-vpa
# Look for:
...
```yaml
triggers:
- type: [kubernetes](../../other/kubernetes/SKILL.md)-workload
  metadata:
...
- Monitor HPA status: `[kubectl](../../other/kubectl/SKILL.md) get hpa -w` to watch target utilization.
...
- Use `[kubectl](../../other/kubectl/SKILL.md) top pods` to validate metrics-server is returning data.
- Add cluster-autoscaler.[kubernetes](../../other/kubernetes/SKILL.md).io/safe-to-evict annotation to pods that can be evicted.
...
## Troubleshooting
- HPA not scaling: check `[kubectl](../../other/kubectl/SKILL.md) describe hpa`, verify metrics-server pod is running, check custom metrics API availability.
- VPA not recommending: verify VPA admission webhook is running, check VPA recommender logs.
- Keda ScaledObject not scaling: `[kubectl](../../other/kubectl/SKILL.md) describe scaledobject`, check Keda operator logs, verify authentication with scaler.
- Cluster Autoscaler not scaling: `[kubectl](../../other/kubectl/SKILL.md) logs -n kube-system cluster-autoscaler`, check IAM permissions, verify ASG max size.
...
- Next skill: **devops-[apm-observability](../../../AI_and_Agents/Operations/apm-[observability](../../Observability_and_SecOps/observability/SKILL.md)/SKILL.md)** — [Observability](../../../../DevOps_and_Cloud/Observability_and_SecOps/observability/SKILL.md) to monitor and inform [autoscaling](../../../../Software_Engineering_and_Other/Backend/autoscaling/SKILL.md)
...
simulate_scale_up() {
  ...
  [kubectl](../../other/kubectl/SKILL.md) scale deployment "$deployment" --replicas="$replicas"

  # Watch pending pods
  while true; do
    PENDING=$([kubectl](../../other/kubectl/SKILL.md) get pods -l app="$deployment" --field-selector status.phase=Pending -o json | jq '.items | length')
    RUNNING=$([kubectl](../../other/kubectl/SKILL.md) get pods -l app="$deployment" --field-selector status.phase=Running -o json | jq '.items | length')
...
- Pre-pull **sidecar images** with `[kubernetes](../../other/kubernetes/SKILL.md)-image-puller` to speed up scale-up
...
- [Audit](../../../../AI_and_Agents/Operations/audit/SKILL.md) **scaling events** with [Kubernetes](../../other/kubernetes/SKILL.md) [audit](../../../../AI_and_Agents/Operations/audit/SKILL.md) logs — who or what triggered a scale action
