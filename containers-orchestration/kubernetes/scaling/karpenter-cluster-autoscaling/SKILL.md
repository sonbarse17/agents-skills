...
The [Kubernetes](../../other/kubernetes/SKILL.md) Cluster Autoscaler scales fixed-shape node groups
...
- Replacing or supplementing the [Kubernetes](../../other/kubernetes/SKILL.md) Cluster Autoscaler with
...
7. **Debug pods stuck `Pending`** by checking Karpenter controller logs
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) describe pod <pod-name> -n <namespace>
   [kubectl](../../other/kubectl/SKILL.md) logs -n kube-system deployment/karpenter -f
   ```
...
- **Symptom:** Pods remain `Pending` indefinitely even though Karpenter
  ...
  **Fix:** No `NodePool` in the cluster satisfies the pod's
  ...
  its `limits`. Check `[kubectl](../../other/kubectl/SKILL.md) describe pod` for scheduling events and
...
**Scenario:** A cluster runs the [Kubernetes](../../other/kubernetes/SKILL.md) Cluster Autoscaler against
