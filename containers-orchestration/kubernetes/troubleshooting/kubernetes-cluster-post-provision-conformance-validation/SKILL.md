...
# [Kubernetes](../../other/kubernetes/SKILL.md) Cluster Post-Provision Conformance Validation

## Purpose

A cluster reporting `[kubectl](../../other/kubectl/SKILL.md) get nodes` all `Ready` is not the same
claim as "this cluster behaves like [Kubernetes](../../other/kubernetes/SKILL.md) is supposed to." Whether
...
[Kubernetes](../../other/kubernetes/SKILL.md) API and core behaviors correctly, and a small set of targeted
...
and
[lightweight-[kubernetes](../../other/kubernetes/SKILL.md)-k3s](../[lightweight-[kubernetes](../kubernetes/SKILL.md)-k3s](../lightweight-[kubernetes](../kubernetes/SKILL.md)-k3s/SKILL.md)/SKILL.md).
...
- Re-validating a cluster after a [Kubernetes](../../other/kubernetes/SKILL.md) version upgrade, a CNI
  swap or migration, or a node pool/instance-type change that could
  affect scheduling or networking behavior.
- Formally certifying that a custom or vendor [Kubernetes](../../other/kubernetes/SKILL.md) distribution
...
- Cluster-admin `[kubectl](../../other/kubectl/SKILL.md)` access to the target cluster (Sonobuoy creates
...
- The `sonobuoy` CLI, version-matched to the target [Kubernetes](../../other/kubernetes/SKILL.md) version's
  supported range (check Sonobuoy's own compatibility matrix — running
  a conformance image built for a much newer/older [Kubernetes](../../other/kubernetes/SKILL.md) minor
...
7. **Layer targeted smoke tests conformance doesn't fully cover.**
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) create namespace smoke-test
   [kubectl](../../other/kubectl/SKILL.md) run dns-check -n smoke-test --image=busybox:1.36 --restart=Never --rm -it -- \
...
8. **Smoke test storage provisioning** if the cluster is expected to
   ...
   ```
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) apply -f smoke-pvc.yaml
   [kubectl](../../other/kubectl/SKILL.md) get pvc smoke-pvc -n smoke-test -w   # expect Bound, not stuck Pending
   ```
...
10. **Tear down all smoke-test resources** and record a pass/fail
    ...
    fine":
    ```bash
    [kubectl](../../other/kubectl/SKILL.md) delete namespace smoke-test
    ```
...
- Re-run the full validation after any change that could plausibly
  affect conformance or networking behavior — a [Kubernetes](../../other/kubernetes/SKILL.md) minor
...
- Version-match the Sonobuoy CLI and conformance image to the target
  [Kubernetes](../../other/kubernetes/SKILL.md) version explicitly; don't assume "latest" is always
...
- **Symptom:** `sonobuoy run` appears to hang indefinitely at "running."
  **Fix:** Check the Sonobuoy namespace's pod status directly
  (`[kubectl](../../other/kubectl/SKILL.md) get pods -n sonobuoy`) rather than only watching
...
- **Symptom:** Conformance passes cleanly, but a specific application
  ...
  **Fix:** Conformance validates the [Kubernetes](../../other/kubernetes/SKILL.md) API/behavior contract in
...
# Name:      [kubernetes](../../other/kubernetes/SKILL.md).default.svc.cluster.local
# Address:   10.96.0.1

[kubectl](../../other/kubectl/SKILL.md) apply -f smoke-pvc.yaml
[kubectl](../../other/kubectl/SKILL.md) get pvc smoke-pvc -n smoke-test
