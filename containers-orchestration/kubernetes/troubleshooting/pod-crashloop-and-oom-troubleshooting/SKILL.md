...
`CrashLoopBackOff` is not itself a root cause — it's [Kubernetes](../../other/kubernetes/SKILL.md)'
...
- A pod shows `CrashLoopBackOff` in `[kubectl](../../other/kubectl/SKILL.md) get pods` and the restart
  count keeps climbing.
- A pod's `Last State` reason is `OOMKilled`, or its previous container
  exited with code `137`.
- `[kubectl](../../other/kubectl/SKILL.md) logs <pod>` shows nothing useful right after a restart.
...
- `[kubectl](../../other/kubectl/SKILL.md)` access to the pod's namespace with permission to read pod
  logs, events, and `describe` output.
- `metrics-server` installed (for `[kubectl](../../other/kubectl/SKILL.md) top pod`) or a Prometheus/
...
1. **Confirm the restart pattern and exit code** before assuming
   anything:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get pods -n <namespace>
   [kubectl](../../other/kubectl/SKILL.md) describe pod <pod> -n <namespace>
   ```
...
2. **Always check the previous container's logs, not just current** —
   ...
   or just starting up:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) logs <pod> -n <namespace> --previous
   [kubectl](../../other/kubectl/SKILL.md) logs <pod> -n <namespace> --previous -c <container>   # multi-container pod
   ```
...
3. **Check events for corroborating detail** the `describe` summary may
   compress:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get events -n <namespace> \
...
4. **If the reason is `OOMKilled`**, determine whether it's an
   ...
   recent usage:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get pod <pod> -n <namespace> -o jsonpath='{.spec.containers[*].resources}'
   [kubectl](../../other/kubectl/SKILL.md) top pod <pod> -n <namespace> --containers
   ```
...
6. **Rule out a probe misconfiguration masquerading as a crash.** A
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get pod <pod> -n <namespace> -o jsonpath='{.spec.containers[*].livenessProbe}'
   ```
...
7. **Check node-level memory pressure** to rule out the node's own OOM
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) describe node <node> | grep -A5 Conditions
   [kubectl](../../other/kubectl/SKILL.md) top node <node>
   ```
...
8. **Debug interactively without modifying the running workload**, when
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) debug <pod> -n <namespace> -it --image=busybox:1.36 --target=<container>
   ```
   Ephemeral containers/`[kubectl](../../other/kubectl/SKILL.md) debug` share the target pod's process
...
10. **Confirm the fix** by watching the restart count stabilize, not
    just disappear once:
    ```bash
    [kubectl](../../other/kubectl/SKILL.md) get pod <pod> -n <namespace> -w
    ```
...
- Always run `[kubectl](../../other/kubectl/SKILL.md) logs --previous` before concluding "no useful
...
- Correlate a suspected OOM with node-level memory pressure, not just
  ...
  bug.
- When a crash loop appears right after a [chaos-engineering](../../../../DevOps_and_Cloud/Observability_and_SecOps/chaos-engineering/SKILL.md) experiment
...
- **Symptom:** `[kubectl](../../other/kubectl/SKILL.md) logs <pod>` shows nothing useful, or just a
...
- **Symptom:** `[kubectl](../../other/kubectl/SKILL.md) delete pod <pod>` is used repeatedly to "fix" a
  ...
  **Fix:** The controller (Deployment/StatefulSet/ReplicaSet) just
  ...
  `[kubectl](../../other/kubectl/SKILL.md) delete pod --force --grace-period=0` to speed things up,
...
```bash
[kubectl](../../other/kubectl/SKILL.md) describe pod payments-worker-6f9d4b5-2xk9p -n payments
...
```bash
[kubectl](../../other/kubectl/SKILL.md) get pod payments-worker-6f9d4b5-2xk9p -n payments \
  -o jsonpath='{.spec.containers[0].resources}'
# {"limits":{"memory":"256Mi"},"requests":{"memory":"128Mi"}}

[kubectl](../../other/kubectl/SKILL.md) top pod payments-worker-6f9d4b5-2xk9p -n payments --containers
...
```bash
[kubectl](../../other/kubectl/SKILL.md) get pod payments-worker-6f9d4b5-2xk9p -n payments -w
...
- [kubernetes-[service-connectivity](../../../../DevOps_and_Cloud/Observability_and_SecOps/service-connectivity/SKILL.md)-troubleshooting](../[kubernetes-[service-connectivity](../../Observability_and_SecOps/service-connectivity/SKILL.md)-troubleshooting](../[kubernetes](../kubernetes/SKILL.md)-[service-connectivity](../../../../DevOps_and_Cloud/Observability_and_SecOps/service-connectivity/SKILL.md)-troubleshooting/SKILL.md)/SKILL.md) — the related readiness-probe failure mode (excluded from Service endpoints without a restart) versus the liveness-probe-triggered restarts covered here.
- [capacity-planning-and-load-testing](../../../site-reliability-engineering/skills/[capacity-planning-and-load-testing](../../Observability_and_SecOps/[capacity-planning](../../Observability_and_SecOps/[capacity](../../../AI_and_Agents/Infrastructure/deploy-model/[capacity](../../Cloud_Providers/azure-skills/skills/microsoft-foundry/models/deploy-model/[capacity](../../Observability_and_SecOps/capacity/SKILL.md)/SKILL.md)/SKILL.md)-planning/SKILL.md)-and-[load-testing](../../../../DevOps_and_Cloud/Observability_and_SecOps/load-testing/SKILL.md)/SKILL.md)/SKILL.md) — [load-testing](../../../../DevOps_and_Cloud/Observability_and_SecOps/load-testing/SKILL.md) methodology for setting real, evidence-based memory requests/limits instead of guessing.
