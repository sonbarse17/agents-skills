...
operational surface into a handful of [Kubernetes](../../other/kubernetes/SKILL.md) CRDs (`CephCluster`,
`CephBlockPool`, `CephFilesystem`, `CephObjectStore`) that are
declarative and [GitOps](../../../common/gitops/gitops/SKILL.md)-friendly, at the cost of still needing to
...
- [Kubernetes](../../other/kubernetes/SKILL.md) ≥ 1.26 and Rook ≥ v1.14 (check the Rook/Ceph/[Kubernetes](../../other/kubernetes/SKILL.md)
...
- Sufficient dedicated CPU/RAM per node running OSDs — each OSD
  ...
  workload pods.
- The Rook Ceph Helm chart or raw operator manifests, plus `[kubectl](../../other/kubectl/SKILL.md)`/
...
1. **Install the Rook operator** (Helm, the recommended path):
   ...
   `[kubectl](../../other/kubectl/SKILL.md) -n rook-ceph get pods -l app=rook-ceph-operator`.
...
3. **Wait for the cluster to reach `HEALTH_OK`** before creating any
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) -n rook-ceph get cephcluster rook-ceph -o jsonpath='{.status.ceph.health}'
...
6. **Create an object store (S3-compatible RGW)** when workloads need
   ...
   access/secret key pairs stored as a [Kubernetes](../../other/kubernetes/SKILL.md) Secret Rook
...
- **Symptom:** `CephCluster` never reaches `HEALTH_OK`; OSD pods are
  ...
  **Fix:** Usually a device selection problem — check
  `[kubectl](../../other/kubectl/SKILL.md) -n rook-ceph logs -l app=rook-ceph-operator` for "no
...
- **Symptom:** PVCs using a Rook StorageClass stay `Pending` indefinitely
  with no clear error in `[kubectl](../../other/kubectl/SKILL.md) describe pvc`.
  **Fix:** Check the CSI provisioner pods
  (`[kubectl](../../other/kubectl/SKILL.md) -n rook-ceph get pods -l app=csi-rbdplugin-provisioner`) for
...
[PostgreSQL](../../../../Software_Engineering_and_Other/Databases/relational/postgresql/SKILL.md) StatefulSet and a shared CephFS volume for a CI artifact
cache.
...
- [prometheus-and-grafana-[monitoring](../../../../observability-monitoring-logging/common/monitoring-strategy/monitoring/SKILL.md)-stack](../../../[observability](../observability/SKILL.md)-and-platform-extras/skills/[prometheus-and-grafana-[monitoring](../../../../observability-monitoring-logging/common/monitoring-strategy/monitoring/SKILL.md)-stack](../../Containers_and_Orchestration/prometheus-and-grafana-[monitoring](../monitoring/SKILL.md)-stack/SKILL.md)/SKILL.md) — scraping the Ceph dashboard/mgr Prometheus exporter for ongoing [capacity](../../../AI_and_Agents/Infrastructure/deploy-model/[capacity](../../Cloud_Providers/azure-skills/skills/microsoft-foundry/models/deploy-model/[capacity](../capacity/SKILL.md)/SKILL.md)/SKILL.md) and health [monitoring](../../../../observability-monitoring-logging/common/monitoring-strategy/monitoring/SKILL.md).
