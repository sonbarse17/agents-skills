...
# Complete [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) Deployment on GKE From Scratch
...
- Deploying a brand-new GKE cluster into a GCP project for the first
  ...
  hierarchy and Shared VPC but has no [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) workload yet.
...
- `gcloud` ≥ 470.0.0, `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)`, and `helm` ≥ 3.14 authenticated against
  the target project.
...
3. **Phase 3 — CNI: VPC-native with Dataplane V2 by default.** GKE
   ...
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get pods -n kube-system -l k8s-app=cilium
   ```
...
5. **Phase 5 — cert-manager with Cloud DNS.** Install cert-manager and
   ...
   planned in Phase 1 to the cluster's [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) ServiceAccount via
...
- **Symptom:** A team installs Calico on a GKE cluster "to enable
  ...
  **Fix:** Recent GKE clusters already run Dataplane V2 (Cilium-based)
  ...
  Confirm via `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get pods -n kube-system -l k8s-app=cilium` before
...
[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -f cloud-dns-staging-issuer.yaml   # validate, then swap to prod
...
- [managed-[kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-eks-aks-gke](../[managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke](../managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke/SKILL.md)/SKILL.md) — full detail for Phase 2's cluster/node pool/Workload Identity provisioning.
