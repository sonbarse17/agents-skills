...
[Infrastructure-as-code](../infrastructure-as-code/SKILL.md) declares intent, but the running cloud account is
the only source of truth for what's actually there — and the two diverge
constantly: a console click-fix during an [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) that never made it
...
- Confirming that infrastructure provisioned by a recent change (a
  Terraform apply, a manual console change, an [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) hotfix) matches
...
2. **Classify each drifted resource before deciding what to do.**
   ...
   - **Out-of-band manual change**: someone changed a setting via
     ...
     [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md), changed an instance type by hand). Decide explicitly:
...
7. **Decide remediation deliberately, and prefer proposing rather than
   ...
   > active [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md), a manually scaled-up instance count keeping a
   > degraded service afloat). Default to [alerting](../../../../observability-monitoring-logging/common/alerting/alerting/SKILL.md) a human with the diff
...
- **Grant drift-detection tooling read-only access by default** —
  ...
  only observe cannot itself cause an [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md), and remediation should be
...
- **Symptom:** A drift-detection job auto-applies `terraform apply` to
  ...
  [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md), which the auto-apply just reverted mid-[incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md).
...
2. Classify the drift (step 2): CloudTrail shows the rule was widened
   manually three weeks ago by an on-call engineer during an [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md)
   ...
   Terraform pipeline (not a manual console fix), since the [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md)
...
7. Because this account has now had two consecutive quarters of manual
   security-group drift from [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) response, recommend adding a
