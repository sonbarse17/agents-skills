...
- Fastlane setup for iOS and [Android](../android/SKILL.md)
...
│   └── [Android](../android/SKILL.md) → Play Internal Testing (no review, 100 testers)
├── External beta (>100 users)
│   ├── iOS → TestFlight External (up to 10k testers, Beta App Review required)
│   └── [Android](../android/SKILL.md) → Open Beta (unlimited, no review)
├── Production release
│   ├── iOS → App Store via App Store Connect (App Review, 24-48h typical)
│   └── [Android](../android/SKILL.md) → Play Console Production track (review hours-days)
└── Enterprise/internal (org-owned devices only)
    ├── iOS → Apple Developer Enterprise Program + MDM
    └── [Android](../android/SKILL.md) → Managed Google Play + EMM / sideload APK
```
...
├── [GitHub](../../ci-cd/github-actions/other/github/SKILL.md) project → [GitHub](../../ci-cd/github-actions/other/github/SKILL.md) Actions (macOS-14 for iOS, ubuntu-latest for [Android](../android/SKILL.md))
...
├── Cross-platform desktop → [CircleCI](../../ci-cd/circleci/other/circleci/SKILL.md) (macOS M1 runner, [Android](../android/SKILL.md) Linux)
...
Configure Fastfile with beta and release lanes for both iOS and [Android](../android/SKILL.md) with proper build and upload steps. Include Matchfile for code signing, Appfile for metadata, and Deliverfile/Supplyfile for store metadata.

### Step 2: Configure Code Signing
Set up Fastlane Match for iOS code signing with encrypted certificates or manual signing for specific targets. Configure [Android](../android/SKILL.md) keystore with env vars.

### Step 3: Set CI/CD Pipeline
Create [GitHub](../../ci-cd/github-actions/other/github/SKILL.md) Actions workflows for iOS (macOS runner) and [Android](../android/SKILL.md) (Ubuntu runner) with secrets management. Add GitLab CI, Bitrise, or [CircleCI](../../ci-cd/circleci/other/circleci/SKILL.md) as needed.

### Step 4: Configure Distribution Tracks
Set up TestFlight Internal/External for iOS and Internal Testing/Closed Alpha/Open Beta for [Android](../android/SKILL.md). Configure phased releases and in-app updates.

### Step 5: Manage Store Metadata
Automate app metadata, screenshots, and pricing with Deliver (iOS) and Supply ([Android](../android/SKILL.md)) to eliminate manual store work.
...
- [Android](../android/SKILL.md) keystore must be base64-encoded in CI secrets, not in repo
...
- In-app updates ([Android](../android/SKILL.md)) must support both flexible and immediate flows
...
### [Android](../android/SKILL.md) Fastfile

```ruby
default_platform(:[android](../android/SKILL.md))

platform :[android](../android/SKILL.md) do
...
### Supplyfile ([Android](../android/SKILL.md) Metadata)
```ruby
...
metadata_path("metadata/[android](../android/SKILL.md)")
...
### [Android](../android/SKILL.md) Code Signing
```bash
# Generate keystore
...
### [GitHub](../../ci-cd/github-actions/other/github/SKILL.md) Actions — [Android](../android/SKILL.md)
```yaml
name: [Android](../android/SKILL.md) Beta
on:
  push:
    branches: [main]
jobs:
  build:
    ...
    steps:
      ...
      - uses: actions/upload-artifact@v4
        with:
          name: [android](../android/SKILL.md)-apk-aab
...
- [Android](../android/SKILL.md): Use `sign-APK` or `sign-aab` step
...
5. google-play-deploy ([Android](../android/SKILL.md))
...
Build number: auto-increment from git
  ...
  [Android](../android/SKILL.md) versionCode = number_of_commits
  [Android](../android/SKILL.md) versionName = manual bump
```
...
# [Android](../android/SKILL.md): derive from git [commit](../../ci-cd/common/git-workflow/commit/SKILL.md) count  
lane :bump do
  increment_version_code(
    version_code: number_of_commits
  )
...
### [Android](../android/SKILL.md) — Play Console Staged Rollout
```kotlin
...
### [Android](../android/SKILL.md) Enterprise (Managed Google Play)
...
## [Android](../android/SKILL.md) App Bundle (AAB)
...
### Fastlane Screengrab ([Android](../android/SKILL.md))
```ruby
lane :screenshots do
  screengrab(
    ...
    locales: ["en-US", "de", "ja"],
    output_directory: "screenshots/[android](../android/SKILL.md)"
  )
  supply(
...
```kotlin
// [Android](../android/SKILL.md) — Build Config Fields in build.gradle.kts
[android](../android/SKILL.md) {
  buildTypes {
    debug {
...
- `mobile/[android](../android/SKILL.md)` — [Android](../android/SKILL.md)-specific build variants
