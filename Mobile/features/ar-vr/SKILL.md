...
### Input Context
- Target platforms (iOS only, [Android](../../platforms/android/SKILL.md) only, or cross-platform)
...
```
Is the app cross-platform (iOS + [Android](../../platforms/android/SKILL.md))?
...
└── [Android](../../platforms/android/SKILL.md) only → ARCore (native Kotlin/Java)

Does the app need LiDAR features?
├── Yes → ARKit (LiDAR on iOS), ARCore Depth API (limited [Android](../../platforms/android/SKILL.md))
...
| LiDAR support | iPad Pro 2020+, iPhone 12 Pro+ | Select [Android](../../platforms/android/SKILL.md) devices |

### Step 2: Scene Setup
Configure ARSession. For world tracking: set `ARWorldTrackingConfiguration` (iOS) or `Config` with `PlaneFindingMode` ([Android](../../platforms/android/SKILL.md)). Enable auto-focus, ambient light estimation, and environment texturing. Handle session interruptions.
...
GPU instancing for repeated objects. Occlusion culling (LiDAR depth on iOS, Depth API on [Android](../../platforms/android/SKILL.md)). Limit tracked images to 10 max. Batch ARAnchor operations. LOD switching based on camera distance. Profile with Xcode SceneKit debugger or [Android](../../platforms/android/SKILL.md) GPU Inspector.
...
Running AR features on unsupported devices crashes the app. Always check `ARConfiguration.isSupported` (iOS) or `ArCoreApk.checkAvailability` ([Android](../../platforms/android/SKILL.md)) before activating AR.
...
│   → Metal (iOS) / Vulkan ([Android](../../platforms/android/SKILL.md)) — full GPU control
...
| Condition | iOS Checklist | [Android](../../platforms/android/SKILL.md) Checklist |
|-----------|---------------|-------------------|
...
- **No session state persistence**: User places furniture in AR, backgrounds app, comes back — everything is gone. Save `ARWorldMap` (iOS) or Cloud Anchors ([Android](../../platforms/android/SKILL.md)) for session restoration.
...
mobile/universal/performance for [profiling](../../../Software_Engineering_and_Other/Frontend/performance/profiling/SKILL.md) AR-specific performance metrics
