...
### Input Context
- Platform (iOS, [Android](../../platforms/android/SKILL.md), Flutter, React Native)
- Push service (APNs, FCM, or both)
- Existing notification infrastructure

### Output Artifact
A markdown document containing permission flow, token registration and refresh, payload structure (alert, data, silent), foreground/background handling, notification channels ([Android](../../platforms/android/SKILL.md)), and platform-specific considerations.
...
- [ ] Notification channels defined ([Android](../../platforms/android/SKILL.md))
...
├── [Android](../../platforms/android/SKILL.md)-only → FCM directly (required for Play Store apps)
├── iOS + [Android](../../platforms/android/SKILL.md) → FCM (unified, single server endpoint)
...
│   └── [Android](../../platforms/android/SKILL.md): notification channel with importance level
├── Silent data sync (content sync, cache refresh)
│   ├── iOS: content-available:1, no alert/badge/sound
│   ├── [Android](../../platforms/android/SKILL.md): data-only payload, no notification key
│   └── Triggers background processing (30s iOS, variable [Android](../../platforms/android/SKILL.md))
├── Rich notification (image, actions, reply)
│   ├── iOS: UNNotificationServiceExtension for media download
│   └── [Android](../../platforms/android/SKILL.md): BigPictureStyle, InboxStyle, action buttons
...
│   └── If denied: never prompt again (iOS), can re-prompt ([Android](../../platforms/android/SKILL.md))
...
└── Proactive re-prompt ([Android](../../platforms/android/SKILL.md) only)
...
### Step 4: Define Notification Channels ([Android](../../platforms/android/SKILL.md))
...
### [Android](../../platforms/android/SKILL.md) — NotificationCompat
```kotlin
class NotificationHelper(private val context: Context) {
  ...
  // [Android](../../platforms/android/SKILL.md) 13+ requires runtime permission (POST_NOTIFICATIONS)
...
class PushService {
  ...
  Future<void> setup() async {
    // Request permission (iOS) / no-op on [Android](../../platforms/android/SKILL.md)
...
class PushService {
  async setup() {
    ...
    if (enabled) {
      // Create notification channels ([Android](../../platforms/android/SKILL.md))
      ...
      messaging().onMessage(async (remoteMessage) => {
        await notifee.displayNotification({
          ...
          [android](../../platforms/android/SKILL.md): { channelId: remoteMessage.data?.channel || 'default' },
...
### [Android](../../platforms/android/SKILL.md) — FCM Token
```kotlin
...
class MyFirebaseService : FirebaseMessagingService() {
  override fun onNewToken(token: String) {
    ...
    api.registerToken(token, "[android](../../platforms/android/SKILL.md)")
  }
}
```
...
async function sendPush(userId, title, body, data = {}) {
  ...
  const message = {
    ...
    [android](../../platforms/android/SKILL.md): {
      notification: {
...
### [Android](../../platforms/android/SKILL.md) — FCM Data Payload
```json
{
  "to": "device_token",
  "priority": "high",
...
### [Android](../../platforms/android/SKILL.md) — Big Picture Style
```kotlin
...
### [Android](../../platforms/android/SKILL.md) — Direct Reply
```kotlin
...
### [Android](../../platforms/android/SKILL.md) — Group Summary
```kotlin
val groupKey = "orders"
...
Future<void> showLocalNotification({
  ...
  await _localNotificationsPlugin.show(
    ...
    title,
    body,
    NotificationDetails([android](../../platforms/android/SKILL.md): androidDetails, iOS: iosDetails),
    payload: payload,
  );
}
```
...
- **No notification channel differentiation ([Android](../../platforms/android/SKILL.md))**: All notifications same importance. Users turn off all instead of just promotions
...
- [Android](../../platforms/android/SKILL.md) notification rate limit: ~100/second per device
...
- [Android](../../platforms/android/SKILL.md): use [Firebase](../../../Software_Engineering_and_Other/Databases/nosql/firebase/SKILL.md) Console > Cloud Messaging > Send Test Message
...
- Test notification channel importance levels ([Android](../../platforms/android/SKILL.md))
...
- `mobile/[android](../../platforms/android/SKILL.md)` — FCM, notification channels, direct reply
