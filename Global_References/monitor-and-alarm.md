...
|---|---|---|---|
| `PartitionCount` per broker | Standard and Express | > recommended limit for broker size (see [Amazon MSK best practices — right-size your cluster](https://docs.aws.amazon.com/msk/latest/developerguide/bestpractices.html#bestpractices-right-size-cluster)) | Approaching partition hard limit. Scale to larger broker size or add brokers to redistribute. |
