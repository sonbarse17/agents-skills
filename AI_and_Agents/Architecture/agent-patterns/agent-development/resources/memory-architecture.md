...
**What it is:**
- Structured sections of the agent's context window
- Always visible to the agent
- Persists across all interactions (in supported frameworks)
...
- Description: guides agent on when/how to use the block
- Value: the actual data content
- Limit: character limit per block (framework-dependent)

**Access:**
- Always in context (no tool call needed to read)
- Agent can edit via memory tools (insert, replace, rewrite)
...
**What it is:**
- Vector database or document store for embedded content
...
**Access:**
- Agent must explicitly search archival memory
- Results brought into context on demand
- Agent can add new entries

**Important notes:**
- NOT auto-populated from context overflow
- Must be explicitly added by the agent
...
**Automatic behavior:**
- Messages automatically move to history when context fills
- Agent may trigger summarization when needed
...
**Reality:** Archival memory must be explicitly added by the agent
...
**Reality:** It's stored and accessible via search tools
