...
3. **Every schema change is judged on lock behaviour, not just correctness.**
   ...
   version matter (`ADD COLUMN … DEFAULT` is cheap on [PostgreSQL](../../../../Software_Engineering_and_Other/Databases/relational/postgresql/SKILL.md) 11+ and a
...
- **Migration safety** — blocking DDL under load (`ALTER TABLE` rewrites, adding
  ...
  without `CONCURRENTLY` ([PostgreSQL](../../../../Software_Engineering_and_Other/Databases/relational/postgresql/SKILL.md)) or `ALGORITHM=INPLACE`/gh-ost/pt-osc
  ([MySQL](../../../../Software_Engineering_and_Other/Databases/relational/mysql/SKILL.md)), `ACCESS EXCLUSIVE` locks queueing behind a long-running query and
