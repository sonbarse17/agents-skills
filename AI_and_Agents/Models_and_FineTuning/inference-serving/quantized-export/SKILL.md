...
The last stop after `[checkpoint-promotion](../../fine-tuning/checkpoint-promotion/SKILL.md)`
...
- Don't validate this override with MMLU or
  ...
  and graders from `[eval-harness-first](../../evaluation/eval-harness-first/SKILL.md)`, run
...
2. **Run 3–5 golden prompts through it** —
   ...
   `[eval-harness-first](../../evaluation/eval-harness-first/SKILL.md)` maintains, not a fresh
   ad hoc set.
...
- `[checkpoint-promotion](../../fine-tuning/checkpoint-promotion/SKILL.md)` — the only valid
  upstream source for this skill. A checkpoint
  without a `PROMOTE` verdict doesn't reach
  export.
- `[eval-harness-first](../../evaluation/eval-harness-first/SKILL.md)` — owns the
