...
4. **Bound every sweep explicitly before starting it.**
   ...
   > foreseeable cost/resource [incident](../../../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md), not an edge case.
...
- Use `wandb.Artifact` for anything a downstream run consumes (a
  ...
  later, matching the [audit](../../../Operations/common/audit/SKILL.md)-trail goal in
...
- **Symptom:** A Bayesian sweep is started with no `--count` and no
  ...
  **Fix:** Always launch `wandb agent` with an explicit `--count`, and
  ...
  paid compute as a preventable [incident](../../../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md), not a "just let it run" default.
