...
and [agent-tool-use-patterns](../../../Models_and_FineTuning/agent-patterns/agent-tool-use-patterns/SKILL.md)/SKILL.md); this
...
5. **Implement a bounded retry with an explicit hard ceiling, not just a
   ...
   > `max_total_calls` in response to a loop [incident](../../../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md), without fixing the
...
8. **Add session-level [alerting](../../../../DevOps_and_Cloud/observability-monitoring-logging/common/alerting/alerting/SKILL.md) on tool-call count and distinct-signature
...
- Periodically review which ceilings have been raised and why; a ceiling
  raised during an [incident](../../../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md) and never revisited is effectively a silently
  weakened safety control.

## Common pitfalls

- **Symptom:** An on-call engineer raises `MAX_ITERATIONS` from 12 to 100
  during an [incident](../../../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md) to "unblock" a stuck workflow, the workflow completes
...
- **Symptom:** A tool call fails with a generic "error" the dispatcher
  ...
  **Fix:** This is a dispatcher problem, not a model problem — return a
  ...
  [agent-tool-use-patterns](../../../Models_and_FineTuning/agent-patterns/agent-tool-use-patterns/SKILL.md)/SKILL.md)) that
...
- **Symptom:** Cost/latency [monitoring](../../../../DevOps_and_Cloud/observability-monitoring-logging/common/monitoring-strategy/monitoring/SKILL.md) only alerts on aggregate spend, so a
...
2. This is an **error-retry storm**: the tool is failing deterministically
   ...
   [agent-tool-use-patterns](../../../Models_and_FineTuning/agent-patterns/agent-tool-use-patterns/SKILL.md)/SKILL.md)), and
...
6. Verification: replaying the original request against the patched
   ...
   [agent-evaluation-and-guardrails](../../../Models_and_FineTuning/evaluation/agent-evaluation-and-guardrails/SKILL.md)/SKILL.md).
7. [Alerting](../../../../DevOps_and_Cloud/observability-monitoring-logging/common/alerting/alerting/SKILL.md): a new per-session alert on `tool_call_count > 15` for this
   agent type is added, so a future recurrence pages before running to the
   ceiling silently.

## Cross-references

- [agent-tool-use-patterns](../../../Models_and_FineTuning/agent-patterns/agent-tool-use-patterns/SKILL.md)/SKILL.md) — the design-time stall detection and tool-risk classification this skill's operational diagnosis builds on.
...
- [agent-evaluation-and-guardrails](../../../Models_and_FineTuning/evaluation/agent-evaluation-and-guardrails/SKILL.md)/SKILL.md) — turning a confirmed loop trigger into a permanent regression case.
