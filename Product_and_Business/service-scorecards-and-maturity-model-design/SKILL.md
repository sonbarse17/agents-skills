...
proxy — a team adds a stub `[runbook](../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md).md` with one sentence in it, the
"has a [runbook](../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md)" check goes green, and the service is exactly as
...
- Deciding how to combine several individual checks (has an on-call
  rotation, has a [runbook](../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md), passes a vulnerability scan, has an SLO
...
- Reviewing a scorecard rubric before or after a security or reliability
  [incident](../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) to check whether the check that should have caught the gap
...
- Investigating a service that shows a high scorecard score but had a
  preventable [incident](../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) — deciding whether a check needs to become
...
- A catalog tool with scorecard/rubric support already populated with
  ...
  see
  [no-code-idp-[service-catalog](../../containers-orchestration/common/other/service-catalog/SKILL.md)-tools-port-cortex-opslevel](../[no-code-idp-[service-catalog](../../DevOps_and_Cloud/Observability_and_SecOps/service-catalog/SKILL.md)-tools-port-cortex-opslevel](../../DevOps_and_Cloud/Observability_and_SecOps/no-code-idp-[service-catalog](../../DevOps_and_Cloud/Observability_and_SecOps/service-catalog/SKILL.md)-tools-port-cortex-opslevel/SKILL.md)/SKILL.md)
...
- Machine-queryable data sources for each check, not just self-reported
  ...
  security checks, an SLO/[observability](../../observability-monitoring-logging/common/fundamentals/observability/SKILL.md) tool's config for latency/error
...
2. **Prefer machine-verifiable checks over self-attestation wherever a
   ...
   [runbook](../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md): yes/no") is the single easiest thing to game; a check that
   queries an actual system is not:
   ```yaml
   # Cortex Scorecard rule — self-attested (weak)
   - title: Has a [runbook](../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md)
     expression: "entity.hasMetadataField('has_runbook')"   # team just sets this true

   # Cortex Scorecard rule — machine-verified (strong)
   - title: Has a [runbook](../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md)
     expression: "entity.hasDocument('[runbook](../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md)') && entity.documents('[runbook](../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md)').last_updated_days_ago() < 180"
   ```
   The stronger version checks both that a [runbook](../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md) document actually
...
- **Symptom:** A team's "has a [runbook](../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md)" check goes green the same day a
  one-line stub file (`# [Runbook](../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md)\nTODO`) is committed, and the check
...
- [no-code-idp-[service-catalog](../../containers-orchestration/common/other/service-catalog/SKILL.md)-tools-port-cortex-opslevel](../[no-code-idp-[service-catalog](../../DevOps_and_Cloud/Observability_and_SecOps/service-catalog/SKILL.md)-tools-port-cortex-opslevel](../../DevOps_and_Cloud/Observability_and_SecOps/no-code-idp-[service-catalog](../../DevOps_and_Cloud/Observability_and_SecOps/service-catalog/SKILL.md)-tools-port-cortex-opslevel/SKILL.md)/SKILL.md) — standing up Cortex Scorecards or OpsLevel Rubrics as a product decision, and the phased-rollout/re-baselining pattern referenced in step 8 here.
...
- [idp-adoption-rollout-and-[change-management](../../observability-monitoring-logging/common/other/change-management/SKILL.md)-strategy](../../observability-monitoring-logging/common/other/change-management/SKILL.md)-strategy](../../observability-monitoring-logging/common/other/change-management/SKILL.md)-strategy/SKILL.md)/SKILL.md) — sequencing a new or tightened scorecard rollout so it doesn't repeat the mandatory-adoption failure modes covered there.
