...
The heart of [Cocos2d](../cocos2d/SKILL.md)-x is the `Director` singleton, responsible for the game's main loop, rendering pipeline, and scene management. The Director orchestrates the `mainLoop()`, which iteratively performs calculations (updating the simulation) and triggers the draw call.
