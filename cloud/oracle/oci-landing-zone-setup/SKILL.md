...
- Know that the tenancy's **home region is permanent** — chosen at
  ...
  ([Vault](../../../Security/cryptography-secrets/vault/SKILL.md) keys, Identity Domains, and some other resources have region
  affinity).
...
1. **Design the compartment hierarchy before creating any resource.** A
   ...
   ├── Security
   │   ├── (Cloud Guard, [Vault](../../../Security/cryptography-secrets/vault/SKILL.md), centralized Object Storage log archive)
   ├── Network
...
- **Symptom:** A Terraform apply that provisions [Vault](../../../Security/cryptography-secrets/vault/SKILL.md) keys or an
  Identity Domain in a newly subscribed region fails or behaves
  inconsistently with the home region's equivalent resource.
  **Fix:** Some resources ([Vault](../../../Security/cryptography-secrets/vault/SKILL.md) master encryption keys, Identity
