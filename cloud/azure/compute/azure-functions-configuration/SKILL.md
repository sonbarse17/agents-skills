...
5. **Separate local development config from deployed Application
   ...
   Key [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md) references) on the Function App:
   ```bash
...
- Store secrets and connection strings as Key [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md) references in
  Application Settings, not as plaintext values, even though Application
  Settings are already encrypted at rest — Key [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md) references add
...
- **Symptom:** `local.settings.json` (containing a real storage account
  ...
  **Fix:** Add `local.settings.json` to `.gitignore` by default in every
  new Function App project, and rely on Application Settings / Key [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md)
...
```
The Cosmos DB connection string is stored as a Key [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md) reference in
