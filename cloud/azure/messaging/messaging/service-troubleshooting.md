...
Scripts (paths below are relative to the skill root, `plugins/azure-skills/skills/azure-diagnostics`, so run them from there): [`scripts/test-messaging-connectivity.sh`](../../monitoring/azure-diagnostics/scripts/test-messaging-connectivity.sh) (bash) and [`scripts/test-messaging-connectivity.ps1`](../../monitoring/azure-diagnostics/scripts/test-messaging-connectivity.ps1) (PowerShell).
