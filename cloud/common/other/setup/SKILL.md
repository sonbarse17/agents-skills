...
Cloud [Serverless](../../../../DevOps_and_Cloud/Containers_and_Orchestration/serverless/SKILL.md) with no credit card required. Once the user has registered and logged in, proceed with API key
generation below.
...
1. When creating this key, include **Project Admin** privileges or higher (Org Owner) so it can create and manage
   [serverless](../../../../DevOps_and_Cloud/Containers_and_Orchestration/serverless/SKILL.md) projects.
...
curl -s -H "Authorization: ApiKey ${EC_API_KEY}" \
  "${EC_BASE_URL}/api/v1/[serverless](../../../../DevOps_and_Cloud/Containers_and_Orchestration/serverless/SKILL.md)/regions" | python3 -m json.tool
```
...
curl -sf -H "Authorization: ApiKey ${EC_API_KEY}" \
  "${EC_BASE_URL}/api/v1/[serverless](../../../../DevOps_and_Cloud/Containers_and_Orchestration/serverless/SKILL.md)/regions" > /dev/null && echo "Authenticated." || echo "Authentication failed."
```
