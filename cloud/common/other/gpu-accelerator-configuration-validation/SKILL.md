...
before it runs. GPU scheduling on [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) has an unusually large number
...
(possibly with wrong numerical results). There is rarely a [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md)-level
...
- `[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md)` access to the cluster and namespace running the job, plus
  `nvidia-smi` access on the node or via `[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) exec` into a running pod
...
- The GPU infrastructure from
  ...
  *against* that infrastructure, it doesn't install it.
  ...
  [pod-crashloop-and-oom-troubleshooting](../../../[kubernetes](../../Containers_and_Orchestration/kubernetes/SKILL.md)-platform/skills/[pod-crashloop-and-oom-troubleshooting](../../../../containers-orchestration/kubernetes/troubleshooting/pod-crashloop-and-oom-troubleshooting/SKILL.md)/SKILL.md).
...
1. **Confirm the device plugin reports the resource the job requests, on
   ...
   silently:
   ```bash
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get nodes -o json \
...
2. **Check that the resource key in the pod spec exactly matches what the
   ...
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get pod <pod> -o jsonpath='{.spec.containers[*].resources.limits}'
...
3. **Verify the resource appears in `limits`, not only `requests` (or vice
   versa left empty).** [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) device plugins are "limit-only"
...
4. **Verify tolerations match the node pool's taints exactly**, including
   ...
   ```bash
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get node <gpu-node> -o jsonpath='{.spec.taints}'
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get pod <pod> -o jsonpath='{.spec.tolerations}'
   ```

5. **Add an in-job fail-fast assertion** as a second line of defense
   independent of the [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) scheduling layer, so a job that somehow
...
6. **Confirm the container image's CUDA toolkit version is compatible
   ...
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) exec -it <pod> -- nvidia-smi --query-gpu=driver_version --format=csv
   # CUDA toolkit baked into the image
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) exec -it <pod> -- nvcc --version
   ```
...
- Never treat "the job finished without a [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md)-level error" as
...
- **Symptom:** A pod requests `nvidia.com/gpu: 1` but the node pool was
  ...
  indefinitely with no obvious reason in `[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) describe pod`.
  **Fix:** Check `[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) describe node <node>` for the actual resource
...
- **Symptom:** A multi-GPU distributed training job only uses one GPU
  ...
  **Fix:** Check the training framework's distributed launch configuration
  ...
  device-count detection) separately from the [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) resource request —
  [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) correctly allocating 4 GPU devices to a pod does not
...
2. If the policy weren't in place, the pod **would schedule** — [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md)
...
- [training-pipeline-orchestration](../../../../AI_and_Agents/Models_and_FineTuning/llmops/training-pipeline-orchestration/SKILL.md)/SKILL.md) — where GPU resource requests should be templated consistently across pipeline steps rather than hand-copied per job.
