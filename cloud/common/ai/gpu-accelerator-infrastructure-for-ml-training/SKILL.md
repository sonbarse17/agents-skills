...
building the [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md)-native GPU platform layer: installing the NVIDIA
...
- Standing up GPU support on a new or existing [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) cluster (on-prem,
  EKS/AKS/GKE, or a [bare-metal](../../../../Software_Engineering_and_Other/Miscellaneous/systems-low-level/bare-metal/SKILL.md) cluster) for the first time.
...
- [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) ≥ 1.24 with Helm ≥ 3.8 for installing the GPU Operator chart.
...
- For managed [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) (EKS/AKS/GKE), decide whether to use the cloud
...
2. **Verify the device plugin is healthy and GPUs are schedulable** before
   ...
   ```bash
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get pods -n gpu-operator
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get nodes -o json | jq '.items[].status.[capacity](../../../AI_and_Agents/Infrastructure/deploy-model/[capacity](../azure-skills/skills/microsoft-foundry/models/deploy-model/[capacity](../../Observability_and_SecOps/capacity/SKILL.md)/SKILL.md)/SKILL.md) | select(.["nvidia.com/gpu"])'
   ```
...
3. **Partition large GPUs with MIG when workloads don't need a full card.**
   ...
   ```
   ```bash
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) label node gpu-node-a100-01 nvidia.com/mig.config=all-3g.20gb --overwrite
   ```
...
4. **Design node pools around workload shape, not just GPU SKU.** Separate
   ...
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) taint nodes gpu-node-a100-01 workload=training:NoSchedule
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) label nodes gpu-node-a100-01 gpu-pool=training gpu-sku=a100-80gb

   # Serving pool: MIG-partitioned L4/A100 slices for inference
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) taint nodes gpu-node-l4-01 workload=serving:NoSchedule
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) label nodes gpu-node-l4-01 gpu-pool=serving gpu-sku=l4
   ```
...
5. **Bin-pack deliberately** rather than letting the default scheduler
   ...
   use a `PriorityClass` plus the [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) scheduler's default bin-packing
...
- Label node pools with the GPU SKU and MIG profile explicitly
  ...
  cost attribution are visible from `[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get nodes --show-labels`
...
- **Symptom:** Installing the NVIDIA GPU Operator on a managed [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md)
...
- **Symptom:** A MIG profile change (e.g. switching a node from
  ...
  **Fix:** Cordon and drain the node before changing its MIG profile label
  (`[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) cordon` then `[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) drain --ignore-daemonsets`) — the GPU
...
```bash
[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) label nodes gpu-node-a100-01 gpu-node-a100-02 \
  gpu-pool=training gpu-sku=a100-80gb
[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) taint nodes gpu-node-a100-01 gpu-node-a100-02 \
...
```
```bash
[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) label nodes gpu-node-a100-03 gpu-node-a100-04 \
  gpu-pool=serving nvidia.com/mig.config=all-1g.10gb --overwrite
[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) taint nodes gpu-node-a100-03 gpu-node-a100-04 \
...
- [training-pipeline-orchestration](../../../../AI_and_Agents/Models_and_FineTuning/llmops/training-pipeline-orchestration/SKILL.md)/SKILL.md) — the pipeline layer that submits training jobs onto the GPU infrastructure built here.
- [model-serving-and-scaling](../../../../AI_and_Agents/Models_and_FineTuning/inference-serving/model-serving-and-scaling/SKILL.md)/SKILL.md) — serving-side [autoscaling](../../../../Software_Engineering_and_Other/Backend/patterns/autoscaling/SKILL.md) and latency concerns for workloads running on the serving GPU pool.
