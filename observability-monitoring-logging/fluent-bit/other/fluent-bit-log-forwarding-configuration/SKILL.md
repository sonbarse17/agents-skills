...
5. **Route to multiple outputs by tag/`Match`**, sending different log
   ...
   ```yaml
     outputs:
       - name: loki
         match: kube.payments.*
         host: loki-gateway.[monitoring](../../../common/monitoring-strategy/monitoring/SKILL.md).svc
         port: 3100
...
- **Symptom:** A node running the Fluent Bit DaemonSet runs low on disk
  ...
  **Fix:** Filesystem buffering was enabled without a bounded
  ...
  node's actual available resources, and pair with [alerting](../../../common/alerting/alerting/SKILL.md) on Fluent
