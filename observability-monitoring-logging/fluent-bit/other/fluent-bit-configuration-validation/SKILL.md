...
- Auditing an existing production Fluent Bit config for `Match`
  ...
  proactive health check rather than only after an [incident](../../../common/incident-detection/incident/SKILL.md).
...
7. **Validate output destination reachability and credentials
   ...
   ```bash
   curl -sf -o /dev/null -w '%{http_code}' http://loki-gateway.[monitoring](../../../common/monitoring-strategy/monitoring/SKILL.md).svc:3100/ready
   ```
...
9. **After deploying, confirm real production behavior matches the
   ...
   ```bash
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) logs -n [monitoring](../../../common/monitoring-strategy/monitoring/SKILL.md) -l app=fluent-bit --tail=200 | grep -i error
   ```
