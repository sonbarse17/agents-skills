...
exporter pipeline that feeds traces *into* Tempo/Jaeger is the [OpenTelemetry](../../../opentelemetry/other/opentelemetry/SKILL.md)
...
- Standing up Tempo or Jaeger as the tracing backend for an
  [OpenTelemetry](../../../opentelemetry/other/opentelemetry/SKILL.md)-instrumented (or Jaeger-native) system.
...
- Traces already arriving in OTLP (or Jaeger native protocol/Thrift)
  format from an [OpenTelemetry](../../../opentelemetry/other/opentelemetry/SKILL.md) Collector or directly-instrumented
  applications — see
  [opentelemetry-instrumentation-and-collector-configuration](../[opentelemetry-instrumentation-and-collector-configuration](../../../Software_Engineering_and_Other/Frontend/[opentelemetry](../opentelemetry/SKILL.md)-instrumentation-and-collector-configuration/SKILL.md)/SKILL.md)
...
- Grafana (see
  [prometheus-and-grafana-[monitoring](../../../common/monitoring-strategy/monitoring/SKILL.md)-stack](../[prometheus-and-grafana-[monitoring](../monitoring/SKILL.md)-stack](../../Containers_and_Orchestration/prometheus-and-grafana-[monitoring](../monitoring/SKILL.md)-stack/SKILL.md)/SKILL.md))
...
2. **Set retention via the compactor**, sized to an explicit
   ...
   specifically, since traces are typically used for near-term [incident](../../../common/incident-detection/incident/SKILL.md)
...
3. **For Jaeger, choose the storage backend based on query needs and
   ...
   --es.server-urls=https://elasticsearch.[observability](../../../common/fundamentals/observability/SKILL.md).svc:9200
...
5. **Decide head-based vs. tail-based sampling deliberately** — head-based
   ...
   # (this belongs in the [OpenTelemetry](../../../opentelemetry/other/opentelemetry/SKILL.md) Collector gateway tier — see
...
7. **Configure trace-to-logs correlation in Grafana's Tempo datasource**
   ...
   datasources:
     - name: Tempo
       type: tempo
       url: http://tempo-query-frontend.[observability](../../../common/fundamentals/observability/SKILL.md).svc:3100
       jsonData:
         tracesToLogsV2:
...
- Wire both trace-to-logs and trace-to-metrics correlation as a standing
  ...
  [incident](../../../common/incident-detection/incident/SKILL.md) — confirm the correlation actually works (a real trace pivots
...
- Version-control Tempo/Jaeger configuration and Grafana datasource
  ...
  alongside the rest of the [observability](../../../common/fundamentals/observability/SKILL.md) stack's IaC, not as hand-edited
...
1. Tempo deployed with S3 object storage and 14-day retention:
   ...
   compactor:
     compaction:
       block_retention: 336h   # 14 days, matching the platform team's agreed [incident](../../../common/incident-detection/incident/SKILL.md)-investigation window
   ```
