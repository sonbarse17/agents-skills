...
1. Install `elastic-[opentelemetry](../../other/opentelemetry/SKILL.md)` via pip (add to `requirements.txt` or equivalent)
1. Run `edot-bootstrap --action=install` during image build to install auto-instrumentation packages for detected
   libraries
1. Wrap the application entrypoint with `[opentelemetry](../../other/opentelemetry/SKILL.md)-instrument` — e.g. `[opentelemetry](../../other/opentelemetry/SKILL.md)-instrument gunicorn app:app`.
