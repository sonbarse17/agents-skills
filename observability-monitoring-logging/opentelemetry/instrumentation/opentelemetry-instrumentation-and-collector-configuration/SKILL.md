...
# [OpenTelemetry](../../other/opentelemetry/SKILL.md) Instrumentation and Collector Configuration

## Purpose

[OpenTelemetry](../../other/opentelemetry/SKILL.md) (OTel) is the vendor-neutral instrumentation layer that sits
between application code and whichever [observability](../../../common/fundamentals/observability/SKILL.md) backend actually
...
- Instrumenting a new service and deciding between auto-instrumentation
  ...
  the code).
- Standing up an [OpenTelemetry](../../other/opentelemetry/SKILL.md) Collector deployment — as a per-node/
...
- Migrating an application off a vendor-specific instrumentation SDK to
  [OpenTelemetry](../../other/opentelemetry/SKILL.md) to decouple it from a specific [observability](../../../common/fundamentals/observability/SKILL.md) vendor.

## Prerequisites & environment

- The **[OpenTelemetry](../../other/opentelemetry/SKILL.md) Collector** — either the core distribution (OTLP
...
1. **Prefer auto-instrumentation as the default starting point** — it
   ...
   java -javaagent:/otel/[opentelemetry](../../other/opentelemetry/SKILL.md)-javaagent.jar \
   ...
   const { NodeSDK } = require('@[opentelemetry](../../other/opentelemetry/SKILL.md)/sdk-node');
   const { getNodeAutoInstrumentations } = require('@[opentelemetry](../../other/opentelemetry/SKILL.md)/auto-instrumentations-node');
   const { OTLPTraceExporter } = require('@[opentelemetry](../../other/opentelemetry/SKILL.md)/exporter-trace-otlp-grpc');
...
3. **Set `service.name` and other resource attributes at SDK
   ...
   as `unknown_service` in every downstream backend, making [dashboards](../../../common/dashboard-design/dashboards/SKILL.md) and
...
5. **Choose a deployment topology deliberately** — an **agent** tier (one
   ...
   exporters:
     otlp:
       endpoint: otel-gateway.[observability](../../../common/fundamentals/observability/SKILL.md).svc:4317
       tls:
         insecure: false
   service:
     pipelines:
...
6. **Configure the gateway's full receiver → processor → exporter
   ...
   exporters:
     prometheusremotewrite:
       endpoint: http://prometheus.[observability](../../../common/fundamentals/observability/SKILL.md).svc:9090/api/v1/write
     loki:
       endpoint: http://loki-gateway.[observability](../../../common/fundamentals/observability/SKILL.md).svc:3100/loki/api/v1/push
     otlp/tempo:
       endpoint: tempo.[observability](../../../common/fundamentals/observability/SKILL.md).svc:4317
       tls:
         insecure: false
...
9. **For tail-based sampling at scale, route by trace ID with a load
   ...
   exporters:
     loadbalancing:
       ...
       resolver:
         k8s:
           service: otel-gateway-headless.[observability](../../../common/fundamentals/observability/SKILL.md).svc
   ```
   See
   [distributed-tracing-with-tempo-and-jaeger](../[distributed-tracing-with-tempo-and-jaeger](../../../DevOps_and_Cloud/Observability_and_SecOps/[distributed-tracing](../../../DevOps_and_Cloud/Observability_and_SecOps/distributed-tracing/SKILL.md)-with-tempo-and-jaeger/SKILL.md)/SKILL.md)
...
- Scrape and dashboard the Collector's own `:8888` internal metrics
  ...
  own [monitoring](../../../common/monitoring-strategy/monitoring/SKILL.md), not something checked only when someone reports missing
  data.
...
5. The gateway's `:8888` internal metrics are scraped by Prometheus and a
   ...
   discovered during an [incident](../../../common/incident-detection/incident/SKILL.md).
