...
# Kibana [Alerting](../alerting/SKILL.md) Rules
...
All [alerting](../alerting/SKILL.md) API calls require either API key auth or Basic auth. Every mutating request must include the `kbn-xsrf`
header.
...
- `all` privileges for the appropriate Kibana feature (e.g., Stack Rules, [Observability](../observability/SKILL.md), Security)
- `read` privileges for Actions and Connectors (to attach actions to rules)

## API Reference

Base path: `<kibana_url>/api/[alerting](../alerting/SKILL.md)` (or `/s/<space_id>/api/[alerting](../alerting/SKILL.md)` for non-default spaces).

| Operation         | Method | Endpoint                                                   |
| ----------------- | ------ | ---------------------------------------------------------- |
| Create rule       | POST   | `/api/[alerting](../alerting/SKILL.md)/rule/{id}`                                  |
| Update rule       | PUT    | `/api/[alerting](../alerting/SKILL.md)/rule/{id}`                                  |
| Get rule          | GET    | `/api/[alerting](../alerting/SKILL.md)/rule/{id}`                                  |
| Delete rule       | DELETE | `/api/[alerting](../alerting/SKILL.md)/rule/{id}`                                  |
| Find rules        | GET    | `/api/[alerting](../alerting/SKILL.md)/rules/_find`                                |
| List rule types   | GET    | `/api/[alerting](../alerting/SKILL.md)/rule_types`                                 |
| Enable rule       | POST   | `/api/[alerting](../alerting/SKILL.md)/rule/{id}/_enable`                          |
| Disable rule      | POST   | `/api/[alerting](../alerting/SKILL.md)/rule/{id}/_disable`                         |
| Mute all alerts   | POST   | `/api/[alerting](../alerting/SKILL.md)/rule/{id}/_mute_all`                        |
| Unmute all alerts | POST   | `/api/[alerting](../alerting/SKILL.md)/rule/{id}/_unmute_all`                      |
| Mute alert        | POST   | `/api/[alerting](../alerting/SKILL.md)/rule/{rule_id}/alert/{alert_id}/_mute`      |
| Unmute alert      | POST   | `/api/[alerting](../alerting/SKILL.md)/rule/{rule_id}/alert/{alert_id}/_unmute`    |
| Update API key    | POST   | `/api/[alerting](../alerting/SKILL.md)/rule/{id}/_update_api_key`                  |
| Create snooze     | POST   | `/api/[alerting](../alerting/SKILL.md)/rule/{id}/snooze_schedule`                  |
| Delete snooze     | DELETE | `/api/[alerting](../alerting/SKILL.md)/rule/{ruleId}/snooze_schedule/{scheduleId}` |
| Health check      | GET    | `/api/[alerting](../alerting/SKILL.md)/_health`                                    |
...
| `consumer`     | string | Owning app: `alerts`, `apm`, `discover`, `infrastructure`, `logs`, `metrics`, `ml`, `[monitoring](../monitoring/SKILL.md)`, `securitySolution`, `siem`, `stackAlerts`, `uptime` |
...
```bash
curl -X POST "https://my-kibana:5601/api/[alerting](../alerting/SKILL.md)/rule/my-rule-id" \
  -H "kbn-xsrf: true" \
...
`.es-query`) and provide the matching `params` object. Use `GET /api/[alerting](../alerting/SKILL.md)/rule_types` to discover params schemas.

## Updating a Rule

`PUT /api/[alerting](../alerting/SKILL.md)/rule/{id}` — send the complete rule body. `rule_type_id` and `consumer` are immutable after creation.
Returns **409 Conflict** if another user updated the rule concurrently; re-fetch and retry.

## Finding Rules

```bash
curl -X GET "https://my-kibana:5601/api/[alerting](../alerting/SKILL.md)/rules/_find?per_page=20&page=1&search=cpu&sort_field=name&sort_order=asc" \
  -H "Authorization: ApiKey <your-api-key>"
```
...
```bash
# Enable
curl -X POST ".../api/[alerting](../alerting/SKILL.md)/rule/{id}/_enable" -H "kbn-xsrf: true"

# Disable
curl -X POST ".../api/[alerting](../alerting/SKILL.md)/rule/{id}/_disable" -H "kbn-xsrf: true"

# Mute all alerts
curl -X POST ".../api/[alerting](../alerting/SKILL.md)/rule/{id}/_mute_all" -H "kbn-xsrf: true"

# Mute specific alert
curl -X POST ".../api/[alerting](../alerting/SKILL.md)/rule/{rule_id}/alert/{alert_id}/_mute" -H "kbn-xsrf: true"

# Delete
curl -X DELETE ".../api/[alerting](../alerting/SKILL.md)/rule/{id}" -H "kbn-xsrf: true"
```
...
> **Preview feature** — available from Elastic Stack 9.3 and Elastic Cloud [Serverless](../../../../Software_Engineering_and_Other/Patterns/data-performance/serverless/SKILL.md). APIs may change.
...
```bash
curl -X PUT "https://my-kibana:5601/api/[alerting](../alerting/SKILL.md)/rule/my-rule-id" \
  -H "kbn-xsrf: true" \
  -H "Content-Type: application/json" \
  -H "Authorization: ApiKey <your-api-key>" \
  -d '{
...
- `group` — which trigger state fires this action (e.g., `"query matched"`, `"Recovered"`). Discover valid groups via
  `GET /api/[alerting](../alerting/SKILL.md)/rule_types`.
...
5. **Set a reasonable check interval.** The minimum recommended interval is `1m`. Very short intervals across many rules
   ...
   `xpack.[alerting](../alerting/SKILL.md).rules.minimumScheduleInterval.value` enforces this.
...
2. **Wrong `consumer` value.** Using an invalid consumer (e.g., `[observability](../observability/SKILL.md)` instead of `infrastructure`) causes a
   400 error. Check the rule type's supported consumers via `GET /api/[alerting](../alerting/SKILL.md)/rule_types`.
...
5. **Rule ID conflicts.** POST to `/api/[alerting](../alerting/SKILL.md)/rule/{id}` with an existing ID returns 409. Either omit the ID to
...
7. **Too many actions per rule.** Rules generating thousands of alerts with multiple actions can clog Task Manager. The
   server setting `xpack.[alerting](../alerting/SKILL.md).rules.run.actions.max` (default varies) limits actions per run. Design rules to use
   alert summaries or limit term sizes.

8. **Long-running rules.** Rules that run expensive queries are cancelled after `xpack.[alerting](../alerting/SKILL.md).rules.run.timeout`
...
**Find rules by tag:** "Show all production [alerting](../alerting/SKILL.md) rules." `GET /api/[alerting](../alerting/SKILL.md)/rules/_find` with
`filter=alert.attributes.tags:"production"` and `sort_field=name` to page through results.

**Pause a rule temporarily:** "Disable rule abc123 until next Monday." `POST /api/[alerting](../alerting/SKILL.md)/rule/abc123/_disable`.
...
- Prefix paths with `/s/<space_id>/api/[alerting](../alerting/SKILL.md)/` for non-default Kibana Spaces.
- Always pair an active action with a `Recovered` action to auto-close PagerDuty, Jira, and ServiceNow incidents.
- Run `GET /api/[alerting](../alerting/SKILL.md)/rule_types` first to discover valid `consumer` values and action group names.
- Use `alert_delay` to suppress transient spikes; use the `flapping` object to reduce noise from unstable conditions.

## Additional Resources

- [Kibana [Alerting](../alerting/SKILL.md) API Reference](https://www.elastic.co/docs/api/doc/kibana/group/endpoint-[alerting](../alerting/SKILL.md))
- [Alerting Concepts](https://www.elastic.co/docs/explore-analyze/[alerting](../alerting/SKILL.md)/alerts)
- [Create and Manage Rules (UI)](https://www.elastic.co/docs/explore-analyze/[alerting](../alerting/SKILL.md)/alerts/create-manage-rules)
- [Rule Action Variables](https://www.elastic.co/docs/explore-analyze/[alerting](../alerting/SKILL.md)/alerts/rule-action-variables)
- [Alerting Production Considerations](https://www.elastic.co/docs/deploy-manage/production-guidance/kibana-[alerting](../alerting/SKILL.md)-production-considerations)
