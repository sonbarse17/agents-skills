...
Connectors store connection information for Elastic services and third-party systems. [Alerting](../alerting/SKILL.md) rules use connectors to
...
| **[Incident](../incident/SKILL.md) Management**     | PagerDuty, Opsgenie, ServiceNow (ITSM, SecOps, ITOM), Jira, Jira Service Management (9.2+), IBM Resilient, Swimlane, Torq, Tines, D3 Security, XSOAR (9.1+), TheHive |
...
| **Logging & [Observability](../observability/SKILL.md)** | Server log, Index, [Observability](../observability/SKILL.md) AI Assistant                                                                                                                        |
...
Access to connectors is granted based on your privileges to [alerting](../alerting/SKILL.md)-enabled features. You need `all` privileges for
...
# Filter connector types by feature (e.g., only those supporting [alerting](../alerting/SKILL.md))
curl -X GET "https://my-kibana:5601/api/actions/connector_types?feature_id=[alerting](../alerting/SKILL.md)" \
  -H "Authorization: ApiKey <your-api-key>"
```
...
Connectors serve as the integration layer across multiple Kibana workflows, not just [alerting](../alerting/SKILL.md) notifications:
...
8. **Monitor connector health.** Failed connector executions are logged in the event log index (`.kibana-event-log-*`).
   ...
   [Event Log Index](https://www.elastic.co/docs/explore-analyze/[alerting](../alerting/SKILL.md)/alerts/event-log-index) for true failure
   rates.
...
10. **Use deduplication keys for on-call connectors.** Set `dedupKey` (PagerDuty) or `alias` (Opsgenie) to
    `{{rule.id}}-{{alert.id}}` to ensure the resolve event closes exactly the right [incident](../incident/SKILL.md). Without this, a new
    [incident](../incident/SKILL.md) is created every time the alert re-fires.
...
- [Alerting Settings (Action Config)](https://www.elastic.co/docs/reference/kibana/configuration-reference/[alerting](../alerting/SKILL.md)-settings#action-settings)
