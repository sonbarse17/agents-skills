...
# Metrics and [Monitoring](../../monitoring-strategy/monitoring/SKILL.md)
...
## 6. Separate "[monitoring](../../monitoring-strategy/monitoring/SKILL.md) is broken" from "the service is broken"

A metrics pipeline that silently drops data during the exact outage you needed it for is worse than no metrics — it gives false confidence. Monitor the [monitoring](../../monitoring-strategy/monitoring/SKILL.md): scrape success/failure, ingestion lag, and rule evaluation failures are metrics too.
...
- **Route this meta-[monitoring](../../monitoring-strategy/monitoring/SKILL.md) through its own low-noise path**, so a storage backend problem doesn't masquerade as "everything's fine, no alerts fired."
