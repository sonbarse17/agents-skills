...
history, and the [incident](../../incident-detection/incident/SKILL.md) channel transcript — actual timestamps, not "around then" — before
...
find out what made that decision look reasonable — a missing alert, a confusing [runbook](../../incident-detection/runbook/SKILL.md), an
...
reverting the bad code — leaves the other four in place for the next [incident](../../incident-detection/incident/SKILL.md).
...
- **Ask "five whys" as a technique, not a religion** — stop when you reach an organizational
  ...
  cause."
- **Look across the whole stack**: code, tests, review, deploy process, [monitoring](../../monitoring-strategy/monitoring/SKILL.md), [alerting](../../alerting/alerting/SKILL.md),
...
An action item like "improve [monitoring](../../monitoring-strategy/monitoring/SKILL.md)" with no owner and no date is a wish, not a
commitment, and it will still be open — unstarted — at the next [incident](../../incident-detection/incident/SKILL.md) review. Every action
...
- **Re-open the [incident](../../incident-detection/incident/SKILL.md) lineage** if an unshipped action item's failure mode recurs — that's
