...
### Trigger
Exact user phrases: SLA, service level agreement, uptime, SLO, error budget, availability, reliability target, service guarantee, SLA penalty, SLA [monitoring](../../monitoring-strategy/monitoring/SKILL.md), service level indicator, SLA breach, SLA reporting, SLA negotiation, service credit, burn rate, feature freeze.
...
- What is the team's current on-call and [incident](../../incident-detection/incident/SKILL.md) response maturity?
- What are the business priorities: maximum reliability, feature velocity, or balanced?

### Output Artifact
SLA framework with SLO definitions, error budget calculations, burn rate [alerting](../../alerting/alerting/SKILL.md), and multi-tier structure.
...
- [ ] SLA reporting automated with [dashboards](../../dashboard-design/dashboards/SKILL.md) per tier and per customer
...
For each SLI, define measurement method: server-side metrics from application performance [monitoring](../../monitoring-strategy/monitoring/SKILL.md), request-based (count successful vs. total requests), time-based (availability measured in minutes of uptime per window), synthetic [monitoring](../../monitoring-strategy/monitoring/SKILL.md) (external probes measuring from user locations).
...
Fast burn rate: consuming budget at a rate that would exhaust it in < 6 hours if sustained. Triggers immediate page (PagerDuty, OpsGenie). Indicates critical [incident](../../incident-detection/incident/SKILL.md) requiring immediate response.
...
Alert thresholds should be calibrated per service based on SLO target, traffic volume, and [incident](../../incident-detection/incident/SKILL.md) response time. A 99.99% SLO service has less error budget and needs faster [alerting](../../alerting/alerting/SKILL.md) than a 99.9% service.

### Step 4: Multi-Tier SLA Structure
Define service tiers based on customer requirements and operational investment:

Critical tier (99.99% uptime, <10ms p99 latency, 15-minute response time): for enterprise customers with revenue-critical dependency on the service. Highest operational investment (dedicated support, proactive [monitoring](../../monitoring-strategy/monitoring/SKILL.md), redundancy across regions). Highest penalty for breach (5-10x monthly fee service credits).
...
3. Is the service customer-facing?
   - YES -> Set SLO based on user experience metrics (request-based measurement, user-perceived latency). Include synthetic [monitoring](../../monitoring-strategy/monitoring/SKILL.md) from user locations.
...
1. Has error budget reached 50% consumption?
   ...
   - NO -> Green status. Normal feature work. Continue [monitoring](../../monitoring-strategy/monitoring/SKILL.md).

2. Has error budget reached 75% consumption?
   - YES -> Orange status. Escalate to engineering manager. Reliability sprint planned. Feature freeze discussions begin. Daily burn rate [monitoring](../../monitoring-strategy/monitoring/SKILL.md).
   - NO -> Continue current status.

3. Has error budget reached 100% consumption?
   - YES -> Red status. Feature freeze activated. All non-critical deployments blocked. Reliability rotation formed. [Incident](../../incident-detection/incident/SKILL.md) review accelerated. Budget replenishment plan required.
   - NO -> Continue current status. Review trend.

4. Is error budget replenishing after exhaustion?
   - YES -> Track replenishment rate. Lift feature freeze when budget exceeds 25% and holding steady. Post-[incident](../../incident-detection/incident/SKILL.md) review completed.
...
### Multi-Window, Multi-Burn-Rate [Alerting](../../alerting/alerting/SKILL.md)
Recommended by Google SRE for balancing detection speed with alert fatigue. Two windows (short and long), two burn rates (fast and slow). Short window + fast burn rate = immediate page (critical [incident](../../incident-detection/incident/SKILL.md) detected quickly). Long window + slow burn rate = daily alert (systemic degradation detected without noise). No alert when burn rate is nominal.
...
Stage 2 — [Monitoring](../../monitoring-strategy/monitoring/SKILL.md): implement instrumentation, build [dashboards](../../dashboard-design/dashboards/SKILL.md), configure alerts, track attainment.
...
Tracking error budgets in a dashboard that nobody checks. The error budget is useless if it is not visible to decision-makers. Mitigation: display error budget prominently on team [dashboards](../../dashboard-design/dashboards/SKILL.md), include in daily standups, and reference in sprint planning.
...
Single-threshold [alerting](../../alerting/alerting/SKILL.md) on error budget creates too many pages. Team becomes desensitized and misses real incidents. Mitigation: implement multi-window, multi-burn-rate [alerting](../../alerting/alerting/SKILL.md). Calibrate thresholds per service. Test alert configurations.
...
- Display error budget consumption on engineering [dashboards](../../dashboard-design/dashboards/SKILL.md) in real time.
...
### [Alerting](../../alerting/alerting/SKILL.md)
- Use multi-window, multi-burn-rate [alerting](../../alerting/alerting/SKILL.md) to balance detection speed and alert fatigue.
...
- Test alert configurations regularly with chaos engineering or [incident](../../incident-detection/incident/SKILL.md) drills.
...
- Provide tier-specific [dashboards](../../dashboard-design/dashboards/SKILL.md) and reports to customers.
...
- Action: immediate page, [incident](../../incident-detection/incident/SKILL.md) response
...
| Response time | {X} minutes | per [incident](../../incident-detection/incident/SKILL.md) | {credit calculation} |
...
Remediation: immediate reliability investment (multi-region deployment, load testing, redundancy). Renegotiated SLA to 99.95% with a 99.99% target for a subset of APIs. Implemented real-time SLO [dashboards](../../dashboard-design/dashboards/SKILL.md) for the customer. Internal SLO set at 99.995% to provide buffer. Lessons learned: never negotiate SLA without validating against current system capability. Include phase-in period in new SLAs.
...
- SLA [monitoring](../../monitoring-strategy/monitoring/SKILL.md) must use request-based measurement for accuracy, not time-based approximation.
...
- [Alerting](../../alerting/alerting/SKILL.md): multi-window, multi-burn-rate (fast burn page, slow burn digest).
- Exclusion windows: planned maintenance excluded from SLO calculation. Configured in [alerting](../../alerting/alerting/SKILL.md) system.
- Rolling windows: 30-day rolling prevents permanent deficit. Services recover after [incident](../../incident-detection/incident/SKILL.md).
...
| Single-threshold [alerting](../../alerting/alerting/SKILL.md) | Alert fatigue. Miss real incidents. | Multi-window, multi-burn-rate. Calibrate per service. |
| Wrong SLIs measured | CPU at 20% but p99 latency at 5s. Users unhappy. | Measure user-facing: latency, error rate, uptime. |
| No maintenance exclusions | Pager on every deploy. | Planned maintenance window configured in [alerting](../../alerting/alerting/SKILL.md). |
