...
This is why SLO definition is its own discipline, distinct from the broader [observability](../../fundamentals/observability/SKILL.md) practice it depends on — see `[observability](../../fundamentals/observability/SKILL.md)` for the signals underneath, but the number itself is a negotiated business artifact, not just a query result.
...
- **See `[dashboards](../../dashboard-design/dashboards/SKILL.md)`** for where the rest of that detail should live instead.
