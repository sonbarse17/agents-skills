...
# [Runbook](../runbook/SKILL.md) Creation

Create effective operational [runbooks](../runbooks/SKILL.md), standard operating procedures, and
troubleshooting guides that any on-call engineer can follow under pressure.

## [Runbook](../runbook/SKILL.md) Template — Full Structure

````markdown
# [Runbook](../runbook/SKILL.md): [Service / Process Name]
...
Brief description of the service, why this [runbook](../runbook/SKILL.md) exists, and when to
use it.
...
## Related [Runbooks](../runbooks/SKILL.md)

- [Link to related [runbook](../runbook/SKILL.md) 1]
- [Link to related [runbook](../runbook/SKILL.md) 2]
...
````markdown
# [Runbook](../runbook/SKILL.md): [PostgreSQL](../../../../Software_Engineering_and_Other/Backend/postgresql/SKILL.md) Database Failover
...
Failover the primary [PostgreSQL](../../../../Software_Engineering_and_Other/Backend/postgresql/SKILL.md) instance to the synchronous replica when
...
### Log Collection for [Incident](../incident/SKILL.md) Investigation

```bash
#!/usr/bin/env bash
# collect-logs.sh — Gather logs from multiple sources for [incident](../incident/SKILL.md) review
set -euo pipefail

INCIDENT_ID="${1:?Usage: collect-logs.sh <[incident](../incident/SKILL.md)-id>}"
OUTDIR="/tmp/[incident](../incident/SKILL.md)-${INCIDENT_ID}"
mkdir -p "$OUTDIR"

echo "Collecting logs for [incident](../incident/SKILL.md) $INCIDENT_ID..."
...
tar czf "${OUTDIR}.tar.gz" -C /tmp "[incident](../incident/SKILL.md)-${INCIDENT_ID}"
...
## [Runbook](../runbook/SKILL.md) Review Checklist

Use this checklist every time a [runbook](../runbook/SKILL.md) is created or updated.

```yaml
content_review:
  - [ ] Title clearly identifies the service and operation
  - [ ] Overview explains WHEN and WHY to use this [runbook](../runbook/SKILL.md)
...
usability_review:
  - [ ] A new team member can follow the [runbook](../runbook/SKILL.md) without tribal knowledge
...
maintenance_review:
  ...
  - [ ] Related [runbooks](../runbooks/SKILL.md) section is up to date
  - [ ] Links to [dashboards](../../dashboard-design/dashboards/SKILL.md) and docs are valid (not broken)
```

## [Runbook](../runbook/SKILL.md) Testing Procedures

```yaml
testing_strategy:
  dry_run:
    frequency: "Every time a [runbook](../runbook/SKILL.md) is created or substantially edited"
  ...
  peer_review:
    ...
    method: "Another engineer follows the [runbook](../runbook/SKILL.md) in staging without help"
    goal: "Confirm the [runbook](../runbook/SKILL.md) is self-contained and unambiguous"

  scheduled_validation:
    frequency: "Quarterly"
    method: "SRE team picks 5 [runbooks](../runbooks/SKILL.md) at random, executes in staging"
    goal: "Catch [runbooks](../runbooks/SKILL.md) that have drifted from production reality"

  incident_triggered:
    trigger: "Any time a [runbook](../runbook/SKILL.md) is used in a real [incident](../incident/SKILL.md)"
    method: "Post-mortem includes [runbook](../runbook/SKILL.md) accuracy assessment"
  ...
  automation_testing:
    ...
    example: |
      # .[github](../../../../ci-cd/github-actions/other/github/SKILL.md)/workflows/[runbook](../runbook/SKILL.md)-lint.yml
      name: Lint [Runbook](../runbook/SKILL.md) Scripts
      on: [pull_request]
      jobs:
        shellcheck:
          ...
          steps:
            ...
            - name: ShellCheck
              run: |
                find [runbooks](../runbooks/SKILL.md)/ -name "*.sh" -exec shellcheck {} +
```
...
```yaml
versioning:
  storage: "Git repository — one directory per service, one file per [runbook](../runbook/SKILL.md)"
  naming: "[runbooks](../runbooks/SKILL.md)/<service>/<operation>.md"
...
Keep a top-level index so engineers can find the right [runbook](../runbook/SKILL.md) quickly.

```markdown
# [Runbook](../runbook/SKILL.md) Index

| Service | [Runbook](../runbook/SKILL.md) | Severity | Owner | Last Tested |
|---------|---------|----------|-------|-------------|
...
- Write [runbooks](../runbooks/SKILL.md) for the engineer at 3 AM — clear, sequential, copy-pasteable
...
- Test [runbooks](../runbooks/SKILL.md) in staging before they are needed in production
- Keep [runbooks](../runbooks/SKILL.md) in version control alongside the code they support
- Assign an owner to every [runbook](../runbook/SKILL.md); ownerless [runbooks](../runbooks/SKILL.md) rot fast
- After every [incident](../incident/SKILL.md), update the relevant [runbook](../runbook/SKILL.md) with lessons learned
- Automate repetitive [runbook](../runbook/SKILL.md) steps into scripts, but keep the [runbook](../runbook/SKILL.md) as
