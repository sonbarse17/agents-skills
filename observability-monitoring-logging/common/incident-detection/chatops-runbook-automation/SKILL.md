...
# ChatOps [Runbook](../runbook/SKILL.md) Automation

## Purpose

During an [incident](../incident/SKILL.md), the fastest path from "we know the fix" to "the fix
is applied" is often a chat command — `/restart checkout-api-prod` typed
directly into the [incident](../incident/SKILL.md) channel — rather than someone context-
...
gap: a [runbook](../runbook/SKILL.md) becomes a slash command any authorized on-call engineer
can trigger from inside the [incident](../incident/SKILL.md) channel, with the action and its
output visible to everyone in the channel in real time. But the same
property that makes ChatOps valuable during an [incident](../incident/SKILL.md) — a chat message
...
convenience into the [incident](../incident/SKILL.md)'s actual root cause. This skill covers
...
- Building a new Slack or Microsoft Teams bot/integration that lets
  on-call trigger diagnostic or remediation [runbooks](../runbooks/SKILL.md) from an [incident](../incident/SKILL.md)
  channel.
- Wiring StackStorm, Rundeck, or a custom webhook-driven automation
  layer to execute a specific [runbook](../runbook/SKILL.md) action (restart a service, scale
...
- Deciding which [runbook](../runbook/SKILL.md) actions are safe to fully automate
...
- Auditing what [runbook](../runbook/SKILL.md) actions were actually executed during a past
  [incident](../incident/SKILL.md), to confirm the record is complete and attributable to a
  specific person.
- Investigating an [incident](../incident/SKILL.md) where a ChatOps command produced an
...
- Existing [runbooks](../runbooks/SKILL.md) already documented as a clear, reproducible
  ...
  [incident-response-and-on-call-management](../../../site-reliability-engineering/skills/[incident-response-and-on-call-management](../[incident-response](../../../DevOps_and_Cloud/Observability_and_SecOps/[incident](../../../DevOps_and_Cloud/Observability_and_SecOps/incident/SKILL.md)-response/SKILL.md)-and-[on-call-management](../../alerting/on-call-management/SKILL.md)/SKILL.md)/SKILL.md).
...
1. **Classify every candidate [runbook](../runbook/SKILL.md) action by risk tier before
...
3. **Require an explicit, time-boxed confirmation step for Tier 2/3
   ...
   > run by someone who misread the [runbook](../runbook/SKILL.md). Always require a distinct
...
4. **Scope authorization per action and per target, not per bot
   ...
   [enterprise-sso-and-idp-federation-configuration](../../../../Security/scanning/security-scanning/SKILL.md)-tooling/skills/[enterprise-sso-and-idp-federation-configuration](../../../../cloud/common/other/enterprise-sso-and-idp-federation-configuration/SKILL.md)/SKILL.md))
...
- **Symptom:** A junior engineer who has access to the ChatOps bot (but
  ...
  failovers) successfully triggers one during an [incident](../incident/SKILL.md), because the
...
- **Symptom:** Months after an [incident](../incident/SKILL.md), a postmortem needs to determine
...
- **Symptom:** A [runbook](../runbook/SKILL.md) action that was originally Tier 2 (reversible,
...
```
```http
POST /api/now/table/[incident](../incident/SKILL.md)/INC0048213/comment
