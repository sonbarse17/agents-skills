...
# [Incident](../incident/SKILL.md) Response and On-Call Management

## Purpose

An outage doesn't become a well-handled [incident](../incident/SKILL.md) just because someone
gets paged — without predefined roles, severity levels, and an
escalation policy, the first ten minutes of a real [incident](../incident/SKILL.md) are spent
...
[incident](../incident/SKILL.md) response around a lightweight [Incident](../incident/SKILL.md) Command System (ICS) so
...
- Standing up (or fixing) an [incident](../incident/SKILL.md) response process from scratch —
...
- On-call is causing burnout or attrition — too many pages, uneven
  ...
  non-urgent issues.
- A recent [incident](../incident/SKILL.md) revealed confusion about who could declare an
  [incident](../incident/SKILL.md), who had authority to make a mitigation call, or who was
...
- [Alerting](../../alerting/alerting/SKILL.md) already wired from the [monitoring](../../monitoring-strategy/monitoring/SKILL.md) stack into that paging tool
  — see
  [Prometheus and Grafana [monitoring](../../monitoring-strategy/monitoring/SKILL.md) stack](../../../[observability](../../../DevOps_and_Cloud/Observability_and_SecOps/observability/SKILL.md)-and-platform-extras/skills/[prometheus-and-grafana-[monitoring](../../monitoring-strategy/monitoring/SKILL.md)-stack](../../../DevOps_and_Cloud/Containers_and_Orchestration/prometheus-and-grafana-[monitoring](../../../DevOps_and_Cloud/Observability_and_SecOps/monitoring/SKILL.md)-stack/SKILL.md)/SKILL.md)
...
- A dedicated [incident](../incident/SKILL.md) communication channel pattern (e.g. a Slack/Teams
  bot that spins up a fresh `#[incident](../incident/SKILL.md)-<id>` channel plus a bridge/call
  line per declared [incident](../incident/SKILL.md)) and, for customer-facing services, a
  status-page tool.
- Clarity on who is authorized to declare an [incident](../incident/SKILL.md) (should be *any*
...
1. **Define severity levels with an explicit paging outcome for each** —
   severity should be decided in seconds during an [incident](../incident/SKILL.md), not debated:
...
2. **Assign [Incident](../incident/SKILL.md) Command System roles** — for anything Sev1/Sev2,
   split coordination from execution:
   - **[Incident](../incident/SKILL.md) Commander (IC):** owns the [incident](../incident/SKILL.md) overall — decides
     severity, decides when to escalate/de-escalate, keeps the [incident](../incident/SKILL.md)
   ...
   - **Scribe:** logs the timeline in real time (timestamps, actions
     taken, decisions made) directly into the [incident](../incident/SKILL.md) channel/doc —
...
3. **Design the on-call rotation:**
   ...
   - **Follow-the-sun** (for globally distributed teams): hand off
     ...
     active [incident](../incident/SKILL.md) blind.
...
5. **Run the [incident](../incident/SKILL.md) end to end:** declare → assemble roles per
   ...
   service's SLO [dashboards](../../dashboard-design/dashboards/SKILL.md) → explicit stand-down/all-clear → hand the
...
- Keep IC and hands-on-fixer as different people for Sev1/Sev2 whenever
  ...
  makes both worse.
- Use a fresh, dedicated channel per [incident](../incident/SKILL.md) (not the team's standing
...
- **Symptom:** A Sev1 page goes unacknowledged for 40 minutes before
  ...
  **Fix:** No escalation timeout was configured (or it silently failed).
  ...
  periodically like the rest of the [alerting](../../alerting/alerting/SKILL.md) stack.
...
- **Symptom:** A follow-the-sun handoff happens with a one-line "all
  ...
  active, unresolved [incident](../incident/SKILL.md) an hour into their shift.
...
1. The fast-burn SLO alert (from
   ...
   step 4.
2. Primary acknowledges within 3 minutes, declares the [incident](../incident/SKILL.md) as
   **Sev1** per the severity table (checkout affects all customers), and
   a fresh `#[incident](../incident/SKILL.md)-2026-0728-01` channel is created automatically.
...
- [secure-cicd-gates](../../../[devsecops](../../../Security/devsecops/SKILL.md)/skills/[secure-cicd-gates](../../../../Security/secure-cicd-gates/SKILL.md)/SKILL.md) — reference when an [incident](../incident/SKILL.md)'s root cause has a security angle (compromised dependency, bypassed pipeline gate) to reason about which gate should have caught it.
