...
Engineering [incident](../incident/SKILL.md) tools like PagerDuty and Opsgenie are optimized for
speed — page the right human in seconds. ServiceNow's [Incident](../incident/SKILL.md), Change,
...
these aren't a choice between one or the other; a real [incident](../incident/SKILL.md) needs
...
caused it, and (if a fix requires a production change mid-[incident](../incident/SKILL.md)) an
...
or the [incident](../incident/SKILL.md)-command process running in parallel (see
[incident-response-and-on-call-management](../../../site-reliability-engineering/skills/[incident-response-and-on-call-management](../../Frontend/[incident-response](../../../DevOps_and_Cloud/Observability_and_SecOps/[incident](../../../DevOps_and_Cloud/Observability_and_SecOps/incident/SKILL.md)-response/SKILL.md)-and-[on-call-management](../../alerting/on-call-management/SKILL.md)/SKILL.md)/SKILL.md)).

## When to use

- Standing up bi-directional sync so a PagerDuty/Opsgenie [incident](../incident/SKILL.md)
  automatically creates and updates a ServiceNow [Incident](../incident/SKILL.md) record (and
...
- Writing or troubleshooting a Change Request workflow for a production
  ...
  approval routing.
- Linking a recurring or unresolved [incident](../incident/SKILL.md) to a Problem record for
  root-cause tracking that outlives any single [incident](../incident/SKILL.md).
- Configuring CMDB-based assignment group routing so a ServiceNow
  [incident](../incident/SKILL.md) lands with the team that actually owns the affected
  Configuration Item, not a generic triage queue.
- Debugging a sync issue: incidents duplicating, status ping-ponging
  between tools, or a ServiceNow [incident](../incident/SKILL.md) stuck in the wrong assignment
  group.
- Deciding how a mid-[incident](../incident/SKILL.md) production fix (a rollback, a config
...
- A ServiceNow instance (`${SNOW_INSTANCE_URL}`, e.g.
  ...
  ([incident](../incident/SKILL.md)/problem read-write) and [change-management](../../other/change-management/SKILL.md) roles as needed
...
- CMDB Configuration Items (CIs) already populated for the services in
  ...
  owner will misroute every [incident](../incident/SKILL.md) against it.
- Clarity on change types your ServiceNow instance supports (Standard,
  ...
  — this must be resolved before an [incident](../incident/SKILL.md), not decided live during
  one.
...
1. **Create the ServiceNow [Incident](../incident/SKILL.md) from the paging tool's [incident](../incident/SKILL.md)
   trigger**, via the Table API, carrying over the paging tool's
   [incident](../incident/SKILL.md) ID as a correlation field so later updates target the same
   ...
   ```json
   {
     ...
     "description": "Auto-created from PagerDuty [incident](../incident/SKILL.md) PD-INC-48213. See link for live timeline.",
     "urgency": "1",
     "impact": "1",
   ...
   (`GET /api/now/table/[incident](../incident/SKILL.md)?sysparm_query=correlation_id=PD-INC-48213`)
...
2. **Sync status changes bidirectionally through a defined mapping**,
   ...
   [incident](../incident/SKILL.md) states explicitly:
...
3. **Route incidents by CMDB CI → assignment group**, not a static
   default queue:
   ```
   [incident](../incident/SKILL.md).cmdb_ci  → looks up →  cmdb_rel_ci (or a dedicated
   ci_to_team mapping table)  →  [incident](../incident/SKILL.md).assignment_group
   ```
   Configure this as a Business Rule or Flow Designer flow on [incident](../incident/SKILL.md)
...
4. **Model change types deliberately** — most ServiceNow instances ship
   ...
   ```json
   {
     ...
     "type": "emergency",
     "justification": "Mitigating active Sev1 [incident](../incident/SKILL.md) INC0048213 — rolling back to v2.14.0",
     "cmdb_ci": "checkout-api",
     "risk": "moderate",
     "assignment_group": "checkout-team"
   }
...
6. **Link recurring/unresolved incidents to a Problem record** for
   root-cause tracking that outlives the [incident](../incident/SKILL.md):
   ```http
   ...
   ```json
   {
     ...
     "cmdb_ci": "checkout-db",
     "[incident](../incident/SKILL.md)": "INC0048213"
   }
   ```
   Then relate every future [incident](../incident/SKILL.md) with the same root cause back to
   this Problem record (`problem_id` field on the [incident](../incident/SKILL.md)) instead of
   letting each recurrence be investigated from scratch.

7. **Guard every inbound webhook/sync call with an idempotency key** —
   use the paging tool's [incident](../incident/SKILL.md) ID (already captured as
   ...
   timeout) doesn't create a duplicate ServiceNow [incident](../incident/SKILL.md) for the same
   underlying page.

## Best practices

- Always carry the paging tool's [incident](../incident/SKILL.md) ID into `correlation_id` and
...
- Link every Change Request made during an [incident](../incident/SKILL.md) back to that
  [incident](../incident/SKILL.md) record explicitly, so a postmortem (see
  [incident-response-and-on-call-management](../../../site-reliability-engineering/skills/[incident-response-and-on-call-management](../../Frontend/[incident-response](../../../DevOps_and_Cloud/Observability_and_SecOps/[incident](../../../DevOps_and_Cloud/Observability_and_SecOps/incident/SKILL.md)-response/SKILL.md)-and-[on-call-management](../../alerting/on-call-management/SKILL.md)/SKILL.md)/SKILL.md))
...
- Validate the workflow/approval configuration itself before depending
  on it in a real [incident](../incident/SKILL.md) — see
...
- **Symptom:** A PagerDuty [incident](../incident/SKILL.md) and its synced ServiceNow record
  ...
  **Fix:** Give the sync flow a clear single source of truth per field
  ...
  [incident](../incident/SKILL.md) is open; ServiceNow becomes authoritative only after
...
- **Symptom:** During an active Sev1, the fix requires a production
  ...
  next weekly CAB meeting — the [incident](../incident/SKILL.md) drags on for hours because the
  ...
  **Fix:** Configure and use the Emergency change type (step 4/5) with a
  ...
  needs to be set up and tested *before* the [incident](../incident/SKILL.md), not requested for
...
- **Symptom:** A retried PagerDuty webhook delivery (e.g. after a
  ...
  [incident](../incident/SKILL.md) for the same underlying page, and both get worked
...
- **Symptom:** A new microservice's incidents consistently land in a
  ...
  **Fix:** The service's CMDB CI either doesn't exist yet or has no
  ...
  exception to patch per [incident](../incident/SKILL.md).

## Worked example

**Scenario:** `checkout-api`'s Sev1 (from the worked example in
[incident-response-and-on-call-management](../../../site-reliability-engineering/skills/[incident-response-and-on-call-management](../../Frontend/[incident-response](../../../DevOps_and_Cloud/Observability_and_SecOps/[incident](../../../DevOps_and_Cloud/Observability_and_SecOps/incident/SKILL.md)-response/SKILL.md)-and-[on-call-management](../../alerting/on-call-management/SKILL.md)/SKILL.md)/SKILL.md))
needs both the PagerDuty [incident](../incident/SKILL.md) and a synced ServiceNow record, plus
an emergency Change Request for the rollback that mitigates it.

1. PagerDuty [incident](../incident/SKILL.md) `PD-INC-48213` triggers; a Flow Designer flow
   (webhook-triggered) creates the ServiceNow [incident](../incident/SKILL.md):
   ```json
   {
     "short_description": "Checkout success rate dropped sharply",
     "urgency": "1",
...
3. The rollback completes and checkout's error rate recovers. The sync
   ...
   PagerDuty's [incident](../incident/SKILL.md) is resolved, using `correlation_id` to target
...
4. Because this is the second time in a month checkout-api has had a
   ...
   across incidents, separate from any single [incident](../incident/SKILL.md)'s record.
