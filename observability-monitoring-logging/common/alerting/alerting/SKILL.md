...
See `[slo-definition](../../sli-slo-sla/slo-definition/SKILL.md)` for where the underlying budget comes from.
...
## 4. Never ship an alert without a [runbook](../../incident-detection/runbook/SKILL.md) link
...
See `[runbooks](../../incident-detection/runbooks/SKILL.md)` for what that document should actually contain.

**Done when:** every page-tier alert links a [runbook](../../incident-detection/runbook/SKILL.md), and the [runbook](../../incident-detection/runbook/SKILL.md) it links to actually exists and is current.
...
State the current page-tier alert count, what fraction are burn-rate based versus flat-threshold, and whether every page-tier alert links a working [runbook](../../incident-detection/runbook/SKILL.md).

Name the honest gap — usually a handful of cause-based alerts still in the page tier, or a [runbook](../../incident-detection/runbook/SKILL.md) link that points at something stale — rather than reporting the alerting setup as fully symptom-based and fatigue-free.
