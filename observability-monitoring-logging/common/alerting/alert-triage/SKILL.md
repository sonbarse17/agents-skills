...
group by `agent.id`, sub-group by time window (~5 min = likely one [incident](../../incident-detection/incident/SKILL.md)), triage each group as a single unit.
...
> **Note:** `find --search` may return 500 errors on [Serverless](../../../../Software_Engineering_and_Other/Patterns/data-performance/serverless/SKILL.md). Use `find --tags` or `list` instead.
...
- The acknowledge script uses the Kibana Detection Engine API, which is compatible with both self-managed and [Serverless](../../../../Software_Engineering_and_Other/Patterns/data-performance/serverless/SKILL.md)
  deployments.
