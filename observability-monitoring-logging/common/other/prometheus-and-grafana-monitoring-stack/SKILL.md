...
# Prometheus and Grafana [Monitoring](../../monitoring-strategy/monitoring/SKILL.md) Stack

## Purpose

Prometheus and Grafana are the de facto open-source metrics stack for
[Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) and cloud-native workloads: Prometheus pulls (scrapes) metrics
on an interval, evaluates [alerting](../../alerting/alerting/SKILL.md) rules against them, and hands firing
alerts to Alertmanager for routing/deduplication/silencing, while Grafana
turns the same time-series data into [dashboards](../../dashboard-design/dashboards/SKILL.md). The stack is simple to
...
too short a window, missing `by()` clauses that collapse [dashboards](../../dashboard-design/dashboards/SKILL.md) to a
single line), [alerting](../../alerting/alerting/SKILL.md) rules that page on transient blips, and
hand-edited Grafana [dashboards](../../dashboard-design/dashboards/SKILL.md) that drift from what's checked into git.
...
[alerting](../../alerting/alerting/SKILL.md) rules and Alertmanager routing that produce actionable pages
instead of noise, and provisioning Grafana as code so [dashboards](../../dashboard-design/dashboards/SKILL.md) survive
a cluster rebuild.
...
- Writing or debugging a PromQL query for a dashboard panel, a
  recording rule, or an [alerting](../../alerting/alerting/SKILL.md) rule.
- Defining or tuning Prometheus [alerting](../../alerting/alerting/SKILL.md) rules and Alertmanager
  routing trees, grouping, inhibition, and silences.
- Provisioning Grafana datasources and [dashboards](../../dashboard-design/dashboards/SKILL.md) declaratively
...
- Reducing alert fatigue — too many pages, duplicate pages across
  teams, or alerts with no clear owner/[runbook](../../incident-detection/runbook/SKILL.md).

## Prerequisites & environment

- [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) cluster with the **kube-prometheus-stack** Helm chart
...
- Cluster RBAC allowing Prometheus's service account to `list`/`watch`
  `Endpoints`, `Service`, and `Pod` objects for [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) service
...
- A notification receiver already provisioned for Alertmanager
  ...
  generic webhook) stored as a [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) `Secret` — never inline in the
  Alertmanager config.
...
1. **Install the stack with an explicit, pinned chart version:**
   ...
   helm upgrade --install kube-prom-stack prometheus-community/kube-prometheus-stack \
     --namespace [monitoring](../../monitoring-strategy/monitoring/SKILL.md) --create-namespace \
     --version 65.5.0 \
     -f values-[monitoring](../../monitoring-strategy/monitoring/SKILL.md).yaml
   ```
...
2. **Add a scrape target for a new service.** Prefer the Prometheus
   ...
   ```yaml
   apiVersion: [monitoring](../../monitoring-strategy/monitoring/SKILL.md).coreos.com/v1
   kind: ServiceMonitor
   metadata:
     name: payments-api
     namespace: [monitoring](../../monitoring-strategy/monitoring/SKILL.md)
     labels:
   ...
   spec:
     ...
     endpoints:
       - port: metrics            # named port on the Service, not a raw port number
         ...
         scrapeTimeout: 10s
   ```
   For non-[Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) targets (a VM, an on-prem host), use a static
...
5. **Add recording rules for anything queried repeatedly** ([dashboards](../../dashboard-design/dashboards/SKILL.md),
...
6. **Write [alerting](../../alerting/alerting/SKILL.md) rules with a `for:` duration** to suppress
   flapping/transient blips, and attach severity + [runbook](../../incident-detection/runbook/SKILL.md) labels so
         ...
         rules:
           - alert: PaymentsAPIHighErrorRate
             ...
             annotations:
               ...
               runbook_url: "https://[runbooks](../../incident-detection/runbooks/SKILL.md).internal/payments-api-error-rate"
...
7. **Configure Alertmanager routing** so alerts reach the right team
   ...
   ```
   Store `SLACK_WEBHOOK_URL`/`PAGERDUTY_ROUTING_KEY` in a [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md)
   `Secret` referenced via `alertmanagerConfigSecret`/`envFrom`, never
   inline.

8. **Provision Grafana datasources and [dashboards](../../dashboard-design/dashboards/SKILL.md) as code**, not
   ...
   datasources:
     - name: Prometheus
       type: prometheus
       access: proxy
       url: http://kube-prom-stack-prometheus.[monitoring](../../monitoring-strategy/monitoring/SKILL.md):9090
       isDefault: true
       jsonData:
         timeInterval: 30s
   ```
...
- **Every alert must have `severity` and `team` (or equivalent
  ...
  clear owner and no [runbook](../../incident-detection/runbook/SKILL.md) is a page nobody knows how to act on.
- **Set `for:` on every [alerting](../../alerting/alerting/SKILL.md) rule** long enough to ride out normal
...
- **Alert on symptoms (error rate, latency, saturation), not on every
  ...
  produces far less noise than [alerting](../../alerting/alerting/SKILL.md) on every internal metric.
...
- **Version-control Grafana [dashboards](../../dashboard-design/dashboards/SKILL.md) and Alertmanager config** in the
  same repo/pipeline as the rest of the platform config — [dashboards](../../dashboard-design/dashboards/SKILL.md)
...
- **Symptom:** A `ServiceMonitor` is applied but the target never shows
  ...
  **Fix:** The `ServiceMonitor`'s `labels` don't match the Prometheus
  ...
  `[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get prometheus -o yaml` for the selector and confirm the
...
- **Symptom:** The same underlying [incident](../../incident-detection/incident/SKILL.md) produces five separate
...
dashboard, and no [alerting](../../alerting/alerting/SKILL.md) — the team wants to be paged only on genuine
...
5. Route `severity="critical", team="payments"` to a PagerDuty
   ...
   the same job if both trip during the same [incident](../../incident-detection/incident/SKILL.md).
...
7. Run `promtool check rules` and `promtool test rules` in CI against
   the [alerting](../../alerting/alerting/SKILL.md) rule file before merging, catching a typo'd metric name
