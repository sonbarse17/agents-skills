...
exact moment they can least afford to learn it: during an [incident](../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md).
...
it is the one that matters during an [incident](../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md). A backup that has never been restored has an
...
- **Restore the way you would during a real [incident](../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md)** — from the actual backup artifact, by
...
- **Replicate backups to a separate account or region** that a single compromised credential
  cannot reach, following the same boundary discipline as `[disaster-recovery](../../../containers-orchestration/common/other/disaster-recovery/SKILL.md)`.
...
## 6. Document the restore procedure as a [runbook](../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md)

During an actual [incident](../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md), the person restoring data is often not the one who built the
backup pipeline, and they are working under time pressure with people watching. A [runbook](../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md)
...
- **State the expected duration** so whoever is running the [incident](../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) can set expectations with
  stakeholders instead of guessing.

**Done when:** a restore [runbook](../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md) exists that a different engineer, not its author, has
