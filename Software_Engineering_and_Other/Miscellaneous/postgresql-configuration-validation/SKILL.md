...
# [PostgreSQL](../../Databases/relational/postgresql/SKILL.md) Configuration Validation

## Purpose

A [PostgreSQL](../../Databases/relational/postgresql/SKILL.md) configuration change that looks correct in isolation can
...
- Before applying any `[postgresql](../../Databases/relational/postgresql/SKILL.md).conf` change (or `ALTER SYSTEM`) to a
...
- As a PR/change-review gate for [infrastructure-as-code](../../../infrastructure-as-code/common/other/infrastructure-as-code/SKILL.md) that manages
  [PostgreSQL](../../Databases/relational/postgresql/SKILL.md) configuration (e.g. a Terraform/[Ansible](../../../infrastructure-as-code/ansible/other/ansible/SKILL.md)-managed
  `[postgresql](../../Databases/relational/postgresql/SKILL.md).conf` template).
...
- The current `pg_hba.conf` and `[postgresql](../../Databases/relational/postgresql/SKILL.md).conf` (or the values
...
- `context = 'postmaster'` (e.g. `max_connections`, `shared_buffers`,
  ...
  reload` will silently accept the new value into `[postgresql](../../Databases/relational/postgresql/SKILL.md).conf` but
...
- **Symptom:** A `work_mem` increase that looked safe per-connection
  causes intermittent out-of-memory kills of the [PostgreSQL](../../Databases/relational/postgresql/SKILL.md) process
  under peak load.
