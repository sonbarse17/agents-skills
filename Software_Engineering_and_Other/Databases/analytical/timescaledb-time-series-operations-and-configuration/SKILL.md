...
TimescaleDB is a [PostgreSQL](../../relational/postgresql/SKILL.md) extension, not a separate database engine —
it adds **hypertables** (transparently partitioned by time, and
optionally by an additional "space" dimension) on top of ordinary
[PostgreSQL](../../relational/postgresql/SKILL.md) tables, along with continuous aggregates, compression, and
...
[PostgreSQL](../../relational/postgresql/SKILL.md) table underneath, everything in
...
retention policies that a plain [PostgreSQL](../../relational/postgresql/SKILL.md) table doesn't have.
...
- TimescaleDB 2.x on [PostgreSQL](../../relational/postgresql/SKILL.md) 13+ assumed for the syntax below —
...
- The `timescaledb` extension created in the target database
  ...
  required `[postgresql](../../relational/postgresql/SKILL.md).conf` change), which should be validated via
...
- Choose the initial chunk interval based on a realistic ingest-rate
  ...
  single hardest-to-fix decision in this skill, on par with [MongoDB](../../nosql/mongodb/SKILL.md)
...
- Keep ordinary [PostgreSQL](../../relational/postgresql/SKILL.md) operational practice (autovacuum tuning,
