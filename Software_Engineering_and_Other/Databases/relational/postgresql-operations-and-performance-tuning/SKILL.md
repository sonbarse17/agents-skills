...
# [PostgreSQL](../postgresql/SKILL.md) Operations and Performance Tuning

## Purpose

[PostgreSQL](../postgresql/SKILL.md)'s default configuration is tuned for compatibility on modest
...
operational work of keeping a [PostgreSQL](../postgresql/SKILL.md) fleet healthy under load:
...
It assumes a working, already-provisioned [PostgreSQL](../postgresql/SKILL.md) instance; for
...
- Planning a major-version upgrade (e.g. [PostgreSQL](../postgresql/SKILL.md) 14 → 16) and deciding
...
- [PostgreSQL](../postgresql/SKILL.md) 13+ assumed for the guidance below; logical replication is
  available from [PostgreSQL](../postgresql/SKILL.md) 10 onward but this skill's examples assume
...
- PgBouncer (or an equivalent pooler, e.g. pgbouncer/pgcat/odyssey)
  installed on a host between the application and [PostgreSQL](../postgresql/SKILL.md) for
...
```sql
-- [postgresql](../postgresql/SKILL.md).conf on primary
wal_level = replica
max_wal_senders = 10
...
```bash
pg_basebackup -h <PRIMARY_HOST> -U replicator -D /var/lib/[postgresql](../postgresql/SKILL.md)/data \
  -Fp -Xs -P -R
```
`-R` writes `[postgresql](../postgresql/SKILL.md).auto.conf` with `primary_conninfo` and creates
`standby.signal` automatically ([PostgreSQL](../postgresql/SKILL.md) 12+); on older supported
...
on the primary if you need synchronous [commit](../../../../DevOps_and_Cloud/ci-cd/common/git-workflow/commit/SKILL.md) guarantees for a specific
...
`max_client_conn`, is what determines load on [PostgreSQL](../postgresql/SKILL.md)'s own
...
workloads, well under `max_connections`, since [PostgreSQL](../postgresql/SKILL.md)'s per-connection
...
```bash
pg_upgrade \
  --old-datadir=/var/lib/[postgresql](../postgresql/SKILL.md)/14/data \
  --new-datadir=/var/lib/[postgresql](../postgresql/SKILL.md)/16/data \
  --old-bindir=/usr/lib/[postgresql](../postgresql/SKILL.md)/14/bin \
  --new-bindir=/usr/lib/[postgresql](../postgresql/SKILL.md)/16/bin \
  --link
```
...
- [postgresql-configuration-validation](../[postgresql-configuration-validation](../../../Software_Engineering_and_Other/Miscellaneous/[postgresql](../../../Software_Engineering_and_Other/Backend/postgresql/SKILL.md)-configuration-validation/SKILL.md)/SKILL.md) — validating `[postgresql](../postgresql/SKILL.md).conf` changes (e.g. `max_connections`, `wal_level`) and connection-limit math before applying any tuning from this skill to production.
