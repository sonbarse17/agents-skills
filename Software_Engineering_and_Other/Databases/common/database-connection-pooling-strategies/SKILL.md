...
engines — **PgBouncer** for [PostgreSQL](../../relational/postgresql/SKILL.md) and **ProxySQL** for [MySQL](../../relational/mysql/SKILL.md)/
...
- Setting up ProxySQL in front of a [MySQL](../../relational/mysql/SKILL.md)/MariaDB replication or Galera
...
- Deciding whether to add a pooler at all for a given workload/database
  ...
  handling (e.g. [PostgreSQL](../../relational/postgresql/SKILL.md)'s process-per-connection model vs. [MySQL](../../relational/mysql/SKILL.md)'s
...
- A target database's actual connection-handling model understood
  before sizing anything: [PostgreSQL](../../relational/postgresql/SKILL.md) forks an OS process per
  ...
  [MySQL](../../relational/mysql/SKILL.md)/MariaDB use a thread per connection (generally cheaper per
...
- Visibility into current connection counts and pool utilization on the
  target database (`pg_stat_activity` for [PostgreSQL](../../relational/postgresql/SKILL.md),
  `SHOW STATUS LIKE 'Threads_connected'` for [MySQL](../../relational/mysql/SKILL.md)/MariaDB) to size
...
### 2. Configure PgBouncer for [PostgreSQL](../../relational/postgresql/SKILL.md)
...
open per database/user pair — is what determines load on [PostgreSQL](../../relational/postgresql/SKILL.md)'s
...
against measured [PostgreSQL](../../relational/postgresql/SKILL.md)-side connection counts and query latency
...
### 3. Configure ProxySQL for [MySQL](../../relational/mysql/SKILL.md)/MariaDB
...
LOAD [MYSQL](../../relational/mysql/SKILL.md) SERVERS TO RUNTIME;
LOAD [MYSQL](../../relational/mysql/SKILL.md) QUERY RULES TO RUNTIME;
SAVE [MYSQL](../../relational/mysql/SKILL.md) SERVERS TO DISK;
SAVE [MYSQL](../../relational/mysql/SKILL.md) QUERY RULES TO DISK;
```
...
[MySQL](../../relational/mysql/SKILL.md)/MariaDB failover (whether via Orchestrator, Galera's own
...
- **Symptom:** ProxySQL's query routing rules appear correct in the
  ...
  **Fix:** The rules were inserted into the admin schema but never
  promoted to runtime. Always run `LOAD [MYSQL](../../relational/mysql/SKILL.md) QUERY RULES TO RUNTIME;`
...
**Scenario:** A checkout service on [MySQL](../../relational/mysql/SKILL.md)/MariaDB with a primary and
...
1. Deploy ProxySQL as a small redundant fleet (2 instances behind an
   ...
   INSERT INTO mysql_replication_hostgroups (writer_hostgroup, reader_hostgroup, check_type)
     VALUES (10, 20, 'read_only');
   LOAD [MYSQL](../../relational/mysql/SKILL.md) SERVERS TO RUNTIME; SAVE [MYSQL](../../relational/mysql/SKILL.md) SERVERS TO DISK;
   ```
