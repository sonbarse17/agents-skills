...
**B+Trees:** The standard for read-optimized storage (e.g., [PostgreSQL](../../relational/postgresql/SKILL.md)). Nodes are aligned to disk pages (typically 4KB-8KB). Search complexity is O(log_B(N)), yielding extremely shallow trees (depth 3-4 for billions of rows). In-place updates trigger page splits and fragmentation, making random writes heavy (Write Amplification).
