...
engines — **pg_dump**/**pg_basebackup** for [PostgreSQL](../../relational/postgresql/SKILL.md),
**mysqldump**/**Percona XtraBackup** for [MySQL](../../relational/mysql/SKILL.md)/MariaDB, and
**mongodump**/**mongorestore** for [MongoDB](../../nosql/mongodb/SKILL.md) — the logical-vs-physical
...
- For point-in-time recovery specifically: continuous WAL archiving
  ([PostgreSQL](../../relational/postgresql/SKILL.md)), binary log retention ([MySQL](../../relational/mysql/SKILL.md)/MariaDB), or oplog-based
  replication ([MongoDB](../../nosql/mongodb/SKILL.md)) already configured and retained for at least as
...
### 2. [PostgreSQL](../../relational/postgresql/SKILL.md): pg_dump for logical, pg_basebackup + WAL archiving for physical/PITR
...
moment:
```ini
# [postgresql](../../relational/postgresql/SKILL.md).conf on the source
archive_mode = on
...
### 3. [MySQL](../../relational/mysql/SKILL.md)/MariaDB: mysqldump for logical, XtraBackup for physical/hot backups
...
start a [MySQL](../../relational/mysql/SKILL.md) instance from; always confirm `--prepare` completed
...
mysqlbinlog --start-datetime="2026-07-28 00:00:00" \
  ...
  binlog.000123 | [mysql](../../relational/mysql/SKILL.md) -u <USER> -p appdb
```

### 4. [MongoDB](../../nosql/mongodb/SKILL.md): mongodump/mongorestore, and oplog-based point-in-time recovery

```bash
mongodump --uri="[mongodb](../../nosql/mongodb/SKILL.md)://<HOST>:27017" --db=appdb --out=/backup/appdb
```
```bash
mongorestore --uri="[mongodb](../../nosql/mongodb/SKILL.md)://<TARGET_HOST>:27017" --db=appdb /backup/appdb/appdb
```
...
```bash
mongodump --uri="[mongodb](../../nosql/mongodb/SKILL.md)://<HOST>:27017" --oplog --out=/backup/appdb
...
- **Symptom:** A Percona XtraBackup restore fails to start [MySQL](../../relational/mysql/SKILL.md), or
