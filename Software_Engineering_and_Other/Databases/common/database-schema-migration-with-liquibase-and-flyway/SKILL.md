...
- name: Start test database
  run: [docker](../../../../DevOps_and_Cloud/containers-orchestration/docker/other/docker/SKILL.md) run -d --name pg-test -e POSTGRES_PASSWORD=<TEST_DB_PASSWORD> -p 5432:5432 postgres:16

- name: Apply migrations from clean state
  run: flyway -url=jdbc:[postgresql](../../relational/postgresql/SKILL.md)://localhost:5432/testdb -user=postgres migrate
...
Note that DDL transactionality differs by engine: [PostgreSQL](../../relational/postgresql/SKILL.md) wraps most
...
[MySQL](../../relational/mysql/SKILL.md)'s DDL is largely non-transactional (each statement commits
...
- **Symptom:** A migration that ran fine in CI and staging locks a
  ...
  **Fix:** CI/staging validated correctness against a small dataset but
  ...
  `CREATE INDEX CONCURRENTLY` in [PostgreSQL](../../relational/postgresql/SKILL.md)) where the engine
...
- [mongodb-operations-and-scaling](../[mongodb-operations-and-scaling](../../../Software_Engineering_and_Other/Databases/[mongodb](../../../Software_Engineering_and_Other/Backend/mongodb/SKILL.md)-operations-and-scaling/SKILL.md)/SKILL.md) — [MongoDB](../../nosql/mongodb/SKILL.md) is schemaless, but index and shard-key changes there deserve the same tracked, reviewed, forward-planned change process described here rather than ad hoc shell commands.
