...
production-alert-fatigue [incident](../../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md) waiting to happen the moment it's
...
- Investigating a spike in Falco alert volume to determine whether it
  reflects a real change in behavior (a genuine [incident](../../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md), a new
...
- The ability to deploy a rule change in a **non-[alerting](../../../DevOps_and_Cloud/observability-monitoring-logging/common/alerting/alerting/SKILL.md) or
...
- A way to trigger a deliberate, known-positive test case (e.g.
  `[kubectl](../../../DevOps_and_Cloud/containers-orchestration/kubernetes/other/kubectl/SKILL.md) exec` a shell into a test pod, or a scripted equivalent of
...
- Basic scripting (`jq`, `grep`, or a small [Python](../../../Software_Engineering_and_Other/Languages/python/python/SKILL.md) script) to summarize
...
1. **Deploy the new/changed rule in a validation posture, not
   production-[alerting](../../../DevOps_and_Cloud/observability-monitoring-logging/common/alerting/alerting/SKILL.md) posture, first.** The simplest approach: keep the
   ...
   ```yaml
   # Falcosidekick config — validation window: cap outbound [alerting](../../../DevOps_and_Cloud/observability-monitoring-logging/common/alerting/alerting/SKILL.md)
...
2. **Let the rule run for a representative period against real
   ...
   ```bash
   [kubectl](../../../DevOps_and_Cloud/containers-orchestration/kubernetes/other/kubectl/SKILL.md) logs -n falco -l app.[kubernetes](../../../DevOps_and_Cloud/containers-orchestration/kubernetes/other/kubernetes/SKILL.md).io/name=falco --since=168h \
...
4. **Distinguish "the rule matched something legitimate" from "the rule
   ...
   this specific match as a one-off investigated [incident](../../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md), not evidence
...
5. **Confirm the rule still fires on a deliberate positive-control
   ...
   ```bash
   [kubectl](../../../DevOps_and_Cloud/containers-orchestration/kubernetes/other/kubectl/SKILL.md) exec -it test-pod -n payments -- /bin/sh
   [kubectl](../../../DevOps_and_Cloud/containers-orchestration/kubernetes/other/kubectl/SKILL.md) logs -n falco -l app.[kubernetes](../../../DevOps_and_Cloud/containers-orchestration/kubernetes/other/kubernetes/SKILL.md).io/name=falco --since=1m \
     | jq -c 'select(.rule == "Shell spawned in payments-api")'
   ```
...
documented exception for a specific [incident](../../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md) investigation) using the
...
unrelated [incident](../../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md), also approved and logged.
...
Positive-control re-test: `[kubectl](../../../DevOps_and_Cloud/containers-orchestration/kubernetes/other/kubectl/SKILL.md) exec` into an actual
...
- [vault-configuration-validation](../[vault-configuration-validation](../../../Security/[vault](../vault/SKILL.md)-configuration-validation/SKILL.md)/SKILL.md) —
  ...
  applied to [Vault](../../cryptography-secrets/vault/SKILL.md) policy/auth configuration instead of Falco rules.
