...
- The user is building a [DevSecOps](../../common/devsecops/SKILL.md) pipeline from scratch and wants a
...
- A CI/CD system with distinguishable pipeline stages (e.g. [GitHub](../../../DevOps_and_Cloud/ci-cd/github-actions/other/github/SKILL.md)
  Actions jobs with `needs:`, GitLab CI `stages:`, [Jenkins](../../../DevOps_and_Cloud/ci-cd/jenkins/other/jenkins/SKILL.md) pipeline
...
1. **Map scan types to pipeline stages by cost and blast radius**, fastest
   and cheapest first:
   - **Pre-[commit](../../../DevOps_and_Cloud/ci-cd/common/git-workflow/commit/SKILL.md) / local**: secret-scanning (Gitleaks), linters — sub-second
   ...
   - **Merge to main / pre-release build**: container image scan (SCA
     ...
     once per merge, not per-[commit](../../../DevOps_and_Cloud/ci-cd/common/git-workflow/commit/SKILL.md)-in-PR, since it needs a built
     artifact.
...
3. **Wire required status checks** to match exactly this table — don't
   mark a tool as a "required" [GitHub](../../../DevOps_and_Cloud/ci-cd/github-actions/other/github/SKILL.md) check if its default behavior would
...
4. **Separate PR gates from release gates explicitly** — a PR gate
   ...
   practical to run on every single [commit](../../../DevOps_and_Cloud/ci-cd/common/git-workflow/commit/SKILL.md)).
...
6. **Consolidate reporting** — export SARIF from SAST/DAST/IaC tools and
   JSON from SCA into one dashboard ([GitHub](../../../DevOps_and_Cloud/ci-cd/github-actions/other/github/SKILL.md) code scanning, a SIEM, or a
...
- Order gates by speed and confidence: fast, high-confidence checks
  ...
  release time and feed a dashboard instead of blocking every [commit](../../../DevOps_and_Cloud/ci-cd/common/git-workflow/commit/SKILL.md).
...
- **Symptom:** Every PR takes 40+ minutes because SAST, full SCA, and a
  ...
  on every [commit](../../../DevOps_and_Cloud/ci-cd/common/git-workflow/commit/SKILL.md).
  **Fix:** Split into fast/blocking PR-time checks (diff-aware SAST,
  ...
  release time instead of every [commit](../../../DevOps_and_Cloud/ci-cd/common/git-workflow/commit/SKILL.md).
...
|----------------------|------------------------------------------|-----------------------|
| Pre-[commit](../../../DevOps_and_Cloud/ci-cd/common/git-workflow/commit/SKILL.md)           | Gitleaks (secrets)                       | Any match             |
...
`.[github](../../../DevOps_and_Cloud/ci-cd/github-actions/other/github/SKILL.md)/workflows/pr-gates.yml`:
```yaml
  ...
  sast:
    ...
    steps:
      - uses: actions/checkout@v4
        with: { fetch-depth: 0 }
      - run: semgrep ci --config p/owasp-top-ten --baseline-[commit](../../../DevOps_and_Cloud/ci-cd/common/git-workflow/commit/SKILL.md) "${{ [github](../../../DevOps_and_Cloud/ci-cd/github-actions/other/github/SKILL.md).event.pull_request.base.sha }}"
...
- [software-composition-analysis-sca](../[software-composition-analysis-sca](../../Software_Engineering_and_Other/Frontend/software-composition-analysis-sca/SKILL.md)/SKILL.md) —
  the [dependency-scanning](../../scanning/dependency-scanning/SKILL.md) stage, run both at PR time (filesystem) and
...
- [secrets-management](../[secrets-management](../../DevOps_and_Cloud/Cloud_Providers/secrets-management/SKILL.md)/SKILL.md) — the
  pre-[commit](../../../DevOps_and_Cloud/ci-cd/common/git-workflow/commit/SKILL.md)/PR-time secret-scanning stage.
- [policy-as-code-guardrails](../[policy-as-code-guardrails](../[policy-as-code](../policy-as-code/SKILL.md)-guardrails/SKILL.md)/SKILL.md) —
