...
Timing checks fire when Process Monitor or AV hooks inflate syscall latency. Calibrate the threshold at startup: measure the guarded path 3 times and use `mean + 3*stddev`. For ptrace checks, verify the TracerPid comm name via `/proc/<pid>/comm` before exiting — it may be an unrelated [monitoring](../../observability-monitoring-logging/common/monitoring-strategy/monitoring/SKILL.md) tool, not a debugger.
