---
name: scan
description: Scans the codebase to generate project-doc.md and AGENTS.md. Use
  when bootstrapping a new agent-driven repo, refreshing project documentation
  after architectural changes, or running a delta scan to detect drift. Runs a
  full scan on first use and a smart delta scan on subsequent runs. Uses
  understand-anything + context-mode when available, falls back to native tools
  otherwise. Only updates AGENTS.md on detected architectural changes with human
  confirmation.
tags:
  - security
  - scan
...
## Tech Stack
- Runtime: [e.g. Node.js 20, [Python](../../../Software_Engineering_and_Other/Languages/python/python/SKILL.md) 3.11]
...
- Database: [e.g. [PostgreSQL](../../../Software_Engineering_and_Other/Databases/relational/postgresql/SKILL.md) via Prisma]
...
## Stack Context
[One-line summary: e.g. "Next.js 14 App Router + Prisma + [PostgreSQL](../../../Software_Engineering_and_Other/Databases/relational/postgresql/SKILL.md) + Tailwind + Vitest"]
...
If the project is MERN stack ([MongoDB](../../../Software_Engineering_and_Other/Databases/nosql/mongodb/SKILL.md) + Express + React + Node.js — detected from package.json / requirements), append a `### MERN Stack Notes` section to AGENTS.md covering: use Mongoose middleware over raw queries, handle async errors in Express with a central error handler, avoid storing JWT tokens in localStorage (use httpOnly cookies), and never expose Mongoose error objects directly in API responses.
