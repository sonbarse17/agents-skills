...
# [Vault](../vault/SKILL.md) Configuration Validation

## Purpose

A [Vault](../vault/SKILL.md) cluster's actual security guarantee is only as strong as its
...
- The user has drafted a new or modified [Vault](../vault/SKILL.md) ACL policy (HCL) and
...
- The user is auditing an existing [Vault](../vault/SKILL.md) installation's policies for
...
- [Vault](../vault/SKILL.md) CLI (`[vault](../vault/SKILL.md)`) matching the target cluster's version, with at
  least read access to policies (`[vault](../vault/SKILL.md) policy list`/`[vault](../vault/SKILL.md) policy
...
- Policy files version-controlled as HCL (or JSON) in a repo, not only
  ever written directly via `[vault](../vault/SKILL.md) policy write` from an ad hoc shell —
  version control is what makes CI-based linting and PR review possible
  at all.
- `[vault](../vault/SKILL.md)-lint` or an equivalent policy-linting tool (community tools
...
1. **Format and syntax-check every policy file before review** — a
   ...
   ```bash
   [vault](../vault/SKILL.md) policy fmt -check policies/payments-prod-read.hcl
   ```
...
4. **Lint policy files in CI on every change**, catching common
   ...
   for f in policies/*.hcl; do
     [vault](../vault/SKILL.md) policy fmt -check "$f" || fail=1
...
7. **Validate a seal-migration plan against a non-production cluster
   ...
   [vault](../vault/SKILL.md) operator seal   # confirm current seal type and status on staging
   ...
   [vault](../vault/SKILL.md) status   # confirm Type reflects the new seal mechanism
   ```
...
8. **Diff policy/auth-method state against the version-controlled source
   ...
   `[vault](../vault/SKILL.md) policy write`/`[vault](../vault/SKILL.md) write auth/.../role/...` changes made
   outside the reviewed change process:
   ```bash
   [vault](../vault/SKILL.md) policy read payments-prod-read > /tmp/live-policy.hcl
...
- Version-control every policy and auth-method role definition, and
  require PR review before applying — a `[vault](../vault/SKILL.md) policy write` run
...
- **Symptom:** Live [Vault](../vault/SKILL.md) policy state has drifted from what's in the
  ...
  **Fix:** Run a periodic diff of live policy/auth-method state against
  ...
  discipline in
  [critical-vulnerability-emergency-response](../../../[devsecops](../devsecops/SKILL.md)/skills/[critical-vulnerability-emergency-response](../../incident-response/critical-vulnerability-emergency-response/SKILL.md)/SKILL.md)).
...
```
CI lint now passes (`[vault](../vault/SKILL.md) policy fmt -check` clean, no wildcard-path
match).
...
the same CI pipeline that ran the lint (not a manual `[vault](../vault/SKILL.md) policy
