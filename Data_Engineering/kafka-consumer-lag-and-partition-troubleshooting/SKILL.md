...
causes is actually responsible in a live [incident](../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md), rather than guessing
...
1. **Quantify the lag and its trend before reacting** — a snapshot lag
   number without a trend can't tell you if it's an active [incident](../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md) or a
   transient blip:
   ```bash
...
- Instrument the consuming application with its own processing-latency
  ...
  [commit](../../DevOps_and_Cloud/ci-cd/common/git-workflow/commit/SKILL.md)) — broker-side lag metrics tell you *that* something is slow,
...
- **Symptom:** A consumer group rebalances every few minutes indefinitely,
  ...
  **Fix:** This is almost always `max.poll.interval.ms` being exceeded by
  ...
  worker acknowledged before [commit](../../DevOps_and_Cloud/ci-cd/common/git-workflow/commit/SKILL.md)) or raise `max.poll.interval.ms` to
...
- **Symptom:** `UnderReplicatedPartitions` spikes cluster-wide right
  ...
  patch), and someone starts an [incident](../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md) response before checking
  ...
  **Fix:** A restarted broker's replicas are legitimately behind until
  ...
  as a real [incident](../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md) if the metric doesn't converge within the expected
...
live during the [incident](../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md) given the ordering-guarantee implications):
...
- [kafka-configuration-validation](../../Software_Engineering_and_Other/Databases/messaging/kafka-configuration-validation/SKILL.md)/SKILL.md) — pre-production checks that catch consumer group and topic misconfigurations before they become live-[incident](../../DevOps_and_Cloud/observability-monitoring-logging/common/incident-detection/incident/SKILL.md) lag/rebalance problems.
