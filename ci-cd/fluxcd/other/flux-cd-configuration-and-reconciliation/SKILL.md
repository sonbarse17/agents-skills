...
Flux CD implements the same [GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md) pull-based reconciliation model
...
Flux's specific CRDs and reconciliation behavior; for [GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md) concepts
...
- Diagnosing a `Kustomization` or `HelmRelease` stuck `False` on
  ...
  doesn't match Git.
- Choosing Flux over Argo CD (or the reverse) for a new [GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md) rollout.

## Prerequisites & environment

- A [Kubernetes](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubernetes/SKILL.md) cluster ≥ 1.26 and the Flux CLI/controllers ≥ v2.3
...
- A Git repository (or OCI registry, for `OCIRepository` sources) Flux
  can authenticate to — a deploy key or a token stored as a [Kubernetes](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubernetes/SKILL.md)
  `Secret`, never embedded in a CRD spec.
- [Kustomize](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kustomize/SKILL.md) ≥ 5.0 (bundled into the `[kustomize](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kustomize/SKILL.md)-controller`) or Helm ≥
  3.x (for `HelmRelease`) knowledge — this skill assumes familiarity
  with [Kustomize](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kustomize/SKILL.md) overlays as covered in
...
- A secrets strategy decided before rollout (SOPS with age/GPG,
  ...
  `[kustomize](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kustomize/SKILL.md)-controller`, but the underlying rule ("never [commit](../../../common/git-workflow/commit/SKILL.md)
...
1. **Bootstrap Flux onto the cluster**, which installs the controllers
   ...
   own installation is itself [GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-managed):
   ```bash
   flux bootstrap [github](../../../github-actions/other/github/SKILL.md) \
     --owner=example-org \
     --repository=[gitops](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-config \
     --branch=main \
     --path=clusters/prod \
     --personal=false
   ```
   ...
   upgrades happen by re-running bootstrap, not manual `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) apply`.
...
2. **Define a `GitRepository` source** pointing at the config repo,
   ...
   kind: GitRepository
   metadata:
     name: [gitops](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-config
     namespace: flux-system
   spec:
     interval: 1m
     url: https://[github](../../../github-actions/other/github/SKILL.md).com/example-org/[gitops](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-config
     ref:
       branch: main
     secretRef:
       name: [gitops](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-config-auth
   ```
...
3. **Define a `Kustomization` reconciling a specific path** from that
   ...
   ```yaml
   apiVersion: [kustomize](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kustomize/SKILL.md).toolkit.fluxcd.io/v1
   kind: Kustomization
   metadata:
...
6. **Wire CI to update the config repo, not the cluster** — the same
   core [GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md) inversion as
   [gitops-workflow](../../../devops/skills/[gitops-workflow](../[gitops](../gitops/SKILL.md)-workflow/SKILL.md)/SKILL.md)
   describes generically: application CI bumps an image tag or chart
   version in the `[gitops](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-config` repo; `source-controller` and
   `[kustomize](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kustomize/SKILL.md)-controller`/`helm-controller` perform the actual cluster
...
7. **Force an immediate reconciliation** when waiting for the next
   ...
   fix):
   ```bash
   flux reconcile source git [gitops](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-config
   ...
   [commit](../../../common/git-workflow/commit/SKILL.md) yet.
...
8. **Roll back by reverting Git**, exactly as in the vendor-neutral
   ...
   ```bash
   git revert <bad-[commit](../../../common/git-workflow/commit/SKILL.md)-sha> && git push
   flux reconcile kustomization payments-api-prod --with-source
...
- Use SOPS-encrypted Secrets committed directly to the config repo
  (Flux's `[kustomize](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kustomize/SKILL.md)-controller` supports native decryption via a
...
- **Symptom:** A `Kustomization` reports `Ready: False` with a vague
  ...
  **Fix:** Check `spec.path` against the source's actual directory
  ...
  typo in the path, fails at the [kustomize](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kustomize/SKILL.md)-build step before any apply
...
- **Symptom:** `prune: true` on a `Kustomization` unexpectedly deletes a
  ...
  [Kustomize](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kustomize/SKILL.md) path during a refactor rather than an intentional removal.
...
```bash
flux bootstrap [github](../../../github-actions/other/github/SKILL.md) --owner=example-org --repository=[gitops](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-config --branch=main --path=clusters/prod
```

```yaml
apiVersion: source.toolkit.fluxcd.io/v1
kind: GitRepository
metadata: { name: [gitops](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-config, namespace: flux-system }
spec:
  interval: 1m
  url: https://[github](../../../github-actions/other/github/SKILL.md).com/example-org/[gitops](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-config
  ref: { branch: main }
...
spec:
  interval: 10m
  sourceRef: { kind: GitRepository, name: [gitops](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-config }
...
[commit](../../../common/git-workflow/commit/SKILL.md) SHA.

## Cross-references

- [gitops-workflow](../../../devops/skills/[gitops-workflow](../[gitops](../gitops/SKILL.md)-workflow/SKILL.md)/SKILL.md) — vendor-neutral [GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md) concepts (repo topology, secrets strategy, rollback-by-revert) this skill implements in Flux-specific terms; read that first if new to [GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md) generally.
