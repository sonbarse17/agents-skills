...
# [Jenkins](../../other/jenkins/SKILL.md) Centralized Shared Library
...
and is untestable in isolation. A **[Jenkins](../../other/jenkins/SKILL.md) Shared Library** solves this by
...
once with the [Jenkins](../../other/jenkins/SKILL.md) controller, so every consuming repo's Jenkinsfile
...
- Standing up [Jenkins](../../other/jenkins/SKILL.md) pipeline standards for a new team/org and wanting
...
- A dedicated Git repository for the shared library (commonly named
  `[jenkins](../../other/jenkins/SKILL.md)-shared-library` or `pipeline-library`), separate from any
  consumer application repo.
- [Jenkins](../../other/jenkins/SKILL.md) **Pipeline: Shared Groovy Libraries** plugin, and the library
  registered in **Manage [Jenkins](../../other/jenkins/SKILL.md) → System → Global Pipeline Libraries**
...
- Repo admin/[Jenkins](../../other/jenkins/SKILL.md) admin coordination: consuming repos need to reference
...
1. **Lay out the library repo in the required structure** — [Jenkins](../../other/jenkins/SKILL.md)
   expects exactly these top-level directories:
   ```
   (root)
   ├── vars/
   │   ├── standardPipeline.groovy      # callable as standardPipeline(...)
   │   └── standardPipeline.txt         # optional: help text shown in [Jenkins](../../other/jenkins/SKILL.md) UI
   ├── src/
...
3. **Put reusable, testable logic in `src/` classes**, not scattered across
   ...
   and can be unit tested independently of a running [Jenkins](../../other/jenkins/SKILL.md) instance:
   ```groovy
   ...
   outside the [Jenkins](../../other/jenkins/SKILL.md) CPS runtime — see
   [jenkins-groovy-scripting-best-practices](../[jenkins-groovy-scripting-best-practices](../[jenkins](../jenkins/SKILL.md)-groovy-scripting-best-practices/SKILL.md)/SKILL.md).
...
- Provide a `.txt` companion file next to each `vars/*.groovy` (e.g.
  ...
  renders in the [Jenkins](../../other/jenkins/SKILL.md) "Global Variables Reference" and saves consumers
...
- **Symptom:** A repo owner wants to delete an old shared-library
  ...
  **Fix:** Before deleting a [Jenkins](../../other/jenkins/SKILL.md) credential or an old library tag,
...
Library repo layout (`[jenkins](../../other/jenkins/SKILL.md)-shared-library`):
```
...
- [secure-cicd-gates](../../../../Security/common/devsecops/SKILL.md)/skills/[secure-cicd-gates](../../../../Security/app-security/secure-cicd-gates/SKILL.md)/SKILL.md) — designing the security gates a shared library's stages should enforce consistently across all consumers.
