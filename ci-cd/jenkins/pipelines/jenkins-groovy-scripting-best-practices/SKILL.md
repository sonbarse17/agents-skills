...
# [Jenkins](../../other/jenkins/SKILL.md) Groovy Scripting Best Practices

## Purpose

[Jenkins](../../other/jenkins/SKILL.md) Pipeline (both per-repo Jenkinsfiles and shared libraries) is
...
resume across [Jenkins](../../other/jenkins/SKILL.md) restarts), both of which produce failure modes that
...
test shared library Groovy without a running [Jenkins](../../other/jenkins/SKILL.md) controller, and the
...
- A [Jenkins](../../other/jenkins/SKILL.md) controller with the **Script Security** plugin (bundled with
  modern [Jenkins](../../other/jenkins/SKILL.md) Pipeline installs) — this is what enforces the sandbox and
  stores approved signatures under **Manage [Jenkins](../../other/jenkins/SKILL.md) → In-process Script
  Approval**.
- Administrator access to approve pending script signatures (only [Jenkins](../../other/jenkins/SKILL.md)
...
- For unit testing: a JVM project (Maven or Gradle) alongside the shared
  ...
  (`lesfurets:[jenkins](../../other/jenkins/SKILL.md)-pipeline-unit`) added as a test dependency, and a
  test framework (JUnit 5 or Spock) to drive it.
- Groovy 2.x/3.x knowledge (whichever your [Jenkins](../../other/jenkins/SKILL.md) version bundles — check
  **Manage [Jenkins](../../other/jenkins/SKILL.md) → System Information** for the exact Groovy version) —
...
2. **Approve the specific signature, don't disable the sandbox.** As an
   admin, go to **Manage [Jenkins](../../other/jenkins/SKILL.md) → In-process Script Approval**, review the
...
5. **Implement `Serializable` on any `src/` class whose instances are held
   across pipeline steps**, since [Jenkins](../../other/jenkins/SKILL.md) periodically serializes the
...
6. **Unit test shared library code with JenkinsPipelineUnit** instead of
   only validating by pushing to a real [Jenkins](../../other/jenkins/SKILL.md) and watching a build:
   ```groovy
   // test/groovy/NodeServicePipelineTest.groovy
   import com.lesfurets.[jenkins](../../other/jenkins/SKILL.md).unit.BasePipelineTest
...
- **Symptom:**
  ...
  `groovy` script outside [Jenkins](../../other/jenkins/SKILL.md).
...
- **Symptom:** After a [Jenkins](../../other/jenkins/SKILL.md) controller restart, a step appears to
...
- **Symptom:** A library author "temporarily" enables running the
  ...
  **Fix:** This should be flagged as a security regression, not a
  ...
  access to the [Jenkins](../../other/jenkins/SKILL.md) controller's JVM (including credentials store
...
[Jenkins](../../other/jenkins/SKILL.md) controller required):
```groovy
...
be unit tested as ordinary Groovy/JUnit with no [Jenkins](../../other/jenkins/SKILL.md) controller —
