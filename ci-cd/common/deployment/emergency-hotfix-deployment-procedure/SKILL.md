...
gates — but a live production [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) (data corruption, a security
...
corners get cut in ways that turn one [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) into two. This skill
...
- A production [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) requires a code change deployed before the next
...
- Defining (in advance, not mid-[incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md)) an organization's emergency
...
- A pre-agreed emergency-change policy defined *before* an [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md)
  ...
  calmly in advance.
- A named [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) commander or equivalent authority (see
...
1. **Confirm the situation actually warrants bypassing the normal
   ...
   with the [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) commander first; see
   [incident-response-and-on-call-management](../../../site-reliability-engineering/skills/[incident-response-and-on-call-management](../../../Software_Engineering_and_Other/Frontend/[incident-response](../../Observability_and_SecOps/[incident](../../Observability_and_SecOps/incident/SKILL.md)-response/SKILL.md)-and-[on-call-management](../../../../observability-monitoring-logging/common/alerting/on-call-management/SKILL.md)/SKILL.md)/SKILL.md)
   for severity definitions.

2. **Branch from the exact [commit](../../git-workflow/commit/SKILL.md) currently running in production**, not
...
3. **Define, in the emergency policy ahead of time, exactly which checks
   ...
   - **Never skip, regardless of urgency:**
     ...
     > prepared are not legitimate time-savers — they turn one [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md)
     ...
     - Named human approval from an accountable role ([incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) commander
...
4. **Deploy through the pipeline's emergency path, not around the
   ...
   environment:
     ...
     # Emergency policy: [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md)-commander team may approve directly;
...
6. **Immediately reconcile the hotfix back into the normal release
   history** once the [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) is resolved — merge the hotfix branch
...
7. **Run a blameless postmortem on the [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) that required the hotfix,
   ...
   [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) is over.

## Best practices

- Decide the "always skip / never skip" list before an [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) happens,
...
- Require a named, accountable approver for every emergency deploy, even
  ...
  pipeline environment approval) — "everyone agreed in the [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md)
  channel" without one clearly accountable approver is not a real
  approval trail.
- Branch the hotfix from the exact production [commit](../../git-workflow/commit/SKILL.md)/tag, never from
...
- **Symptom:** Under [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) pressure, someone deploys directly to
...
- **Symptom:** The hotfix fixes the immediate [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) but is never
  merged back into `main`, and the next regular release accidentally
  reverts the fix because it was branched from an older [commit](../../git-workflow/commit/SKILL.md).
  **Fix:** Reconcile the hotfix into mainline immediately after the
  [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) (step 6), and verify the next release build actually still
...
- **Symptom:** An emergency deploy ships with no rollback plan because
  ...
  turning a contained [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) into an extended one.
...
- **Symptom:** There is no pre-agreed emergency policy, so every [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md)
  ...
  **Fix:** Write and socialize the emergency-change policy in advance
  ...
  authority before the next [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md), not during it.
...
1. **Severity confirmed** by the [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) commander as Sev-1 (active
   customer financial impact) per the org's [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) severity policy —
...
3. **Minimum required checks, per the pre-agreed policy:**
   ...
   - Not skipped: a new targeted test reproducing the double-charge race
     ...
     diff; named sign-off from the [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) commander, recorded in the
     [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md) channel and in the pipeline's environment-approval log.
