...
### Input Context
- [Commit](../../git-workflow/commit/SKILL.md) message convention (Conventional Commits, Angular convention, custom)
- Output format (Keep a Changelog, custom Markdown, [GitHub](../../../github-actions/other/github/SKILL.md) release, Slack message)
- Tool preference (git-cliff, standard-version, release-please, semantic-release, auto-changelog)
- Source of truth (git log, [GitHub](../../../github-actions/other/github/SKILL.md) releases, JIRA)
...
- [ ] Conventional [commit](../../git-workflow/commit/SKILL.md) convention established
- [ ] Parsing rules defined ([commit](../../git-workflow/commit/SKILL.md) scopes, types, breaking changes)
...
│   Per-package changelogs, [GitHub](../../../github-actions/other/github/SKILL.md) releases, PR-based
├── npm package, simple → standard-version
│   npm-aware, version bump + changelog + tag
├── Full CI/CD pipeline → semantic-release
│   Automated release from CI, npm/[GitHub](../../../github-actions/other/github/SKILL.md)/GCR publish
└── Custom needs → custom script with conventional-changelog
    Flexible, integrate with any workflow
```

### Conventional [Commit](../../git-workflow/commit/SKILL.md) Format
```
...
{% for group, commits in commits | group_by(attribute="group") %}
  ### {{ group | upper_first }}
  {% for [commit](../../git-workflow/commit/SKILL.md) in commits %}
    - {% if [commit](../../git-workflow/commit/SKILL.md).scope %}**{{ [commit](../../git-workflow/commit/SKILL.md).scope }}:** {% endif %}{{ [commit](../../git-workflow/commit/SKILL.md).message | upper_first }}
      {% if [commit](../../git-workflow/commit/SKILL.md).breaking %}[**breaking**]{% endif %}
  {% endfor %}
{% endfor %}
"""
trim = true
...
commit_preprocessors = [
  { pattern = "\\(#(\\d+)\\)", replace = "([#${1}](https://[github](../../../github-actions/other/github/SKILL.md).com/myorg/myapp/pull/${1}))" },
]
...
### Added
- New feature description ([#42](https://[github](../../../github-actions/other/github/SKILL.md).com/org/repo/pull/42))

### Changed
- Updated dependency from v1 to v2

### Fixed
- Bug fix description ([#41](https://[github](../../../github-actions/other/github/SKILL.md).com/org/repo/pull/41))
...
[2.0.0]: https://[github](../../../github-actions/other/github/SKILL.md).com/org/repo/releases/tag/v2.0.0
```

### Step 4: CI Automation

```yaml
# .[github](../../../github-actions/other/github/SKILL.md)/workflows/release.yml
name: Release
on:
  push:
    branches: [main]
...
```yaml
# .[github](../../../github-actions/other/github/SKILL.md)/workflows/release-please.yml
name: Release Please
on:
  push:
    branches: [main]
...
| Breaking changes buried | Users miss critical upgrade info | BREAKING CHANGE: in [commit](../../git-workflow/commit/SKILL.md) footer always |
...
| Generate at release time, not per-[commit](../../git-workflow/commit/SKILL.md) | One consistent document per version |
...
| Validate [commit](../../git-workflow/commit/SKILL.md) messages with commitlint | Catch non-conventional commits before merge |
...
[MAJOR.MINOR.PATCH]: https://[github](../../../github-actions/other/github/SKILL.md).com/org/repo/releases/tag/vMAJOR.MINOR.PATCH
```
...
## Handoff
Hand off to `dev-loop-[git-workflow](../../git-workflow/git-workflow/SKILL.md)` for version tagging strategy. Hand off to `dev-loop-[code-review](../code-review/SKILL.md)` for PR-based changelog entries.
...
├── Has any [commit](../../git-workflow/commit/SKILL.md) with BREAKING CHANGE or feat!
│   └── MAJOR bump (1.0.0 → 2.0.0)
│       └── Update all consumers for breaking API changes
│
├── Has any [commit](../../git-workflow/commit/SKILL.md) with feat (and no breaking)
│   └── MINOR bump (1.0.0 → 1.1.0)
│       └── New features, backward compatible
│
├── Has any [commit](../../git-workflow/commit/SKILL.md) with fix, perf, refactor
...
```
What type is the [commit](../../git-workflow/commit/SKILL.md)?
...
- **Release automation pipeline**: Generate changelog automatically as part of the release CI pipeline. Tag the release [commit](../../git-workflow/commit/SKILL.md), generate changelog, create [GitHub](../../../github-actions/other/github/SKILL.md) Release with changelog content. Never manual.
...
| Ignoring Conventional Commits | Changelog is a mess of random messages | Enforce [commit](../../git-workflow/commit/SKILL.md) convention in CI |
...
- **Limit [commit](../../git-workflow/commit/SKILL.md) history depth**: When generating changelog for large repos, limit to last 1000 commits. Use `git log --max-count=1000` to avoid slow full-history traversal.
- **Cache tag-to-[commit](../../git-workflow/commit/SKILL.md) mapping**: Cache the tag-to-[commit](../../git-workflow/commit/SKILL.md) hash mapping. Avoids repeated git operations when rendering multiple changelogs (e.g., per-package in [monorepo](../../../../Software_Engineering_and_Other/Frontend/build-tools/monorepo/SKILL.md)).
