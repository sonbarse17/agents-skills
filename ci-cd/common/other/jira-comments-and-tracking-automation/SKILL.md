...
2. **Structure a decision comment so it stands alone:**
   ...
   - **Links** — PR, [commit](../../git-workflow/commit/SKILL.md) SHA, design doc/Confluence page, related
     ticket.
       ...
       "content": [
         {
           "type": "paragraph",
           "content": [
             { "type": "text", "text": "Deploy status: v2.14.1 deployed to production at 2026-07-28T14:32:00Z. [Commit](../../git-workflow/commit/SKILL.md) a91fbc2. Pipeline run: " },
...
7. **Post a genuinely useful deploy notification, not a log dump.**
   Include: what shipped (version/[commit](../../git-workflow/commit/SKILL.md)), where (environment), when
...
- **Symptom:** A CI pipeline posts a new comment for every stage (lint
  ✅, build ✅, unit tests ✅, integration tests ✅...) on every [commit](../../git-workflow/commit/SKILL.md),
...
back to anything useful — and this repeats for every [commit](../../git-workflow/commit/SKILL.md).
...
[Commit](../../git-workflow/commit/SKILL.md): a91fbc2 ("fix: correct clock-skew tolerance in SSO token
validation", PR #482)
    ...
    "content": [
      {
        "type": "paragraph",
        "content": [
          { "type": "text", "text": "Deploy status: v2.14.1 deployed to production (eu-west-1) at 2026-07-28T14:32:00Z. [Commit](../../git-workflow/commit/SKILL.md) a91fbc2 (\"fix: correct clock-skew tolerance in SSO token validation\", PR #482). Pipeline run #4821. Smoke tests green; EU login error rate back to baseline as of 14:40 UTC." }
        ]
      }
    ]
  }
