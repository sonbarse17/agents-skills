...
[commit](../../git-workflow/commit/SKILL.md) and production. The discipline this demands is upstream of the deploy pipeline itself: if
...
**Every [commit](../../git-workflow/commit/SKILL.md) on the default branch should be deployable at any moment — if it isn't, that's a
...
instead of long-lived feature branches (see `[feature-flags](../../other/feature-flags/SKILL.md)`), and a CI signal from `[ci-pipelines](../ci-pipelines/SKILL.md)`
...
**Done when:** any [commit](../../git-workflow/commit/SKILL.md) on main can be deployed to production without someone first checking
...
**Done when:** every gate between [commit](../../git-workflow/commit/SKILL.md) and production is either an automated check with a
...
off) without a rollback. See `[feature-flags](../../other/feature-flags/SKILL.md)` for the mechanics.
...
and the pipeline should be the *only* way production changes — no manual [kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) apply or console
...
Lead time ([commit](../../git-workflow/commit/SKILL.md) to production) and deploy frequency are the two numbers that tell you whether
