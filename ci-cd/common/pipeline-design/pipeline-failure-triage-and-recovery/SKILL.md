...
- The same [commit](../../git-workflow/commit/SKILL.md) passes on re-run with no code change, suggesting
...
- Read access to the CI platform's run logs and, ideally, per-step timing
  and cache-hit metadata ([GitHub](../../../github-actions/other/github/SKILL.md) Actions run logs and `actions/cache`
  hit/miss output, GitLab CI job traces and cache job artifacts, [Jenkins](../../../jenkins/other/jenkins/SKILL.md)
...
- The ability to re-run a single job/stage in isolation, not only the
  ...
  something to build ad hoc ([GitHub](../../../github-actions/other/github/SKILL.md) Actions "Re-run failed jobs", GitLab
  "Retry" per job, [Jenkins](../../../jenkins/other/jenkins/SKILL.md) "Replay"/stage restart where supported).
...
2. **Classify the failure before deciding on a recovery action:**
   ...
   - **Flaky** — the *same [commit](../../git-workflow/commit/SKILL.md)*, unchanged, fails intermittently across
...
3. **Re-run at the narrowest scope that actually re-tests the failure.**
   ...
   - [GitHub](../../../github-actions/other/github/SKILL.md) Actions: `gh run rerun <run-id> --failed` re-runs only failed
     jobs.
   ...
   - [Jenkins](../../../jenkins/other/jenkins/SKILL.md) (declarative): use `Restart from Stage` for a specific stage
...
4. **For suspected cache/dependency corruption, invalidate the cache
   ...
   state:
   ```yaml
   # [GitHub](../../../github-actions/other/github/SKILL.md) Actions actions/cache — bump the key suffix to force a miss
...
**Scenario:** A [GitHub](../../../github-actions/other/github/SKILL.md) Actions job `test-integration` fails on a PR with
