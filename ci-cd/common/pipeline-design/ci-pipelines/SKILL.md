...
For concrete [GitHub](../../../github-actions/other/github/SKILL.md) Actions patterns — caching, matrix builds, SHA pinning, OIDC, reusable
...
A pipeline that produces a different answer on a re-run of the same [commit](../../git-workflow/commit/SKILL.md) is not a merge signal,
...
**Done when:** re-running the same [commit](../../git-workflow/commit/SKILL.md)'s pipeline twice, a week apart, produces the same
pass/fail result.
...
changed, the cache is valid regardless of which branch produced it. See `[build-optimization](../../build/build-optimization/SKILL.md)` for
...
make two builds of the "same" [commit](../../git-workflow/commit/SKILL.md) different. Build once in CI, publish it with a
content-addressed or immutable version, and have every later stage pull that exact artifact.
Details on where and how to store it belong to `[artifact-management](../../build/artifact-management/SKILL.md)`.
