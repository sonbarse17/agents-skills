...
Argo Rollouts replaces the [Kubernetes](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubernetes/SKILL.md) `Deployment` controller with a
...
- Wiring an `AnalysisTemplate` to a specific metrics backend (Prometheus,
  [Datadog](../../../../DevOps_and_Cloud/Observability_and_SecOps/datadog/SKILL.md), CloudWatch, Wavefront, New Relic, or a [Kubernetes](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubernetes/SKILL.md) `Job`-based
...
- Argo Rollouts controller ≥ 1.6 installed cluster-wide, plus the
  `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) argo rollouts` plugin (or standalone `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md)-argo-rollouts`
  binary) matching the controller's major version for CLI operations and
  the terminal dashboard (`[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) argo rollouts dashboard`).
- For **canary with real traffic-weighting** (not just replica-ratio
  ...
  (with the `nginx.ingress.[kubernetes](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubernetes/SKILL.md).io/canary*` annotations Argo
...
1. **Convert a `Deployment` to a `Rollout`** — same `spec.template`,
   ...
   `Deployment` deliberately, or use `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) argo rollouts` tooling to
   assist migration (`argo rollouts convert deployment` pattern varies by
   version — verify by checking `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) argo rollouts --help` for the
   installed version).
...
3. **Configure blue-green** when you want a full environment swap with an
   ...
   `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) argo rollouts promote payments-api`;
...
6. **Operate the rollout: inspect, pause, promote, abort, roll back:**
   ```bash
   [kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) argo rollouts get rollout payments-api --watch
   [kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) argo rollouts promote payments-api            # skip remaining pause/advance one step
   [kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) argo rollouts promote payments-api --full      # skip straight to 100%
   [kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) argo rollouts pause payments-api               # manually pause an in-progress rollout
   [kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) argo rollouts abort payments-api               # halt, revert traffic to stable
   [kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) argo rollouts undo payments-api                # revert to previous revision entirely
   ```
...
7. **Set `dryRun`/`measurementRetention` and dashboard for [observability](../../../../DevOps_and_Cloud/Observability_and_SecOps/observability/SKILL.md)
   ...
   ```bash
   [kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) argo rollouts dashboard   # local web UI: http://localhost:3100
   [kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) get analysisrun -n <ns>
   [kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) describe analysisrun <name> -n <ns>   # per-measurement results
   ```
...
- Manage `Rollout` specs the same way as any other [GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-tracked
  resource — see
...
- **Symptom:** A canary rollout is stuck at an intermediate `setWeight`
  ...
  **Fix:** Distinguish a manual `pause: {}` (no duration — waits for an
  ...
  `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) get analysisrun -n <ns>` and `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) describe analysisrun
...
- **Symptom:** Argo CD shows the `Rollout` as perpetually `Progressing`,
  never `Healthy`, even though `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) argo rollouts get rollout` shows
...
- **Symptom:** `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) argo rollouts abort` was run to stop a bad
...
- **Symptom:** Promoting via `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) argo rollouts promote --full` was
