...
Argo CD is the reconciler that makes `[gitops](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)` real, but knowing the [GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md) model doesn't tell you
...
For stuck-sync states, sync waves, and the `[argocd](../argocd/SKILL.md) app` command reference, read
`../../../Global_References/[argocd](../argocd/SKILL.md)-troubleshooting.md`.
...
[commit](../../../common/git-workflow/commit/SKILL.md) to add a new service everywhere it belongs. ApplicationSets go further, generating
...
**Done when:** onboarding a new service to an existing environment pattern is one small [commit](../../../common/git-workflow/commit/SKILL.md), not
a manual `[argocd](../argocd/SKILL.md) app create`.
...
needs a CRD, ConfigMap, or Secret to exist first. Sync waves (`[argocd](../argocd/SKILL.md).argoproj.io/sync-wave`
...
metadata:
  annotations:
    [argocd](../argocd/SKILL.md).argoproj.io/sync-wave: "-1"   # applied before wave 0 resources
```
...
`[operators-and-crds](../../../../DevOps_and_Cloud/Containers_and_Orchestration/operators-and-crds/SKILL.md)` for what those resources are and `[kubernetes-operations](../../Containers_and_Orchestration/[kubernetes](../../Containers_and_Orchestration/kubernetes/SKILL.md)-operations/SKILL.md)` for reading the
...
Applications on manual sync long-term reintroduces the human-in-the-loop step [GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md) exists to
...
`[release-management](../../../common/deployment/release-management/SKILL.md)` rather than disabling automation outright.
...
without diagnosing just repeats the same failure. Check `[argocd](../argocd/SKILL.md) app get <name> --show-operation` and
the resource tree for the specific resource that isn't healthy, then look at its own events —
Argo CD is reporting the [Kubernetes](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubernetes/SKILL.md)-level problem, not causing it. See `[kubernetes-operations](../../Containers_and_Orchestration/[kubernetes](../../Containers_and_Orchestration/kubernetes/SKILL.md)-operations/SKILL.md)` for
...
Application use the `default` project with unrestricted scope. This is what turns "anyone can [commit](../../../common/git-workflow/commit/SKILL.md)
an Application manifest" from a security risk into a bounded one. See `[multi-tenancy](../../../../DevOps_and_Cloud/Containers_and_Orchestration/multi-tenancy/SKILL.md)` for the
