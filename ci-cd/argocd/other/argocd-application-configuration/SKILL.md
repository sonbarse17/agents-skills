...
difference between a [GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md) setup that quietly self-heals drift and one
that either does nothing on its own (manual toil) or auto-prunes resources
nobody meant to delete. This skill assumes you already know *why* [GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)
...
- The application's manifests already exist in a Git repo Argo CD can
  reach — plain YAML, [Kustomize](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kustomize/SKILL.md), or Helm; this skill focuses on the
  `Application` resource wrapping any of those, not on templating itself.
- `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md)` access to the cluster for direct inspection when the
  `[argocd](../argocd/SKILL.md)` CLI/UI view isn't enough (`[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) get application -n [argocd](../argocd/SKILL.md)`,
  `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) describe`).
...
1. **Write the base `Application` manifest**, one per deployable unit:
   ...
   spec:
     project: default
     source:
       repoURL: https://[github](../../../github-actions/other/github/SKILL.md).com/example/[gitops](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-config.git
       targetRevision: main
...
2. **Choose the sync policy deliberately, per environment.** Three
   ...
   syncPolicy:
     automated:
       ...
       selfHeal: true      # revert out-of-band ([kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) edit) drift
...
7. **Apply and verify:**
   ```bash
   [kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) apply -f payments-api-prod-application.yaml
...
- Write custom health checks for every CRD your `Application`s manage that
  ...
  how to check it) hides real provisioning failures from [dashboards](../../../../DevOps_and_Cloud/Observability_and_SecOps/dashboards/SKILL.md) and
  alerts.
...
- **Symptom:** After removing a Deployment from the Git overlay (thinking
  ...
  **Fix:** This is `prune: true` doing exactly what it's configured to do
  ...
  from Git until deliberately deleted via `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md)`.
...
- **Symptom:** An `Application` is stuck `Progressing` indefinitely for a
  ...
  even though `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) get` shows the resource's `status.phase: Ready`.
...
- **Symptom:** A `PreSync` migration Job fails once, is fixed, and re-run
  ...
  **Fix:** The failed Job wasn't cleaned up because
  ...
  `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) delete job` every time.
