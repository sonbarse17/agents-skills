...
Argo Events is a [Kubernetes](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubernetes/SKILL.md)-native event framework that decouples *event
...
Workflow, creating/patching a [Kubernetes](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubernetes/SKILL.md) resource, calling another
...
[GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-managed cluster as everything else. This matters operationally
...
- Triggering an Argo Workflows pipeline (see
  ...
  from an external event: a [GitHub](../../../github-actions/other/github/SKILL.md) webhook, a file landing in S3/GCS, a
...
- Replacing a bespoke webhook-receiver Deployment/polling script with a
  declarative, [GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-managed `EventSource`/`Sensor` pair.
...
- Network reachability for the event's origin to hit the cluster: for
  webhook-based `EventSource`s ([GitHub](../../../github-actions/other/github/SKILL.md), Gitlab, generic webhook), an
...
- If triggers target Argo Workflows, the Argo Workflows controller
  ...
  [Kubernetes](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubernetes/SKILL.md) API, so ordinary RBAC applies.
...
2. **Define an `EventSource`** for the origin of events. Webhook example
   (generic HTTP webhook — [GitHub](../../../github-actions/other/github/SKILL.md)/GitLab have dedicated event source
...
6. **Verify and debug:**
   ```bash
   [kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) get eventsource,sensor,eventbus -n automation
   [kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) logs -n automation deploy/upload-webhook-eventsource -f
   [kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) logs -n automation deploy/upload-processor-sensor -f
   [kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) describe sensor upload-processor -n automation   # trigger conditions/status
   ```
...
- Use dedicated `EventSource` types ([GitHub](../../../github-actions/other/github/SKILL.md), GitLab, Stripe, etc.) with
...
- Version and [GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-manage `EventSource`/`Sensor`/`EventBus` manifests
  the same as any other cluster resource — treat them as part of the
  [GitOps](../../../../DevOps_and_Cloud/Containers_and_Orchestration/gitops/SKILL.md)-tracked config repo, not a one-off `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) apply` nobody
  remembers making.
...
- **Symptom:** A `Sensor` trigger fires but the target `Workflow` never
  ...
  **Fix:** Check the `Sensor` Pod's own logs and `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) describe
...
- **Symptom:** A generic webhook `EventSource` is receiving forged/
  ...
  **Fix:** The generic `webhook` EventSource type has no built-in
  ...
  type (e.g., `[github](../../../github-actions/other/github/SKILL.md)` with a configured webhook secret) which validates
...
triggers the pipeline, and `[kubectl](../../../../DevOps_and_Cloud/Containers_and_Orchestration/kubectl/SKILL.md) describe sensor upload-processor -n
