...
# [GitHub](../../other/github/SKILL.md) Actions Single-Repo Workflows

## Purpose

Most repos' CI needs are served by workflow YAML that lives entirely
within that repo under `.[github](../../other/github/SKILL.md)/workflows/`, defining its own triggers,
jobs, and steps. This skill covers the concrete [GitHub](../../other/github/SKILL.md) Actions mechanics
...
`.[github](../../other/github/SKILL.md)/actions/` for within-repo reuse — as distinct from the
...
this skill focuses on [GitHub](../../other/github/SKILL.md) Actions' actual YAML syntax and runner
behavior.

## When to use

- A repository needs its first CI workflow, or an existing
  `.[github](../../other/github/SKILL.md)/workflows/*.yml` file is failing and needs debugging.
...
- Extracting duplicated step sequences within one repo's workflows into a
  local composite action under `.[github](../../other/github/SKILL.md)/actions/`.
...
- [GitHub](../../other/github/SKILL.md) Actions enabled on the repository (Settings → Actions → General).
- Runner availability: [GitHub](../../other/github/SKILL.md)-hosted runners (`ubuntu-latest`,
...
- Secrets already stored in **Settings → Secrets and variables → Actions**
  ...
  YAML.
- Familiarity with YAML expression syntax (`${{ }}`) and [GitHub](../../other/github/SKILL.md)'s built-in
  contexts (`[github](../../other/github/SKILL.md).*`, `secrets.*`, `matrix.*`, `needs.*`).
...
1. **Scope triggers precisely.** Use `paths:`/`paths-ignore:` in a
   ...
   on:
     ...
     push:
       branches: [main]
   concurrency:
     group: ci-${{ [github](../../other/github/SKILL.md).workflow }}-${{ [github](../../other/github/SKILL.md).ref }}
...
5. **Extract repeated step sequences into a local composite action**
   under `.[github](../../other/github/SKILL.md)/actions/<name>/action.yml` when the same steps recur
   across jobs *within this repo* — this is the single-repo analog of a
   shared library, scoped to one repo:
   ```yaml
   # .[github](../../other/github/SKILL.md)/actions/setup-toolchain/action.yml
   ...
   ```yaml
   steps:
     - uses: ./.[github](../../other/github/SKILL.md)/actions/setup-toolchain
     - run: npm test
   ```
...
6. **Gate deploy jobs with `environment:` protection rules** rather than
   ...
   deploy-staging:
     ...
     steps:
       - run: ./deploy.sh staging ${{ [github](../../other/github/SKILL.md).sha }}
   ```

7. **Pin every third-party action to a full [commit](../../../common/git-workflow/commit/SKILL.md) SHA or, at minimum, a
...
- When step logic is copy-pasted three or more times *within this repo*,
  extract a local composite action (`.[github](../../other/github/SKILL.md)/actions/`); when it's
...
- **Symptom:** A required check named `test` passes even though one
  ...
  **Fix:** With `fail-fast: false`, [GitHub](../../other/github/SKILL.md) still reports the overall job
...
- **Symptom:** A workflow that ran safely for months suddenly executes
  ...
  **Fix:** This is the risk of tracking `@main`/`@latest` on a third-party
  action; pin to a full [commit](../../../common/git-workflow/commit/SKILL.md) SHA (most defensible) or at least a
...
- **Symptom:** Two pushes to the same branch in quick succession both run
  ...
  **Fix:** Add `concurrency: { group: ci-${{ [github](../../other/github/SKILL.md).ref }}, cancel-in-progress:
...
`.[github](../../other/github/SKILL.md)/workflows/ci.yml`:
```yaml
name: ci
on:
  pull_request:
...
concurrency:
  group: ci-${{ [github](../../other/github/SKILL.md).ref }}
  cancel-in-progress: true
