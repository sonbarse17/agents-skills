...
`stages:`/`rules:` model that differs in specifics from [GitHub](../../../github-actions/other/github/SKILL.md) Actions or
[Jenkins](../../../jenkins/other/jenkins/SKILL.md) even though the underlying pipeline concepts overlap (see
...
- Designing merge-request pipelines (`merge_request_event`) alongside
  ...
  [commit](../../../common/git-workflow/commit/SKILL.md).
...
2. **Use `workflow:` to control whether a pipeline runs at all**, before
   ...
   pipelines for the same [commit](../../../common/git-workflow/commit/SKILL.md) (once as a branch pipeline, once as a
...
5. **Share pipeline templates across projects with `include:`** —
   ...
   discipline as a [Jenkins](../../../jenkins/other/jenkins/SKILL.md) shared library or [GitHub](../../../github-actions/other/github/SKILL.md) Actions reusable
...
- Set `workflow:rules` once at the top of the file to prevent duplicate
  branch + merge-request pipelines firing for the same [commit](../../../common/git-workflow/commit/SKILL.md), rather than
...
- **Symptom:** A job that should run on merge requests runs twice — once
  ...
  **Fix:** Add a top-level `workflow:rules:` block that picks exactly one
  pipeline source per [commit](../../../common/git-workflow/commit/SKILL.md) context (`merge_request_event` OR
