...
contract MyVaultTest is Test {
    MyVault [vault](../../Security/cryptography-secrets/vault/SKILL.md);
    address alice = makeAddr("alice");
    uint256 constant BAL = 1000e18;

    function setUp() public {
        [vault](../../Security/cryptography-secrets/vault/SKILL.md) = new MyVault();
        deal(alice, BAL);
    }
    ...
    function test_Deposit() public {
        vm.prank(alice);
        uint256 shares = [vault](../../Security/cryptography-secrets/vault/SKILL.md).deposit{value: 100e18}(100e18, alice);
    ...
    function testFuzz_Deposit_RoundTrip(uint256 amount) public {
        ...
        vm.prank(alice);
        uint256 shares = [vault](../../Security/cryptography-secrets/vault/SKILL.md).deposit{value: amount}(amount, alice);
        vm.prank(alice);
        uint256 returned = [vault](../../Security/cryptography-secrets/vault/SKILL.md).withdraw(shares, alice, alice);
        assertEq(returned, amount, "Roundtrip preserves value");
    }

    // Invariant test
    function invariant_Solvency() public {
        assertGe(address([vault](../../Security/cryptography-secrets/vault/SKILL.md)).balance, [vault](../../Security/cryptography-secrets/vault/SKILL.md).totalSupply());
    }
}
```
...
contract VaultHandler is Test {
    ...
    function deposit(uint256 amount, uint256 seed) public {
        ...
        vm.prank(u);
        [vault](../../Security/cryptography-secrets/vault/SKILL.md).deposit{value: amount}(amount, u);
    }
}
```
