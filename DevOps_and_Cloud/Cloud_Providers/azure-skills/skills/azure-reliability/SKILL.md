...
| Generate multi-region IaC | ✅ Yes (additions for the secondary region + Front Door) | `[azure-prepare](../../../[azure-prepare](../azure-prepare/SKILL.md)/SKILL.md)` for full new-app IaC scaffolding |
| Deploy IaC for reliability changes | ✅ Yes (runs `azd up` / `terraform apply` / `az deployment` itself, after user confirmation) | `[azure-deploy](../[azure-deploy](../../../../Infrastructure_as_Code/azure-deploy/SKILL.md)/SKILL.md)` for general/non-reliability deploys |
| Validate pre-deployment | Reliability checks only | `[azure-validate](../[azure-validate](../../../azure-validate/SKILL.md)/SKILL.md)` for full validation |
