...
> **Orchestrator relationship:** Called by `[azure-app-onboard](../../../[azure-app-onboard](../azure-app-onboard/SKILL.md)/SKILL.md)` at Step 3, or standalone for code readiness checks. When called by orchestrator, return control to `[azure-app-onboard](../../../[azure-app-onboard](../azure-app-onboard/SKILL.md)/SKILL.md)` after writing artifacts — do NOT invoke downstream phases directly.

Phase 1 of 4 in AppOnboard pipeline. Session: `.copilot-azure/sessions/{session-id}/`. Reads `context.json`. Writes `components[]`, `repo{}`, `detectedInfra[]`. Produces `prereq-output.json`. Schema: [`prereq-schemas.ts`](../../../../../Global_References/prereq-schemas.ts) — `PrereqOutput`, `BuildRequirements`. Direct entry supported.

## When NOT to Use

| Signal | Redirect |
|--------|----------|
| Validate infrastructure (Bicep/TF/azure.yaml) | **[azure-validate](../[azure-validate](../../../azure-validate/SKILL.md)/SKILL.md)** |
| Generate IaC | **[azure-prepare](../../../[azure-prepare](../azure-prepare/SKILL.md)/SKILL.md)** |
| End-to-end idea-to-production | **[azure-app-onboard](../../../[azure-app-onboard](../azure-app-onboard/SKILL.md)/SKILL.md)** |
| Run `azd up` or deploy | **[azure-deploy](../[azure-deploy](../../../../Infrastructure_as_Code/azure-deploy/SKILL.md)/SKILL.md)** |
...
> **ONLY exception — two sanctioned contexts, both consent-gated:** (a) code the agent **modified** during migration/remediation (see [remediation-protocol.md](../../../../../Global_References/remediation-protocol.md) step 6), or (b) code the agent **wrote** from scratch on the zero-code path (see [zero-code-path.md](../../../../../Global_References/zero-code-path.md)). In either case, install/build/test runs ONLY via the user-confirmed build-validation gate ([build-check.md](../../../../../Global_References/build-check.md) Step 3), after the user answers that specific per-command consent prompt. General prior consent never counts.
...
- **Exists** → ⛔ read [session-protocol.md](../../../../../Global_References/[azure-app-onboard](../../../[azure-app-onboard](../azure-app-onboard/SKILL.md)/SKILL.md)-prereq_session-protocol.md) for resume/fresh gate. Do NOT proceed until user answers.
...
⛔ Verify `context.json` exists on disk. Read [readiness-gate.md](../../../../../Global_References/readiness-gate.md) (verdicts, tiers, batch-then-approve, fast-track) then [prereq-artifacts.md](../../../../../Global_References/prereq-artifacts.md) (write procedures, schemas).

### Step 5: Present Findings

Per [readiness-gate.md § Present Findings](../../../../../Global_References/readiness-gate.md) — show verdicts grouped by severity before proceeding.

### Step 6: Remediation (conditional)

⛔ **You MUST read [remediation-protocol.md](../../../../../Global_References/remediation-protocol.md)** IF any ❌ FAIL verdict, 🔧 Recommended Fix, or ⚠️ WARN with `fixPhase: "prereq"` exists. Contains remediation loop, static verification, re-eval mandate, post-remediation artifact updates, and the build-validation consent gate. If all verdicts are ✅ PASS or ⚠️ WARN without `fixPhase: "prereq"`, skip to Step 7.
...
| 2 | `cloudSdkFindings[]` non-empty (user chose "Continue evaluation anyway") | Present the cloud-SDK → Azure swap mapping as 🔶 blockers, then `ask_user` with this exact prompt: **"🔶 Cloud SDK migration required — these dependencies must be swapped before this app can deploy to Azure. (Redirect to [azure-cloud-migrate](../../../[azure-cloud-migrate](../azure-cloud-migrate/SKILL.md)/SKILL.md) / Stop — swap manually and re-run)"** — Redirect sets `routeToSkill: "[azure-cloud-migrate](../../../[azure-cloud-migrate](../azure-cloud-migrate/SKILL.md)/SKILL.md)"`, Stop halts. ⛔ Pipeline stops — do NOT proceed to architecture planning, and do NOT offer a "continue to prepare" option; the app can't deploy until the deps are swapped. |
| 3 | Orchestrator + no `routeToSkill` | Tell the user: "✅ Your app has been evaluated and is ready — let's plan your Azure deployment." Then invoke `[azure-app-onboard](../../../[azure-app-onboard](../azure-app-onboard/SKILL.md)/SKILL.md)`. ⛔ Do NOT stop, do NOT wait for user input, do NOT narrate internal handoffs. The user already consented to the full pipeline at scope triage. |
| 4 | Direct + ready/readyWithCaveats + no Azure infra | `ask_user`: "Deploy to Azure (full pipeline)" → invoke `[azure-app-onboard](../../../[azure-app-onboard](../azure-app-onboard/SKILL.md)/SKILL.md)` / "Not now" |
| 5 | Direct + ready/readyWithCaveats + existing Azure infra | `ask_user`: "Start fresh" → invoke `[azure-app-onboard](../../../[azure-app-onboard](../azure-app-onboard/SKILL.md)/SKILL.md)` / "Use existing infra" → invoke `[azure-prepare](../../../[azure-prepare](../azure-prepare/SKILL.md)/SKILL.md)` / "Not now" |
| 6 | Direct + blocked | Report blocker summary + "Fix and re-run." |

Severity tiers (🛑🔶❌🔧⚠️✅) are defined in [readiness-gate.md](../../../../../Global_References/readiness-gate.md).
...
| Prereq output | `prereq-output.json` | prepare phase (via `[azure-app-onboard](../../../[azure-app-onboard](../azure-app-onboard/SKILL.md)/SKILL.md)`) |
