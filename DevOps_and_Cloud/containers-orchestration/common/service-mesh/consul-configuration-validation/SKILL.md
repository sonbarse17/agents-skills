...
- Before applying a new or changed service definition, config entry
  ...
  [datacenter](../../../../Software_Engineering_and_Other/Miscellaneous/systems-low-level/datacenter/SKILL.md).
...
- Auditing an existing [datacenter](../../../../Software_Engineering_and_Other/Miscellaneous/systems-low-level/datacenter/SKILL.md)'s intentions for unintended
  wildcard-allow rules before a security review.

## Prerequisites & environment

- The `consul` CLI matching (or compatible with) the target [datacenter](../../../../Software_Engineering_and_Other/Miscellaneous/systems-low-level/datacenter/SKILL.md)'s
...
- For CI-integrated validation: either a disposable Consul dev-mode
  ...
  against, or a non-production [datacenter](../../../../Software_Engineering_and_Other/Miscellaneous/systems-low-level/datacenter/SKILL.md) that mirrors production
  topology closely enough that subset/intention checks are meaningful.
- `consul-k8s`-managed CRDs (on [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)) can additionally be
  validated with standard `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply --dry-run=server`, which
...
7. **Wire validation into CI** as a gate before any config entry or
   intention reaches a production [datacenter](../../../../Software_Engineering_and_Other/Miscellaneous/systems-low-level/datacenter/SKILL.md):
   ```bash
...
8. **For `consul-k8s` CRDs, dry-run against the API server first**, then
   ...
   acceptance by [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) doesn't guarantee the `consul-k8s` controller
   successfully reconciled it into Consul's catalog):
   ```bash
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply --dry-run=server -f service-intentions.yaml
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -f service-intentions.yaml
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) get serviceintentions payments-api -o yaml | \
     grep -A3 status
   ```
...
- Always follow `consul config write` with `consul config read` (or the
  [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) CRD's `status` field) to confirm the entry landed as
...
- **Symptom:** An intention explicitly allows `checkout-service →
  ...
  **Fix:** A more specific or higher-precedence intention (or the
  [datacenter](../../../../Software_Engineering_and_Other/Miscellaneous/systems-low-level/datacenter/SKILL.md)'s default policy) is overriding it — run `consul intention
...
- **Symptom:** A `consul-k8s` `ServiceIntentions` CRD applies cleanly per
  `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply` with no errors, but the corresponding Consul intention
  never takes effect.
  **Fix:** CRD acceptance by the [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) API server only confirms
  ...
  get` directly against Consul, not just `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get`.
...
- **Symptom:** To unblock a failing intention check during
  ...
  **Fix:** A wildcard allow intention removes mesh authorization
  ...
  [datacenter](../../../../Software_Engineering_and_Other/Miscellaneous/systems-low-level/datacenter/SKILL.md) where possible, with an explicit tracked task to revert it
...
change applied to the real production [datacenter](../../../../Software_Engineering_and_Other/Miscellaneous/systems-low-level/datacenter/SKILL.md) with `consul config
