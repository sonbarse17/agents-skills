...
1. **Read-only.** Allowed: read IaC/backup config, `aws rds describe-db-snapshots`,
   ...
   `gcloud/az` equivalents, `velero get backups`, `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get volumesnapshot`,
   `terraform plan`, reading restore-test records and [runbooks](../../../../observability-monitoring-logging/common/incident-detection/runbooks/SKILL.md). **Never** run a
...
4. **Recovery scope includes the things people forget.** Not just the primary
   ...
   registries, CI/CD config, [dashboards](../../../../observability-monitoring-logging/common/dashboard-design/dashboards/SKILL.md) and alert definitions, and the [runbook](../../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md)
...
- Inventory **stateful** assets and rank them by what their loss would cost:
  ...
  registries.
- Find the stated bar: RTO/RPO in an SLA, ADR, [runbook](../../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md), or ticket. Record where
...
- Look for evidence of past restores or game days: test records, [incident](../../../../observability-monitoring-logging/common/incident-detection/incident/SKILL.md)
  reports, [runbook](../../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md) sign-offs, CI jobs that restore into a scratch environment.
...
- **Verification** — no restore test in the last N months, tests that restore but
  ...
  [alerting](../../../../observability-monitoring-logging/common/alerting/alerting/SKILL.md) on backup *job failure* (silent failure is the norm here), retention
...
- **Failover & continuity** — never-exercised failover, no documented decision
  ...
  provider outage.
- **Documentation** — no restore [runbook](../../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md), or one referencing renamed
  resources/retired tooling; recovery knowledge held by one person.
  ([Runbook](../../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md) drafting: `/[runbook](../../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md)`.)
...
- [ ] Backup-job *failure* [alerting](../../../../observability-monitoring-logging/common/alerting/alerting/SKILL.md) was checked — silent failure is the default.
