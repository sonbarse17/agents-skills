...
attacks via a lightweight agent (a [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) DaemonSet/sidecar, a host
...
[Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-native tools like Chaos Mesh/LitmusChaos in that it works
consistently across [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md), VMs, and bare metal without needing a
...
- Standing up Gremlin in a [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) cluster, EC2 fleet, or ECS
...
- Migrating an existing [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-native chaos setup (Chaos Mesh/
...
- The Gremlin agent installed on every target:
  - **[Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md):** the `gremlin` Helm chart, deployed as a DaemonSet so
...
- The same automated abort/[observability](../../../../observability-monitoring-logging/common/fundamentals/observability/SKILL.md) wiring called for generically in
  [chaos-engineering-and-resilience-testing](../../../site-reliability-engineering/skills/[chaos-engineering-and-resilience-testing](../../../Software_Engineering_and_Other/Frontend/[chaos-engineering](../chaos-engineering/SKILL.md)-and-resilience-testing/SKILL.md)/SKILL.md) —
  Gremlin's own halt conditions (below) are necessary but should be paired
  with your own SLI-based [alerting](../../../../observability-monitoring-logging/common/alerting/alerting/SKILL.md), not relied on alone.
...
1. **Install the agent scoped to the intended blast-radius environment**
   ...
   ```bash
   # [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md): install via Helm, scoped to a specific namespace's
...
3. **Define a state attack** (process kill, container/host shutdown) to
   ...
   whatever platform the agent runs on ([Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md), EC2, or bare metal).
...
5. **Always set an explicit halt condition** tied to a real health check
   ...
   ```json
   {
     "haltCondition": {
       "type": "metric",
       "metricSource": "[datadog](../../../../observability-monitoring-logging/datadog/other/datadog/SKILL.md)",
...
7. **Use the Gremlin Terraform provider to version-control recurring
   ...
   alongside other [infrastructure-as-code](../../../../infrastructure-as-code/common/other/infrastructure-as-code/SKILL.md), instead of only ever
...
**Scenario:** The `payments-api` team, running on [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) with the
