...
The built-in [Kubernetes](../../other/kubernetes/SKILL.md) `HorizontalPodAutoscaler` (HPA) can only scale on
...
[keda-configuration-validation](../keda-configuration-validation/SKILL.md)/SKILL.md).
...
- A [Kubernetes](../../other/kubernetes/SKILL.md) cluster with the KEDA operator installed (commonly via
...
- Credentials for the event source (SASL/mTLS for Kafka, IAM role or
  ...
  as a [Kubernetes](../../other/kubernetes/SKILL.md) `Secret`, referenced via `TriggerAuthentication` —
  never inlined directly into the `ScaledObject` spec.
- The standard [Kubernetes](../../other/kubernetes/SKILL.md) `metrics-server` is **not** required for KEDA
...
- **Symptom:** A `ScaledJob` for a batch workload leaves large numbers
  ...
  slowing down `[kubectl](../../other/kubectl/SKILL.md)` operations and cluttering the namespace.
...
- [keda-configuration-validation](../keda-configuration-validation/SKILL.md)/SKILL.md) — validating this `ScaledObject`/`TriggerAuthentication` configuration (auth, thresholds, cooldowns) before it reaches production.
