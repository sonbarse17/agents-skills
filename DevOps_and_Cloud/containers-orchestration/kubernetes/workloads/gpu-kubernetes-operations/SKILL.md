...
# GPU [Kubernetes](../../other/kubernetes/SKILL.md) Operations
...
- [Kubernetes](../../other/kubernetes/SKILL.md) 1.28+ cluster with GPU-capable nodes
...
# Verify installation
[kubectl](../../other/kubectl/SKILL.md) get pods -n gpu-operator
[kubectl](../../other/kubectl/SKILL.md) get nodes -o json | jq '.items[].status.allocatable["nvidia.com/gpu"]'
```
...
[kubectl](../../other/kubectl/SKILL.md) label nodes gpu-node-01 nvidia.com/mig.config=all-1g.10gb --overwrite

# Verify MIG instances
[kubectl](../../other/kubectl/SKILL.md) exec -it nvidia-device-plugin-xxxxx -n kube-system -- nvidia-smi mig -lgi

# Check available MIG resources
[kubectl](../../other/kubectl/SKILL.md) get nodes gpu-node-01 -o json | jq '.status.allocatable | with_entries(select(.key | startswith("nvidia.com")))'
```
...
[kubectl](../../other/kubectl/SKILL.md) patch clusterpolicy/cluster-policy \
  --type merge \
...
| Symptom | Check | Fix |
|---------|-------|-----|
| Pod stuck in Pending | `[kubectl](../../other/kubectl/SKILL.md) describe pod` for GPU resource events | Verify node has allocatable GPUs, check taints/tolerations |
