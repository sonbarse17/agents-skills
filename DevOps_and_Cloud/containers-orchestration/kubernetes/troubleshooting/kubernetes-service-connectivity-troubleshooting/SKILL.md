...
# [Kubernetes](../../other/kubernetes/SKILL.md) Service Connectivity Troubleshooting

## Purpose

A [Kubernetes](../../other/kubernetes/SKILL.md) `Service` is just a stable virtual IP and DNS name backed
...
- `[kubectl](../../other/kubectl/SKILL.md) get endpoints`/`endpointslices` for a Service returns empty
...
- `[kubectl](../../other/kubectl/SKILL.md)` access to inspect `Service`, `Endpoints`/`EndpointSlice`,
...
1. **Check the Service's selector and its Endpoints together, side by
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get svc <name> -n <namespace> -o jsonpath='{.spec.selector}{"\n"}'
   [kubectl](../../other/kubectl/SKILL.md) get endpoints <name> -n <namespace>
   [kubectl](../../other/kubectl/SKILL.md) get endpointslice -n <namespace> -l [kubernetes](../../other/kubernetes/SKILL.md).io/service-name=<name>
   ```
   ...
   none are currently `Ready` ([Kubernetes](../../other/kubernetes/SKILL.md) only populates Endpoints with
   ready pod IPs).

2. **Compare the selector against actual pod labels exactly**:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get pods -n <namespace> --show-labels
   ```
...
3. **If labels match but Endpoints are still empty, check pod
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get pods -n <namespace> -o wide
...
4. **Test the readiness probe's target manually** from inside the
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) exec <pod> -n <namespace> -- curl -sf localhost:<port><path>
   ```
...
5. **Once Endpoints are populated, test the pod IP directly**, bypassing
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) run net-test -n <namespace> --image=busybox:1.36 --rm -it --restart=Never -- \
...
6. **If the pod IP works but the Service `ClusterIP`/DNS name doesn't,
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get pods -n kube-system -l k8s-app=kube-proxy
   [kubectl](../../other/kubectl/SKILL.md) logs -n kube-system <kube-proxy-pod>
   ```
...
7. **For DNS-specific failures**, test resolution directly and check
   CoreDNS itself:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) exec <pod> -n <namespace> -- nslookup <service>.<namespace>.svc.cluster.local
   [kubectl](../../other/kubectl/SKILL.md) get pods -n kube-system -l k8s-app=kube-dns
   [kubectl](../../other/kubectl/SKILL.md) logs -n kube-system -l k8s-app=kube-dns
   ```
...
8. **For a headless Service (`clusterIP: None`) or StatefulSet DNS**
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get svc <name> -n <namespace> -o yaml | grep clusterIP
   [kubectl](../../other/kubectl/SKILL.md) get statefulset <name> -n <namespace> -o jsonpath='{.spec.serviceName}'
   ```
...
- Diagnose Service/selector/Endpoints/labels with exact `[kubectl](../../other/kubectl/SKILL.md)`
...
- **Symptom:** A Service has zero Endpoints despite pods that are
  ...
  **Fix:** Compare `[kubectl](../../other/kubectl/SKILL.md) get svc -o jsonpath='{.spec.selector}'`
  against `[kubectl](../../other/kubectl/SKILL.md) get pods --show-labels` directly — a label typo,
...
- **Symptom:** A pod intermittently appears and disappears from a
  ...
  **Fix:** Its readiness probe is intermittently failing — check the
  ...
  Endpoints purely due to failing readiness is [Kubernetes](../../other/kubernetes/SKILL.md) working
...
```bash
[kubectl](../../other/kubectl/SKILL.md) get svc payments-api -n payments -o jsonpath='{.spec.selector}{"\n"}'
# {"app":"payments"}

[kubectl](../../other/kubectl/SKILL.md) get endpoints payments-api -n payments
# NAME           ENDPOINTS   AGE
# payments-api   <none>      3h

[kubectl](../../other/kubectl/SKILL.md) get pods -n payments --show-labels
...
```bash
[kubectl](../../other/kubectl/SKILL.md) patch deployment payments-api -n payments --type merge \
...
```bash
[kubectl](../../other/kubectl/SKILL.md) exec -n reporting deploy/reporting-job -- \
...
[kubectl](../../other/kubectl/SKILL.md) get networkpolicy -n reporting -o yaml
...
spec:
  ...
  policyTypes: ["Egress"]
  egress:
    - to: [{ namespaceSelector: { matchLabels: { [kubernetes](../../other/kubernetes/SKILL.md).io/metadata.name: kube-system } } }]
      ports:
...
```bash
[kubectl](../../other/kubectl/SKILL.md) apply -f allow-dns-egress.yaml
[kubectl](../../other/kubectl/SKILL.md) exec -n reporting deploy/reporting-job -- \
  nslookup billing.finance.svc.cluster.local
