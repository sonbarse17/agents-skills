...
# [Kubernetes](../../other/kubernetes/SKILL.md) Node Maintenance and Troubleshooting
...
one of the most common self-inflicted causes of a [Kubernetes](../../other/kubernetes/SKILL.md) outage: a
...
- A node shows `NotReady` in `[kubectl](../../other/kubectl/SKILL.md) get nodes` and needs root-cause
  diagnosis before deciding whether to wait, intervene, or replace it.
- A `[kubectl](../../other/kubectl/SKILL.md) drain` is hanging or timing out and it's unclear whether
  that's expected (a `PodDisruptionBudget` correctly blocking it) or a
  different problem.
- Bringing a node back into service with `[kubectl](../../other/kubectl/SKILL.md) uncordon` after
...
- `[kubectl](../../other/kubectl/SKILL.md)` access with permission to cordon/drain/uncordon nodes and to
...
- SSH or console access to the node itself for OS-level diagnosis when
  ...
  — `[kubectl](../../other/kubectl/SKILL.md)` alone often can't explain *why* a node stopped reporting
  Ready.
- Understanding that `[kubectl](../../other/kubectl/SKILL.md) drain` does not evict DaemonSet-managed
...
1. **Diagnose a `NotReady` node before doing anything else**, if that's
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) describe node <node>
   ```
...
3. **Before draining for planned maintenance, check what's actually
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get pods -A -o wide --field-selector spec.nodeName=<node>
   [kubectl](../../other/kubectl/SKILL.md) get pdb -A
   ```
...
4. **Cordon the node** to stop new pods from being scheduled onto it
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) cordon <node>
   ```

5. **Drain the node**, letting `[kubectl](../../other/kubectl/SKILL.md) drain` respect
   PodDisruptionBudgets by default:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) drain <node> \
     --ignore-daemonsets \
     --delete-emptydir-data \
     --timeout=300s
   ```
   `[kubectl](../../other/kubectl/SKILL.md) drain` cordons the node automatically as part of this
...
6. **If the drain hangs or times out, diagnose before forcing
   anything**:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) get pdb -A -o wide
   [kubectl](../../other/kubectl/SKILL.md) describe pdb <pdb-name> -n <namespace>
   ```
...
8. **Uncordon the node** to make it schedulable again:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) uncordon <node>
   ```
...
- Give `[kubectl](../../other/kubectl/SKILL.md) drain` a `--grace-period` (or trust the pod's own
...
- **Symptom:** `[kubectl](../../other/kubectl/SKILL.md) drain` hangs indefinitely or times out.
  **Fix:** Check `[kubectl](../../other/kubectl/SKILL.md) get pdb -A` for a `PodDisruptionBudget`
...
- **Symptom:** A node is drained for routine maintenance and a service
  ...
  **Fix:** With no PDB, eviction-respecting drain has nothing to check
  ...
  maintenance window, and check `[kubectl](../../other/kubectl/SKILL.md) get pods -o wide
  --field-selector spec.nodeName=<node>` plus `[kubectl](../../other/kubectl/SKILL.md) get pdb -A`
  together before draining, not just PDB existence in isolation.

- **Symptom:** Pods stuck `Terminating` are cleared with
  `[kubectl](../../other/kubectl/SKILL.md) delete pod --force --grace-period=0` to unblock a drain.
  **Fix:**
  ...
  longer logically exists (check `[kubectl](../../other/kubectl/SKILL.md) get volumeattachments`
...
- **Symptom:** A node goes `NotReady` after a transient network blip,
  ...
  **Fix:** The default `pod-eviction-timeout` (5 minutes) passed before
  ...
  toleration for `node.[kubernetes](../../other/kubernetes/SKILL.md).io/not-ready`/`node.[kubernetes](../../other/kubernetes/SKILL.md).io/
...
- **Symptom:** `[kubectl](../../other/kubectl/SKILL.md) uncordon` is run, but expected workloads don't
...
- **Symptom:** `[kubectl](../../other/kubectl/SKILL.md) delete node <node>` is run directly to remove a
...
```bash
[kubectl](../../other/kubectl/SKILL.md) get pods -n payments -o wide --field-selector spec.nodeName=node-3
# payments-api-7d9f6-abc12   1/1   Running   node-3

[kubectl](../../other/kubectl/SKILL.md) get pdb -n payments
...
```bash
[kubectl](../../other/kubectl/SKILL.md) scale deployment payments-api -n payments --replicas=3
[kubectl](../../other/kubectl/SKILL.md) get pdb payments-api-pdb -n payments
# ALLOWED DISRUPTIONS: 1
```

```bash
[kubectl](../../other/kubectl/SKILL.md) cordon node-3
[kubectl](../../other/kubectl/SKILL.md) drain node-3 --ignore-daemonsets --delete-emptydir-data --timeout=300s
# node/node-3 drained
```
...
```bash
[kubectl](../../other/kubectl/SKILL.md) uncordon node-3
[kubectl](../../other/kubectl/SKILL.md) get nodes
# node-3   Ready

[kubectl](../../other/kubectl/SKILL.md) scale deployment payments-api -n payments --replicas=2
```
