...
# [Kubernetes](../../other/kubernetes/SKILL.md) Networking

Every networking problem in [Kubernetes](../../other/kubernetes/SKILL.md) is a chain: client → DNS → Service → Endpoints → pod IP →
...
`[kubectl](../../other/kubectl/SKILL.md) get endpoints <svc>` (or `endpointslices`) tells you immediately whether the Service's
...
```bash
[kubectl](../../other/kubectl/SKILL.md) get svc <name> -o yaml | grep -A3 selector
[kubectl](../../other/kubectl/SKILL.md) get pods --show-labels -l <same-selector>
[kubectl](../../other/kubectl/SKILL.md) get endpoints <name>
```
...
**Done when:** `[kubectl](../../other/kubectl/SKILL.md) get endpoints` shows the expected pod IPs, or you've identified exactly why
it doesn't.
...
- **Test from inside**, not outside: `[kubectl](../../other/kubectl/SKILL.md) exec` into a pod and run `nslookup`/`getent hosts`
