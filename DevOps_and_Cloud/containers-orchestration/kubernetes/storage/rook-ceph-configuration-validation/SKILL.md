...
A `CephCluster` CRD can report `Ready` in [Kubernetes](../../other/kubernetes/SKILL.md) terms — the
...
because [Kubernetes](../../other/kubernetes/SKILL.md)-level pod health and Ceph's own internal
...
- A `CephCluster` shows `HEALTH_WARN`/`HEALTH_ERR` and the specific
  cause isn't obvious from the [Kubernetes](../../other/kubernetes/SKILL.md)-level CRD status alone.
...
- Periodic (e.g. weekly, or pre-change) health verification as part of
  an operational [runbook](../../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md), not only during incidents.
- Validating a Ceph cluster's health *before* a planned disruptive
  operation (node drain, [Kubernetes](../../other/kubernetes/SKILL.md) upgrade, device replacement) to
...
- A Rook-Ceph cluster already deployed per
  [rook-ceph-storage-operations](../[rook-ceph-storage-operations](../rook-ceph-storage-operations/SKILL.md)/SKILL.md),
  with the `rook-ceph-tools` deployment running (`[kubectl](../../other/kubectl/SKILL.md) -n rook-ceph
  get pods -l app=rook-ceph-tools`) — most of the checks below assume a
  shell into this pod (`[kubectl](../../other/kubectl/SKILL.md) -n rook-ceph exec -it deploy/rook-ceph-tools -- bash`).
- `[kubectl](../../other/kubectl/SKILL.md)` read access to the `rook-ceph` namespace and the
...
1. **Start from `ceph status`, not the [Kubernetes](../../other/kubernetes/SKILL.md) CRD's `Ready`
   field** — they measure different things:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) -n rook-ceph exec deploy/rook-ceph-tools -- ceph status
   ```
...
2. **Get the specific reason behind any non-`HEALTH_OK` state** —
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) -n rook-ceph exec deploy/rook-ceph-tools -- ceph health detail
   ```
...
3. **Cross-check the [Kubernetes](../../other/kubernetes/SKILL.md)-level CRD status against the Ceph-level
   status** — they should agree, but confirm rather than assume:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) -n rook-ceph get cephcluster rook-ceph -o jsonpath='{.status.phase}{"\n"}{.status.ceph.health}{"\n"}{.status.ceph.details}'
   [kubectl](../../other/kubectl/SKILL.md) -n rook-ceph get pods -o wide | grep -E 'osd|mon|mgr'
   ```
   A `CephCluster` reporting [Kubernetes](../../other/kubernetes/SKILL.md)-level `Ready`/`Progressing` with
   ...
   operator's own logs
   (`[kubectl](../../other/kubectl/SKILL.md) -n rook-ceph logs -l app=rook-ceph-operator --tail=200`)
...
4. **Verify OSD placement matches the intended failure domain**, not
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) -n rook-ceph exec deploy/rook-ceph-tools -- ceph osd tree
   ```
...
5. **Confirm the CRUSH map's rule set matches the pool's declared
   `failureDomain`**:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) -n rook-ceph exec deploy/rook-ceph-tools -- ceph osd crush rule dump
   [kubectl](../../other/kubectl/SKILL.md) -n rook-ceph get cephblockpool replicapool -o jsonpath='{.spec.failureDomain}'
   ```
...
6. **Check placement group (PG) state for anything other than
   `active+clean`**:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) -n rook-ceph exec deploy/rook-ceph-tools -- ceph pg stat
   [kubectl](../../other/kubectl/SKILL.md) -n rook-ceph exec deploy/rook-ceph-tools -- ceph pg dump_stuck
   ```
...
7. **Check [capacity](../../../AI_and_Agents/Infrastructure/deploy-model/[capacity](../../Cloud_Providers/azure-skills/skills/microsoft-foundry/models/deploy-model/[capacity](../capacity/SKILL.md)/SKILL.md)/SKILL.md) headroom before it becomes an outage**:
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) -n rook-ceph exec deploy/rook-ceph-tools -- ceph df
   [kubectl](../../other/kubectl/SKILL.md) -n rook-ceph exec deploy/rook-ceph-tools -- ceph osd df
   ```
...
8. **Validate CSI provisioning actually works end-to-end**, not just
   ...
   ```bash
   [kubectl](../../other/kubectl/SKILL.md) apply -f - <<'EOF'
   apiVersion: v1
   ...
   EOF
   [kubectl](../../other/kubectl/SKILL.md) get pvc rook-validation-test -w   # confirm it reaches Bound
   [kubectl](../../other/kubectl/SKILL.md) delete pvc rook-validation-test
   ```
...
- Treat `ceph status`/`ceph health detail` as the source of truth for
  storage health, and the [Kubernetes](../../other/kubernetes/SKILL.md) CRD status as a secondary,
  sometimes-lagging signal — never certify a cluster healthy from
  `[kubectl](../../other/kubectl/SKILL.md) get cephcluster` output alone.
...
- Alert on `ceph health detail`'s specific warning codes
  ...
  incidents — see
  [prometheus-and-grafana-[monitoring](../../../../observability-monitoring-logging/common/monitoring-strategy/monitoring/SKILL.md)-stack](../../../[observability](../observability/SKILL.md)-and-platform-extras/skills/[prometheus-and-grafana-[monitoring](../../../../observability-monitoring-logging/common/monitoring-strategy/monitoring/SKILL.md)-stack](../../Containers_and_Orchestration/prometheus-and-grafana-[monitoring](../monitoring/SKILL.md)-stack/SKILL.md)/SKILL.md).
- Include the end-to-end PVC provisioning smoke test (step 8) in CI/CD
  or a post-upgrade [runbook](../../../../observability-monitoring-logging/common/incident-detection/runbook/SKILL.md) — a cluster can be `HEALTH_OK` at the Ceph
...
- **Symptom:** `[kubectl](../../other/kubectl/SKILL.md) get cephcluster` shows `phase: Ready`, but an
  application backed by Rook storage experiences I/O errors or extreme
  latency.
  **Fix:** The CRD's [Kubernetes](../../other/kubernetes/SKILL.md)-level `Ready` reflects operator
...
- **Symptom:** `ceph osd tree` shows the expected number of OSDs and
  ...
  **Fix:** `failureDomain: host` only protects against failures Ceph's
  ...
  that [Kubernetes](../../other/kubernetes/SKILL.md) lists them as separate `Node` objects.
...
```bash
[kubectl](../../other/kubectl/SKILL.md) -n rook-ceph exec deploy/rook-ceph-tools -- ceph osd reweight-by-utilization
```
...
- [etcd-backup-restore-and-cluster-health](../[etcd-backup-restore-and-cluster-health](../../Containers_and_Orchestration/etcd-backup-restore-and-cluster-health/SKILL.md)/SKILL.md) — analogous quorum/health-[monitoring](../../../../observability-monitoring-logging/common/monitoring-strategy/monitoring/SKILL.md) discipline applied to etcd rather than Ceph.
- [prometheus-and-grafana-[monitoring](../../../../observability-monitoring-logging/common/monitoring-strategy/monitoring/SKILL.md)-stack](../../../[observability](../observability/SKILL.md)-and-platform-extras/skills/[prometheus-and-grafana-[monitoring](../../../../observability-monitoring-logging/common/monitoring-strategy/monitoring/SKILL.md)-stack](../../Containers_and_Orchestration/prometheus-and-grafana-[monitoring](../monitoring/SKILL.md)-stack/SKILL.md)/SKILL.md) — continuous [alerting](../../../../observability-monitoring-logging/common/alerting/alerting/SKILL.md) on the Ceph health signals checked manually here.
