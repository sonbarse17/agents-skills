...
Every [Kubernetes](../../other/kubernetes/SKILL.md) distribution — kubeadm, Cluster API, K3s, [OpenShift](../../../openshift/other/openshift/SKILL.md),
...
managed-[Kubernetes](../../other/kubernetes/SKILL.md) users get for free but self-managed cluster operators
...
- TLS client certificates for etcd's peer/client API (kubeadm places
  these at `/etc/[kubernetes](../../other/kubernetes/SKILL.md)/pki/etcd/{ca,server,peer,healthcheck-client}.{crt,key}`
...
- Know your topology: **stacked etcd** (etcd runs as a static pod
  ...
  **external etcd** (a dedicated etcd cluster the API servers point to)
  ...
  stop/start.
- On managed [Kubernetes](../../other/kubernetes/SKILL.md) (EKS/AKS/GKE), etcd is entirely provider-managed
...
1. **Take a snapshot on a schedule, not just before risky changes**:
   ...
   ETCDCTL_API=3 etcdctl snapshot save /backup/etcd-snapshot-$(date +%Y%m%d%H%M%S).db \
     ...
     --cacert=/etc/[kubernetes](../../other/kubernetes/SKILL.md)/pki/etcd/ca.crt \
     --cert=/etc/[kubernetes](../../other/kubernetes/SKILL.md)/pki/etcd/healthcheck-client.crt \
     --key=/etc/[kubernetes](../../other/kubernetes/SKILL.md)/pki/etcd/healthcheck-client.key
   ```
     ...
     jobTemplate:
       spec:
         template:
           spec:
             hostNetwork: true
             nodeSelector: { node-role.[kubernetes](../../other/kubernetes/SKILL.md).io/control-plane: "" }
             tolerations:
               - key: node-role.[kubernetes](../../other/kubernetes/SKILL.md).io/control-plane
                 effect: NoSchedule
             containers:
               - name: etcd-snapshot
                 ...
                 args:
                   - >
                     ...
                     --key=/etc/[kubernetes](../../other/kubernetes/SKILL.md)/pki/etcd/healthcheck-client.key
                 volumeMounts:
                   - { name: etcd-certs, mountPath: /etc/[kubernetes](../../other/kubernetes/SKILL.md)/pki/etcd, readOnly: true }
                   - { name: backup, mountPath: /backup }
             restartPolicy: OnFailure
             volumes:
               - { name: etcd-certs, hostPath: { path: /etc/[kubernetes](../../other/kubernetes/SKILL.md)/pki/etcd } }
...
5. **Restore from a snapshot — a full control-plane recovery procedure,
   ...
   mv /etc/[kubernetes](../../other/kubernetes/SKILL.md)/manifests/etcd.yaml /tmp/etcd.yaml.bak
   ...
   mv /tmp/etcd.yaml.bak /etc/[kubernetes](../../other/kubernetes/SKILL.md)/manifests/etcd.yaml
...
6. **Verify the restored cluster before declaring it recovered**:
   ...
   ETCDCTL_API=3 etcdctl endpoint health --cluster -w table \
     --cacert=/etc/[kubernetes](../../other/kubernetes/SKILL.md)/pki/etcd/ca.crt --cert=... --key=...
   [kubectl](../../other/kubectl/SKILL.md) get nodes
   [kubectl](../../other/kubectl/SKILL.md) get pods -A | grep -v Running
   ```
...
- Practice a full restore in a non-production environment on a real
  schedule (quarterly, or after any etcd/[Kubernetes](../../other/kubernetes/SKILL.md) version bump) —
