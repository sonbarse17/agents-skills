...
[OpenShift](../openshift/SKILL.md) is a [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) distribution that layers additional
...
— on top of vanilla [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md). These differences are exactly where
[Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-general knowledge silently fails on [OpenShift](../openshift/SKILL.md): a manifest
...
- Exposing a Service externally on [OpenShift](../openshift/SKILL.md) (Routes vs. Ingress, and
...
- Debugging a pod that fails to schedule/start with an SCC-related
  ...
  constraint`).
- Creating and scoping a Project ([OpenShift](../openshift/SKILL.md)'s Namespace wrapper) with
...
- Migrating manifests written for vanilla [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) (EKS/AKS/GKE) to
  run on [OpenShift](../openshift/SKILL.md)/ROSA without being blocked by SCCs or missing Routes.

## Prerequisites & environment

- `oc` CLI ([OpenShift](../openshift/SKILL.md)'s `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)` superset) matched to the target
  cluster's [OpenShift](../openshift/SKILL.md) version — [OpenShift](../openshift/SKILL.md) versions track upstream
  [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) on a roughly 3-releases-behind cadence (e.g. [OpenShift](../openshift/SKILL.md) 4.16
  ships [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) 1.29), so confirm compatibility before assuming a
  [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) feature/API version is available.
...
- Familiarity with vanilla [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) concepts this skill builds on —
  for CNI/networking specifics on [OpenShift](../openshift/SKILL.md)'s default SDN/OVN-[Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)
  see [cni-networking-calico-flannel](../[cni-networking-calico-flannel](../cni-networking-calico-flannel/SKILL.md)/SKILL.md)
  for the general Calico/Flannel concepts ([OpenShift](../openshift/SKILL.md) itself defaults to
  OVN-[Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md), a separate CNI from either).

## Step-by-step guidance

1. **Expose a Service with a Route** rather than (or in addition to) an
   Ingress — Routes are [OpenShift](../openshift/SKILL.md)-native and integrate with the
   platform's built-in router (HAProxy-based) without requiring a
   separately installed Ingress controller:
   ```yaml
   apiVersion: route.[openshift](../openshift/SKILL.md).io/v1
   kind: Route
   metadata:
     name: payments-api
     namespace: payments
   ...
   ```
   [OpenShift](../openshift/SKILL.md) also supports standard [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) `Ingress` objects (the
...
2. **Create a Project** ([OpenShift](../openshift/SKILL.md)'s Namespace wrapper, adding
   ...
   ```
   A Project *is* a Namespace with additional [OpenShift](../openshift/SKILL.md)-managed
   annotations — `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) get namespace payments` shows the same object.
...
3. **Diagnose SCC rejections** when a pod fails to start:
   ...
   ```
   The default `restricted-v2` SCC ([OpenShift](../openshift/SKILL.md) ≥ 4.11) forbids running as
...
4. **Install an Operator via OLM** rather than raw manifests, for any
   ...
   ```bash
   oc get packagemanifests -n [openshift](../openshift/SKILL.md)-marketplace | grep cert-manager
   ```
   ```yaml
   ...
   metadata:
     ...
     namespace: [openshift](../openshift/SKILL.md)-operators
   spec:
     channel: stable-v1
     name: [openshift](../openshift/SKILL.md)-cert-manager-operator
     source: redhat-operators
     sourceNamespace: [openshift](../openshift/SKILL.md)-marketplace
     installPlanApproval: Automatic
   ```
   ```bash
   oc apply -f cert-manager-subscription.yaml
   oc get csv -n [openshift](../openshift/SKILL.md)-operators   # confirm Succeeded phase
   ```
...
7. **Validate before promoting a manifest written for vanilla
   [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)** to run on [OpenShift](../openshift/SKILL.md)/ROSA:
   ```bash
...
- Prefer fixing container images to run as an arbitrary non-root UID
  ...
  keeps the same image portable to non-[OpenShift](../openshift/SKILL.md) [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) too.
...
- Scope Projects with `ResourceQuota` and `LimitRange` from creation,
  ...
  is a common path to one team's workload starving node [capacity](../../../AI_and_Agents/Infrastructure/deploy-model/[capacity](../../Cloud_Providers/azure-skills/skills/microsoft-foundry/models/deploy-model/[capacity](../../Observability_and_SecOps/capacity/SKILL.md)/SKILL.md)/SKILL.md) for
  others.
...
- When comparing [OpenShift](../openshift/SKILL.md)/ROSA to vanilla managed [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) options,
  ...
  existing [OpenShift](../openshift/SKILL.md) investment) rather than defaulting to it as "the
  enterprise choice" without a specific driver — see
  [managed-[kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-eks-aks-gke](../[managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke](../managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke/SKILL.md)/SKILL.md)
  for the vanilla-cloud-managed alternative comparison.
- Keep track of which [OpenShift](../openshift/SKILL.md) version maps to which upstream
  [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) minor version when consulting general [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)
...
- **Symptom:** A Deployment that runs fine on EKS/GKE fails to schedule
  on [OpenShift](../openshift/SKILL.md) with `unable to validate against any security context
  constraint`.
...
- **Symptom:** An Ingress object applied on [OpenShift](../openshift/SKILL.md) doesn't produce
  the expected external hostname/behavior seen on other clouds.
  **Fix:** Confirm whether the cluster's Ingress Operator actually
  translated it into a Route (`oc get route`) and whether [OpenShift](../openshift/SKILL.md)-
...
externally reachable HTTPS host served by [OpenShift](../openshift/SKILL.md)'s built-in router —
...
- [managed-[kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-eks-aks-gke](../[managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke](../managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke/SKILL.md)/SKILL.md) — comparison point for vanilla cloud-managed [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md); ROSA runs inside a customer AWS account alongside these options.
- [ingress-nginx-configuration](../[ingress-nginx-configuration](../../../Software_Engineering_and_Other/Frontend/ingress-nginx-configuration/SKILL.md)/SKILL.md) — contrast with [OpenShift](../openshift/SKILL.md)'s native Route/router model when a workload needs to be portable across both.
- [cert-manager-tls-automation](../[cert-manager-tls-automation](../cert-manager-tls-automation/SKILL.md)/SKILL.md) — automating Route/Ingress TLS certificates on [OpenShift](../openshift/SKILL.md) via the OLM-distributed cert-manager Operator shown above.
