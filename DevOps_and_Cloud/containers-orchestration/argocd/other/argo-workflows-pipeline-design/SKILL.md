...
Argo Workflows is a [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-native workflow engine where each step of a
...
- Deciding whether a batch/data workload belongs in Argo Workflows versus
  the org's existing CI system ([Jenkins](../../../../ci-cd/jenkins/other/jenkins/SKILL.md), [GitHub](../../../../ci-cd/github-actions/other/github/SKILL.md) Actions, GitLab CI).
...
- RBAC: the `ServiceAccount` a `Workflow` runs as needs only the
  ...
  Pod under a real ServiceAccount, so ordinary [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) RBAC applies.
...
5. **Schedule recurring runs with `CronWorkflow`** rather than a plain
   [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) `CronJob` running a script, when the job is itself a
   multi-step graph:
   ```yaml
   apiVersion: argoproj.io/v1alpha1
   kind: CronWorkflow
