...
[Nomad](../nomad/SKILL.md) is a single-binary, general-purpose scheduler that runs
...
[Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) — one binary for both server and client roles, no separate
etcd cluster to operate, and a smaller set of core concepts (jobs,
task groups, tasks, scheduler types). It is not a [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)
replacement in every case: [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)' ecosystem (CRDs, service mesh
integrations, the broader operator pattern) is deeper, and most
organizations already standardized on [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) won't switch wholesale.
[Nomad](../nomad/SKILL.md) earns its place where the operational simplicity, mixed
...
topology decisions specific to [Nomad](../nomad/SKILL.md).

## When to use

- Writing or reviewing a [Nomad](../nomad/SKILL.md) job specification (HCL) for a service,
  batch, or system workload.
- Deciding whether a workload/environment is better served by [Nomad](../nomad/SKILL.md) or
  [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md), when neither is already an unchangeable given.
- Configuring [Nomad](../nomad/SKILL.md) server/client topology, including federation across
  regions/datacenters.
- Setting up a rolling update, canary, or blue/green deployment strategy
  for a [Nomad](../nomad/SKILL.md) job.
- Running mixed workloads (containers alongside raw-exec binaries,
  Java, or VM tasks) under a single scheduler.

## Prerequisites & environment

- [Nomad](../nomad/SKILL.md) servers (an odd number, typically 3 or 5, for Raft quorum) and
  one or more [Nomad](../nomad/SKILL.md) client nodes — check the specific [Nomad](../nomad/SKILL.md) version's
...
- A driver enabled on client nodes for each task type in use (`[docker](../../../docker/other/docker/SKILL.md)`,
...
- Consul (optional but common) for service discovery and health checks
  referenced from job specs via the `service` stanza; [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md) (optional)
...
- The `[nomad](../nomad/SKILL.md)` CLI matching (or compatible with) the cluster's server
  version.
...
1. **Choose the scheduler type per job based on the workload's actual
   ...
   - `service` — long-running processes that should be restarted on
     failure and kept at a target count (the [Nomad](../nomad/SKILL.md) analog of a
     [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) `Deployment`).
   - `batch` — run-to-completion workloads (the [Nomad](../nomad/SKILL.md) analog of a
     [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) `Job`), optionally on a `periodic` schedule (the analog
     of a `CronJob`).
...
2. **Write the job spec with explicit resource requests and a matching
   ...
   job "checkout-api" {
     ...
     group "api" {
       ...
       task "api" {
         driver = "[docker](../../../docker/other/docker/SKILL.md)"
   ...
   routes to instances passing it — the [Nomad](../nomad/SKILL.md) equivalent of a
   [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) readiness probe gating Service endpoint membership.
...
3. **Configure an update strategy for rolling deploys, rather than
   ...
   [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) `Deployment`'s `maxSurge`/`maxUnavailable` combined with a
   readiness-gated rollout, expressed in [Nomad](../nomad/SKILL.md)'s own vocabulary.
...
5. **Configure server/client topology and federation deliberately** for
   multi-region deployments — each region runs its own [Nomad](../nomad/SKILL.md) server
   ...
   `server_join` / `[nomad](../nomad/SKILL.md) server join`, not by adding cross-region
   nodes to the same Raft quorum.

6. **Decide [Nomad](../nomad/SKILL.md) vs. [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) per environment/workload class**, not
   as an org-wide either/or: [Nomad](../nomad/SKILL.md) tends to fit environments needing a
   ...
   [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)' control-plane footprint is disproportionate; [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)
   ...
   [Nomad](../nomad/SKILL.md)'s simplicity. Running both for different workload classes in
...
- Set explicit `resources { cpu, memory }` on every task — an
  ...
  as an unset [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) pod resource request.
...
- Enable ACLs and Consul/[Vault](../../../../Security/cryptography-secrets/vault/SKILL.md) integration (via the `service` and
...
- Keep `raw_exec` disabled unless a specific, understood workload needs
  unsandboxed process execution; prefer the `[docker](../../../docker/other/docker/SKILL.md)` or `exec` (chroot/
...
- Version-control job specs (HCL) the same as any other deployment
  artifact, and apply them through `[nomad](../nomad/SKILL.md) job plan` (a dry-run diff)
  before `[nomad](../nomad/SKILL.md) job run`, mirroring `terraform plan`/`apply` discipline.
- Reassess the [Nomad](../nomad/SKILL.md)-vs-[Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) decision per new workload class
...
- **Symptom:** [Nomad](../nomad/SKILL.md) client nodes are running arbitrary unsandboxed
...
- **Symptom:** A multi-region [Nomad](../nomad/SKILL.md) deployment suffers slow leader
...
- **Symptom:** An operator with only a network path to the [Nomad](../nomad/SKILL.md) API
  (no credentials) is able to submit or stop jobs.
  **Fix:** ACLs were never bootstrapped on the cluster ([Nomad](../nomad/SKILL.md) ACLs are
...
all-at-once [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) `CronJob` but needs to move to a [Nomad](../nomad/SKILL.md) cluster
...
a dry-run first:
```bash
[nomad](../nomad/SKILL.md) job plan hourly-report-batch.[nomad](../nomad/SKILL.md).hcl
[nomad](../nomad/SKILL.md) job run hourly-report-batch.[nomad](../nomad/SKILL.md).hcl
```
`[nomad](../nomad/SKILL.md) job plan` shows the diff against the currently running job
...
- [knative-[serverless](../../../../Software_Engineering_and_Other/Patterns/data-performance/serverless/SKILL.md)-configuration](../[knative-[serverless](../serverless/SKILL.md)-configuration](../knative-[serverless](../serverless/SKILL.md)-configuration/SKILL.md)/SKILL.md) — a [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)-native scheduling/scaling model to compare against when deciding whether [Nomad](../nomad/SKILL.md)'s simpler operational footprint is the better fit for a given workload.
- [dapr-distributed-runtime-configuration](../[dapr-distributed-runtime-configuration](../../../Software_Engineering_and_Other/Frontend/dapr-distributed-runtime-configuration/SKILL.md)/SKILL.md) — Dapr's sidecar building blocks can run on [Nomad](../nomad/SKILL.md)-scheduled tasks the same as on [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) pods, for teams wanting consistent state/pub-sub/service-invocation APIs across both schedulers.
- [aws-lambda-packaging-and-configuration](../[aws-lambda-packaging-and-configuration](../../Cloud_Providers/[aws-lambda](../../Cloud_Providers/aws-lambda/SKILL.md)-packaging-and-configuration/SKILL.md)/SKILL.md) — a fully managed alternative to self-operating a [Nomad](../nomad/SKILL.md) (or [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md)) cluster at all, worth weighing when the workload doesn't need long-running processes or non-containerized task types.
