...
# [Kustomize](../kustomize/SKILL.md) Overlay Management

## Purpose

[Kustomize](../kustomize/SKILL.md) solves the "same app, different environment" problem without a
...
correctly and knowing when [Kustomize](../kustomize/SKILL.md) is the right tool versus Helm.
...
- Deciding whether a given repo/team should use [Kustomize](../kustomize/SKILL.md), Helm, or
  [Kustomize](../kustomize/SKILL.md)-post-processing a Helm chart's rendered output.
- Debugging `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -k` output that doesn't match what was
  expected from the overlay.

## Prerequisites & environment

- [Kustomize](../kustomize/SKILL.md) is built into `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)` (`[kubectl](../../../kubernetes/other/kubectl/SKILL.md) [kustomize](../kustomize/SKILL.md)` / `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -k`)
  since `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)` 1.14, but the bundled version often lags the standalone
  `[kustomize](../kustomize/SKILL.md)` CLI — for current features (e.g. `replacements`,
  `components`), install the standalone binary (`[kustomize](../kustomize/SKILL.md)` ≥ 5.0) and
  use `[kustomize](../kustomize/SKILL.md) build | [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -f -` in CI rather than relying on
  `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)`'s embedded version.
- `[kustomize](../kustomize/SKILL.md) edit` subcommands (`[kustomize](../kustomize/SKILL.md) edit set image`, `[kustomize](../kustomize/SKILL.md)
...
- A repo layout convention agreed with the team (`base/` +
  ...
  mixable feature patches introduced in [Kustomize](../kustomize/SKILL.md) v3+).
- If [GitOps](../../../common/gitops/gitops/SKILL.md)-managed (Argo CD, Flux), confirm the controller's supported
  [Kustomize](../kustomize/SKILL.md) version — Argo CD bundles its own [kustomize](../kustomize/SKILL.md) binary version
...
2. **Write the base as a fully valid, environment-agnostic manifest set**
   ...
   apiVersion: [kustomize](../kustomize/SKILL.md).config.k8s.io/v1beta1
   kind: Kustomization
   resources:
     - deployment.yaml
     - service.yaml
     - configmap.yaml
   commonLabels:
     app.[kubernetes](../../../kubernetes/other/kubernetes/SKILL.md).io/name: payments-api
   ```
   ...
   configMapGenerator:
     - name: payments-api-config
       ...
       files:
         - config/app.yaml
   ```
   [Kustomize](../kustomize/SKILL.md) appends a content hash to the generated name
...
6. **Use `components/` for optional, mixable capability** (e.g. a
   ...
   apiVersion: [kustomize](../kustomize/SKILL.md).config.k8s.io/v1alpha1
   kind: Component
   patches:
...
7. **Render and diff before applying**:
   ```bash
   [kustomize](../kustomize/SKILL.md) build overlays/prod > /tmp/rendered-prod.yaml
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) diff -k overlays/prod
   [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -k overlays/prod
   ```
   `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) diff` against the live cluster is the single most valuable
   step before applying to a shared environment — it surfaces drift
   introduced by manual `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) edit`s as well as overlay mistakes.

8. **Decide [Kustomize](../kustomize/SKILL.md) vs. Helm vs. both**:
   - Pure [Kustomize](../kustomize/SKILL.md): best when there is no need to distribute the
   ...
   - Both: run `helm template` to render a vendor chart, then feed the
     output through [Kustomize](../kustomize/SKILL.md) as a resource to apply organization-
     ...
     ```
     [Kustomize](../kustomize/SKILL.md)'s native `helmCharts:` field can also invoke `helm
     template` inline during `[kustomize](../kustomize/SKILL.md) build`, avoiding a separate
     render step, at the cost of requiring `helm` on the build machine
     and `--enable-helm` on older [Kustomize](../kustomize/SKILL.md) versions.

## Best practices

- Keep the base deployable and testable in isolation
  (`[kustomize](../kustomize/SKILL.md) build base | [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply --dry-run=server -f -`); if it
...
- [Commit](../../../../ci-cd/common/git-workflow/commit/SKILL.md) the rendered output of `[kustomize](../kustomize/SKILL.md) build` for at least the prod
...
- **Symptom:** `[kustomize](../kustomize/SKILL.md) build` succeeds, but the applied Service no
  longer selects any Pods after adding a `commonLabels` entry to an
  overlay.
  **Fix:** `commonLabels` rewrites both the resource's labels *and*
  matching `spec.selector` fields where [Kustomize](../kustomize/SKILL.md) recognizes the
  ...
  get the same rewrite in every [Kustomize](../kustomize/SKILL.md) version — inspect the
  rendered output (`[kustomize](../kustomize/SKILL.md) build`) and confirm selectors match labels
...
- **Symptom:** A ConfigMap/Secret generated via `configMapGenerator`
  ...
  **Fix:** Something outside the [Kustomize](../kustomize/SKILL.md)-managed resource tree
  ...
  instead of going through [Kustomize](../kustomize/SKILL.md)'s reference rewriting. Only
...
- **Symptom:** `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -k` behaves differently in CI than on a
  developer's laptop for the same overlay.
  **Fix:** `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)`'s embedded [Kustomize](../kustomize/SKILL.md) version is pinned to that
  `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)` release and commonly lags the standalone `[kustomize](../kustomize/SKILL.md)` CLI by
  ...
  a pinned standalone `[kustomize](../kustomize/SKILL.md)` binary version in both CI and local
  tooling (`[kustomize](../kustomize/SKILL.md) build | [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -f -`) rather than `[kubectl](../../../kubernetes/other/kubectl/SKILL.md)
  apply -k`.
...
```bash
[kustomize](../kustomize/SKILL.md) build overlays/prod | [kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply --dry-run=server -f -
[kubectl](../../../kubernetes/other/kubectl/SKILL.md) diff -k overlays/prod
[kubectl](../../../kubernetes/other/kubectl/SKILL.md) apply -k overlays/prod
```
...
- [helm-chart-authoring](../[helm-chart-authoring](../../../DevOps_and_Cloud/Containers_and_Orchestration/helm-chart-authoring/SKILL.md)/SKILL.md) — when to package as a distributable Helm chart instead of (or alongside) [Kustomize](../kustomize/SKILL.md) overlays.
