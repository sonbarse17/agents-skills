...
## Purpose
Create and configure development containers — [Docker](../docker/SKILL.md)-based, consistent development environments defined by a `devcontainer.json` and Dockerfile. Dev containers ensure all team members (and CI) use identical tools, runtimes, and configurations, eliminating "works on my machine" issues.

## Agent Protocol

### Trigger
Exact user phrases: "dev container", "devcontainer.json", "development container", "VS Code Dev Container", "[GitHub](../../../../ci-cd/github-actions/other/github/SKILL.md) Codespaces", "dev environment setup", "reproducible dev environment", "containerized dev", "[Docker](../docker/SKILL.md) dev environment", "remote container".

### Input Context
- Language runtime (Node.js, [Python](../../../../Software_Engineering_and_Other/Languages/python/python/SKILL.md), Go, Rust, Java, .NET, Ruby, PHP)
- Build tools (npm, pip, cargo, maven, gradle, make, cmake)
- Services needed ([PostgreSQL](../../../../Software_Engineering_and_Other/Databases/relational/postgresql/SKILL.md), Redis, [MySQL](../../../../Software_Engineering_and_Other/Databases/relational/mysql/SKILL.md), [MongoDB](../../../../Software_Engineering_and_Other/Databases/nosql/mongodb/SKILL.md), RabbitMQ, Elasticsearch)
...
### Output Artifact
Complete dev container configuration with Dockerfile, devcontainer.json, and [Docker](../docker/SKILL.md) Compose (if multi-service).
...
- [ ] Multi-service setup via [Docker](../docker/SKILL.md) Compose (if needed)
...
│   base: mcr.microsoft.com/devcontainers/[typescript](../../../../Software_Engineering_and_Other/Frontend/common/typescript/SKILL.md)-node:20
│   → apt install tools → npm install → done
├── Single runtime + databases → [Docker](../docker/SKILL.md) Compose
│   app container + postgres + redis containers
│   → depends_on, network, healthcheck
├── Polyglot / multiple runtimes → Feature-based
│   base: debian + features (node, [python](../../../../Software_Engineering_and_Other/Languages/python/python/SKILL.md), go)
│   → ghcr.io/devcontainers/features/*
├── GPU / CUDA development → GPU-enabled base
│   base: mcr.microsoft.com/devcontainers/cuda
│   → nvidia-[docker](../docker/SKILL.md) runtime
...
Container ([Docker](../docker/SKILL.md))
├── Application code (mounted volume)
├── Language runtime (Node, [Python](../../../../Software_Engineering_and_Other/Languages/python/python/SKILL.md), Go, etc.)
...
└── [Docker](../docker/SKILL.md)-outside-[Docker](../docker/SKILL.md) (optional)
    ↓
[Docker](../docker/SKILL.md) Compose (optional multi-service)
├── App container
├── [PostgreSQL](../../../../Software_Engineering_and_Other/Databases/relational/postgresql/SKILL.md) / [MySQL](../../../../Software_Engineering_and_Other/Databases/relational/mysql/SKILL.md) / [MongoDB](../../../../Software_Engineering_and_Other/Databases/nosql/mongodb/SKILL.md)
├── Redis / Memcached
└── Other services
```
...
// .devcontainer/devcontainer.json
{
  ...
  "image": "mcr.microsoft.com/devcontainers/[typescript](../../../../Software_Engineering_and_Other/Frontend/common/typescript/SKILL.md)-node:20",
  ...
  // Option C: Use [Docker](../docker/SKILL.md) Compose
  ...
  "features": {
    ...
    "ghcr.io/devcontainers/features/[docker](../docker/SKILL.md)-outside-of-[docker](../docker/SKILL.md):1": {},
  ...
  "extensions": [
    ...
    "ms-vscode.vscode-[typescript](../../../../Software_Engineering_and_Other/Frontend/common/typescript/SKILL.md)-next",
    ...
    "ms-azuretools.vscode-[docker](../docker/SKILL.md)"
  ],
  ...
  "settings": {
    ...
    "[typescript](../../../../Software_Engineering_and_Other/Frontend/common/typescript/SKILL.md).updateImportsOnFileMove.enabled": "always",
    "[typescript]": {
      "editor.defaultFormatter": "esbenp.prettier-vscode"
    }
  },
  ...
  "remoteEnv": {
    "DATABASE_URL": "[postgresql](../../../../Software_Engineering_and_Other/Databases/relational/postgresql/SKILL.md)://user:pass@localhost:5432/myapp",
...
ARG VARIANT="20"
FROM mcr.microsoft.com/devcontainers/[typescript](../../../../Software_Engineering_and_Other/Frontend/common/typescript/SKILL.md)-node:${VARIANT}

# Install additional system packages
RUN apt-get update && export DEBIAN_FRONTEND=noninteractive \
    && apt-get install -y --no-install-recommends \
        [postgresql](../../../../Software_Engineering_and_Other/Databases/relational/postgresql/SKILL.md)-client \
        redis-tools \
        ripgrep \
        fd-find \
        bat \
...
### Step 3: [Docker](../docker/SKILL.md) Compose for Multi-Service
...
services:
  app:
    ...
    volumes:
      - ..:/workspaces:cached
    environment:
      DATABASE_URL: [postgresql](../../../../Software_Engineering_and_Other/Databases/relational/postgresql/SKILL.md)://user:pass@postgres:5432/myapp
  ...
  postgres:
    ...
    volumes:
      - postgres-data:/var/lib/[postgresql](../../../../Software_Engineering_and_Other/Databases/relational/postgresql/SKILL.md)/data
    environment:
      POSTGRES_USER: user
  ...
  validate:
    ...
    steps:
      ...
      - name: Run post-create
        run: [docker](../docker/SKILL.md) run --rm ghcr.io/myorg/devcontainer /bin/bash -c ".devcontainer/post-create.sh && npm test"
```
...
| State loss on rebuild | Database data lost when container rebuilds | Use [Docker](../docker/SKILL.md) volumes for persistent data |
| Slow rebuilds | Installing everything from scratch | Use [Docker](../docker/SKILL.md) layer caching, feature cache |
...
| Practice | Rationale |
|----------|-----------|
| Use [Docker](../docker/SKILL.md) Compose for multi-service | DB, cache, and other services in isolated containers |
...
{
  "features": {
    "ghcr.io/devcontainers/features/[kubectl](../../../kubernetes/other/kubectl/SKILL.md):1": {},
    "ghcr.io/devcontainers/features/helm:1": {},
    "ghcr.io/devcontainers/features/[docker](../docker/SKILL.md)-from-[docker](../docker/SKILL.md):1": {},
...
### Multi-root Workspace ([Monorepo](../../../../Software_Engineering_and_Other/Frontend/build-tools/monorepo/SKILL.md))
```jsonc
{
  "name": "My [Monorepo](../../../../Software_Engineering_and_Other/Frontend/build-tools/monorepo/SKILL.md)",
...
`json
{
  ...
  "image": "mcr.microsoft.com/devcontainers/[typescript](../../../../Software_Engineering_and_Other/Frontend/common/typescript/SKILL.md)-node:20",
  "features": {
    "ghcr.io/devcontainers/features/[docker](../docker/SKILL.md)-in-[docker](../docker/SKILL.md):2": {},
...
- **Resource limits**: Set CPU and memory limits per container. Use [docker](../docker/SKILL.md) stats for [monitoring](../../../../observability-monitoring-logging/common/monitoring-strategy/monitoring/SKILL.md).
...
- **Parallel builds**: Use [docker](../docker/SKILL.md) buildx bake for multi-service builds. Leverage --parallel flag.
...
- **Startup [profiling](../../../../Software_Engineering_and_Other/Frontend/performance/profiling/SKILL.md)**: Profile container startup with [docker](../docker/SKILL.md) events. Identify slow initialization.
...
### Secrets Management
- **Never in image**: Use [Docker](../docker/SKILL.md) secrets or .env files mounted at runtime. Never COPY secrets into image.
