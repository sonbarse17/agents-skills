...
- The user wants to set [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) `securityContext` fields (`runAsNonRoot`,
...
- [Docker](../../other/docker/SKILL.md) or another OCI-compatible builder (Buildah, [Podman](../../../podman/other/podman/SKILL.md), BuildKit)
  supporting multi-stage builds (standard in [Docker](../../other/docker/SKILL.md) since `18.09`+, and
...
1. **Split the build into stages**, so build-only tooling (compilers,
   ...
   ```dockerfile
   # syntax=[docker](../../other/docker/SKILL.md)/dockerfile:1
...
4. **Set a read-only root filesystem** at deploy time, with an explicit
   ...
   ```yaml
   # [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) pod securityContext
   securityContext:
     runAsNonRoot: true
...
- Enforce hardening properties at the [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) admission layer in
...
- **Symptom:** Switching to a distroless base image breaks the ability to
  `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) exec -it` into the container to debug an issue.
  **Fix:** This is expected — distroless images have no shell by design.
  Use `[kubectl](../../../kubernetes/other/kubectl/SKILL.md) debug <pod> -it --image=busybox --target=<container>` to
...
- **Symptom:** A Dockerfile sets `USER app` but the running pod in
  [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) still shows the process running as root (UID 0).
  **Fix:** Something in the deploy path (`runAsUser: 0` in the pod spec,
  ...
  [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) pod spec takes precedence over the Dockerfile `USER`
...
matching [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) `securityContext`.
...
ENTRYPOINT ["/server"]
```
Corresponding [Kubernetes](../../../kubernetes/other/kubernetes/SKILL.md) pod hardening:
```yaml
apiVersion: v1
kind: Pod
metadata:
