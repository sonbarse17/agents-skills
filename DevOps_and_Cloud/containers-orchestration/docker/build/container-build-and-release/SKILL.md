...
- Adding vulnerability scanning or SBOM generation to the image build
  pipeline.
- Publishing a versioned release image to a registry (GHCR, [Docker](../../other/docker/SKILL.md) Hub,
  ECR, ACR, GCR/Artifact Registry).

## Prerequisites & environment

- [Docker](../../other/docker/SKILL.md) Engine ≥ 24 or a compatible builder (BuildKit is default in
  [Docker](../../other/docker/SKILL.md) ≥ 23; enable explicitly on older versions with
  `DOCKER_BUILDKIT=1`) — BuildKit enables cache mounts and multi-stage
  parallelism used below.
- `[docker](../../other/docker/SKILL.md) buildx` for multi-architecture builds (bundled with [Docker](../../other/docker/SKILL.md)
  Desktop and modern [Docker](../../other/docker/SKILL.md) Engine installs).
...
- Optional but recommended: `trivy`, `grype`, or the registry's built-in
  scanner for vulnerability scanning; `syft` or `[docker](../../other/docker/SKILL.md) buildx imagetools`
  for SBOM generation.
...
1. **Use multi-stage builds** to separate build-time dependencies from the
   ...
   ```dockerfile
   # syntax=[docker](../../other/docker/SKILL.md)/dockerfile:1.7
...
3. **Order layers from least to most frequently changing** so [Docker](../../other/docker/SKILL.md)'s
...
4. **Run as a non-root user** and drop unnecessary capabilities; avoid
   `--privileged` runs and [Docker](../../other/docker/SKILL.md) socket mounts in production containers.
...
5. **Build multi-arch images with buildx** when targeting mixed
   ...
   production nodes):
   ```bash
   [docker](../../other/docker/SKILL.md) buildx create --use --name multiarch-builder
   [docker](../../other/docker/SKILL.md) buildx build \
...
8. **Generate and attach an SBOM** for supply-chain traceability:
   ...
   [docker](../../other/docker/SKILL.md) buildx imagetools create --tag $IMAGE:1.4.2 --annotation index:sbom=sbom.spdx.json ...
   ```
...
9. **Push to the registry and record the resulting digest** as a build
   ...
   ```bash
   DIGEST=$([docker](../../other/docker/SKILL.md) buildx imagetools inspect $IMAGE:1.4.2 --format '{{json .Manifest.Digest}}')
   ```
...
- **Symptom:** A secret used during build (private package registry
  token) is discoverable via `[docker](../../other/docker/SKILL.md) history` or by extracting layers,
...
jobs:
  build-scan-push:
    ...
    steps:
      ...
      - uses: [docker](../../other/docker/SKILL.md)/setup-qemu-action@v3
      - uses: [docker](../../other/docker/SKILL.md)/setup-buildx-action@v3

      - name: Log in to GHCR
        uses: [docker](../../other/docker/SKILL.md)/login-action@v3
        with:
          registry: ghcr.io
      ...
      - name: Build (local, for scan)
        uses: [docker](../../other/docker/SKILL.md)/build-push-action@v6
        with:
          context: .
          load: true
...
- [release-versioning-and-changelog-automation](../../../../ci-cd/common/other/changelog-automation/SKILL.md)/SKILL.md)/SKILL.md)
- [artifact-and-dependency-management](../../../../ci-cd/common/build/artifact-and-dependency-management/SKILL.md)
