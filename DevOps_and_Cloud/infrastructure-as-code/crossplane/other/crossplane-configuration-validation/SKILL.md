...
is higher-stakes than a typical [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) manifest typo. This skill
covers the validation tooling — schema validation, render/dry-run, and
patch-path checking — that should run before a
[crossplane-[kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md)-native-provisioning](../[crossplane-[kubernetes](../../Containers_and_Orchestration/kubernetes/SKILL.md)-native-provisioning](../../Containers_and_Orchestration/crossplane-[kubernetes](../../Containers_and_Orchestration/kubernetes/SKILL.md)-native-provisioning/SKILL.md)/SKILL.md)-built
...
- For live-cluster cross-checks: `[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md)` read access to inspect
...
3. **Confirm every `fromFieldPath` actually exists in the XRD's
   ...
   ```bash
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) explain xdatabaseinstance.spec.parameters --recursive
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) explain instance.rds.aws.upbound.io.spec.forProvider --recursive
   ```
...
4. **Test a representative Claim against the XRD's schema via
   ...
   ```bash
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) apply --dry-run=server -f databaseclaim.yaml
   ```
...
7. **After a Composition/XRD change ships, cross-check a live `Claim`'s
   ...
   ```bash
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get instance.rds.aws.upbound.io -o jsonpath='{.items[0].spec.forProvider}'
   ```
...
8. **Validate `CompositionRevision` pinning explicitly** when a
   ...
   ```bash
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get compositions.apiextensions.crossplane.io xdatabaseinstances.aws -o jsonpath='{.status.currentRevision}'
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get databaseclaim payments-db -n payments -o jsonpath='{.spec.compositionRevisionRef.name}'
   ```
...
- Cross-check `fromFieldPath`/`toFieldPath` against `[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) explain`
...
- **Symptom:** `crossplane beta validate` passes cleanly on an XRD/
  ...
  **Fix:** Schema-level validation (`crossplane beta validate`) checks
  ...
  (`[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) apply --dry-run=server`) against a real cluster as a
...
4. Dry-run a representative Claim requesting the new tier against a
   ...
   ```bash
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) apply --dry-run=server -f databaseclaim-xlarge-test.yaml
   ```
...
- [infrastructure-as-code-terraform](../../../devops/skills/[infrastructure-as-code-terraform](../[infrastructure-as-code](../infrastructure-as-code/SKILL.md)-terraform/SKILL.md)/SKILL.md) — the `terraform plan`-review discipline this skill's render/dry-run workflow mirrors for a [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md)-native provisioning model.
...
- [opa-gatekeeper-policy-authoring](../../../../Security/policy-as-code/opa-gatekeeper-policy-authoring/SKILL.md)/SKILL.md) — enforcing organization-wide constraints (e.g. disallowed instance types) on Claims/Composites as an admission-time policy layer complementary to this skill's schema/render validation.
