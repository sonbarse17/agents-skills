...
# Crossplane: [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md)-Native Infrastructure Provisioning
...
[Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) control-plane extension rather than as a standalone CLI tool
run against a state file: cloud resources are represented as
[Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) custom resources, and Crossplane's providers continuously
...
plan-then-apply, state-file-tracked model — not a [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) wrapper
...
resource now lives inside the same [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) API server that runs
...
- Deciding whether Crossplane or Terraform is the better fit for a new
  ...
  is already [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md)-centric (an internal developer platform, a
  [GitOps](../../../../containers-orchestration/common/gitops/gitops/SKILL.md)-managed fleet).
...
- Migrating a subset of Terraform-managed infrastructure that would
  benefit from a [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md)-native, self-service front end, without
  necessarily replacing Terraform everywhere.

## Prerequisites & environment

- A [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) cluster ≥ 1.26 with Crossplane ≥ v1.16 installed (via
...
- Cloud provider credentials scoped per Crossplane `Provider`
  ...
  `provider-family` split providers) stored as a [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) `Secret`
...
- If also running Argo CD or Flux, decide how Crossplane's continuous
  reconciliation interacts with [GitOps](../../../../containers-orchestration/common/gitops/gitops/SKILL.md)-managed manifests that create
  Claims — see
  [gitops-workflow](../../../devops/skills/[gitops-workflow](../[gitops](../gitops/SKILL.md)-workflow/SKILL.md)/SKILL.md);
  the two are complementary ([GitOps](../../../../containers-orchestration/common/gitops/gitops/SKILL.md) manages the Claim manifest, Crossplane
...
2. **Provision a single managed resource directly** first, to confirm
   ...
   ```
   ```bash
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get instance.rds.aws.upbound.io test-db -o jsonpath='{.status.conditions}'
   ```
   A `Ready: True`/`Synced: True` condition pair confirms the resource
   was actually created in AWS, not just accepted by the [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) API
   server.
...
5. **Application teams provision via a namespaced `Claim`**, not by
   ...
   ```
   ```bash
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get databaseclaim payments-db -n payments -o jsonpath='{.status.conditions}'
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get secret payments-db-conn -n payments   # connection details Crossplane populated
   ```
...
6. **Let Crossplane's continuous reconciliation handle drift** —
   ...
   ```bash
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get managed   # aggregate view across all managed resource kinds
   ```
...
8. **Decommission a Claim deliberately.**
   ...
   ```bash
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get databaseclaim payments-db -n payments -o jsonpath='{.spec.compositeDeletePolicy}'
   [kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) delete databaseclaim payments-db -n payments   # only after confirming backups/intent
   ```
...
- Treat `[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get managed`'s `SYNCED`/`READY` columns as two
  independent signals worth checking separately, the same discipline
  applied to Argo CD's `sync`/`health` split in
  [argocd-sync-failure-and-drift-investigation](../../../[gitops](../gitops/SKILL.md)-argo-ecosystem/skills/[argocd-sync-failure-and-drift-investigation](../[argocd](../argocd/SKILL.md)-sync-failure-and-drift-investigation/SKILL.md)/SKILL.md).
- Pair Crossplane Claims with [GitOps](../../../../containers-orchestration/common/gitops/gitops/SKILL.md) (Flux or Argo CD) so Claim
  manifests themselves are version-controlled and reviewed the same way
  any other [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md) manifest is, rather than created ad hoc via
  `[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) apply` — see
...
- Don't reach for Crossplane just because a team is [Kubernetes](../../../../containers-orchestration/kubernetes/other/kubernetes/SKILL.md)-native —
...
- **Symptom:** A `Claim` is created and never becomes `Ready`, with no
  obvious error in `[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) describe databaseclaim`.
  **Fix:** Check the underlying `Composite` resource and managed
  resource directly (`[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get composite`, `[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get managed`) —
...
- **Symptom:** `[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get managed` shows a resource `READY: True` but
...
provision [PostgreSQL](../../../../Software_Engineering_and_Other/Databases/relational/postgresql/SKILL.md) databases via a `DatabaseClaim`, without any team
...
```bash
[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get databaseclaim payments-db -n payments -o jsonpath='{.status.conditions}'
[kubectl](../../../../containers-orchestration/kubernetes/other/kubectl/SKILL.md) get secret payments-db-conn -n payments -o jsonpath='{.data.endpoint}' | base64 -d
```
...
- [gitops-workflow](../../../devops/skills/[gitops-workflow](../[gitops](../gitops/SKILL.md)-workflow/SKILL.md)/SKILL.md) — how Claim manifests themselves should be version-controlled and reviewed via [GitOps](../../../../containers-orchestration/common/gitops/gitops/SKILL.md) rather than applied ad hoc.
