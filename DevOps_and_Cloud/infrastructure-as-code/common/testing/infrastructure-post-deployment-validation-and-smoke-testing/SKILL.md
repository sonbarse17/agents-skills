...
`terraform apply` exiting 0, a [CloudFormation](../../../cloudformation/other/cloudformation/SKILL.md) stack reaching
`UPDATE_COMPLETE`, or `[ansible](../../../ansible/other/ansible/SKILL.md)-playbook` reporting no failed tasks all
...
conflated until an [incident](../../../../DevOps_and_Cloud/Observability_and_SecOps/incident/SKILL.md) proves they weren't the same thing.

## When to use

- Immediately after any `terraform apply`, [CloudFormation](../../../cloudformation/other/cloudformation/SKILL.md) stack
  create/update, or [Ansible](../../../ansible/other/ansible/SKILL.md) playbook run that provisions or reconfigures
...
- Investigating an [incident](../../../../DevOps_and_Cloud/Observability_and_SecOps/incident/SKILL.md) where the deployment tooling reported success
...
- The deployment tool's own state/output as the source of truth for
  *what* to check: Terraform outputs (`terraform output -json`),
  [CloudFormation](../../../cloudformation/other/cloudformation/SKILL.md) stack outputs (`aws [cloudformation](../../../cloudformation/other/cloudformation/SKILL.md) describe-stacks
  --query Stacks[0].Outputs`), or an [Ansible](../../../ansible/other/ansible/SKILL.md) fact/registered variable —
...
- For functional smoke tests specifically: a defined "healthy" contract
  ...
  [dashboards](../../../../DevOps_and_Cloud/Observability_and_SecOps/dashboards/SKILL.md).
...
2. **Derive what to check from the deployment tool's own outputs**,
   ...
   ```
   For [CloudFormation](../../../cloudformation/other/cloudformation/SKILL.md), the equivalent is reading `Outputs` from
   `describe-stacks` rather than assuming a resource's name/ARN pattern;
   for [Ansible](../../../ansible/other/ansible/SKILL.md), register the actual result of a task
...
5. **For [CloudFormation](../../../cloudformation/other/cloudformation/SKILL.md), use `ValidateService`-equivalent hooks or
   `cfn-signal`/custom resources where the platform provides them**,
   rather than only checking stack status:
   ```bash
   aws [cloudformation](../../../cloudformation/other/cloudformation/SKILL.md) describe-stacks --stack-name checkout-api-prod \
     --query 'Stacks[0].StackStatus'
   # "UPDATE_COMPLETE" means [CloudFormation](../../../cloudformation/other/cloudformation/SKILL.md) finished its operations —
...
6. **For [Ansible](../../../ansible/other/ansible/SKILL.md), assert against registered results rather than trusting
   the play recap alone**:
   ```yaml
   - name: Check application health endpoint post-deploy
     [ansible](../../../ansible/other/ansible/SKILL.md).builtin.uri:
   ...
   - name: Fail the play explicitly if health check never passed
     [ansible](../../../ansible/other/ansible/SKILL.md).builtin.fail:
...
7. **Decide and wire the failure response before it's needed** — a
   ...
   > known-good plan, [CloudFormation](../../../cloudformation/other/cloudformation/SKILL.md) stack rollback, redeploying a prior
   > CodeDeploy revision) is itself a change to production and should be
   > exercised/tested before relying on it during a real [incident](../../../../DevOps_and_Cloud/Observability_and_SecOps/incident/SKILL.md) — an
...
- **Symptom:** `terraform apply`/[CloudFormation](../../../cloudformation/other/cloudformation/SKILL.md) stack update reports
...
- **Symptom:** A smoke test run immediately after deploy fails, but
  ...
  **Fix:** The service takes a few seconds to become ready (connection
  ...
  `retries`/backoff to the smoke test (as in the [Ansible](../../../ansible/other/ansible/SKILL.md) example, step
...
- [aws-[cloudformation](../../../cloudformation/other/cloudformation/SKILL.md)-templates](../[aws-[cloudformation](../cloudformation/SKILL.md)-templates](../aws-[cloudformation](../cloudformation/SKILL.md)-templates/SKILL.md)/SKILL.md) — stack status and output patterns referenced for the [CloudFormation](../../../cloudformation/other/cloudformation/SKILL.md)-specific validation steps here.
