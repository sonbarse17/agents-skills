...
on-call engineer needs during an [incident](../incident/SKILL.md), or a resource attribute typo
...
- Auditing an existing production Collector config for the common,
  ...
  check, not only after an [incident](../incident/SKILL.md).
...
4. **Review sampling configuration for what it actually discards, not
   ...
   > failing/slow requests an on-call engineer needs during an [incident](../incident/SKILL.md) —
...
- **Symptom:** A tail-sampling change is deployed to "reduce storage
  cost," and during the next [incident](../incident/SKILL.md) the on-call engineer can't find any
  ...
  **Fix:** The sampling policy applied a flat probabilistic rate with no
  ...
  re-review the change with "what would this drop during an [incident](../incident/SKILL.md)" as
...
3. Review the sampling change against step 4's "what does this discard"
   ...
   an [incident](../incident/SKILL.md) investigation would need. Approved as-is.
