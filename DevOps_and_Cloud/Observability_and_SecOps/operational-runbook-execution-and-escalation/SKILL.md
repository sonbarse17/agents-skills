...
# Operational [Runbook](../runbook/SKILL.md) Execution and Escalation
...
edge of that documentation. A junior engineer who deviates from a [runbook](../runbook/SKILL.md)
mid-[incident](../incident/SKILL.md) because a step "looks wrong" or a fix "seems obvious" is
making a judgment call without the system context (blast radius, prior
outages, why the [runbook](../runbook/SKILL.md) says what it says) that would let a senior
engineer make that same call safely. This skill covers executing
[runbooks](../runbooks/SKILL.md) and deployment procedures exactly as written, triaging alerts
methodically (acknowledge, assess severity, apply the documented fix or
escalate), recognizing the explicit escalation criteria a [runbook](../runbook/SKILL.md) should
...
what turns [runbook](../runbook/SKILL.md)-following into real operational judgment over time.

## When to use

- You are the first responder on a paged alert and need to identify,
  read, and execute the correct [runbook](../runbook/SKILL.md) rather than debug from scratch.
- You are running an established deployment, rollback, or maintenance
  ...
  a scaling procedure) that already has a written, approved [runbook](../runbook/SKILL.md).
- A [runbook](../runbook/SKILL.md) step doesn't produce the expected result, or the situation
  doesn't match what the [runbook](../runbook/SKILL.md) describes, and you need to decide
  whether to keep going, try something adjacent, or escalate.
- You've just finished (or handed off) a [runbook](../runbook/SKILL.md) execution and need to
...
- Read access to the team's [runbook](../runbook/SKILL.md) repository/wiki, mapped from alert
  name/tag to the specific [runbook](../runbook/SKILL.md) (e.g. an alert's annotation links
  directly to its [runbook](../runbook/SKILL.md) URL — if it doesn't, that gap itself is worth
...
- Access to the paging tool (PagerDuty, Opsgenie, Grafana OnCall, or
  ...
  — see
  [incident-response-and-on-call-management](../../../site-reliability-engineering/skills/[incident-response-and-on-call-management](../../../Software_Engineering_and_Other/Frontend/[incident-response](../../Observability_and_SecOps/[incident](../../Observability_and_SecOps/incident/SKILL.md)-response/SKILL.md)-and-[on-call-management](../on-call-management/SKILL.md)/SKILL.md)/SKILL.md)
  ...
  first responder.
- Read access to the relevant logs/[dashboards](../dashboards/SKILL.md)/metrics the [runbook](../runbook/SKILL.md)
  references for verification steps.
- Scoped, least-privilege access to perform *only* the actions the
  [runbook](../runbook/SKILL.md) documents (e.g. restart a specific service, run a specific
  script) — not blanket production admin access "just in case." If a
  [runbook](../runbook/SKILL.md) step requires access you don't have, that is itself an
...
- A place to log deviations and post-shift notes (a ticket, a shared
  on-call log, a [runbook](../runbook/SKILL.md)-feedback channel) that a [runbook](../runbook/SKILL.md) owner actually
...
1. **Acknowledge the alert immediately** in the paging tool, before doing
   ...
   and muddies the [incident](../incident/SKILL.md) record.

2. **Identify the correct [runbook](../runbook/SKILL.md) from the alert itself**, not from
   memory or a guess at what's probably wrong. Most well-instrumented
   alerts link directly to a [runbook](../runbook/SKILL.md) URL in their annotation/description;
   if this one doesn't, say so explicitly when you escalate or write up
   the shift rather than proceeding on assumption.

3. **Assess severity using the [runbook](../runbook/SKILL.md)'s/team's documented triage
   criteria**, not personal judgment made under pressure. A [runbook](../runbook/SKILL.md)
...
4. **Execute the [runbook](../runbook/SKILL.md)'s steps in order, verifying after each one**,
   ...
   Example [runbook](../runbook/SKILL.md) excerpt (`[runbooks](../runbooks/SKILL.md)/payments-api-high-memory.md`):
   ```markdown
   # [Runbook](../runbook/SKILL.md): payments-api high memory usage alert
...
5. **Treat the [runbook](../runbook/SKILL.md)'s escalation triggers as hard stops, not
   ...
   work. The [runbook](../runbook/SKILL.md)'s escalation point exists specifically because
...
6. **Escalate with a structured handoff**, not a vague "it's still
   broken": state what alert fired, what [runbook](../runbook/SKILL.md) you followed, exactly
...
7. **After resolution or handoff, log any deviation from the written
   [runbook](../runbook/SKILL.md)** — a step that didn't match reality, an escalation trigger
   ...
   moment. This is what lets the [runbook](../runbook/SKILL.md) owner fix the document instead
   of the same ambiguity tripping up the next first responder too.

## Best practices

- Treat the [runbook](../runbook/SKILL.md) as ground truth for the duration of the [incident](../incident/SKILL.md),
  even if you believe you know a faster or better way — propose the
  improvement afterward, as a documented change to the [runbook](../runbook/SKILL.md), not as a
  live improvisation during an active page.
- Timebox each step mentally before you start it (the [runbook](../runbook/SKILL.md)'s stated
...
- When a [runbook](../runbook/SKILL.md) step is ambiguous, out of date, or simply wrong, say so
  explicitly in the [incident](../incident/SKILL.md) record and follow up with a ticket to fix
  the [runbook](../runbook/SKILL.md) — don't silently "figure it out" and leave the document
  wrong for the next person.
- Keep a personal (or team) log of every [runbook](../runbook/SKILL.md) you've executed and
...
- **Symptom:** A [runbook](../runbook/SKILL.md) step's escalation trigger is met (e.g. "a recent
  ...
  **Fix:** This is a genuinely risky pitfall, not a minor process
  ...
  safety margin the [runbook](../runbook/SKILL.md)'s author built in deliberately, often based
  on a prior [incident](../incident/SKILL.md) where exactly this improvisation made things
...
- **Symptom:** A [runbook](../runbook/SKILL.md) step's "expected result" clearly doesn't match
  ...
  **Fix:** Stop at the first mismatch between expected and actual result
  and treat it as an implicit escalation trigger even if the [runbook](../runbook/SKILL.md)
  doesn't name that exact case — a [runbook](../runbook/SKILL.md)'s steps are usually written
...
- **Symptom:** An alert fires at 2am, the assigned [runbook](../runbook/SKILL.md) is two
  ...
  **Fix:** If the [runbook](../runbook/SKILL.md) clearly doesn't match current reality within
  the first few minutes, escalate immediately and flag the [runbook](../runbook/SKILL.md) as
  stale in the same message — don't burn the acute [incident](../incident/SKILL.md) window
...
- **Symptom:** A responder performs an action that isn't in the [runbook](../runbook/SKILL.md)
  ...
  **Fix:** This is a destructive-action risk, not routine
  ...
  outage that isn't explicitly documented and approved in the [runbook](../runbook/SKILL.md) is
  ...
  escalate to, get it added to the [runbook](../runbook/SKILL.md) afterward so it's a documented
...
- **Symptom:** A shift ends, the ticket is closed as resolved, and no
  note is made that step 3 of the [runbook](../runbook/SKILL.md) actually failed the first time
  ...
  **Fix:** Log every deviation, even ones that resolved themselves on
  retry, in the shift log or ticket before closing — a [runbook](../runbook/SKILL.md) owner who
...
2. **Find the [runbook](../runbook/SKILL.md)** via the alert's linked
   `[runbooks](../runbooks/SKILL.md)/checkout-api-error-rate.md` — it exists and looks current.
3. **Assess severity**: the [runbook](../runbook/SKILL.md)'s triage step says check the "5xx
   rate" panel; it's at 4%, above the [runbook](../runbook/SKILL.md)'s documented 2% Sev2
   threshold — classify Sev2 and note it in the [incident](../incident/SKILL.md) channel per the
   severity table in
   [incident-response-and-on-call-management](../../../site-reliability-engineering/skills/[incident-response-and-on-call-management](../../../Software_Engineering_and_Other/Frontend/[incident-response](../../Observability_and_SecOps/[incident](../../Observability_and_SecOps/incident/SKILL.md)-response/SKILL.md)-and-[on-call-management](../on-call-management/SKILL.md)/SKILL.md)/SKILL.md).
...
5. **Execute step 2**: restart the two flagged pods per the documented
   ...
   matching the [runbook](../runbook/SKILL.md)'s "expected" description. Looks resolved.
...
7. **This matches an explicit escalation trigger** ("recurs within 30
   ...
   handoff: alert name, [runbook](../runbook/SKILL.md) followed, steps 1-4 completed with
...
9. **After stand-down**, the junior engineer logs the deviation in the
   ...
   root cause was a connection-pool leak not covered by this [runbook](../runbook/SKILL.md)."
   This note becomes the seed of a [runbook](../runbook/SKILL.md) update once the senior
...
- [incident-response-and-on-call-management](../../../site-reliability-engineering/skills/[incident-response-and-on-call-management](../../../Software_Engineering_and_Other/Frontend/[incident-response](../../Observability_and_SecOps/[incident](../../Observability_and_SecOps/incident/SKILL.md)-response/SKILL.md)-and-[on-call-management](../on-call-management/SKILL.md)/SKILL.md)/SKILL.md) — the severity levels, paging behavior, and escalation timeouts this skill's triage and escalation steps operate within.
