...
- **Under what circumstance** — mid-[incident](../incident/SKILL.md), weekly review, [capacity](../../../AI_and_Agents/Infrastructure/deploy-model/[capacity](../azure-skills/skills/microsoft-foundry/models/deploy-model/[capacity](../../Observability_and_SecOps/capacity/SKILL.md)/SKILL.md)/SKILL.md) planning.
...
A dashboard meant for spotting an active [incident](../incident/SKILL.md) should default to a short, high-resolution window — last hour, one-minute resolution — so a spike is visible as a spike, not smoothed into a flat line.
...
Every page-tier alert (see `[alerting](../alerting/SKILL.md)`) should link the specific dashboard, or the specific panel, that shows the condition it fired on — not a link to a generic team dashboard the engineer then has to search through.
