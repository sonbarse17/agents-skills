...
- An ephemeral deploy target dedicated to template testing (a
  `template-ci` [Kubernetes](../kubernetes/SKILL.md) namespace, or a disposable Humanitec/Score
...
3. **Check out the freshly generated repository and run its own build**,
   ...
   [docker](../docker/SKILL.md) build -t "template-ci:${GITHUB_RUN_ID}" .
   ```
...
4. **Deploy the built artifact to an ephemeral environment**, using
   ...
   [docker](../docker/SKILL.md) compose -f compose.yaml up -d
...
6. **Tear down every resource this pipeline created**, on both success and
   ...
   ```bash
   [docker](../docker/SKILL.md) compose -f compose.yaml down -v
...
3. `[docker](../docker/SKILL.md) build` on the generated repository succeeds — the Dockerfile's
   base image update is compatible with the generated `go.mod`.
4. `score-compose generate` and `[docker](../docker/SKILL.md) compose up` deploy it locally in
