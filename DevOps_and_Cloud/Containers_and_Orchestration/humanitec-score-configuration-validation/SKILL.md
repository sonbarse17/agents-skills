...
- A policy engine for org-specific rules beyond the base schema — Conftest
  ...
  for Rego authoring if the org already runs Gatekeeper for [Kubernetes](../kubernetes/SKILL.md)
...
2. **Add a CI job that runs schema validation on every pull request
   ...
   "looks fine" ([GitHub](../../../ci-cd/github-actions/other/github/SKILL.md) Actions example; the same job model applies to
   GitLab CI/[Jenkins](../../../ci-cd/jenkins/other/jenkins/SKILL.md) with equivalent syntax):
   ```yaml
   # .[github](../../../ci-cd/github-actions/other/github/SKILL.md)/workflows/validate-score.yml
         ...
         - name: Install score-compose
           run: |
             curl -Lo score-compose.tar.gz \
               https://[github](../../../ci-cd/github-actions/other/github/SKILL.md).com/score-spec/score-compose/releases/latest/download/score-compose_linux_amd64.tar.gz
