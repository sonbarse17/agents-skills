...
[OpenShift](../openshift/SKILL.md) is a [Kubernetes](../kubernetes/SKILL.md) distribution that layers additional
...
— on top of vanilla [Kubernetes](../kubernetes/SKILL.md). These differences are exactly where
[Kubernetes](../kubernetes/SKILL.md)-general knowledge silently fails on [OpenShift](../openshift/SKILL.md): a manifest
...
owns, with a specific shared-responsibility split that changes [incident](../../Observability_and_SecOps/incident/SKILL.md)
...
- Migrating manifests written for vanilla [Kubernetes](../kubernetes/SKILL.md) (EKS/AKS/GKE) to
  run on [OpenShift](../openshift/SKILL.md)/ROSA without being blocked by SCCs or missing Routes.

## Prerequisites & environment

- `oc` CLI ([OpenShift](../openshift/SKILL.md)'s `[kubectl](../kubectl/SKILL.md)` superset) matched to the target
  cluster's [OpenShift](../openshift/SKILL.md) version — [OpenShift](../openshift/SKILL.md) versions track upstream
  [Kubernetes](../kubernetes/SKILL.md) on a roughly 3-releases-behind cadence (e.g. [OpenShift](../openshift/SKILL.md) 4.16
  ships [Kubernetes](../kubernetes/SKILL.md) 1.29), so confirm compatibility before assuming a
  [Kubernetes](../kubernetes/SKILL.md) feature/API version is available.
...
- Familiarity with vanilla [Kubernetes](../kubernetes/SKILL.md) concepts this skill builds on —
  for CNI/networking specifics on [OpenShift](../openshift/SKILL.md)'s default SDN/OVN-[Kubernetes](../kubernetes/SKILL.md)
  ...
  OVN-[Kubernetes](../kubernetes/SKILL.md), a separate CNI from either).
...
1. **Expose a Service with a Route** rather than (or in addition to) an
   ...
   ```
   [OpenShift](../openshift/SKILL.md) also supports standard [Kubernetes](../kubernetes/SKILL.md) `Ingress` objects (the
...
2. **Create a Project** ([OpenShift](../openshift/SKILL.md)'s Namespace wrapper, adding
   ...
   annotations — `[kubectl](../kubectl/SKILL.md) get namespace payments` shows the same object.
...
6. **Understand the ROSA shared-responsibility split** before treating
   any [incident](../../Observability_and_SecOps/incident/SKILL.md) as fully self-serviceable:
   - **Red Hat operates**: the control plane (API server, etcd,
     scheduler), control-plane node patching/upgrades, and
     infrastructure nodes (router, registry, [monitoring](../../Observability_and_SecOps/monitoring/SKILL.md) stack)
   ...
   Confirm which category an [incident](../../Observability_and_SecOps/incident/SKILL.md) falls into before opening either
...
7. **Validate before promoting a manifest written for vanilla
   [Kubernetes](../kubernetes/SKILL.md)** to run on [OpenShift](../openshift/SKILL.md)/ROSA:
   ```bash
...
- Prefer fixing container images to run as an arbitrary non-root UID
  ...
  keeps the same image portable to non-[OpenShift](../openshift/SKILL.md) [Kubernetes](../kubernetes/SKILL.md) too.
...
- When comparing [OpenShift](../openshift/SKILL.md)/ROSA to vanilla managed [Kubernetes](../kubernetes/SKILL.md) options,
...
- Keep track of which [OpenShift](../openshift/SKILL.md) version maps to which upstream
  [Kubernetes](../kubernetes/SKILL.md) minor version when consulting general [Kubernetes](../kubernetes/SKILL.md)
...
- **Symptom:** A ROSA cluster [incident](../../Observability_and_SecOps/incident/SKILL.md) (e.g. slow API server responses)
