...
[Kubernetes](../kubernetes/SKILL.md) control plane, a managed model registry, a managed drift-
[monitoring](../../Observability_and_SecOps/monitoring/SKILL.md) product, or all three. This skill is the path for teams that
cannot or choose not to depend on any vendor-managed ML service at all:
running on any [Kubernetes](../kubernetes/SKILL.md) cluster (a cloud-provisioned one used purely for
...
[monitoring](../../Observability_and_SecOps/monitoring/SKILL.md) — operated by the team itself. The tradeoff is real and this
...
- Deciding between Kubeflow Pipelines and Ray as the orchestration layer
  ...
  than assumed.
- Honestly evaluating whether a team has the operational [capacity](../../../AI_and_Agents/Infrastructure/deploy-model/[capacity](../../Cloud_Providers/azure-skills/skills/microsoft-foundry/models/deploy-model/[capacity](../../Observability_and_SecOps/capacity/SKILL.md)/SKILL.md)/SKILL.md) to run
  this path versus one of the managed-cloud alternatives in this skill
  family, before committing to it.

## Prerequisites & environment

- A [Kubernetes](../kubernetes/SKILL.md) cluster ≥ 1.24 already provisioned — this skill does
...
- `helm` ≥ 3.14 and `[kubectl](../kubectl/SKILL.md)`, plus the NVIDIA GPU Operator's
...
- A decision, made deliberately and with realistic staffing in mind,
  about whether this team actually has the operational [capacity](../../../AI_and_Agents/Infrastructure/deploy-model/[capacity](../../Cloud_Providers/azure-skills/skills/microsoft-foundry/models/deploy-model/[capacity](../../Observability_and_SecOps/capacity/SKILL.md)/SKILL.md)/SKILL.md) (on-call,
...
- No dependency on any cloud IAM federation mechanism (IRSA, Azure AD
  ...
  self-hosted services here are [Kubernetes](../kubernetes/SKILL.md) Secrets or a self-hosted
...
1. **Phase 1 — cluster foundation.** Confirm the cluster is healthy and
   ...
   cluster (see
   [etcd-backup-restore-and-cluster-health](../../../[kubernetes](../kubernetes/SKILL.md)-platform/skills/[etcd-backup-restore-and-cluster-health](../etcd-backup-restore-and-cluster-health/SKILL.md)/SKILL.md)),
...
2. **Phase 2 — GPU node pools.** Install the NVIDIA GPU Operator and
   ...
   [kubectl](../kubectl/SKILL.md) taint nodes -l gpu-pool=training workload=training:NoSchedule
   [kubectl](../kubectl/SKILL.md) taint nodes -l gpu-pool=serving workload=serving:NoSchedule
   ```
...
3. **Phase 3 — orchestration engine decision: Kubeflow Pipelines vs.
   ...
   - **Ray (via the KubeRay operator)** — a [Python](../../../Software_Engineering_and_Other/Languages/python/SKILL.md)-native distributed
     ...
     [ray-distributed-ml-orchestration](../[ray-distributed-ml-orchestration](../../../Data_Engineering/ray-distributed-ml-orchestration/SKILL.md)/SKILL.md).
...
7. **Phase 7 — on-cluster serving and scaling.** Deploy KServe (or
   ...
   here, so this is a plain [Kubernetes](../kubernetes/SKILL.md) Secret that must be rotated on the
...
8. **Phase 8 — [monitoring](../../Observability_and_SecOps/monitoring/SKILL.md) and drift detection.** Run a self-hosted
   ...
   [alerting](../../Observability_and_SecOps/alerting/SKILL.md) discipline from
...
- Scope every in-cluster service account and [Kubernetes](../kubernetes/SKILL.md) Secret to the
...
- **Symptom:** A GPU-requesting training pipeline step in Phase 5 sits
  ...
  **Fix:** Phase 2's GPU node pool was skipped, under-sized, or not yet
  scaled when Phase 5 went live. Confirm real `nvidia.com/gpu` [capacity](../../../AI_and_Agents/Infrastructure/deploy-model/[capacity](../../Cloud_Providers/azure-skills/skills/microsoft-foundry/models/deploy-model/[capacity](../../Observability_and_SecOps/capacity/SKILL.md)/SKILL.md)/SKILL.md)
  with `[kubectl](../kubectl/SKILL.md) get nodes -o json | jq '.items[].status.[capacity](../../../AI_and_Agents/Infrastructure/deploy-model/[capacity](../../Cloud_Providers/azure-skills/skills/microsoft-foundry/models/deploy-model/[capacity](../../Observability_and_SecOps/capacity/SKILL.md)/SKILL.md)/SKILL.md)["nvidia.com/gpu"]'`
...
- **Symptom:** A long-running batch training job on the Phase 2 training
  ...
  **Fix:** Node pool taints/tolerations from Phase 2 weren't actually
  ...
  nodes — re-verify `[kubectl](../kubectl/SKILL.md) describe node` shows the expected taint on
...
- **Symptom:** A model promoted through Phase 4's registry and deployed
  ...
  **Fix:** Phase 8's [monitoring](../../Observability_and_SecOps/monitoring/SKILL.md) was stood up after, not before, Phase 7's
  ...
  to fall back on if the self-hosted [monitoring](../../Observability_and_SecOps/monitoring/SKILL.md) job's cron schedule was
...
- **Symptom:** Months into operating this platform, the team is
  ...
  [incident](../../Observability_and_SecOps/incident/SKILL.md) response than on actual ML work, and morale/velocity suffers.
...
residency and [audit](../../../AI_and_Agents/Operations/audit/SKILL.md) requirements), on a [Kubernetes](../kubernetes/SKILL.md) cluster provisioned via
...
[kubectl](../kubectl/SKILL.md) taint nodes gpu-node-01 gpu-node-02 workload=training:NoSchedule
[kubectl](../kubectl/SKILL.md) taint nodes gpu-node-03 workload=serving:NoSchedule
...
helm install postgres bitnami/[postgresql](../../../Software_Engineering_and_Other/Backend/postgresql/SKILL.md) --namespace mlflow --create-namespace \
...
[kubectl](../kubectl/SKILL.md) apply -f readmission-model-inferenceservice.yaml
...
- [kubernetes-cluster-provisioning-with-kubeadm-and-cluster-api](../../../[kubernetes](../kubernetes/SKILL.md)-platform/skills/[kubernetes-cluster-provisioning-with-kubeadm-and-cluster-api](../[kubernetes](../kubernetes/SKILL.md)-cluster-provisioning-with-kubeadm-and-cluster-api/SKILL.md)/SKILL.md) — Phase 1's cluster bootstrap for a genuinely cloud-agnostic/on-prem cluster.
- [etcd-backup-restore-and-cluster-health](../../../[kubernetes](../kubernetes/SKILL.md)-platform/skills/[etcd-backup-restore-and-cluster-health](../etcd-backup-restore-and-cluster-health/SKILL.md)/SKILL.md) — Phase 1's control-plane durability, the same discipline applied to Phase 4's self-hosted Postgres/MinIO.
- [managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke](../../../[kubernetes](../kubernetes/SKILL.md)-platform/skills/[managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke](../managed-[kubernetes](../kubernetes/SKILL.md)-eks-aks-gke/SKILL.md)/SKILL.md) — an alternative Phase 1 cluster foundation when using a cloud-provisioned control plane purely for raw compute, without its managed ML services.
...
- [model-[monitoring](../../Observability_and_SecOps/monitoring/SKILL.md)-and-drift-detection](../[model-[monitoring](../../Observability_and_SecOps/monitoring/SKILL.md)-and-drift-detection](../../../AI_and_Agents/Models_and_FineTuning/model-[monitoring](../../Observability_and_SecOps/monitoring/SKILL.md)-and-drift-detection/SKILL.md)/SKILL.md) — Phase 8's drift/quality [monitoring](../../Observability_and_SecOps/monitoring/SKILL.md).
