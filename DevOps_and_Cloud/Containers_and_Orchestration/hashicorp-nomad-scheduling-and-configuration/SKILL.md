...
[Kubernetes](../kubernetes/SKILL.md) — one binary for both server and client roles, no separate
etcd cluster to operate, and a smaller set of core concepts (jobs,
task groups, tasks, scheduler types). It is not a [Kubernetes](../kubernetes/SKILL.md)
replacement in every case: [Kubernetes](../kubernetes/SKILL.md)' ecosystem (CRDs, service mesh
integrations, the broader operator pattern) is deeper, and most
organizations already standardized on [Kubernetes](../kubernetes/SKILL.md) won't switch wholesale.
...
- Deciding whether a workload/environment is better served by [Nomad](../nomad/SKILL.md) or
  [Kubernetes](../kubernetes/SKILL.md), when neither is already an unchangeable given.
...
- A driver enabled on client nodes for each task type in use (`[docker](../docker/SKILL.md)`,
...
1. **Choose the scheduler type per job based on the workload's actual
   ...
   - `service` — long-running processes that should be restarted on
     ...
     [Kubernetes](../kubernetes/SKILL.md) `Deployment`).
   - `batch` — run-to-completion workloads (the [Nomad](../nomad/SKILL.md) analog of a
     [Kubernetes](../kubernetes/SKILL.md) `Job`), optionally on a `periodic` schedule (the analog
     of a `CronJob`).
   - `system` — one instance per eligible client node (the [Nomad](../nomad/SKILL.md) analog
     of a [Kubernetes](../kubernetes/SKILL.md) `DaemonSet`), for node-level agents like log
     shippers or [monitoring](../../Observability_and_SecOps/monitoring/SKILL.md) exporters.
...
2. **Write the job spec with explicit resource requests and a matching
   ...
   [Kubernetes](../kubernetes/SKILL.md) readiness probe gating Service endpoint membership.
...
3. **Configure an update strategy for rolling deploys, rather than
   ...
   [Kubernetes](../kubernetes/SKILL.md) `Deployment`'s `maxSurge`/`maxUnavailable` combined with a
...
6. **Decide [Nomad](../nomad/SKILL.md) vs. [Kubernetes](../kubernetes/SKILL.md) per environment/workload class**, not
   ...
   [Kubernetes](../kubernetes/SKILL.md)' control-plane footprint is disproportionate; [Kubernetes](../kubernetes/SKILL.md)
...
- Set explicit `resources { cpu, memory }` on every task — an
  ...
  as an unset [Kubernetes](../kubernetes/SKILL.md) pod resource request.
...
- Keep `raw_exec` disabled unless a specific, understood workload needs
  unsandboxed process execution; prefer the `[docker](../docker/SKILL.md)` or `exec` (chroot/
...
- Reassess the [Nomad](../nomad/SKILL.md)-vs-[Kubernetes](../kubernetes/SKILL.md) decision per new workload class
...
all-at-once [Kubernetes](../kubernetes/SKILL.md) `CronJob` but needs to move to a [Nomad](../nomad/SKILL.md) cluster
...
- [knative-[serverless](../serverless/SKILL.md)-configuration](../[knative-[serverless](../serverless/SKILL.md)-configuration](../knative-[serverless](../serverless/SKILL.md)-configuration/SKILL.md)/SKILL.md) — a [Kubernetes](../kubernetes/SKILL.md)-native scheduling/scaling model to compare against when deciding whether [Nomad](../nomad/SKILL.md)'s simpler operational footprint is the better fit for a given workload.
- [dapr-distributed-runtime-configuration](../[dapr-distributed-runtime-configuration](../../../Software_Engineering_and_Other/Frontend/dapr-distributed-runtime-configuration/SKILL.md)/SKILL.md) — Dapr's sidecar building blocks can run on [Nomad](../nomad/SKILL.md)-scheduled tasks the same as on [Kubernetes](../kubernetes/SKILL.md) pods, for teams wanting consistent state/pub-sub/service-invocation APIs across both schedulers.
- [aws-lambda-packaging-and-configuration](../[aws-lambda-packaging-and-configuration](../../Cloud_Providers/[aws-lambda](../../Cloud_Providers/aws-lambda/SKILL.md)-packaging-and-configuration/SKILL.md)/SKILL.md) — a fully managed alternative to self-operating a [Nomad](../nomad/SKILL.md) (or [Kubernetes](../kubernetes/SKILL.md)) cluster at all, worth weighing when the workload doesn't need long-running processes or non-containerized task types.
