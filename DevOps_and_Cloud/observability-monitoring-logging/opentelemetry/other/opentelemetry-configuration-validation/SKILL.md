...
# [OpenTelemetry](../opentelemetry/SKILL.md) Configuration Validation

## Purpose

An [OpenTelemetry](../opentelemetry/SKILL.md) Collector config that parses as valid YAML and starts
...
on-call engineer needs during an [incident](../../../common/incident-detection/incident/SKILL.md), or a resource attribute typo
...
- Before merging or deploying any change to an [OpenTelemetry](../opentelemetry/SKILL.md) Collector
...
- Auditing an existing production Collector config for the common,
  ...
  check, not only after an [incident](../../../common/incident-detection/incident/SKILL.md).
...
1. **Run the Collector's own structural validation** before anything
   ...
   ```bash
   helm template otel-collector open-telemetry/[opentelemetry](../opentelemetry/SKILL.md)-collector \
...
4. **Review sampling configuration for what it actually discards, not
   ...
   > failing/slow requests an on-call engineer needs during an [incident](../../../common/incident-detection/incident/SKILL.md) —
...
- **Symptom:** A tail-sampling change is deployed to "reduce storage
  cost," and during the next [incident](../../../common/incident-detection/incident/SKILL.md) the on-call engineer can't find any
  ...
  **Fix:** The sampling policy applied a flat probabilistic rate with no
  ...
  re-review the change with "what would this drop during an [incident](../../../common/incident-detection/incident/SKILL.md)" as
...
3. Review the sampling change against step 4's "what does this discard"
   ...
   an [incident](../../../common/incident-detection/incident/SKILL.md) investigation would need. Approved as-is.
