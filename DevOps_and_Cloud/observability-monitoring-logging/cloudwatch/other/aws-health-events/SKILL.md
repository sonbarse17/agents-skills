...
Use this skill when investigating an [incident](../../../common/incident-detection/incident/SKILL.md) and you need to check for AWS-side
...
**[Incident](../../../common/incident-detection/incident/SKILL.md) Investigation (automatic activation):**

- An active [incident](../../../common/incident-detection/incident/SKILL.md) may be caused by an AWS service disruption or degradation.
...
- You need to determine whether an AWS-side event is the root cause or a
  contributing factor to the current [incident](../../../common/incident-detection/incident/SKILL.md).
...
## Step 1: Gather [Incident](../../../common/incident-detection/incident/SKILL.md) Context

Before searching Health events, extract key details from the current [incident](../../../common/incident-detection/incident/SKILL.md):

1. **Affected AWS services** — identify the service(s) experiencing issues
   (e.g., EC2, RDS, Lambda, ELB, ECS).
2. **Timeframe** — determine when the [incident](../../../common/incident-detection/incident/SKILL.md) started and its current duration.
...
5. **Symptoms** — note the observed symptoms (latency spikes, 5xx errors,
   connection timeouts, throttling, [capacity](../../../AI_and_Agents/Infrastructure/deploy-model/[capacity](../azure-skills/skills/microsoft-foundry/models/deploy-model/[capacity](../../Observability_and_SecOps/capacity/SKILL.md)/SKILL.md)/SKILL.md) errors).
...
the [incident](../../../common/incident-detection/incident/SKILL.md) context. All calls must target the **us-east-1** endpoint.
...
|----------|-------------|
| By service | Use the `services` filter with the AWS Health service code (e.g., `EC2`, `RDS`, `ELASTICLOADBALANCING`), because service-specific events are most likely to correlate with the [incident](../../../common/incident-detection/incident/SKILL.md). |
| By time range | Use `startTimes` with a `from` value set to 7 days before the [incident](../../../common/incident-detection/incident/SKILL.md) start, because events that started before the [incident](../../../common/incident-detection/incident/SKILL.md) may still be active and causing impact. |
...
| By availability zone | Use the `availabilityZones` filter when the [incident](../../../common/incident-detection/incident/SKILL.md) is isolated to a specific AZ, because AZ-scoped events have the highest correlation with AZ-specific failures. |
...
|-------|-----------------|
| `service` | Must match one of the affected services from the [incident](../../../common/incident-detection/incident/SKILL.md) context, or a related service from the Service Dependency Map |
| `eventTypeCategory` | Prioritize `issue` events for active incidents; include `scheduledChange` if the [incident](../../../common/incident-detection/incident/SKILL.md) coincides with a maintenance window |
| `eventTypeCode` | Match against known operational event patterns (e.g., `AWS_EC2_OPERATIONAL_ISSUE`, `AWS_RDS_MAINTENANCE`) |
| `statusCode` | Prioritize `open` events; include `closed` only if the event ended within 2 hours of the [incident](../../../common/incident-detection/incident/SKILL.md) start |
| `startTime` / `endTime` | The event's active period must overlap with the [incident](../../../common/incident-detection/incident/SKILL.md) timeframe |
| `region` / `availabilityZone` | Must match the [incident](../../../common/incident-detection/incident/SKILL.md)'s affected region or AZ |
...
2. **Keep** events where the active period (startTime to endTime, or to present
   if open) overlaps with the [incident](../../../common/incident-detection/incident/SKILL.md) timeframe.
3. **Keep** events where the `region` or `availabilityZone` matches the
   [incident](../../../common/incident-detection/incident/SKILL.md)'s affected region/AZ.
4. **Discard** `accountNotification` events unless the [incident](../../../common/incident-detection/incident/SKILL.md) context
   specifically suggests an account-level issue (e.g., abuse notification,
   certificate expiry).
5. **Discard** `closed` events that ended more than 2 hours before the [incident](../../../common/incident-detection/incident/SKILL.md)
...
When the [incident](../../../common/incident-detection/incident/SKILL.md) context includes specific resource identifiers:
...
2. Perform exact string matching of each entity's `entityValue` against the
   [incident](../../../common/incident-detection/incident/SKILL.md) context resource identifiers.
...
## Step 6: Correlate with [Incident](../../../common/incident-detection/incident/SKILL.md)

Score each Health event for relevance to the current [incident](../../../common/incident-detection/incident/SKILL.md) using the
following criteria:
...
- **Service match**: The event's service code matches one of the affected
  services from the [incident](../../../common/incident-detection/incident/SKILL.md) context.
- **Timeframe overlap**: The event's active period (start time through end time,
  or through present if still open) intersects with the [incident](../../../common/incident-detection/incident/SKILL.md)'s timeframe
...
- **Region/AZ match**: The event's region or availability zone matches the
  [incident](../../../common/incident-detection/incident/SKILL.md)'s affected region or AZ.
- **Resource match**: At least one affected entity's `entityValue` matches a
  resource identifier from the [incident](../../../common/incident-detection/incident/SKILL.md) context.
...
- Closed events with High relevance should be noted as potential recent causes
  if the [incident](../../../common/incident-detection/incident/SKILL.md) started shortly after the event closed.

### When resource identifiers are unavailable

If the [incident](../../../common/incident-detection/incident/SKILL.md) context does not include specific resource identifiers, score
...
└── NO → Continue with [incident](../../../common/incident-detection/incident/SKILL.md) investigation flow below
...
Does the [incident](../../../common/incident-detection/incident/SKILL.md) involve a specific availability zone?
...
- **Correlate with support cases**: If a Health event references a service
  disruption, check if related support cases exist using the [support-cases](../../../../cloud/common/other/support-cases/SKILL.md) skill.
