...
- **Name the [runbook](../runbook/SKILL.md) after the symptom**, not the fix — "Disk usage above 90%" not "Cleanup
  script."
- **Put the link in the alert**, see `[alerting](../../alerting/alerting/SKILL.md)` — a [runbook](../runbook/SKILL.md) is only as good as its
...
- **Give an explicit escalation path for every branch**, including who to page and what
  channel to use — see `[on-call-management](../../alerting/on-call-management/SKILL.md)`.
...
- **Run every [runbook](../runbook/SKILL.md) during a game day** — see `[chaos-engineering](../../../../containers-orchestration/common/other/chaos-engineering/SKILL.md)` — not just read it.
