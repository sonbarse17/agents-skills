...
system is actually fragile. A blameless postmortem treats the [incident](../../incident-detection/incident/SKILL.md) as
...
collapsing a multi-cause [incident](../../incident-detection/incident/SKILL.md) into one scapegoat cause, and making
sure action items actually get done instead of accumulating in a backlog
nobody revisits.

## When to use

- After any Sev1/Sev2 [incident](../../incident-detection/incident/SKILL.md) is resolved and stood down (see
...
- A near-miss occurred — no customer impact, but the system was one step
  away from a real [incident](../../incident-detection/incident/SKILL.md) — and the team wants to capture the lesson
...
- Reviewing why past postmortem action items never got implemented and a
  similar [incident](../../incident-detection/incident/SKILL.md) just recurred.

## Prerequisites & environment

- A collected [incident](../../incident-detection/incident/SKILL.md) record: the scribe's live timeline, relevant
  [dashboards](../../dashboard-design/dashboards/SKILL.md)/logs, and the [incident](../../incident-detection/incident/SKILL.md) chat transcript (see
  [incident-response-and-on-call-management](../[incident-response-and-on-call-management](../[incident-response](../../../DevOps_and_Cloud/Observability_and_SecOps/[incident](../../../DevOps_and_Cloud/Observability_and_SecOps/incident/SKILL.md)-response/SKILL.md)-and-[on-call-management](../../alerting/on-call-management/SKILL.md)/SKILL.md)/SKILL.md)
...
- A facilitator who was not the primary hands-on responder for the
  [incident](../../incident-detection/incident/SKILL.md), where staffing allows.
...
3. **Facilitate blamelessly, explicitly:**
   ...
   - The facilitator should be someone other than the person most
     centrally involved in causing or fixing the [incident](../../incident-detection/incident/SKILL.md), so the
...
4. **Run contributing-factor analysis, not single-root-cause analysis.**
   ...
   Why was there no validation step?   → It was deprioritized after the
                                          last [incident](../../incident-detection/incident/SKILL.md)'s action item stalled.
...
5. **Categorize each action item** by what it does: *prevent recurrence*
   (fix the proximate cause), *detect faster* ([alerting](../../alerting/alerting/SKILL.md)/[monitoring](../../monitoring-strategy/monitoring/SKILL.md) gap),
...
6. **Publish broadly and review in a recurring forum.** Post the
   ...
   (internal wiki/doc index), and bring it to a standing "[incident](../../incident-detection/incident/SKILL.md)
...
- Write for a reader who wasn't in the room and doesn't have the
  [incident](../../incident-detection/incident/SKILL.md)'s context — precise UTC timestamps, full service names, no
...
- Explicitly look for at least two to three independent contributing
  ...
  analysis stopped too early, not that the [incident](../../incident-detection/incident/SKILL.md) was simple.
- Revisit old postmortems periodically when a new, similar [incident](../../incident-detection/incident/SKILL.md)
...
- **Symptom:** The postmortem draft reads "Jane forgot to update the
  ...
  more reluctant to volunteer details in [incident](../../incident-detection/incident/SKILL.md) channels.
...
- **Symptom:** 5-whys stops after one iteration at "the engineer ran the
  wrong command," and three months later a near-identical [incident](../../incident-detection/incident/SKILL.md)
...
- **Symptom:** A well-written postmortem is finished, reviewed once by
  ...
  **Fix:** Publish to a shared, searchable location and bring it to a
  cross-team recurring [incident](../../incident-detection/incident/SKILL.md)-review forum, not just the originating
  team's internal channel.

- **Symptom:** The postmortem meeting is scheduled two weeks after the
  [incident](../../incident-detection/incident/SKILL.md), and the discussion spends most of its time arguing about
  what order things happened in.
  **Fix:** Schedule within 48-72 hours while memory is fresh, and draft
  the timeline from the scribe's live [incident](../../incident-detection/incident/SKILL.md)-channel log (captured
  during the [incident](../../incident-detection/incident/SKILL.md) itself) rather than reconstructing it from
...
- **Impact:** 100% of checkout traffic affected for 20 minutes (14:05-
  ...
  in a single [incident](../../incident-detection/incident/SKILL.md).
...
- **Outcome:** the postmortem is published to the internal wiki, brought
  to the monthly cross-team [incident](../../incident-detection/incident/SKILL.md) review, and the error-budget freeze
  from
  [slo-sli-and-error-budget-design](../[slo-sli-and-error-budget-design](../slo-sli-and-error-budget-design/SKILL.md)/SKILL.md)
...
- [incident-response-and-on-call-management](../[incident-response-and-on-call-management](../[incident-response](../../../DevOps_and_Cloud/Observability_and_SecOps/[incident](../../../DevOps_and_Cloud/Observability_and_SecOps/incident/SKILL.md)-response/SKILL.md)-and-[on-call-management](../../alerting/on-call-management/SKILL.md)/SKILL.md)/SKILL.md) — the scribe's live timeline and severity classification captured during the [incident](../../incident-detection/incident/SKILL.md) are the raw input to this postmortem.
