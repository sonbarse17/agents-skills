...
Assess APM service health using [Observability APIs](https://www.elastic.co/docs/solutions/[observability](../../fundamentals/observability/SKILL.md)/apis),
...
- **[Observability](../../fundamentals/observability/SKILL.md) APIs** ([Observability APIs](https://www.elastic.co/docs/solutions/[observability](../../fundamentals/observability/SKILL.md)/apis)): Use the
  **SLOs API** ([Stack](https://www.elastic.co/docs/api/doc/kibana/group/endpoint-slo) |
  [Serverless](https://www.elastic.co/docs/api/doc/[serverless](../../../../Software_Engineering_and_Other/Patterns/data-performance/serverless/SKILL.md)/group/endpoint-slo)) to get SLO definitions, status, burn
  rate, and error budget. Use the **[Alerting](../../alerting/alerting/SKILL.md) API**
  ([Stack](https://www.elastic.co/docs/api/doc/kibana/group/endpoint-[alerting](../../alerting/alerting/SKILL.md)) |
  [Serverless](https://www.elastic.co/docs/api/doc/[serverless](../../../../Software_Engineering_and_Other/Patterns/data-performance/serverless/SKILL.md)/group/endpoint-[alerting](../../alerting/alerting/SKILL.md))) to list and manage [alerting](../../alerting/alerting/SKILL.md)
...
- **[Observability](../../fundamentals/observability/SKILL.md) Labs:** [Observability Labs](https://www.elastic.co/[observability](../../fundamentals/observability/SKILL.md)-labs) and
  [APM tag](https://www.elastic.co/[observability](../../fundamentals/observability/SKILL.md)-labs/blog/tag/apm) for patterns and troubleshooting.
...
- **Availability:** ES|QL is available in **Elasticsearch 8.11+** (technical preview; GA in 8.14). It is **always
  available** in
  [Elastic [Observability](../../fundamentals/observability/SKILL.md) [Serverless](../../../../Software_Engineering_and_Other/Patterns/data-performance/serverless/SKILL.md) Complete tier](https://www.elastic.co/docs/solutions/[observability](../../fundamentals/observability/SKILL.md)/[observability](../../fundamentals/observability/SKILL.md)-[serverless](../../../../Software_Engineering_and_Other/Patterns/data-performance/serverless/SKILL.md)-feature-tiers).
...
node skills/[observability](../../fundamentals/observability/SKILL.md)/service-health/scripts/apm-correlations.js latency-correlations --service-name <name> [--start <iso>] [--end <iso>] [--last-minutes 60] [--transaction-type <t>] [--transaction-name <n>] [--space <id>] [--json]

# Failed transaction correlations
node skills/[observability](../../fundamentals/observability/SKILL.md)/service-health/scripts/apm-correlations.js failed-correlations --service-name <name> [--start <iso>] [--end <iso>] [--last-minutes 60] [--transaction-type <t>] [--transaction-name <n>] [--space <id>] [--json]

# Test Kibana connection
node skills/[observability](../../fundamentals/observability/SKILL.md)/service-health/scripts/apm-correlations.js test [--space <id>]
```
...
`/api/[alerting](../../alerting/alerting/SKILL.md)/rules/_find?search=apm&search_fields=tags&per_page=100&filter=alert.attributes.executionStatus.status:active`.
...
- **APM Correlations (when only a subpopulation is affected):** Run
  `node skills/[observability](../../fundamentals/observability/SKILL.md)/service-health/scripts/apm-correlations.js latency-correlations|failed-correlations --service-name <name> [--start ...] [--end ...]`
...
1. Resolve service X and time range. Call **SLOs API** and **[Alerting](../../alerting/alerting/SKILL.md) API**; run **ES|QL** on
...
3. Answer: Healthy / Degraded / Unhealthy with reasons and next steps (e.g.
   [Observability Labs](https://www.elastic.co/[observability](../../fundamentals/observability/SKILL.md)-labs)).

### Example: "Why is service Y slow?"

1. Service Y and slowness time range. Call **SLOs API** and **[Alerting](../../alerting/alerting/SKILL.md) API**; run **ES|QL** for Y and dependencies;
...
3. Summarize (e.g. p99 up; dependency Z elevated) and recommend (investigate Z; [Observability](../../fundamentals/observability/SKILL.md) Labs for latency).

### Example: Correlate service to infrastructure ([OpenTelemetry](../../../opentelemetry/other/opentelemetry/SKILL.md))
...
- Run **ES|QL** or Elasticsearch search on infrastructure/metrics indices filtered by those resource values and the
  [incident](../../incident-detection/incident/SKILL.md) time range. Check **CPU usage** and **memory consumption** (e.g. `system.cpu.total.norm.pct`); look for
...
- Use **[Observability](../../fundamentals/observability/SKILL.md) APIs** ([SLOs API](https://www.elastic.co/docs/api/doc/kibana/group/endpoint-slo),
  [Alerting API](https://www.elastic.co/docs/api/doc/kibana/group/endpoint-[alerting](../../alerting/alerting/SKILL.md))) and **ES|QL** on
  `traces*apm*,traces*otel*`/`metrics*apm*,metrics*otel*` (8.11+ or [Serverless](../../../../Software_Engineering_and_Other/Patterns/data-performance/serverless/SKILL.md)), filtering by `service.name` (and
  ...
  declaring health. Do not query `.alerts*` indices when determining currently active alerts; use the [Alerting](../../alerting/alerting/SKILL.md) API
...
- When analyzing **only application metrics ingested via [OpenTelemetry](../../../opentelemetry/other/opentelemetry/SKILL.md)**, use the ES|QL **TS** (time series) command for
  efficient metrics queries. The TS command is available in **Elasticsearch 9.3+** and is **always available** in
  Elastic [Observability](../../fundamentals/observability/SKILL.md) [Serverless](../../../../Software_Engineering_and_Other/Patterns/data-performance/serverless/SKILL.md).
