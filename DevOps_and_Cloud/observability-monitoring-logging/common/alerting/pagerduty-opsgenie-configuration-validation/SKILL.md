...
before an [incident](../../incident-detection/incident/SKILL.md) is the first time anyone finds out otherwise.
...
4. **Check `num_loops`/repeat behavior is non-zero** for any policy
   ...
   if nobody acknowledges, leaving the [incident](../../incident-detection/incident/SKILL.md) open and unescalated
...
- **Symptom:** An escalation policy has three rungs configured, and it
  looks properly redundant in the console — but during a real [incident](../../incident-detection/incident/SKILL.md),
...
- **Symptom:** A schedule was set up with a rotation that has an implicit
  ...
  **Fix:** Proactively query resolved on-call coverage 4-6 weeks forward
  ...
  users, rather than discovering the gap only when an [incident](../../incident-detection/incident/SKILL.md) falls
  into it.
...
- **Symptom:** `num_loops` on a production escalation policy is left at
  ...
  with the [incident](../../incident-detection/incident/SKILL.md) still unacknowledged and no further notification.
