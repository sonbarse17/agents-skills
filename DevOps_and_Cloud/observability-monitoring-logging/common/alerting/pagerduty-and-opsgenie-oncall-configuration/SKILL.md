...
The process itself (severity levels, [Incident](../../incident-detection/incident/SKILL.md) Command roles, rotation
...
- Routing alerts from a [monitoring](../../monitoring-strategy/monitoring/SKILL.md)/[alerting](../alerting/SKILL.md) source (Prometheus
  Alertmanager, [Datadog](../../../datadog/other/datadog/SKILL.md), CloudWatch) into the correct PagerDuty service
  or Opsgenie team, including severity-based routing.
- Configuring alert deduplication so a flapping check or a burst of
  correlated alerts from one root cause produces one [incident](../../incident-detection/incident/SKILL.md), not one
  page per alert.
...
2. **Write the escalation policy with at least two independent rungs.**
   ...
   ```
   `num_loops: 2` means the whole chain repeats twice before the [incident](../../incident-detection/incident/SKILL.md)
...
3. **Route alerts to the correct service/team with explicit rules, not
   ...
   you route on payload fields before an event even creates an [incident](../../incident-detection/incident/SKILL.md):
   ```yaml
   ...
   route on a stable field the [monitoring](../../monitoring-strategy/monitoring/SKILL.md) source guarantees is present
...
4. **Deduplicate at the source, using a stable key, not the tool's
   ...
   for the same underlying problem collapses them into one [incident](../../incident-detection/incident/SKILL.md):
   ```json
   {
...
- Deduplicate as close to the alert source as practical (a stable
  ...
  grouping if the [monitoring](../../monitoring-strategy/monitoring/SKILL.md) stack supports it.
...
- **Symptom:** One flapping check or one root-cause outage generates
  ...
  **Fix:** Add a stable `dedup_key`/`alias` at the event-submission layer
  ...
  [incident](../../incident-detection/incident/SKILL.md) instead of creating new ones, and check whether the
  [monitoring](../../monitoring-strategy/monitoring/SKILL.md) source itself (Alertmanager `group_by`) should be
...
- **Symptom:** A maintenance window is set on an entire service to
  silence a known noisy alert, and a real, unrelated [incident](../../incident-detection/incident/SKILL.md) on that
...
flaps every 30 seconds for ten minutes produces one PagerDuty [incident](../../incident-detection/incident/SKILL.md),
not twenty:
...
- [servicenow-itsm-integration](../../incident-detection/servicenow-itsm-integration/SKILL.md)/SKILL.md) — syncing incidents created by this PagerDuty/Opsgenie configuration into a ServiceNow [incident](../../incident-detection/incident/SKILL.md)/change record for ITIL-heavy organizations.
- [chatops-[runbook](../../incident-detection/runbook/SKILL.md)-automation](../[chatops-[runbook](../runbook/SKILL.md)-automation](../../../Software_Engineering_and_Other/Frontend/chatops-[runbook](../runbook/SKILL.md)-automation/SKILL.md)/SKILL.md) — triggering automated [runbook](../../incident-detection/runbook/SKILL.md) actions from the [incident](../../incident-detection/incident/SKILL.md) this escalation policy pages someone into.
- [incident-response-and-on-call-management](../../../site-reliability-engineering/skills/[incident-response-and-on-call-management](../../../Software_Engineering_and_Other/Frontend/[incident-response](../[incident](../incident/SKILL.md)-response/SKILL.md)-and-[on-call-management](../on-call-management/SKILL.md)/SKILL.md)/SKILL.md) — the severity levels, rotation design, and [Incident](../../incident-detection/incident/SKILL.md) Command process this tool configuration implements.
