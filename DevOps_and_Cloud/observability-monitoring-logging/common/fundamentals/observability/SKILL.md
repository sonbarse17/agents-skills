...
1. **Read-only.** Read [monitoring](../../monitoring-strategy/monitoring/SKILL.md)/[alerting](../../alerting/alerting/SKILL.md) config (Prometheus rules, Grafana
   [dashboards](../../dashboard-design/dashboards/SKILL.md), alertmanager, [Datadog](../../../datadog/other/datadog/SKILL.md)/CloudWatch definitions as code) and query
   metrics/logs read-only. Never edit [dashboards](../../dashboard-design/dashboards/SKILL.md), silence/modify alerts, or
   change config.
...
- Identify the stack: metrics (Prometheus/CloudWatch/[Datadog](../../../datadog/other/datadog/SKILL.md)), logs (ELK/Loki/
  CloudWatch), traces (OTel/Jaeger/Tempo/X-Ray), [dashboards](../../dashboard-design/dashboards/SKILL.md), [alerting](../../alerting/alerting/SKILL.md)/on-call
...
- **[Alerting](../../alerting/alerting/SKILL.md) quality** — alerts on causes not symptoms (page on "CPU high"
  instead of "users seeing errors"), no alert for the failure modes that
  actually cause outages (the "would we know?" gap), alerts with no [runbook](../../incident-detection/runbook/SKILL.md)
...
- **[Dashboards](../../dashboard-design/dashboards/SKILL.md)** — no single "is the service healthy?" view for critical
  services, [dashboards](../../dashboard-design/dashboards/SKILL.md) that don't map to how the system fails, stale/broken
  panels.
- **Operational readiness** — no log retention or too-short retention for
  ...
  [monitoring](../../monitoring-strategy/monitoring/SKILL.md) of the user-facing path, missing deploy/version annotations to
...
a [runbook](../../incident-detection/runbook/SKILL.md) link.
...
- `/[incident](../../incident-detection/incident/SKILL.md)` — real incidents are the best evidence of a detection gap.
- `/[runbook](../../incident-detection/runbook/SKILL.md)` — every page needs a [runbook](../../incident-detection/runbook/SKILL.md); alerts without one are a `DOC` finding.
- `/[k8s-review](../../../../containers-orchestration/kubernetes/troubleshooting/k8s-review/SKILL.md)`, `/[db-review](../../../../AI_and_Agents/Operations/common/db-review/SKILL.md)` — instrumentation gaps at the workload/data layer.
- `/[release-readiness](../../../../ci-cd/common/pipeline-design/release-readiness/SKILL.md)` — whether *this* release would be caught going wrong.
...
- [ ] Every proposed alert specifies symptom-based condition, threshold
      rationale, severity, routing, and a [runbook](../../incident-detection/runbook/SKILL.md) link.
