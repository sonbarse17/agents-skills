...
Getting this consistent matters more than getting it clever, because levels are what [alerting](../../alerting/alerting/SKILL.md) and [dashboards](../../dashboard-design/dashboards/SKILL.md) filter on downstream. The failure mode in both directions is common:
...
That single field collapses what used to be a cross-team log-grepping exercise into one query. See `[distributed-tracing](../../distributed-tracing/distributed-tracing/SKILL.md)` for how it gets propagated across service boundaries in the first place.
