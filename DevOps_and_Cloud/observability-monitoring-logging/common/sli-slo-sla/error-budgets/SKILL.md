...
reliable — it just usually gets negotiated informally, [incident](../../incident-detection/incident/SKILL.md) by [incident](../../incident-detection/incident/SKILL.md), by whoever's
...
already agreed with the business, see `[slo-definition](../slo-definition/SKILL.md)`.
...
- **Alert on burn rate, not just on SLO breach** — a fast burn early in the window predicts
  exhaustion before it happens; see `[alerting](../../alerting/alerting/SKILL.md)` and `[slo-definition](../slo-definition/SKILL.md)` for the mechanics.
...
- **Make burn rate visible on a dashboard the whole team sees**, not something only surfaced
  during a postmortem — see `[dashboards](../../dashboard-design/dashboards/SKILL.md)`.
...
A single fast burn is an [incident](../../incident-detection/incident/SKILL.md). A budget that's exhausted every month, or a service that
...
- **A service that's chronically exhausted** signals a systemic reliability problem that ad
  hoc [incident](../../incident-detection/incident/SKILL.md) fixes won't solve — see `[root-cause-analysis](../../root-cause-analysis/root-cause-analysis/SKILL.md)` for finding the contributing
  factors across the pattern, not just one [incident](../../incident-detection/incident/SKILL.md).
