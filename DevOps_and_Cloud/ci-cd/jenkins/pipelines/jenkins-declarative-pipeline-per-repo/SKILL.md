...
# [Jenkins](../../other/jenkins/SKILL.md) Declarative Pipeline Per Repo
...
alongside it, and doesn't require a [Jenkins](../../other/jenkins/SKILL.md) administrator to touch a
separate job configuration. This skill covers [Jenkins](../../other/jenkins/SKILL.md) **Declarative
...
- A [Jenkins](../../other/jenkins/SKILL.md) controller (2.401+ LTS recommended for current Declarative
...
3. **Model real stages, not one giant `script` block.** Each logical phase
   ...
   [Jenkins](../../other/jenkins/SKILL.md) UI/API shows per-stage pass/fail and timing:
...
7. **Validate syntax before pushing** using the [Jenkins](../../other/jenkins/SKILL.md) CLI or the
   `Jenkinsfile` linter endpoint:
   ```bash
   curl -s -X POST -u ${JENKINS_USER}:${JENKINS_API_TOKEN} \
     -F "jenkinsfile=<Jenkinsfile" \
     https://[jenkins](../../other/jenkins/SKILL.md).example.com/pipeline-model-converter/validate
   ```
...
- Reference credentials only by ID (`withCredentials`, `credentialsId:`),
  never inline — [Jenkins](../../other/jenkins/SKILL.md) masks values referenced this way in console
...
- Use `junit`/`archiveArtifacts`/`recordIssues` (Warnings Next Generation
  plugin) to surface structured results in the [Jenkins](../../other/jenkins/SKILL.md) UI rather than only
...
- Version-pin plugin-provided steps where the plugin has known breaking
  ...
  [Jenkins](../../other/jenkins/SKILL.md) upgrade), since a per-repo Jenkinsfile has no central place to
...
- **Symptom:** A credential value shows up in plaintext in the build
  ...
  **Fix:** Never assign a credential to a plain environment variable via
  ...
  keep the secret variable inside its scope; [Jenkins](../../other/jenkins/SKILL.md) only masks values it
...
- **Symptom:** Two builds of the same branch run concurrently and race on
  ...
  **Fix:** Add `options { disableConcurrentBuilds() }`, or narrow it with
  `disableConcurrentBuilds(abortPrevious: true)` ([Jenkins](../../other/jenkins/SKILL.md) 2.263+) so a new
...
- [secure-cicd-gates](../../../../Security/common/devsecops/SKILL.md)/skills/[secure-cicd-gates](../../../../Security/app-security/secure-cicd-gates/SKILL.md)/SKILL.md) — designing the security scan stages to add into this pipeline.
