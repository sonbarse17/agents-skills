...
- Standing up a private package registry or pull-through proxy/mirror
  (Artifactory, Nexus, [GitHub](../../../github-actions/other/github/SKILL.md) Packages, Verdaccio, a cloud-native
...
- A chosen artifact/package registry solution: a managed offering ([GitHub](../../../github-actions/other/github/SKILL.md)
...
2. **[Commit](../../git-workflow/commit/SKILL.md) lockfiles and enforce them in CI** so builds are reproducible
...
6. **Define and enforce artifact retention** so storage doesn't grow
   ...
   [GitHub](../../../github-actions/other/github/SKILL.md) Container Registry cleanup via a scheduled workflow using
...
- Treat major dependency upgrades as their own reviewed change with a
  ...
  contract.
- Record artifact provenance (which [commit](../../git-workflow/commit/SKILL.md)/pipeline run produced a given
...
- **Symptom:** A build that passed last week fails today with no code
  ...
  version.
  **Fix:** This indicates missing or unenforced lockfiles — [commit](../../git-workflow/commit/SKILL.md) the
...
2. For any environment that hadn't cached it yet, pin an explicit
   ...
   regenerate `package-lock.json`, then [commit](../../git-workflow/commit/SKILL.md) the lockfile update.
