...
2. **Match the DR pattern to the tier, not the other way around:**
   ...
   - **Pilot light**: core data stores (e.g. a replicated database)
     are always running in the DR region at minimal [capacity](../../../AI_and_Agents/Infrastructure/deploy-model/[capacity](../azure-skills/skills/microsoft-foundry/models/deploy-model/[capacity](../../Observability_and_SecOps/capacity/SKILL.md)/SKILL.md)/SKILL.md); compute
...
3. **Implement cross-region, cross-account backup replication.**
   ...
   resource "aws_backup_plan" "tier0" {
     ...
     rule {
       ...
       copy_action {
         destination_vault_arn = "arn:aws:backup:eu-west-1:<DR_ACCOUNT_ID>:backup-[vault](../../../../Security/cryptography-secrets/vault/SKILL.md):tier0-dr-[vault](../../../../Security/cryptography-secrets/vault/SKILL.md)"
         lifecycle {
           delete_after = 90
         }
       }
   ...
   ```
   Equivalent patterns: Azure Backup with a Recovery Services [vault](../../../../Security/cryptography-secrets/vault/SKILL.md) in a
   paired region plus cross-subscription [vault](../../../../Security/cryptography-secrets/vault/SKILL.md) access, or GCP Backup and
...
7. **Protect backups against ransomware/mass-deletion specifically**:
   enable immutability (AWS Backup [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md) Lock, Azure Backup immutable
...
- **Test restores, not just backup job success** — a green checkmark on
  ...
  recoverable.
- **Enable backup immutability/[vault](../../../../Security/cryptography-secrets/vault/SKILL.md) lock for anything protecting
...
- **Symptom:** Ransomware or a compromised automation credential deletes
  ...
  **Fix:** Backups shared the same account/credentials/trust boundary as
  ...
  production). Enable backup [vault](../../../../Security/cryptography-secrets/vault/SKILL.md) immutability/lock and store DR
...
- **Symptom:** An engineer, cleaning up "old" DR infrastructure to save
  ...
  **Fix:** **Never tear down DR-region infrastructure (a promoted-
  capable read replica, a cross-region backup [vault](../../../../Security/cryptography-secrets/vault/SKILL.md)/bucket) as a cost
...
2. Given the tight RTO/RPO, select **warm standby**: run a scaled-down
   (e.g. 25% [capacity](../../../AI_and_Agents/Infrastructure/deploy-model/[capacity](../azure-skills/skills/microsoft-foundry/models/deploy-model/[capacity](../../Observability_and_SecOps/capacity/SKILL.md)/SKILL.md)/SKILL.md)) copy of the checkout stack continuously in a
...
3. Set up AWS Backup with a daily cross-account, cross-region copy
   ...
   account, with [Vault](../../../../Security/cryptography-secrets/vault/SKILL.md) Lock enabled in compliance mode so backups cannot
...
5. Schedule a live failover test during a planned low-traffic maintenance
   ...
   pre-warming an additional [capacity](../../../AI_and_Agents/Infrastructure/deploy-model/[capacity](../azure-skills/skills/microsoft-foundry/models/deploy-model/[capacity](../../Observability_and_SecOps/capacity/SKILL.md)/SKILL.md)/SKILL.md) buffer.
6. Document and rehearse fail-back: after the test, reverse the process
   ...
   default.
7. Schedule monthly restore-verification jobs against the backup [vault](../../../../Security/cryptography-secrets/vault/SKILL.md)
